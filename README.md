...
## 108. Full role-by-role system audit — two real backend security gaps, seven frontend permission mismatches

Prompted by a request to audit every user role and remove anything
not genuinely needed. Worked systematically: mapped the complete nav
configuration against all 16 roles, then swept every dashboard page
and component for the specific pattern already proven productive in
the prior session — action handlers (`onClick`, `.mutate()`, `<form>`)
with no `useAuthStore` backing them at all.

**Two genuine backend security gaps, not just frontend display bugs**
— found by checking what the backend actually enforced, not assumed:

- **Suspicious Transactions**: the endpoint for dismissing/confirming
  a fraud flag had no role check whatsoever. Any authenticated
  community member, not just financial officers, could act on it
  directly via the API, regardless of what the UI showed. Fixed to
  match the same "Auditor reviews, doesn't execute" split already
  established for payment reversals.
- **Meeting Summary**: the AI-provider-calling endpoint had no role
  check at all — any authenticated user could trigger it, bypassing
  the nav-level restriction entirely. Fixed to require Community
  Admin/Chairman/Secretary. Both backed by new HTTP-level tests
  confirming an unauthorized role is genuinely blocked and an
  authorized one still succeeds.

**Seven frontend permission mismatches found and fixed**, each
verified against the actual backend authority rather than guessed:
Funerals list page ("Record a funeral directly" and funeral-opening
Approve/Reject both showing to every role that can browse funerals —
Treasurer, Auditor, Collector, every family officer); the funeral
detail page's "Close collection" button (same gap); and Suspicious
Transactions' frontend gate matching the backend fix above.
Deliberately left "Request an opening" untouched after investigation
— concluded the broader access might be intentional, humane design
(any family member reporting a death during bereavement) rather than
a bug, and the backend already gives a clear, honest error rather
than crashing, so it didn't meet the bar the other fixes did.

**Three navigation adjustments** made after checking backend
authorization first, not just moving links: Auditor gained visibility
into Expenses and Liabilities (already backend-authorized to view,
just missing the link, and directly relevant to an audit role);
Platform Admin lost the Support link after tracing the actual ticket
-routing code and confirming a ticket they submitted would become
permanently unreachable by anyone; Contribution Rules — three
rate-editing forms and an exemption checkbox had zero gating at all,
fixed properly, then safely widened to Treasurer/Financial
Secretary/Auditor now that the page can't let them do anything that
would 403.

**Confirmed already correct, not touched**: Audit Log's
platform_admin/community_admin-only restriction (the code's own
docstring says "nobody else can reach this at all" — deliberate, not
a gap); Guest's minimal dashboard view (appropriate for a temporary,
on-the-spot desk-worker account); the funeral committee, family fund,
and welfare-campaign fixes from the immediately preceding session.

207 backend tests re-verified across `funerals` and `ai_features`
(including the new permission tests — 4 for Suspicious Transactions,
5 for Meeting Summary), full frontend production build, vitest — all
clean. `community-settings`, `feature-flags`, and `platform-admins`
were flagged by the same sweep and individually audited to close this
out: each already has a real, correct backend service-layer check
(Community Admin-only branding/workflow settings; Platform Admin-only
feature flags and admin management, both enforced in the service
layer via `is_platform_admin`), and each page's nav link is already
restricted to that same single role — the "multiple roles share nav
access but only some can act" pattern that caused every other fix
this session simply doesn't apply where only one role can reach the
page at all.

## 109. Traditional Leader report crash fix, member wallet feature, family expenses link, and role-distinctive dashboard redesigns

A large, multi-part session working through several concrete complaints
in one pass.

**Reports crash fixed**: a screenshot showed Traditional Leader's
/reports page failing to load entirely. Root cause: an earlier privacy
fix changed the backend's Outstanding Members response shape for this
role specifically (aggregate-only, no individual names — that part was
correct) but the frontend was never updated to handle it, doing
`data.members.map(...)` unconditionally. Fixed to handle both shapes.

**Family data isolation**: investigated thoroughly rather than assumed
fine — traced every access pattern (search, list, and direct-by-ID
lookup) and confirmed the backend already refuses to let a Family Head
widen their view to another family's members under any of them. No
changes needed.

**Collector name on receipts**: a real bug — the receipt generator read
`payment.collected_by.get_full_name()` (the shared login account's
often-blank Django name) instead of the actual `collector_name` field
built specifically for this. Fixed at the one shared source function,
covering PDF, thermal print, and Bluetooth receipts at once. Two new
tests confirm the real name now appears correctly.

**Member wallet feature — built end to end**: "If he doesn't get
change, the balance should be credited to the member's wallet." New
`MemberWallet`/`WalletTransaction` models (a real, auditable ledger, not
just a number), `record_payment` extended with a
`credit_overpayment_to_wallet` flag that caps what's applied to the
obligation at exactly what's owed and credits the real excess, and
`apply_wallet_to_obligation` for spending that credit against a
genuinely different, future obligation later. Two new API endpoints,
13 backend tests, and a real frontend UI: a "no change, credit to
wallet" checkbox that only appears once an entered amount exceeds the
balance, and an "Apply credit" button when a member already has a
balance. Also fixed a related bug surfaced while building this: the
amount field had a hard `max` that silently blocked entering any
overpayment at all.

**Family Expenses nav link**: family officers had no direct way to
reach their own family's expense management, which already existed
behind a dynamic `/family-fund/{familyId}` URL a static nav link can't
point to without knowing the family in advance. Built a small resolver
page at `/my-family-expenses` that finds the signed-in officer's own
family (the same `linked_member_id` detection used elsewhere) and
lands them there directly, never exposing a family picker.

**Dashboard redesign — five of the platform's distinct executive
dashboards restructured**, not just relabeled, each around what that
role actually does:
- Community Operations: a triage-desk structure — pending approvals
  lead the page in a banner, ahead of any static number, since this is
  the busiest, most reactive role.
- Financial Oversight: a genuine ledger-book spread — money in and
  money out side by side as a real two-column ledger.
- Today's Collections: the "Today" section now reads like an actual
  receipt tape, since this is literally a running count of cash in a
  pocket.
- Community Oversight (Chief): rebuilt as a written chronicle — numbers
  woven into flowing prose the way a real briefing would state them,
  since this role is explicitly for reading, not operating. This is the
  most structurally different of all six.
- Your Family's Affairs: the KPI grid replaced with a ledger-page
  summary line, consistent with the "House of [family name]" chapter
  framing already established.

Notification Officer's dispatch-board framing was already reasonably
distinct and left alone.

While redesigning the Financial dashboard, running the full test suite
(not just the build) caught a real issue before it shipped: an existing
test exists specifically to prevent a previously-fixed crash (these
roles never receive a `gift_cash` field in their real data). The
redesign changed a label the test checked for; traced through to
confirm the new code still handles the real data shape correctly, then
updated the assertion rather than just chasing a green checkmark.

Also raised directly rather than silently acted on: a request that
executives' personal accounts get "the donation button" like regular
members conflicts with a structural, deliberate rule verified multiple
times in this project (executives can never be registered as donation
recipients) — flagged for the person to confirm rather than either
ignoring the request or shipping a permanently-empty button.

130 backend tests re-verified across `dashboard`, the new wallet suite,
and `ai_features`, full frontend production build, and vitest — all
clean.

## 110. Deceased Rep nomination integrated into funeral opening, expanded analytics, and a corrected donation-eligibility rule

**Donation eligibility corrected**, superseding the previous session's
change: only the Deceased Rep is now excluded from receiving donations
— not every executive. "Community members and executives with a
personal dashboard... are set to receive donations, since their
personal dashboard is the same as the community member accounts." The
old test enforcing the previous rule was replaced with two tests
proving the new one, not just patched to pass.

**Collector edit access**: a Collector can now request a payment
reversal, but deliberately scoped to only a payment they themselves
collected — never blanket authority over anyone's entries
community-wide. Still goes through the same two-person approval as
every other reversal.

**Deceased Rep's analytics dashboard expanded** to actually surface
data the backend was already computing but the frontend never showed:
collection progress, a full contributions/gift-cash/expenses
breakdown per funeral, and their own family's member-by-member
compliance with real amounts owed. Kept the deliberately quiet, formal
-statement tone already established for this page.

**"Deceased Rep" relabeling**: found and fixed every user-facing
occurrence, including four separate places that were auto-generating
the display label directly from the raw role value (`bereaved_rep` →
"bereaved rep") — sidebar, profile page, audit log, and messaging all
had this same pattern. Built one small, shared `formatRole` helper
rather than patching each individually. The underlying role value
stays `bereaved_rep` everywhere else (database, permissions, API) —
only what's displayed changed.

**Rep nomination integrated into the funeral opening request itself**:
"the ledger opening has to ask for that person to be nominated." A
Family Head can now nominate a Deceased Rep in the same request that
opens the ledger — both succeed or fail together in one transaction.
Deliberately kept consistent with the existing two-party oversight
pattern rather than granting instant authority: a Family Head's
nomination always starts pending, "submitted to the committee" (a
Community Admin/Secretary/Chairman still has to approve it), the same
as when a Secretary or Chairman creates one. Scoped so a Family Head
can only ever nominate for their own family. Nomination stays
optional — a funeral can still be requested without one, using the
separate Families-page flow later. A real bug was caught and fixed
during testing: the new family-head-scope check didn't handle
`actor=None`, which the existing, unrelated direct-creation code path
legitimately allows.

191 backend tests re-verified across `funerals` and `families`, full
frontend production build, and vitest — all clean.

## 111. Role-based analytics for every executive role, matched to actual authority — family and community levels

A large batch built around one principle, applied consistently:
analytics depth should match what a role can actually DO, not be the
same data relabeled three or four times over. Traced each role's real
authority directly in the codebase before writing any dashboard code,
rather than guessing.

**Multi-funeral payment efficiency** (from the previous session,
documented here): a new "Pay all at once" panel on Front Desk settles
every outstanding obligation across every active funeral in one
submission. The real design problem: this platform already refuses a
newer funeral's payment if an older one is still unpaid. Solved by
always processing oldest funeral first within the batch, so each
payment clears the way for the next rather than fighting the same
rule from within its own batch. A misleading, never-triggerable
wallet-overpayment parameter was caught and removed while building
this, rather than shipped as dead code that looked functional.

**Family-level differentiation**: Family Head, Secretary, and
Treasurer used to get byte-identical dashboard data. Traced the real
authority split already enforced elsewhere on this platform (Head and
Treasurer can both approve a family expense; Secretary can only
record one) and matched analytics to it: Head gets the combined
financial picture and a real approval queue; Treasurer gets that plus
genuinely deeper detail (fund-by-fund breakdown, full
approved/pending/rejected split); Secretary gets their own
record-keeping history instead of an approval queue they have no
authority over. The approve/reject buttons are real and functional,
reusing the same backend endpoint and mutation hook the Family Fund
page itself already used and had tested.

**Family Head task oversight**: found `tasks_assigned_by_family_head`
already existed in tasks/services.py, with a docstring explicitly
describing it as "a Family Head's own oversight view" — built but
never wired into anything. Now a real panel: status counts, an
overdue count, and a working approval queue (Approve inline; Reject
links to the full Tasks page, since a rejection needs a written note).

**Community-level task oversight**: the same principle applied
upward. Community Admin, Chairman, Secretary, and Traditional Leader
(who also gained task-assignment authority in an earlier batch) now
see real, aggregate task data — woven into the Chief's existing
chronicle-style prose rather than a tile grid, staying consistent with
that page's established tone.

**Financial officer role differentiation**: Treasurer, Financial
Secretary, and Auditor also used to get identical data. Treasurer and
Financial Secretary can both genuinely request a payment reversal
(funerals.services.REVERSAL_REQUEST_ROLES), so both now get the real,
actionable list, not just a count. Auditor can never request or
approve one — their dashboard reflects their actual authority instead:
a suspicious-transaction review summary and reversal history to
examine, not a to-do list of actions they're not authorized to take.

**A real bug caught by the test suite, not just the build**: adding
the new, required `task_summary` field broke an existing test whose
whole purpose is preventing a previously-fixed crash for these exact
roles. The mock data predated the new field, but more importantly this
exposed that the new UI code wasn't defensive enough — a missing
field would have crashed the entire page rather than degrading
gracefully. Fixed both the test data and the frontend code itself.

122 backend tests added across three new test files, full backend
regression sweep (dashboard, tasks, family_funds — 178+ tests), full
frontend production build, and vitest — all clean.

## 112. Member sorting/filtering, PDF/CSV export, and bulk CSV upload — plus a real DRF gotcha tracked down and fixed

**Member sort and filter**: `search_members` extended with a gender
filter and a `sort_by` option (name, family, age either direction,
gender, status, membership number) — the existing, already-correct
family-scoping for Family Head/Secretary/Treasurer left completely
untouched, since this only adds new filter dimensions on top of it.

**Export to PDF and CSV**: "all data should be downloaded or
printable." A new endpoint reuses `search_members` directly rather
than a separate query, so an exported list can never show more than
the same person would already see on the Members page — the
family-scoping is enforced once, not reimplemented. CSV avoids adding
a new binary-format dependency (no `openpyxl`/`xlsxwriter` in this
project); it opens directly in Excel, which is what the request
actually needed.

**A genuine, hard-won debugging session**: building the HTTP-level
tests for this feature, every single one failed with a mysterious 404
— the URL resolved correctly in isolation, the view was provably
correct, but requests through the real authentication flow kept
failing before even reaching the view. Traced this by systematically
eliminating variables — bypassing JWT authentication entirely with
`force_authenticate` (worked), testing with and without the query
string (isolated it), adding print statements inside Django's actual
authentication internals — down to the exact cause: the query
parameter was named `?format=csv`, which collides with a name DRF
itself reserves internally for content negotiation. Since no renderer
was registered for "csv", DRF was silently failing content negotiation
and returning 404 before authentication or the view ever ran. Renamed
to `?export_format=`. This was a genuine DRF gotcha, not a mistake in
the endpoint's actual logic — but it would have been very easy to ship
broken without ever knowing why.

**Bulk CSV upload**: "executive should have access to upload data when
necessary." Each row goes through the exact same `register_member`
every single registration on this platform already uses — the same
duplicate detection, the same Family Head/Secretary
own-family-only restriction. A dedicated test proves a Family Head's
upload lands in their own family even when the CSV explicitly names a
different one — the existing restriction inside `register_member`
wins regardless. One bad row never aborts the whole batch; each row
succeeds or fails independently, reported by row number.

229 backend tests re-verified (`members`, `reports`, `accounts`), full
frontend production build, and vitest — all clean.

## 113. Platform Admin user management, a granular capability system, and a full multi-tenant data-isolation audit

**"User Management"**: the existing Platform Administrators page
expanded rather than replaced, with a genuinely new Community
Administrators section spanning every community in one view.
`list_community_admins` and its view already existed but were only
reachable by expanding one community at a time on the Communities
page — `list_all_community_admins` is the one-query, actually
-discoverable version a real cross-platform user management screen
needs. Deliberately never a third section for ordinary community
members — that stays each Community Admin's own responsibility.

**"System Settings" — a real, granular capability system for Platform
Admin accounts**: "system settings should provide more option that
will restrict or give access to the platform admin based on what they
can do." A new `platform_admin_capabilities` JSON field, five
capabilities (communities, access periods, community admins, other
platform admins, feature flags) each independently toggleable, wired
into every actual endpoint performing that action — not a flag that
merely exists. Two deliberate safety properties, both directly tested:
defaults are permissive (a capability never explicitly set means
"allowed," so no existing account silently loses access the moment
this field starts existing), and a Platform Admin can never modify
their own capabilities, only another's — without that, an accidental
self-restriction of `manage_platform_admins` would be a genuine,
permanent lockout with no path back in.

**A full multi-tenant data-isolation audit**, prompted by "each
community should have their own database... managed by the platform
admin but the community admin should have access, should be like a
SaaS platform." Rather than attempt a literal separate-database-per
-tenant migration — a large, high-risk architectural change for a
system this mature, deliberately not attempted without a direct
conversation first — audited whether the *logical* isolation this
platform already relies on is actually solid. Checked every one of
the 99 direct object lookups (`get_object_or_404`) across all 14
backend apps and every one of the 8 ViewSet `get_queryset()` methods
platform-wide. Every single one is properly scoped to the requester's
own community. Where something looked unscoped at first glance (a
messaging Channel lookup, a support ticket lookup, an audit log
community lookup), traced it through to the actual service-layer
authorization function doing the real check, rather than trusting a
comment — each one held up correctly under direct verification.
`families/views.py` already documents this itself: "HARD tenant
boundary... this line is what makes 'no community can access another
community's data' true at the database-query level." The audit
confirms that claim is accurate, not just asserted.

250 backend tests re-verified (`tenants`, `accounts`), full frontend
production build, and vitest — all clean.

## 114. User Management & System Settings, extended down two more tiers — Community Admin over their community's executives, Family Head over their own family

The exact same pattern from Platform Admin's capability system,
applied one and two tiers down, sharing one authorization function
rather than three separate, potentially-drifting implementations.

**`can_restrict_features_for(actor, target)`** — the single rule
deciding who can restrict whom, genuinely different per tier: a
Community Admin can restrict any executive role in their own
community (Family Head included) — never a peer Community Admin,
never an ordinary member, never across communities, never themselves.
A Family Head can restrict their own family's Secretary/Treasurer and
ordinary members — never a different family's people, never a
community-wide executive, never themselves. `list_manageable_users`
and `set_disabled_features` both call this exact same function, so the
list of who's shown as manageable and who's actually allowed to be
restricted can never disagree with each other.

Deliberately a single, generic `disabled_features` list per account
(the Sidebar's own nav hrefs) rather than a role-specific capability
dict like Platform Admin's — the same mechanism has to serve both a
Community Admin restricting any executive role and a Family Head
restricting their own officers or members. Can only ever take a
feature away from what a role already grants, never add one beyond
it; an empty list (the default) means no restriction — the same
permissive-by-default principle already used for Platform Admin
capabilities.

One unified "User Management & System Settings" page serves both
tiers — the backend's manageable-users endpoint already decides what's
manageable purely from the caller's own role, so a Community Admin and
a Family Head see the same UI with different data underneath, not two
parallel pages that could drift apart.

The real enforcement point: the Sidebar's own nav filter now checks
`disabled_features` directly, on top of the existing role-based
filter — a restricted account genuinely stops seeing that link, not
just a setting that exists without effect.

20 new backend tests, all passing on the first run including the two
most important ones — a Community Admin genuinely cannot reach a
different community's executive, and a Family Head genuinely cannot
reach a different family's officer — full regression sweep on
`accounts` (110 tests), full frontend production build, and vitest —
all clean.

## 115. The Town Elders ledger system — a genuinely separate ledger, not just a different rate

Built to fully satisfy "the community ledger is for all community
members excluding town elders who have their own ledger and they pay
higher than all the member." Extended an existing but partial
foundation (`Member.is_town_leader`, `FuneralEvent.town_leader_amount`)
rather than a parallel data model — safer, and genuinely achieves the
separation:

- `Member.town_elder_title` (Chief, Queen Mother, Linguist, Other Town
  Executive) — "they should also be registered as town elders."
- `ContributionObligation.RateType.TOWN_ELDER` — a real, distinct
  third rate type, no longer folded into "general" the way it used to
  be. Every report already filtered by `rate_type="general"`
  specifically (never "general OR town_elder") now naturally excludes
  Town Elders, which is what actually achieves "their own ledger" —
  confirmed with a dedicated regression test that a Town Elder who
  happens to belong to a family is still correctly excluded from that
  family's own ledger too.
- `transfer_to_town_elder()` — Community Admin/Chairman/Secretary
  only, matching "community admin or community executive."
- `set_town_elder_rate()` — Traditional Leader only, a deliberately
  separate authority from the community-wide rate-setters, matching
  "the king sets the price... the town leader/king is the head of the
  elders' ledger."
- A dedicated Town Elders Ledger report and page, with one deliberate,
  narrow privacy exception: Traditional Leader gets full member-level
  detail specifically here (they're running it), while staying
  aggregate-only everywhere else on the platform per the existing
  rule — a carve-out, not a general loosening.

A real bug surfaced by writing genuine HTTP-level tests rather than
only calling service functions directly: a `NameError` from an
unimported `status` module that unit tests alone would never have
caught, since it only fires on a real request through the actual view.

A separate, subtler bug caught in the test-writing process itself: a
newly-registered family member auto-enrolls into any already-open
funeral immediately, before a same-turn Town Elder transfer takes
effect (transfers are deliberately never retroactive) — fixed by
tracing the actual enrollment timing rather than assuming the test's
own logic was right.

37 new backend tests (21 + 5 HTTP-level + regression additions), full
regression sweep across `members`, `reports`, `contribution_rules`,
and `accounts` (288 tests), full frontend production build, and
vitest — all clean.

## 116. Four-category Collector system with multi-party approval, plus Family Head's narrower restriction scope and a verified executive/billing separation

**Family Head's system-settings scope narrowed**: "the abusua can't
assign work that was supposed to be assigned by the community
executive or admin — the abusua in his system settings should be
restricted to what he can tick or not." A Family Head's own
restrictable-feature set is now genuinely smaller than a Community
Admin's — `/tasks` is deliberately excluded, since gating who can even
see the Tasks feature is community-wide workforce management, not a
family-level decision (a Family Head keeps their own, unrelated
authority to assign tasks WITHIN their family, built earlier —
this is specifically about restricting others' visibility, a
different kind of authority).

**Executive-dashboard billing separation, verified rather than
assumed**: "no executive dashboard should be billed... those
dashboards are special." Traced this directly rather than trusting it
was already true — confirmed every funeral obligation is recorded
against a Member, never a User, with a dedicated test proving that
deleting an executive's login entirely leaves their billing history
completely untouched.

**A genuine four-category Collector system, each with its own creator
and its own approvers** — the biggest piece. General (community
ledger; created by Chairman/Community Admin; needs the Treasurer plus
either the Secretary or Chairman), Family (family ledger; created by
the Family Head; needs that family's own Treasurer AND Secretary),
Donation (gift ledger, kept separate from mandatory funeral
contributions; same family-officer approval as Family), and Town
Elders (created by the Traditional Leader; needs two OTHER community
executives). A nomination is genuinely inert — the member's login role
never changes to "collector" — until every required approval is in;
a single rejection ends it outright, and the same approver can never
vote twice on the same nomination.

A real design bug caught by writing the tests, not found by
inspection: Town Elder's two approval slots share the exact same
eligible-role set, and the first version of the matching logic let
one approver satisfy both slots by role-overlap alone — silently
turning "two other executives" into "one." Fixed with genuine
distinct-approver matching instead of role-set overlap.

21 new backend tests (service-level and full HTTP round-trip), full
regression sweep across `members`, `accounts`, and `funerals` (143
tests), full frontend production build, and vitest — all clean.

## 117. Daily combined ledger totals, the "no double payment" rule, and a per-funeral Town Elder rate override

**Daily combined Town Elders + Community ledger total**: "after each
day of every funeral the system should be able to calculate all money
received from the town elders ledger, the community ledger and sum
them together." `funeral_daily_breakdown` now breaks out both figures
separately per day plus their combined sum, plus a running grand
total for the whole funeral. Deliberately excludes the family's own
ledger from this specific combined figure — that's a different,
already-separate thing shown elsewhere on this platform.

**The "no double payment" rule — a real gap found and closed, with a
deliberate, explicit behavior change**: "one person is not allowed to
pay twice of each funeral, but can decide to pay more than the
required amount." Nothing previously stopped a second, separate
payment against an obligation that was already fully settled. Adding
this guard directly contradicted an EXISTING test that verified the
old behavior (a "further generous payment" feature from an earlier
session) — treated the user's current, explicit instruction as
superseding that earlier design rather than silently keeping both,
and rewrote the test to prove the new rule while adding a companion
test confirming overpaying within a SINGLE payment still works fine,
and another confirming multiple genuine partial installments are
unaffected.

**A per-funeral Town Elder rate override**: "when a king or queen or
anyone from the town elders dies, the community price can be set
again." `create_funeral_event`/`request_funeral_event` now accept an
explicit `town_leader_amount` for one funeral only, without touching
the community's standing default — the same "supply an amount for
this funeral only" flexibility `own_family_amount` already had, wired
through both service functions and both API serializers.

**Re-confirmed, not just assumed**: Town Elders from different
families genuinely pay the identical rate; a member correctly pays
the own-family rate on their own family's simultaneous funeral and
the general rate on a different family's, and the earlier batch-pay
feature genuinely settles both in one action — each backed by its own
test rather than left as an assumption.

11 new backend tests, full regression sweep across `funerals` and
`reports` (234 tests), full frontend production build, and vitest —
all clean.

## 118. Family Head's User Management gets role-assignment options, and both toggle interfaces get optimistic saves

**Family Head can now assign all four family roles from one place**:
"let some act as treasurer, family contribution and donation
collector, and treasurer, secretary as well." Consolidates what
previously required visiting two different pages — officer
appointment (Family Fund page) and collector nomination (Collector
Nominations page) — into the same "manage my family" experience on
User Management. Deliberately reuses both already-tested actions
rather than duplicating either: Secretary/Treasurer take effect
immediately via the existing `assignOfficer` action; Family/Donation
Collector go through the same nomination-and-nothing-collects-until
-approved workflow every other collector on this platform requires —
this panel starts that nomination, it never bypasses it. The backend's
`ManageableUsersView` now resolves the Head's own family once and
returns it, so the frontend needs no second round trip.

**Both feature-restriction toggle interfaces now save optimistically**:
the checkbox reflects its new state the instant it's clicked, not
after a full round trip, with genuine rollback if the save actually
fails — applied consistently to both Platform Admin's System Settings
and the shared Community Admin/Family Head User Management page. The
global "disable every checkbox while any one is saving" behavior is
gone now that each toggle already shows its own result instantly.

2 new backend tests, full regression sweep across `accounts` and
`members` (45 tests), full frontend production build, and vitest —
all clean.

## 119. Payment transparency dashboards, QR receipt verification, and Traditional Leader gains Town Elders ledger management

A large batch, rebuilt in full after a mid-session sandbox reset wiped
the working directory and installed dependencies — rebuilt
methodically from the last delivered zip rather than guessed at, with
every piece re-verified by an actual test run, not assumed correct
because it had been built once already.

**Payment transparency**: "make it transparent to the community
treasurer to have data of those who have paid... collectors should
also have a place where they can see those who have paid and who have
not... each treasurer should have access to data of those who have
paid in his family and who haven't." A new `members_payment_status_report`
function returns both sides — paid and outstanding — in one call,
optionally scoped to a single family. Wired into three dashboards:
Community Treasurer/Financial Secretary (community-wide), Collector
(community-wide, capped at 20), and Family Treasurer (scoped to their
own family only).

**QR receipt verification**: "all receipt printed should have the QR
scanner so when they scan it should confirm the amount they paid and
who received the pay." `ContributionPayment.qr_payload` — a real,
scannable URL, the same pattern already used for membership cards —
plus a `verify_receipt` function and a login-protected verification
page it lands on, confirming amount, collector, payer, and funeral.
The QR code is embedded directly into the printed receipt PDF itself
(not just shown separately on a web page), verified by actually
generating a PDF and confirming it's valid, not just checking the
code runs without an exception.

**Confirmed, not assumed**: no endpoint anywhere in this codebase lets
a recorded payment be edited directly — only the existing reversal
workflow can change one. A structural test proves this by searching
the actual view source for any such class.

**Traditional Leader can now manage Town Elders ledger membership**:
"the town leader should also have user management and system settings
to manage the town elders ledger." The chief joins Community
Admin/Chairman/Secretary as authorized to both add members to the
Town Elders ledger and — new — remove them, via a symmetric
`remove_from_town_elder` function that respects the same
never-retroactive rule every other rate change on this platform
follows. The ledger page itself now doubles as this management
surface, with a Remove action next to each elder.

47 new/rebuilt backend tests, full regression sweep across `members`,
`reports`, and `dashboard` (320 tests total, run in verified,
complete batches after an earlier attempt silently truncated), full
frontend production build, and vitest — all clean.

## 120. Sidebar navigation grouped into sections — a genuine usability fix, not a cosmetic one

"Make sure all the user dashboard navigation is easier to use and
should be user friendly." A concrete, verifiable problem: 35 nav links
had accumulated in one long, flat, ungrouped list over the course of
this project — a role like Community Admin ends up with 15+ visible
links at once, genuinely hard to scan.

Every link now declares a `section` (Platform, Funerals & Collections,
Finance & Oversight, Administration, My Space, or none for Dashboard
itself), and the sidebar renders a small header above each group —
but only for sections that actually have a visible link for the
current account, so a role with just one or two links from a section
never sees an empty, pointless header. This was a reorganization of
existing links, not new ones: every link's own `roles` array was
double-checked against what it was before the change (the trickiest
ones — User Management, Town Elders Ledger, Collector Nominations —
verified explicitly) to make sure regrouping never silently changed
who can see what.

Full frontend production build, vitest, and the existing
feature-restriction test suite (which reads these exact hrefs) — all
clean.

## 121. A complete, full-platform regression sweep — every Django app, individually verified, two real bugs caught and fixed

Prompted by nothing more specific than "continue" — used the opening
to do something genuinely valuable given how much has accumulated
across this project: run every single Django app's test suite, one
at a time (or in small groups), each confirmed with a complete "Ran N
tests / OK" result rather than trusting a combined run that could
silently truncate. 1,136 tests total, across all 21 apps.

**Two real bugs found, both the same root cause**: `record_payment`
requires a non-empty `collector_name` — the right rule for a human
collector at a front desk — but two call sites elsewhere in the
codebase were never updated when that requirement was added:

- **A genuine production bug**: the Mobile Money self-service payment
  flow (confirmed automatically by a webhook or a poll, with no human
  collector involved at all) was silently broken — every MoMo
  contribution payment would have failed outright. Fixed with an
  honest, explicit label, `"Mobile Money (self-service)"`, rather than
  fabricating a person's name.
- **A test-only bug**: three `record_payment` calls inside the
  real-time WebSocket broadcast tests were missing the same field —
  fixed by adding a realistic collector name, matching the convention
  used everywhere else in this codebase's tests.

Every app re-verified individually after each fix, plus a full
frontend production build and vitest — all clean. Full tally by app:
accounts 114, tenants 160, contribution_rules 18, families 45,
family_funds 38, funeral_logistics 35, funerals 159, gifts 63, members
123, ai_features+audit_log+communication+messaging 80,
notifications+payments+support+tasks 72, welfare+realtime 32,
dashboard 112, reports 85.

## 122. A real, held wallet for every ledger — community, family, and Town Elders — with Paystack self-service deposits

The biggest architectural addition to this platform: "members can have
their own wallet where they can deposit money and can use to pay for
their contribution... each family should have their wallet being
managed by the family treasurer... the town elder should also have
their wallet as well."

**Design decision made and held to**: a genuinely new `LedgerWallet`
model — deliberately never folded into the existing `FamilyFund`,
which is explicitly documented as private, voluntary family savings,
structurally isolated from every community ledger. Mixing mandatory
funeral-contribution money into it would have been exactly the
confusion this feature exists to avoid. One unified model (not three
parallel ones) scoped by `rate_type` (general / own_family /
town_elder) — matching `ContributionObligation.RateType` exactly, so
routing is never a second, separate decision that could drift out of
sync.

**Automatic routing wired directly into `record_payment`**: every
contribution payment now credits the matching wallet the instant it's
recorded, with a full, auditable `LedgerWalletTransaction` log
(traceable back to its source payment) — never just an incremented
number. A real gap was caught and fixed along the way:
`apply_wallet_to_obligation` (spending a pre-loaded member wallet
balance) originally bypassed this routing entirely, meaning
wallet-settled payments would never have reached any ledger wallet —
found by writing the "deposit now, spend later" test, not by
inspection.

**Paystack wallet top-ups**: `initiate_momo_wallet_deposit` lets a
member charge their own phone directly to top up their personal
`MemberWallet`, completing a `MomoPaymentRequest.TargetType.WALLET_DEPOSIT`
extension that existed only in a docstring and unfinished `TargetType`
choice before this batch — finished properly, migrated, and verified
that confirming the same deposit twice (a realistic webhook-plus-poll
race) never double-credits the wallet.

**A real, existing performance guardrail legitimately exceeded and
honestly adjusted**: adding wallet crediting pushed `record_payment`'s
query count past an existing bounded-queries test. Verified directly
(not assumed) that the new cost is fixed and doesn't scale with
member count before raising the threshold, with an honest comment
explaining exactly why it changed.

24 new backend tests across the wallet routing, the Paystack deposit
flow, and the idempotency guarantee — all clean, alongside a full
regression sweep of `funerals` (183) and `payments` (26).

## 123. The Reports page crash — found, fixed, and rigorously proven

A real bug report, investigated directly rather than guessed at:
"the report feature wasn't working." Root cause: the backend
deliberately omits `gift_cash` from collections reports for Treasurer,
Chairman, Secretary, Financial Secretary, and Auditor ("the funeral
committee should have access to all the money paid except the
donations" — an intentional, documented design decision), but
`CollectionsReportCard` on `/reports` read `report.gift_cash.count`
unconditionally, crashing the whole page for exactly the roles that
use it most. The same bug *class* had already been fixed on the
dashboard page — this was a second, separate instance on a different
page that was missed.

Fixed by rendering the Gift Cash section only when the field is
actually present, matching what the type already documented. Proven
rigorously, not just asserted: temporarily reverted the fix, confirmed
the new test genuinely fails with the exact original crash pointing
at the exact broken line, then restored the fix and confirmed all
tests pass.

## 124. Traditional Leader gets a full "manage my Town Elders" experience

"The chief should also have user management and system settings
features to manage town elders, including adding town elders and the
town elders' executive, and assigning tasks to them." Extended the
same `can_restrict_features_for`/`list_manageable_users` authority
system Community Admin and Family Head already use — scoped to
`Member.is_town_leader`, not to any particular `User.role`, since a
Town Elder can otherwise hold any ordinary account type. The Chief's
own restrictable-feature set is narrowed to just the Town Elders
Ledger itself, the same "domain-scoped, not community-wide" principle
already established for Family Head.

On the User Management page itself: a Chief-only panel to add new
Town Elders and their executives (a title itself is what makes someone
part of this group's "executive" — Chief, Queen Mother, and Linguist
are all just as much this panel's concern as an ordinary elder), and
to assign a task directly to any current elder — reusing the
platform's existing task-assignment action rather than building a
parallel one.

6 new backend tests, full regression sweep across `accounts` (120),
full frontend production build, and vitest — all clean.

## 125. Ledger Wallets get a frontend, the withdrawal system was found already complete, and members can now self-service deposit

**A significant discovery before building anything new**: investigating
what was needed for wallet withdrawals turned up a complete, already
-working system from earlier work — `withdraw_from_ledger_wallet`
with correct role-based authorization (including family-scoping),
`ledger_wallets_for` for access scoping, full views/URLs, and 24
passing tests. Rather than build a second, competing approval-workflow
design on top of it, removed the redundant model already in progress
and verified the existing system directly.

**The first frontend for the entire Ledger Wallet feature**: a new
`/ledger-wallets` page, role-aware without needing to know the role
itself — it simply renders whatever `ledger_wallets_for` returns
(the community wallet for Treasurer, only their own family's for a
Family Treasurer, the Town Elders wallet for the Chief), each with
full transaction history and a working withdrawal form.

**A real gap closed**: `initiate_momo_wallet_deposit` (the Paystack
self-service top-up) had a tested service function but no view or URL
at all — completely unreachable over HTTP. Built the missing
serializer, view, and URL, verified with a full HTTP round-trip test.

**Members can now actually deposit**: a "My Wallet" section on the My
Receipts page — balance, and a Mobile Money top-up form reusing the
exact same polling infrastructure (`usePollMomoStatus`) every other
Mobile Money charge on this platform already depends on, rather than
a second, parallel implementation.

**Investigated and correctly left alone**: whether any authenticated
community member being able to view another member's wallet balance
was a privacy gap. Traced the actual permission logic rather than
assuming — confirmed this matches the platform's own deliberate,
already-established design ("every member of the community can
already see the roster" — read-only member data is community-wide by
design; only edits are restricted). Not a bug, and changing it would
have been an unrequested, incorrect scope-creep.

7 new/verified backend tests, full regression sweep across `members`
and `payments` (151 tests), full frontend production build, and
vitest — all clean.

## 126. Funeral contribution rules gap analysis, and Town Elders get genuine per-title and per-individual rates

A formal specification document ("real-world funeral contribution
rules") arrived with an explicit instruction: study the existing
system first, map its six ledger concepts (Community, Family, Town
Elders, Guest, In-Law, Asupedeɛ) against what's actually built, and
identify gaps before writing any code — never redesign what already
works.

**The investigation (not from memory — traced directly against the
code)**:
- **Already fully satisfied**: Community Ledger, the family exemption
  (a family member never gets both a general AND an own_family
  obligation), the family tier hierarchy (Head/senior "uncle"/junior
  "nephew"/woman), minimum-not-cap on every ledger, multi-payment
  traceability, and the core of the Guest Ledger (`GiftDonation` with
  a `donor_category="guest"`, hometown, and which relative they're
  attending for).
- **Real, confirmed gaps**: Town Elders all paying one identical flat
  rate regardless of title — a direct violation of the document's own
  stated rule; no delegation/headcount field for group guest gifts;
  In-Law Contribution and Asupedeɛ genuinely don't exist anywhere in
  the codebase (In-Law appears only as a free-text example string in
  a code comment).
- **Flagged rather than silently decided**: whether to expand the
  4-tier family hierarchy into the document's named categories
  (Grandparents/Uncles/Nephews) — the current system already satisfies
  the functional requirement (different positions, different amounts,
  seniority-aware) with coarser labels; expanding the taxonomy is a
  real design choice with migration implications, not a clear bug.

**Town Elders per-title and per-individual rates — built and verified**:
the clearest, most direct gap. Added nullable per-title rate fields
(Chief/Queen Mother/Linguist/Other) on both `Community` (the
configurable default) and `FuneralEvent` (snapshotted at creation,
matching every other rate field's own pattern) — additive and
non-breaking, so a community that never configures these keeps its
existing flat-rate behavior exactly as before. Added a standing,
per-individual override field on `Member` for "Elder A pays a
genuinely different amount than Elder B even with the same title,"
which takes priority over the per-title rate, which in turn takes
priority over the flat fallback.

10 new tests, full regression sweep across `funerals` (193), `members`
(123), and `tenants` (160) — all clean.

## 127. Asupedeɛ — a genuinely new, genuinely separate contribution ledger

The second gap from the funeral contribution rules document, and the
first entirely-new ledger built from scratch. "Do NOT simply add
Asupedeɛ to the normal Family Ledger... it must remain identifiable...
because it has a different purpose, collection period, collection
process, financial classification, reporting category."

**A real architectural decision made deliberately, not by default**:
Asupedeɛ needed to be *additive* — a member can owe both their
ordinary family/general/town-elder contribution AND Asupedeɛ on the
same funeral simultaneously. `ContributionObligation` has a hard
`one_obligation_per_member_per_funeral` constraint that hundreds of
existing `.get(funeral_event=, member=)` lookups across this entire
codebase depend on — reusing it as a fourth `rate_type` would have
meant either breaking that constraint (a serious risk to code that
already works) or silently conflating two obligations that must stay
independently traceable. Built as its own model instead —
`AsupedeObligation`/`AsupedePayment` — the same precedent
`gifts.GiftDonation` already established for a genuinely different
ledger type.

**What's built**: `activate_asupede()` — a deliberate, explicit
activation step (not automatic for every funeral, since the exact
burial-morning timing is often only confirmed well after a funeral's
ledger first opens) that sets the amount and an optional collection
window and generates one obligation per eligible member, reusing the
platform's existing, already-configurable `eligible_members_queryset`
rather than inventing a second eligibility concept.
`record_asupede_payment()` mirrors the mandatory ledger's own rules
exactly (minimum-not-cap, required collector name, no second payment
once settled, full idempotency) plus the one genuinely new rule this
ledger needs: a payment outside the configured collection window is
rejected outright — with no window configured, it's payable any time,
never an invented default time (like "6am-10am") that was never
actually specified.

**Wired into the Ledger Wallet system, with three real gaps caught
and fixed along the way**: Asupedeɛ payments route into their own,
distinct wallet scope (family-scoped, since the money exists to
support the deceased's family) — never folded into that family's
ordinary contribution wallet. Extending `LedgerWallet` for a fourth
scope surfaced three places that only ever checked for the exact
`FAMILY` scope and would have silently missed the new one: the
model's own `CheckConstraint` (which family-scoped wallet gets a
`family` FK), the cross-family withdrawal leakage guard, and
`ledger_wallets_for`'s own family-branch query. All three fixed and
covered by dedicated tests before considering this feature complete.

19 new tests, full regression sweep across `funerals` (212) and
`tenants` (160) — all clean.

## 128. Age-based mandatory contribution eligibility, with a real leap-year bug caught before it shipped

The last piece from the funeral contribution rules document's
member-registration section: "once a community member gets to 20
years and he's not schooling or an apprentice he has to mandatory pay
for contribution... even if the person is a student or an apprentice
and more than 25 years should still have to pay."

**The rule itself**, added directly into `eligible_members_queryset`
(the same function every funeral's obligation generation already
calls, so this needed no separate integration point): under 20 is
always exempt; 20-24 stays exempt only while genuinely recorded as a
student or apprentice; 25+ is mandatory regardless of occupation
status. A member with no recorded birth date is deliberately never
silently exempted — defaulting to mandatory, since quietly shrinking
an existing community's obligated membership the moment this shipped
would have been its own dangerous assumption.

**A real bug caught before it reached the actual rule**: the cutoff
dates are computed with `date.replace(year=...)`, which raises
`ValueError` on February 29th when the target year isn't a leap year.
A latent crash that would only surface one day in roughly 1,461 —
exactly the kind of bug that's easy to miss and expensive to hit in
production. Added a safe `_years_ago()` helper (falls back to Feb 28)
and used it everywhere a cutoff date is computed, rather than leaving
the raw, unsafe call in code that runs every single day this platform
is in use.

**"Fetch them out" — `members_needing_age_review`**: not a one-time
notification, since both facts it depends on change on their own over
time. Surfaces three genuinely distinct situations for a Family
Head/Secretary to check: a member who just turned 20 (their first year
of possible obligation), a 20-24-year-old currently recorded as
student/apprentice (a time-limited, status-dependent exemption worth
periodic re-verification), and a member who just turned 25 while still
recorded as student/apprentice (about to become obligated regardless).
A separate `members_missing_birth_date` surfaces the different,
data-completeness gap of members this rule can't even evaluate yet.

Precise day-boundary testing caught two sign errors in my own test
helper (not the actual rule) before they were trusted — fixed and
re-verified against the exact day before/after each threshold.

18 new tests, full regression sweep across `contribution_rules` (36),
`members` (123), and `funerals` (220) — all clean.

## 129. Asupedeɛ becomes reachable — the view/URL layer, with two real bugs caught while proving it actually works

Closing the biggest reachability gap from the last several batches:
Asupedeɛ had a complete, tested backend with nothing callable outside
a Django shell. Four new actions on `FuneralEventViewSet` — activate,
list obligations, record a payment, and a summary — reusing the exact
same collecting-role/desk-worker/self-payment authorization the
mandatory ledger's own `record_payment` already enforces, rather than
inventing a separate permission model for this ledger.

**Two real bugs caught by insisting on an actual HTTP round trip,
not by trusting the code looked right**:
- My own test compared a UUID response field against a plain string
  (`o["member"] == str(...)`) — always `False` in Python regardless
  of whether the values matched, since `UUID.__eq__` doesn't compare
  against strings. Fixed by comparing `str(o["member"])` instead.
- My test assumed Community Admin could record a payment directly.
  This platform's actual, existing authorization model restricts
  payment-collecting to the Collector role specifically (plus desk
  workers and self-payment) — Community Admin was never in that set,
  for any ledger. Not a bug in the code; a wrong assumption in the
  test, corrected once the actual permission check made it visible.

Both were caught specifically because every new endpoint got a real
request through it before being called done — exactly the discipline
that's caught similar issues earlier in this project.

3 new HTTP-level tests, full regression sweep across all of
`funerals` — 223 tests, run in verified, complete batches after an
earlier attempt truncated silently — all clean.

## 130. Four more configuration/reporting endpoints reachable — Town Elder per-title rates, family position rates, age review

Continuing to close reachability gaps: Town Elder per-title rate
configuration (Traditional-Leader-only, mirroring `TownElderRateView`'s
own established pattern exactly), family position rate configuration
(Community Admin/Chairman/Secretary, same authority as the existing
family-tier rates), and both age-eligibility reports
(`members_needing_age_review`/`members_missing_birth_date`) all get
real endpoints for the first time.

**A real inconsistency caught by testing every authorization path,
not just the happy one**: the first version let the serializer's own
exception handling convert an authorization failure into a generic
DRF 400. Every other rate-setting endpoint on this platform
(`TownElderRateView` included) deliberately catches that same
`ValidationError` in the view itself and returns 403 — "wrong role"
and "bad input" are different failures with different meanings to a
frontend. Fixed by moving the try/except into the view, matching the
established convention, and caught specifically because a
Community-Admin-cannot-do-this test was written and actually run
before calling this done.

12 new tests, full regression sweep across `contribution_rules` (44)
— all clean.

## 131. In-Law Contribution — the last unstarted ledger, and the only one built backwards on purpose

The final ledger from the funeral contribution rules document, and
architecturally the odd one out among all six: "do NOT automatically
create a financial obligation solely because a relationship exists...
only after approval should the system generate the actual financial
obligation." Every other ledger on this platform auto-enrolls every
eligible member the moment a funeral is created — this one does the
opposite by design.

**Targets a whole family, not a member** — a deliberate, real
architectural decision: the connection this represents is between two
families through a marriage, and the amount is a single, negotiated
figure for that family collectively, not a per-member rate lookup.
`InLawContributionRequest` (pending/approved/rejected) is the request
itself; `InLawObligation` is only ever created the moment a request is
approved, never before — a rejected or still-pending request produces
no financial obligation at all, proven directly by a test that checks
the obligation table stays empty.

**Two distinct, deliberately separate authorities**: the deceased
family's own leadership (or a community-wide executive) initiates the
request; a genuinely different community-wide executive approves it.
A Family Head can never approve their own family's request — "approving
your own request would defeat the point of a separate approval step
entirely" — verified directly rather than assumed.

**Payment mirrors every other ledger's own rules** (minimum-not-cap,
required collector name, no second payment once settled) with one
addition specific to a family-level obligation: `paid_by_member` must
actually belong to the obligated family, checked directly rather than
trusted.

15 new tests, full regression sweep across every funeral-contribution
-rules test file built across this whole thread (79 tests) — all
clean.

## 132. In-Law and Guest delegation verified complete, plus the first genuine frontend for this whole thread

**Verified, not just built**: reran the In-Law contribution HTTP tests
and the full `funerals`/`gifts` regression sweep (241 + 68 tests, in
verified complete batches) after last turn's URL-routing and
authorization-status fixes — confirming both actually hold, not just
assuming they did because the code looked right.

**The first real frontend for anything in this entire funeral
-contribution-rules thread**: rather than build seven separate,
disconnected pages, extended the existing `funeral_full_ledger_breakdown`
report and its already-existing frontend card
(`FourLedgerBreakdownCard`) to show all seven ledgers together — a
genuinely consolidated view matching the original document's own
"funeral contribution overview" request. Added `in_law_summary`
(mirroring `asupede_summary`'s exact shape) so the backend report
actually has something to show; the frontend card now displays Town
Elders' mandatory contribution ledger (which existed in the backend
response already but was never rendered), Asupedeɛ, and In-Law
alongside the original four. Committee-role gift-cash restrictions
are correctly left untouched — that rule is specifically about
voluntary gifts, not mandatory-style contributions, so Asupedeɛ and
In-Law were never candidates for stripping.

5 new backend tests (one catching yet another Decimal-string
formatting assumption in my own test, not a real bug), full
regression sweep across `reports` (89), full frontend production
build, and vitest — all clean.

## 133. Welfare requests and disbursement — the last major gap from the contribution-scope rules document

The most substantial genuinely-missing piece from the contribution
-scope document: "the system should support welfare requests
separately from contributions... Collection -> Verification ->
Approval -> Disbursement -> Recipient acknowledgement -> Audit."
Before this, money could flow INTO a welfare fund (the existing
`ContributionCampaign`/`WelfareObligation` system) but there was no
way for a member to request money OUT of one, and no disbursement
concept existed at all.

**A deliberate design choice**: `WelfareRequest` links to the
existing `ContributionCampaign` rather than inventing a second,
parallel "fund" concept — a campaign's own scope (`family=None` vs
`family` set) already IS the fund's scope, so a request against a
family-scoped campaign is inherently a family-fund request with no
extra field needed. Reuses the exact same isolation/authorization
rule already established for obligations: community-wide leadership
or the owning family's own executives administer requests; a member
sees only their own.

**Approval and disbursement kept as two genuinely separate, auditable
steps** — "do not automatically mark funds as spent simply because a
campaign has collected money." A request can be approved for less
than requested; disbursement can be for less than approved; and a
final, distinct `acknowledge_welfare_disbursement` step requires the
requester themselves (never anyone else) to confirm they actually
received it.

**A real inconsistency caught and fixed during the view layer**: my
first version let `decide`/`disburse` delegate their authorization
check to the service, which meant a genuine state error ("this
request is already decided" — 400-worthy) would incorrectly surface
as the same 403 as an authorization failure. Fixed by checking
authorization explicitly in the view first, matching the pattern
already established for Town Elder rate-setting — verified with a
dedicated test asserting 400, not 403, for the state-error case.

24 new tests (21 service-level, 3 HTTP round-trip), full regression
sweep across all of `welfare` (75 tests) — all clean.

## 134. Town Elders scope and Selected Members targeting, plus a second orphaned-signature bug caught immediately

The remaining two gaps from the contribution-scope rules document:
"the campaign may apply only to Senior family members, family
executives, specific contributors" (§10) and a dedicated Town Elders
scope (§1). Added `ContributionCampaign.target_type`
(ALL_ELIGIBLE/TOWN_ELDERS/SELECTED_MEMBERS) and a new
`CampaignTargetMember` model for the explicit list — both additive,
defaulting to ALL_ELIGIBLE so every existing campaign's behavior is
completely unchanged. `set_campaign_target_members` replaces the
whole list each call (never accumulates stale entries) and is locked
the moment a campaign has already generated obligations — "never
allow scope to be changed casually after obligations have already
been generated," the document's own principle applied to WHO gets
targeted, not just the family/community scope it already governed.

**A second orphaned-function-signature bug, caught immediately by
the same discipline that caught the first one weeks ago**: inserting
`set_campaign_target_members` before `initiate_community_campaign`
via string replacement dropped that function's own `def` line and
duplicated its `@transaction.atomic` decorator onto the new function
instead. Caught by checking the actual function list in the file
right after the edit, rather than assuming the edit landed cleanly —
fixed and verified with the full `welfare` regression sweep before
treating it as safe.

INDIVIDUAL and GROUP scopes from the document (§1) are covered by
SELECTED_MEMBERS at the data level — a one-member list is
functionally an individual target, a many-member list is a group;
no separate mechanism was needed. CUSTOM scope was deliberately not
built — the document itself describes it only as "authorized
administrators define a specific target population where necessary"
with no concrete rule specified, and building a generic rule engine
for an unspecified requirement would be speculative rather than
responsive to an actual need.

8 new tests, full regression sweep across all of `welfare` (83
tests) — all clean.

## 134. A correction, plus scope-aware campaign notifications

**Correcting the record**: last turn's summary claimed "Owner Type/
Target Population fields and 'selected members' targeting... I
haven't started either of these." That was wrong. On investigating
this turn, `ContributionCampaign.target_type` (ALL_ELIGIBLE/
TOWN_ELDERS/SELECTED_MEMBERS) and the `CampaignTargetMember` model
already existed, fully wired into `_eligible_members_for`, with a
migration and 8 passing tests including an HTTP round-trip and a
test proving the target list can't be silently changed once
obligations already exist — directly matching §49-50's immutability
rule. This must have been built in an earlier part of this same
thread's work that wasn't accurately reflected in that summary. Flagging
this directly rather than letting an inaccurate status stand.

With that reconciled, sections 9-10 (target population, selected
members) and the practical intent behind the Town Elders/Individual/
Group scopes are genuinely satisfied — Individual and Group targeting
both reduce to the same `SELECTED_MEMBERS` mechanism (one row
selected vs. many), which needed no separate scope dimension.

**What was actually still missing, and is now built**: §43 —
"notifications must respect scope... do NOT notify all community
members unless the campaign scope says COMMUNITY." Verified directly
that campaign activation sent zero notifications to anyone. Added
`notify_eligible_members_of_campaign`, wired into both activation
points (immediate for community-wide, final Community-Admin approval
for family-scoped) — passing in the exact same eligibility queryset
the campaign already uses to generate its own obligations, so the
notification scope can never quietly disagree with the obligation
scope.

4 new tests proving cross-family notification isolation specifically
(not just obligation isolation), full regression sweep across
`welfare` (87) and `notifications` (7) — all clean.

## 135. Audit log visibility fixed, and three new Registration Officer roles

**Audit log fix**: "all the community logs should go to the community
administrator not the platform admin. The platform admin should only
see the logs of the community admins only, not members." Verified the
actual prior behavior first — Platform Admin saw every entry across
every community regardless of actor. Fixed `list_audit_log` to
restrict Platform Admin's view to entries whose actor is a Community
Admin (or the Platform Admin's own actions, or system-generated
entries with no actor) — an ordinary Family Head, Collector, or
member's own action now stays out of that view entirely. A genuine
Django superuser still bypasses this, matching every other
business-level restriction on this platform. Community Admin's own
visibility into their own community was already correct and needed
no change. 5 new tests, full existing suite (14) still passes.

**Three new Registration Officer roles**: `family_registration_officer`
(scoped to their own family only — the exact same restriction Family
Head/Secretary already have, extended to this new dedicated role),
`town_registration_officer` (community-wide jurisdiction, but can only
register town elders — registering an ordinary member is rejected),
and `community_registration_desk` (registers into any family
community-wide). All three wired into the existing registration,
bulk-import, and member-listing scope logic — not just the single
entry point. The "one person registered once" rule needed no new
work; the existing Ghana-Card and name+phone duplicate detection
already applies regardless of which registration role is doing the
registering. 8 new tests, full `members` regression sweep (123) —
all clean.

## 136. Registration form redesign — background fields, backend and frontend both

"The registration form should be redesigned to ask more information
about the person, mother, father, background." Five new fields on
`Member`: `mother_name`, `father_name`, `hometown` (ancestral origin,
kept distinct from `address`, which is current residence),
`marital_status`, and `spouse_name`. All free text or a simple choice
field, all optional — every existing registration flow is completely
unaffected.

Wired through the full stack, not just the model: `register_member`
and `update_member` in services, both the read and write serializers
(the registration serializer and the update serializer use subtly
different field definitions — verified this before editing, since an
identical-looking `str_replace` could easily have landed in the wrong
one), and the actual `RegisterMemberDialog` frontend form itself,
inside its existing "more details" collapsible section rather than a
new one. Spouse's name only appears once marital status is set to
married, rather than always showing an input that's irrelevant most
of the time.

4 new backend tests (including a full HTTP round-trip), full
`members` regression sweep (135), full frontend production build,
and vitest — all clean.

## 137. Family Head/Traditional Leader get more system-settings options — and a real gap from last turn's new roles caught and fixed

**Two real gaps found before extending anything**: last turn's three
new Registration Officer roles were never wired into the existing
executive-management system. A Community Admin or Family Head
couldn't manage these accounts' feature restrictions at all —
`_COMMUNITY_EXECUTIVE_ROLES`/`_FAMILY_EXECUTIVE_ROLES` didn't include
them, and `list_manageable_users`' Family Head branch used its own
separate, hardcoded role tuple (a second list that could drift from
the first, exactly the failure mode its own docstring warns against).
Both fixed — Family Registration Officer is now manageable by both
Community Admin and their own Family Head; Town Registration Officer
and Community Registration Desk by Community Admin. Traditional
Leader needed no change here — their own scope is based on
`Member.is_town_leader`, not role, so it already covered the new
Town Registration Officer correctly.

**The actual settings expansion**: "the family head... should have
more options in the system settings... same as the town leader."
`/family-fund` and `/ledger-wallets` — genuine platform features that
had simply never been added to the master restrictable-features list
since they were built — are now real options. Family Head's own
restrictable set gained Welfare Contributions and Family Fund (both
squarely family-internal financial tools, the same category as their
existing three options); Traditional Leader's gained Ledger Wallets
(the Town Elders' own wallet scope). Community Admin's already-full
list picks up both automatically.

Two existing tests asserted the old, narrower Traditional Leader
scope by name ("...ledger_only") — updated to reflect the intentional
expansion rather than treated as a regression. 9 new tests, full
`accounts` regression sweep (129) — all clean.

## 138. Database download/upload — most of this already existed; the real gap was "upload to update"

Investigated before building anything, and found the core of this
request already in place: `MembersExportView` (CSV/PDF download,
scoped per role exactly like the Members page itself) and
`bulk_register_members` (CSV upload, same family-scoping) both
already existed. Two genuine gaps, both fixed:

1. **The export was missing most fields.** Only 9 columns — nowhere
   near enough to be a working copy of the underlying record. Now
   carries every field a re-upload can actually write back:
   `membership_number`, background fields (mother/father/hometown/
   marital status/spouse), address, occupation, Ghana Card, emergency
   contact — the full round trip "download it or upload to update
   it" actually asked for.

2. **Upload only ever created new members — it couldn't update
   existing ones.** `bulk_register_members` now checks each row for a
   `membership_number`: a match updates that member's own record
   (through the exact same `update_member` every other edit on this
   platform goes through, with the exact same family-scoping);
   no match falls through to the existing create path unchanged.
   Deliberately chose "pass through whatever non-empty fields are
   present" over trying to diff against the existing record's current
   values — the latter risked real bugs (an uploaded empty string and
   a stored `None` aren't the same "no change," particularly for
   `ghana_card_number`).

**Verified rather than assumed for Traditional Leader**: their
`search_members`/export access was already deliberately community
-wide (not elders-only) — a pre-existing, documented design decision,
not a gap — so "same applies to the Town Leader" was already true
before this turn. Confirmed directly with a test rather than taking
the docstring's word for it.

9 new tests, full regression sweep across `members` and `reports`
(233) — all clean.

## 139. Funeral request/creation forms — closing the gap between what the backend already accepted and what the forms ever asked for

"The funeral request form... any form has to be updated to make it
greater." Checked both funeral forms against their own backend
serializers before writing anything, and found a real, concrete gap
in each: `deceased_date_of_birth`, `burial_date`, `funeral_date`, and
`collection_end_date` were all already accepted by
`RequestFuneralEventSerializer` and `FuneralEventCreateSerializer` —
some even already declared in the frontend's own TypeScript types —
but neither `RequestFuneralOpeningDialog` (the Family Head's form)
nor `CreateFuneralDialog` (the Community Admin's) ever actually asked
for any of them.

Both forms now expose all four, in the same collapsible "more
details" section the registration form redesign established last
turn — optional, never a blocker for a quick ledger opening. Burial
date carries a note connecting it to Asupedeɛ's own collection
window, since that's the concrete reason this field exists on this
platform at all, not just a generic date field.

No backend changes needed — every field asked for here was already
fully supported server-side. Full frontend production build and
vitest — clean.

## 140. Two more backend-ahead-of-frontend gaps closed: gift delegation, and In-Law Contribution's entire missing UI

Kept looking for the same pattern that fixed the funeral forms last
turn — a fully-built backend capability nobody had ever wired into
the actual interface.

**Guest delegation** — `delegation_size` (the "Town A sent 15 people,
one representative gives on their behalf" field, built several
batches back) was completely absent from `RecordGiftDialog`. Added to
its existing "more details" section, with a plain-language note
rather than exposing the raw field name.

**In-Law Contribution had no frontend at all** — the entire request
-approve-pay workflow this platform has had a complete, tested
backend for was reachable only from a test file. This was a bigger
gap than a missing field, so building the actual panel surfaced a
real backend gap too: there was no way to even discover an
obligation's ID once a request was approved, since nothing exposed
it. Fixed by nesting the obligation directly inside the request
serializer's own response (null until approved) rather than adding a
second, separate lookup endpoint — verified this actually round-trips
correctly for both the pending and approved cases before building
anything on top of it, since a reverse one-to-one relation with no
related row raises `DoesNotExist` rather than returning `None` by
default, and that's exactly the kind of assumption worth confirming
rather than trusting.

Built `InLawContributionsPanel` end to end — request form, pending
-request list with approve/reject for authorized executives, and a
payment-recording form once approved — and wired it into the funeral
detail page right alongside the Gift Ledger panel.

2 new backend tests for the obligation-nesting fix, full regression
sweep on the affected backend test files (20 tests) and the full
frontend production build — all clean.

## 141. Full-system audit — a real 400-vs-403 bug found in Announcements; 34 other candidates checked and ruled safe

Continuing the audit from last turn's 35 flagged `get_object_or_404`
calls with no obvious community scoping. Checked each individually
rather than trusting the mechanical search:

**Ruled safe (34 of 35)** — every one delegates its real authorization
check to a service function that DOES properly scope by community or
family: `nominate_collector` checks `member.community_id`,
`set_disabled_features` calls the already-verified
`can_restrict_features_for`, `can_access_channel` and
`can_access_ticket` both check community/family membership directly.
A handful are legitimately unscoped by design — Platform Admin-only
objects with no community concept at all (`HomepageImage`,
`PlanInterestSubmission`), and the memorial page/tribute endpoints,
which are explicitly, deliberately public.

**One real bug found and fixed** — `ResubmitAnnouncementView`,
`ApproveAnnouncementView`, and `RejectAnnouncementView` all had a
genuine authorization check buried in their service functions
(`_is_own_communitys_admin`, `is_platform_admin`) that got converted
into a generic DRF 400 by the serializer's own exception handling —
the same "wrong role, not bad input" class of bug already fixed
several times this build (Town Elder rates, welfare requests), just
never caught here until this audit. Verified the actual buggy
behavior with a real HTTP request before fixing — a different
community's admin resubmitting someone else's announcement returned
400, not 403. Fixed all three the same way: authorization checked
explicitly in the view first (403), genuine state errors ("already
decided") still reach the serializer and correctly return 400.

7 new tests covering both the fix and the state-error case, full
`tenants` regression sweep (167) — all clean.

**Full-system verification total this audit**: 1,367 backend tests
across all 21 Django apps, full frontend production build, and
vitest — all passing, run in complete, verified batches with no
skipped apps.

## 142. Deployment logs reviewed — one real fix (npm's new install-script policy), one clarification (Railway's log shows success)

**Railway log is not an error.** Walked through every line: "no
migrations to apply" means every migration from recent pushes was
already applied in a prior deploy (expected, since the last several
pushes were view/status-code fixes with no model changes); static
files collected successfully; the container started; Daphne bound to
its port and is listening. "HTTP/2 support not enabled" is an
info-level notice, not a warning about anything broken — it only
matters if the app needs to speak HTTP/2 directly to clients, which
it doesn't here, since Railway's own edge terminates TLS/HTTP2 and
forwards plain HTTP/1.1 to the container. Nothing to fix; nothing was
broken.

**The Vercel npm warning was real, and worth fixing proactively.**
Recent npm versions (11.16+, shipped mid-2026) introduced a new
supply-chain security policy: dependency install scripts are no
longer trusted automatically — a project must explicitly list which
packages are allowed to run them, via an `allowScripts` field in
`package.json`. Currently this is advisory (scripts still run, just
with a warning), but npm v12 makes it enforcing by default, meaning
the exact same, currently-harmless warning could become an actual
build failure the moment Vercel's build image updates its npm
version — entirely outside this project's own control. `sharp`
(pulled in transitively by Next.js itself for image optimization, not
even a direct dependency) is the one package this project's build
flagged. Added `"allowScripts": {"sharp": true}` — a name-only entry
so it survives sharp's own routine version bumps without
re-triggering the warning, rather than a version-pinned one that
would need updating on every dependency bump.

Verified with a full, clean `npm install` and production build
before considering this actually fixed, not just plausible.

## 143. HomepageImage CRUD verified, chief photos seeded, and an important storage-persistence risk flagged

Finished what was left mid-turn: 17 tests for the HomepageImage
edit/reactivate/delete work (full CRUD, matching "edit, add, upload,
or delete"), full `tenants` regression (188) — all clean.

**The 5 uploaded chief photos are now seeded automatically on
deploy.** A new idempotent management command
(`seed_homepage_images`) creates one `HomepageImage` row per photo
(captioned, ordered for the carousel) — verified directly: ran it
twice to confirm no duplicates, and confirmed the public endpoint
actually serves all 5 with working URLs before calling it done. Wired
into `entrypoint.sh` so it runs on every deploy with no manual step,
same as migrations and `collectstatic` already do.

**A real, platform-wide risk surfaced while wiring this up, worth
being direct about**: `settings.py`'s storage backend is plain
`FileSystemStorage` — every uploaded photo on this entire platform
(member photos, gift receipts, community logos, announcement images,
and now these homepage photos) is written to the app container's own
local disk. Railway's container filesystem is typically ephemeral —
rebuilt from the deploy image on every redeploy — unless a persistent
volume is explicitly attached. If one isn't already configured, these
five photos (and every other uploaded image) would vanish the very
next time this deploys, even though the database records referencing
them would still exist, leaving broken image links. This wasn't
something to quietly work around; it needs a real answer — either
confirm a Railway volume is already mounted at the media path, or
this is worth fixing properly (a Railway volume, or moving to
S3/Cloudinary-style storage) before this matters for real data.

## 144. Richer community-creation form — identity fields, backend and frontend both

"When creating a new community, the form should require a lot of
information." Seven new fields on `Community`: `district`,
`traditional_authority_name` (the reigning chief's name and title),
`contact_phone`, `contact_email`, `address`, `estimated_population`,
and `registration_notes`. All optional at the service/API layer
(a Platform Admin who genuinely doesn't have this information yet
isn't blocked) — the frontend form is what actually asks for them up
front, matching the same "more details, never a hard blocker" pattern
established for the registration and funeral forms.

Wired through the full stack: `onboard_new_community`, the editable
allowlist in `update_community` (so these can be corrected after the
fact, not just set once at creation), `CommunitySerializer`,
`OnboardCommunitySerializer`, `UpdateCommunitySerializer`, and the
actual `CreateCommunityDialog` form UI.

19 new backend tests (including full HTTP round-trips for both create
and edit), full `tenants` regression (193), full frontend production
build, and vitest — all clean.

## 144. Optional S3-compatible storage — the storage-persistence risk, addressed without waiting for an answer

Rather than leave last turn's storage question sitting unresolved,
built the fix that works regardless of which answer comes back:
`storages`/`boto3` added, and `STORAGES` in `settings.py` now checks
for `AWS_STORAGE_BUCKET_NAME` — if it's set, every upload (member
photos, receipts, community logos, homepage images, everything)
goes to a real S3-compatible bucket instead of local disk; if it
isn't, behavior is completely unchanged from before. Works with real
AWS S3, or any S3-compatible provider (Cloudflare R2, Backblaze B2,
DigitalOcean Spaces, MinIO) via `AWS_S3_ENDPOINT_URL`.

Verified both configuration states actually pass system checks
(installed real `django-storages`/`boto3` in the sandbox rather than
assuming version numbers), and reran the file-upload-heavy test
suites (`tenants`: 193, `members`: 144 — 337 total) with the default,
unconfigured local-storage path to confirm nothing broke.

**This doesn't replace answering the original question** — if a
Railway Volume is already attached to the backend service, that's
still the simpler fix and nothing here needs to change. But if it
isn't, setting five environment variables now makes storage genuinely
durable with no further code changes:
...
AWS_S3_ENDPOINT_URL=<only for non-AWS providers, e.g. R2's account URL>
```

## 145. Homepage video, funeral-tagged announcements, and notice board notifications — with three real bugs caught along the way

**Homepage pictures diagnosed and expanded.** The "not showing" report
is most likely the storage-persistence risk flagged the last three
turns — seeded database rows surviving a redeploy while the actual
files on disk don't, still an open question for you to confirm.
Separately, a real and fixable gap: the frontend's homepage-image
client only ever had `upload`/`deactivate` — the edit/reactivate/
delete built two turns ago was never wired to any UI. Fixed, and
`HomepageImage` now supports genuine video file uploads alongside
photos (mutually exclusive — a slide is one or the other, enforced in
`clean()`), with the public hero rotator and the Platform Admin's
management screen both rebuilt to handle either media type.

**Funeral announcements with photo and video.** `Announcement` gained
an optional `funeral_event` link (tag a notice board post to a
specific funeral) and a real `video` file field alongside the
existing video-link option. The submission form now offers a funeral
dropdown and a genuine video upload, not just a link field.

**Notice board → member notifications, and sort/filter — both
genuinely missing, now built.** Approving an announcement now
notifies every linked member of that community; verified it never
fires for a still-pending or rejected one, and never crosses
community lines. The notification list gained `?category=`,
`?is_read=`, and `?ordering=` — all optional, combinable, verified to
correctly AND together rather than silently OR-ing everything — with
real dropdown controls added to the Notifications page itself.

**Three real bugs caught before they shipped, each the same way**:
by testing the actual behavior rather than trusting the code looked
right.
1. The new `.update()` call for editing a homepage slide's caption
   was missing an explicit `Content-Type: application/json` header —
   this file's plain `fetch()` calls don't set one automatically the
   way the shared `request()` wrapper elsewhere does.
2. `submit_announcement` briefly used `full_clean()`, which
   incorrectly rejected `reviewed_by`/`reviewed_at` as "required" even
   though those fields are meant to stay empty until a Platform Admin
   actually reviews the post later — reverted to the simpler creation
   path, since no cross-field validation was actually needed here.
3. My own test suite's pagination handling was wrong — DRF wraps list
   responses in `{"count": ..., "results": [...]}`, and iterating the
   raw response directly produced confusing `TypeError`s and wrong
   counts until corrected.

Full regression: `tenants` (205 tests), `notifications` (20 tests),
full frontend production build, and vitest — all clean.

## 146. Google Drive backup/restore — backend complete, with a real duplicate bug caught before it shipped

"Each community admin will have a section... where they can import a
Google Drive link to back up their system data, and should also have
options to retrieve them back." Investigated the actual constraints
before designing anything: this platform never authenticates to
Google's own Drive API on anyone's behalf (no OAuth flow, no stored
Drive credentials) — instead, a Community Admin exports their own
member/family roster as a downloadable JSON file, saves it to their
own Drive as "anyone with the link," and later pastes that link back
in to restore it. A `_extract_google_drive_file_id` parser handles the
handful of real link shapes Drive's own Share button produces (file
-view links, `?id=` links, a bare file id), rejecting anything else
with a clear message rather than guessing.

**Deliberately scoped to members and families, not funerals or
payments** — those already have their own audit trail, reversal
workflow, and business-rule validation (minimum-not-cap, no double
-payment) that a blind restore would bypass entirely. Every restored
row goes through the exact same `create_family`/`register_member`/
`update_member`/`bulk_register_members` every other write on this
platform already goes through.

**A real bug caught by testing "restore the same backup twice," a
genuinely realistic scenario**: a member row without a
`membership_number` (which a genuine export from this platform always
carries, but a hand-edited or partial backup file might not) got
re-created as a duplicate on a second restore. Fixed by pre-checking
for an existing member with the same name in the same family before
ever reaching the shared bulk-registration path — without touching
that shared function's own matching logic, since it's also used by
the ordinary CSV-upload feature and changing its behavior there
wasn't warranted.

A `CommunityBackupRecord` tracks every export and restore — who, when,
how many members/families — so backup history is visible, not
invisible. 19 new tests (Drive link parsing, export, restore,
idempotency, cross-community isolation, and the full HTTP round trip
with the Drive fetch itself mocked), full `tenants` regression sweep
(224 tests) — all clean.

**Not yet built**: the frontend UI for this — no "Data Backup"
section exists in the Community Admin's navigation yet. The backend
is fully reachable via API but has no page to actually use it from.

## 147. Backup frontend built, plus a sidebar search filter for easier navigation

**Data Backup UI, on the existing Community Settings page** — no new
nav link needed, since it's already reachable through the link every
Community Admin already has. Realized partway through that the
backend's `communities/<id>/backup/...` endpoints needed a community
id the frontend's own `user` object doesn't carry — rather than add
that broadly, followed the exact convention this platform already
established for this (`/tenants/my-community/branding/`) and built
matching `my-community/backup/...` shortcuts that resolve the acting
user's own community server-side. 4 more backend tests for these,
full download-backup / paste-a-Drive-link-to-restore / history UI
built on top.

**A search filter for the sidebar** — "make sure all the user role
types are having easier navigation." The sidebar itself already had
extensive, carefully-reasoned structure (grouped sections, mobile
auto-hide, executive/personal context switching) from earlier work,
so rather than redo any of that, added the one thing genuinely
missing: a quick filter box (shown only once a role has enough links,
6+, to be worth filtering) that narrows the visible list by label as
you type, with a clear "no pages match" state rather than a
confusing empty list.

Full frontend production build and vitest — clean.

**Not yet done**: Platform Admin's "more control to manage the
community platform" — the third part of the original request.
Everything already built this build cycle (community CRUD,
subscription reminders, homepage images, announcement review, billing
records) already gives Platform Admin a great deal of control; what
specifically is still missing hasn't been investigated yet.

## 148. Upload-from-computer restore — a second real restore bug caught, this one more serious than the first

"Should also be able to upload from my computer to synchronize."
Refactored the restore logic first: extracted a shared
`_restore_community_backup_from_data` that both the Drive-link path
and a new direct file-upload path call, so there's exactly one place
the actual families/members restore logic (and its duplicate-safety
checks) lives, not two copies that could drift apart.

**A second, more serious restore bug caught this turn**, found by
testing the single most realistic scenario for this entire feature:
export a real backup, then restore it into a *different* (or fresh)
community — the actual "recover after data loss" or "set up a new
deployment from an old backup" use case, not just "restore into the
same community it came from." `bulk_register_members` — by design,
to catch a genuine CSV typo rather than silently registering a
stranger — treats a `membership_number` that doesn't match any
existing member as an outright error, not a new registration. A real
export's membership numbers come from the community it was exported
FROM, and won't exist yet in a different one being restored into, so
every single member in a real cross-community restore was being
rejected as an error rather than created. Fixed by only trusting a
row's `membership_number` when a member with that exact number
already exists in the target community; otherwise it's dropped and the
row falls back to the same by-name matching every other row already
uses — without touching `bulk_register_members`' own behavior, since
that's also relied on by ordinary CSV uploads.

Also caught, and fixed, a wrong test premise of my own: I'd initially
written a test expecting a 403 when a different community's admin
used the "my-community" upload-restore endpoint — but there's no
`community_id` parameter on that endpoint at all, so there was never
a way to even target someone else's community through it. Rewrote the
test to verify the actual guarantee instead: their restore succeeds,
but only ever lands in their own community.

8 new backend tests (including the full realistic export → restore
-into-a-fresh-community round trip that caught the bug above), full
`tenants` regression (236 tests), full frontend production build, and
vitest — all clean. The Community Settings page now offers both
restore paths side by side: paste a Drive link, or upload a file
already on the Community Admin's own device.

## 149. Platform Admin's "more control" — a landing page that finally surfaces what already existed, plus one genuinely missing action

"The platform admin should have more control to manage all the
community platform." Investigated rather than guessed: most of the
actual capability already existed (billing records, a full
`BillingRecordsPanel` UI, subscription tracking) — what didn't exist
was a landing page that surfaced any of it, or a way to actually send
a reminder.

**`platform_overview` expanded** with unpaid billing totals,
communities needing subscription attention, and open support ticket
counts — real metrics, not decorative ones, each backed by a test.
Caught a genuine bug while wiring this in: a redundant local `import
Decimal` elsewhere in the same `build_dashboard` function was
shadowing the module-level import for the ENTIRE function scope (a
classic Python gotcha — a name assigned or imported anywhere in a
function body is treated as local throughout that function, even
before the line that does it), producing an `UnboundLocalError` the
moment the new code tried to use `Decimal`. Removed the redundant
import rather than working around it.

**The one genuinely missing piece**: sending a subscription reminder
had zero UI anywhere, despite the backend being fully built and
tested weeks ago. Added a "Needs attention" section to the Communities
page — every community with an unpaid fee or expiring/expired access,
with a one-click reminder (optional custom message).

**The Platform dashboard landing page** now actually links to
everything: Subscriptions & Billing, Support Queue, Revenue, Feature
Flags, and User Management all appear in the department directory —
several of these existed as fully-built pages that were simply never
linked from the one place a Platform Admin actually starts their day.

Caught real duplicate function definitions while wiring the frontend
API client — `createBillingRecord`/`markBillingRecordPaid`/
`waiveBillingRecord` already existed from earlier work; removed my
duplicates rather than leaving two competing copies.

4 new dashboard tests, full regression: `tenants` (236), `dashboard`
(116), full frontend production build, and vitest — all clean.

## 150. Permission system extended: Community Admin manages plain members, bulk-apply, and Family Head data download — all wired into the UI

Continuing last turn's backend work (widened Community Admin's scope
to plain Community Members, bulk-apply for a whole family or all
members at once, Family Head's own data export) with the frontend to
actually use it.

**User Management & System Settings page**: a collapsible "Apply
settings in bulk" section — a Family Head sees "apply to my whole
family"; a Community Admin sees a family picker ("apply to that
family") alongside "apply to all community members." Both reuse the
exact same `restrictable_features` checklist the existing one
-account-at-a-time toggle already uses — no separate, potentially
-drifting list of options. A "Download my family's data" button
appears for Family Head, using the same file-download pattern already
established for the Community Admin's own backup.

Corrected outdated page copy that still said ordinary members "stay
each Family Head's own responsibility, never yours to manage" — no
longer accurate now that Community Admin's scope includes them too.

Full regression: `accounts` (140 tests), `families` (52 tests), full
frontend production build, and vitest — all clean.

**Not yet addressed**: whether the "gift donation collector" part of
the original message needs a distinct role, or is already covered by
the existing generic Collector role — this hasn't been discussed with
the person directly yet.

## 151. SMS payment tracking built; major discovery about how much of the "arrears" ask already exists

Investigated the full request before building anything, since it
touches several already-mature systems. Two important discoveries:

**The tracking link already existed.** `ContributionPayment.qr_payload`
(`/verify-receipt/<id>`) was already built for QR-code receipts — the
exact link this request wanted texted to a payer. And a full,
already-tested `SmsProvider` (Twilio) already existed too — it just
had never been wired to a direct, payment-time send.

**Built**: `send_payment_tracking_sms` — the second delivery option
alongside the existing "mark receipt printed," texting
`payment.obligation.member.phone` directly (deliberately NOT routed
through the `Notification`/`DeliveryAttempt` system, since that
targets a registered User account and the person being handed a
receipt very often has none) with a plain confirmation message and
the payment's own tracking link. 5 tests (successful send, a member
with no phone number rejected clearly, SMS genuinely unconfigured
handled without crashing, a Twilio failure response correctly
reflected rather than swallowed, full HTTP round trip), full `reports`
regression (94 tests) — all clean.

**A major discovery for the "arrears" part of the request**: the rule
"must settle an older debt before a new contribution can be recorded"
**already exists and already blocks the payment** —
`_find_older_unpaid_obligation` inside `record_payment`, tested and
working. What's still genuinely missing, not yet built: a dedicated
"Arrears Collector" role/desk that can collect against a *closed*
funeral's outstanding balance (the existing rule only helps once
someone tries to pay toward a *new* funeral — it doesn't give anyone
anywhere to actually pay the old debt if that old funeral's own desk
is no longer staffed), a name/code lookup surfacing a member's arrears
at the table, and the edit-with-Treasurer-approval workflow for
correcting a missed arrears payment.

## 152. Arrears Collector role and cross-funeral arrears lookup — verified with a real request before building anything

Before writing any code, verified empirically (a real HTTP request, not
just reading the code) that the existing "Collector" role can already
record a payment against a *closed* funeral's obligation — 201, no
error. This confirmed the mechanical foundation for an arrears
collector already existed; what was genuinely missing was the
distinct role and the lookup.

**`ARREARS_COLLECTOR`** — a new, distinct role (not a flag on
Collector), matching "we will have two type of collectors" directly.
Wired into `PAYMENT_COLLECTING_ROLES` (the same community-wide, not
desk-scoped, payment authority Collector already has) and into
`_COMMUNITY_EXECUTIVE_ROLES` (manageable by Community Admin through
the existing User Management screen — no separate admin surface
needed).

**`lookup_member_arrears`** — "when the person standing in front of
the table mentions their name or code, their arrears they own should
show." Every outstanding obligation a member has across *every*
funeral regardless of status, oldest first, matching the same
debt-priority order the existing block-until-settled rule already
uses. Reachable at `GET /members/<id>/arrears/`, returning each
obligation with its funeral's name and status so an Arrears Collector
can see exactly what's owed and where.

8 new tests (role authority, cross-funeral lookup, oldest-first
ordering, a fully-paid obligation correctly excluded, cross-community
isolation), full regression: `accounts` (140), `members` (144), and
the directly-affected `funerals` test files (44) — all clean.

**Still not built**: the edit-with-Treasurer-approval workflow for
correcting a missed arrears payment, the desk-opening session form
(collector name + print/SMS default, entered once), the "see all
funerals open for collection" list, and any frontend UI for the
arrears system itself.

## 153. Full verification of the arrears correction workflow and the two new scoped roles

Completed the verification left over from last turn. Full regression:
`accounts` (140 tests), `members` (144 tests), plus the directly
affected `funerals` test files (73 tests) already confirmed last turn.
Frontend TypeScript check, production build, and vitest all pass
cleanly with the new roles in place. No frontend changes were needed
for the new role labels themselves, since `formatRole`'s existing
generic fallback (replace underscores with spaces) already produces
sensible labels for all three: "arrears collector," "family arrears
officer," and "town elders arrears officer."

Also confirmed the frontend has no strict role type union that could
have broken from adding new backend roles. Roles are typed as plain
strings throughout, so this was a safe, additive change with nothing
else to update.

Style note, applying from this point forward: written without em
dashes, using plain sentences and commas instead, per the person's
request. This applies to new code comments and to conversation
responses, not as a retroactive rewrite of everything already
written this way earlier in the build.

## 154. Desk session, funerals summary, and a real syntax bug caught before delivery

Two remaining pieces from the original collector request. Both are
frontend work on top of the backend already built.

A new `useDeskSession` hook persists the collector's own name and
their print versus SMS choice to sessionStorage for the whole shift.
The front-desk page now opens with a one-time setup form, not a
per-payment one, and every payment recorded afterward reuses that
name and delivery choice automatically. The SMS-tracking send built
two turns ago is now actually wired into the payment flow, since it
had never been connected to a screen before. A small header
indicator shows who is currently collecting, with a way to reopen
the setup form if a different person takes over the desk.

A collapsed-by-default "funerals currently open for collection"
summary was added to the same page, so a collector can see what is
active without leaving the search screen.

One real bug caught before delivery. While adding the funerals
summary, an earlier edit of mine accidentally deleted the
`OpenDeskForm` function's own declaration line, leaving its body
orphaned and breaking the file with a syntax error. Caught this by
actually re-running the TypeScript check rather than assuming the
edit was clean, since the error only showed up on the real compiler
pass. Fixed and reverified: TypeScript check, full production build,
and vitest all pass, and `/front-desk` specifically confirmed present
in the build output.

## 155. Frontend for the arrears lookup and correction workflow

The last two pieces of the collector request now have real screens,
not just API endpoints.

On the Front Desk page, once a member is looked up, an Arrears
section now appears below their active obligations whenever they
owe anything from a closed funeral, deliberately kept separate from
the active list above it rather than merged into one, since arrears
and an active funeral's own contribution are genuinely different
things. Each arrears line offers two actions: record a payment
directly (reusing the exact same authorization the backend already
enforces for Arrears Collector, Family Arrears Officer, and Town
Elders Arrears Officer), or flag it as already paid but never
reflected, which opens the correction request form and sends it to
the Community Treasurer rather than changing anything immediately.

A new Arrears Corrections page gives the Treasurer their own
approval queue, pending requests with the member, funeral, amount,
and reason all visible, plus a permanent decided list so nothing
ever disappears once acted on. Deliberately built as a review screen
only, not a request form, since requesting already happens naturally
from the Front Desk at the moment someone's arrears actually show
up. Added to the sidebar under Finance and Oversight, visible to the
Treasurer and every arrears-collecting role.

Full frontend verification: TypeScript check, production build (with
both `/front-desk` and `/arrears-corrections` confirmed present in
the output), and vitest, all clean. No backend changes this turn,
this was entirely wiring the frontend to endpoints already built and
tested in earlier turns.

## 156. Naming confirmed: Arrears Collector is the Community Arrears Officer

The person confirmed directly: "the arrears collector for the
community and the community arrears officer are the same role."
This resolves the open naming question flagged several turns back.

The stored role value stays exactly `arrears_collector`, unchanged,
since that is already deployed with real data and tests depending on
it. Only the human-readable label changed, on both sides:

Backend: `Role.ARREARS_COLLECTOR`'s display label is now "Community
Arrears Officer", so it reads consistently in Django admin alongside
Family Arrears Officer and Town Elders Arrears Officer.

Frontend: added an entry to `formatRole`'s override table, since the
frontend only ever sees the raw role string and applies its own
generic underscore-to-space fallback, which would still have shown
the old label regardless of the backend change. Also updated two
places on the Arrears Corrections page that named "Arrears Collector"
specifically, since all three arrears-collecting roles (community,
family, and Town Elders) can request a correction, not only the
community-level one.

Verified: the label change did produce a migration (Django tracks
choices as part of the field definition), but it changes no stored
data, only field metadata. Reran the directly affected test suites
(70 tests) to confirm, plus a full frontend TypeScript check,
production build, and vitest. All clean.

## 157. WebSocket authentication fixed, a real gap closed before this deployment

A final review pass through this build's own list of flagged, honest
gaps turned up one that mattered enough to fix now rather than leave
open: `realtime/consumers.py` accepted any WebSocket connection to a
funeral's live ledger channel with no authentication check at all. A
funeral's own memorial page is deliberately public and its URL
contains that same funeral id, so anyone who ever viewed a public
memorial page already had everything needed to connect directly to
this channel and watch every payer's name and amount live, with no
login required at all. This was not a theoretical risk, it was a
genuine, reachable privacy leak.

Fixed by validating a real access token at connect time, the same
JWT check DRF's own authentication already performs for every REST
request. A browser's WebSocket API cannot set a custom Authorization
header, so the token is passed as a query parameter instead
(`?token=...`) and checked by hand using the same token classes and
the same community-aware authentication class already used
elsewhere. A connecting user's own community is then checked against
the funeral's, matching the same isolation the REST API already
enforces, and a Platform Admin can still connect to any community's
funeral, exactly as they already can everywhere else. A connection
failing either check is closed immediately, before ever joining the
group or receiving anything.

Verified with 5 new tests covering both directions: no token
rejected, an invalid token rejected, a different community's user
rejected, alongside the two existing tests (now updated to include a
real token) confirming a legitimate connection still works and still
receives live payment events correctly. Frontend's
`useFuneralLiveUpdates` hook updated to actually send the token it
already had available. Full frontend verification (TypeScript check,
production build, vitest) and a full system and migration check, all
clean.

## 158. Family Head and Traditional Leader can now assign executive roles too, plus suspend and reactivate, backend complete

"The community admin should be able to add new executive user roles
and assign task to, edit executive role, edit members, suspend, or
delete executive role, same as each family head should be able to
add new family executive role, and the town leader should also have
these features. The arrears collector should be available to each of
the family head to assign to someone, and the community admin should
also be able to add or assign the arrears collector role to someone."

Investigated first: `assign_role_to_member` already existed and
already covered most of "add new executive," "edit executive role,"
and "delete executive role" (via the existing revoke), but only for
Community Admin, and `ASSIGNABLE_COMMUNITY_ROLES` had never been
updated to include the newer roles built in later turns
(`arrears_collector`, `family_arrears_officer`,
`family_registration_officer`, `town_registration_officer`,
`town_elders_arrears_officer`, `community_registration_desk`).
Extended this same function to a genuine three-tier design: Community
Admin keeps their full, community-wide authority; Family Head gets
their own narrower slice (family secretary, family treasurer, family
arrears officer, family registration officer) scoped to their own
family only; Traditional Leader gets their own slice (town
registration officer, town elders arrears officer) scoped to members
who are genuinely Town Elders.

Two real permission-scoping issues caught and fixed before this
shipped, not left for later:

`AssignRoleSerializer`'s `role` field was a `ChoiceField` restricted
to `ASSIGNABLE_COMMUNITY_ROLES` alone, meaning a Family Head trying to
assign `family_arrears_officer` would have been rejected by the
serializer before ever reaching the new, correctly scoped
authorization logic underneath it. Changed to a plain `CharField`,
since the real validation already lives in the three-tier check
inside `assign_role_to_member` itself.

Adding Traditional Leader to the existing `MEMBER_REGISTRATION_ROLES`
set (the one gating `CanManageMembers`) would have also granted them
general member-registration and editing rights community-wide, well
beyond what was actually asked for. Built a separate, dedicated
permission class instead (`CanAssignExecutiveRoles`) so the three
actor tiers can reach these specific actions without picking up
unrelated authority they were never meant to have.

**Suspend and reactivate, a new capability**, distinct from revoke:
suspending only flips a login's `is_active` off (already the exact
flag Django's own authentication checks, verified with a real login
attempt before building on it) without touching their role at all, so
reactivating later restores them exactly as they were rather than
needing to be reassigned from scratch.

19 new tests (Family Head assigning family-scoped roles including a
rejection outside their own family, Traditional Leader assigning
Town-Elders-scoped roles, Community Admin assigning the newly
available arrears collector role, and the full suspend/reactivate
cycle including a real login attempt against a suspended account),
plus a full `members` regression sweep (163 tests). One test's own
expectation was wrong and got corrected along the way: a Family Head
reaching for a different family's member correctly returns 404, not
403, since that member was never visible in their own scoped queryset
to begin with, not a case of "found but forbidden."

**Not yet done: any frontend UI for this.** The existing family-role
assignment panel still only offers secretary/treasurer/collector
options, nothing shows a Traditional Leader an equivalent screen for
Town Elders, and there is no suspend/reactivate button anywhere yet.
Backend is fully built, tested, and reachable by API only.

## 159. Frontend built for Family Head and Traditional Leader role assignment, plus suspend/reactivate, with a real bug caught right before shipping

Finishing last turn's backend-only work with real screens.

Family Head's existing role-assignment panel now offers Family
Arrears Officer and Family Registration Officer, alongside the
existing Secretary, Treasurer, and the two collector types, wired
through the same `assign_role_to_member` path already used for those,
with optional username/password fields shown only when the picked
member has no login yet.

Traditional Leader had no equivalent screen at all before this. Built
one: a new section on the Town Elders page lets them assign Town
Registration Officer or Town Elders Arrears Officer, searching only
among people already confirmed as Town Elders.

Suspend and Reactivate buttons added to every entry in the User
Management list, with a clear "Suspended" tag shown inline.

**A real bug caught while wiring this up, not after.** The list of
manageable users only ever returned each person's own login (User)
id, but the suspend and reactivate endpoints are scoped by their
linked Member id, a different id entirely tied to a different
underlying model. Every click would have quietly failed. Fixed by
adding the linked member's id to that list's own response, verified
directly with a real Python shell command that Django's `getattr`
correctly returns `None` for someone with no linked member rather
than raising, before ever building the fix on top of that assumption.

3 new end-to-end tests covering the exact sequence the frontend
actually performs (fetch the list, read the member id off of it, then
suspend using that id), plus a null-member-id case verified not to
crash. Full regression: `members` (166 tests), `accounts` (140 tests),
full frontend TypeScript check, production build, and vitest. A
system and migration check confirmed this needed no new migration at
all, view and serializer changes only.

## 160. Barcode scanning at the desk, and a real regression in receipt printing caught and fixed

"Printing of receipt should have a bar code scanner. Also the receipt
should be very easy to print. Once paid printing should be so easier
and should support either connecting printer by Bluetooth, wireless,
or cable."

Investigated first, since this platform already had a fair amount
built here from an earlier pass: `openReceiptPrintWindow` (cable and
wireless, via a system-registered printer) and `bluetoothPrinter.ts`
(direct Web Bluetooth, no OS driver needed) already existed and
already worked, wrapped in a polished `PrintReceiptButton` component
that offers both.

**A real regression found in the process.** The desk-session payment
forms built a few turns back (`QuickCashForm`, `ArrearsPaymentForm`)
never actually used `PrintReceiptButton` at all — they called the
lower-level system-print path directly, silently dropping the
Bluetooth option that had already been built and working before this
platform ever had a desk session. Fixed properly, not patched around:
`useDeskSession` now also asks which way the printer connects, right
alongside the print-versus-SMS choice already asked at desk-opening
time, so a Bluetooth printer only needs picking once per shift rather
than being re-offered after every single payment. A print failure now
also reads clearly as a print failure, not a payment failure, since
the payment itself already succeeded by that point.

**The barcode scanner, genuinely new.** Every member already had a
real, scannable QR code on their digital membership card
(`Member.qr_payload`, a URL to that member's profile), built for
exactly this moment but never wired to a scan-to-look-up flow at the
desk. Built `QrScannerModal` using the browser's native
`BarcodeDetector` API rather than adding an external scanning library,
the same honest Chrome/Edge-only limitation already accepted for
Bluetooth printing — a "Scan" button appears next to the Front Desk
search box only where the browser actually supports it, opens the
camera, and jumps straight to that member's payment screen the moment
a card is recognized, without ever navigating away from the desk.

7 new tests for the URL-parsing logic (a real member QR code accepted
in several real shapes, an unrelated QR code correctly rejected rather
than silently doing something unexpected), full frontend verification
(TypeScript check, production build with `/front-desk` specifically
confirmed present and grown, vitest), all clean.

## 161. A real scannable QR code on the printed receipt itself, both print paths

"The receipt should have a bar scan code so that they can scan to
confirm their payment." A genuinely different, narrower ask than last
turn's desk-side scanner (which reads a member's card to look them up)
— this is about the payer's own printed slip carrying a code they can
scan themselves, after the fact, to confirm what they paid.

Investigated first: the payment's own verification link
(`payment.qr_payload`) and the PDF receipt already embedded a QR code
correctly. The gap was specifically the plain-text receipt, the one
actually produced at the desk via cable/system printing or Bluetooth,
which ended in nothing but "Thank you." with no code at all.

Refactored `generate_qr_code_base64` into a generic, reusable
`generate_qr_code_base64_for(payload)`, since it had only ever
accepted a Member before. Added a new, separate endpoint
(`/receipts/contribution-payments/<id>/qr-code/`) rather than changing
the existing plain-text endpoint's response shape, which three
different screens already call expecting a plain string back.

Both printer paths now genuinely carry the code, not just one of
them: `openReceiptPrintWindow` renders it as an image alongside the
text for cable/system printing, and `bluetoothPrinter.ts` now
implements the real ESC/POS "GS ( K" QR code command sequence so a
Bluetooth thermal printer renders it in hardware directly, no bitmap
image ever generated or sent over the slower BLE link. Wired into the
Front Desk's own print flow and into the more general
`PrintReceiptButton` component (used by My Receipts and the family
fund pages too), so this isn't limited to the desk alone. A QR code
fetch failure never blocks the receipt itself from printing — the
code is an enhancement on top of it, not a dependency of it.

4 new backend tests (the verify URL matches the payment's own
qr_payload, the returned image is a real, decodable PNG rather than a
plausible-looking string, cross-community isolation), full `reports`
regression (115 tests including the members sanity check), full
frontend verification (TypeScript check, production build, vitest),
system and migration check confirming this needed no new migration at
all.

## 162. The actual scroll bug fixed across every modal dialog in the system

"I can't scroll some of the forms down, so make it great so that I
can scroll them down and when filling." A screenshot of the Register
Member form, cut off at the bottom of the screen with no way to reach
the rest of it, showed the real, reproducible bug directly.

Root cause: `DialogShell`, the shared wrapper nine different family
dialogs are built on (Add, Rename, Merge, Delete, Deactivate,
Transfer Members, Family Rate, Bereaved Rep, History Drawer), had no
`max-height` or `overflow` at all. Any form taller than the viewport
simply ran off the screen, submit button included, with nothing to
scroll. Fixed once, in that one shared component, which immediately
fixed all nine dialogs built on it at the same time.

The remaining dialogs each maintain their own separate modal markup
rather than sharing one component, so each needed its own fix.
Restructured into a sticky header (title, description, close button
always visible) over a scrollable body for Register Member (the one
in the screenshot), Create Funeral, Record Payment, Record Gift
Donation, and Request a Funeral Opening. The two Mobile Money payment
dialogs got a lighter version of the same fix, given how short their
own content already is.

12 dialogs touched in total. Full frontend verification: TypeScript
check, production build, and vitest, all clean.

**What this covers, and what it doesn't.** This fixes the concrete,
reported bug everywhere it existed. It is not yet the fuller
"modernize every form" redesign the person also asked for — spacing,
visual hierarchy, and field grouping in each individual form haven't
been touched yet, and full-page forms (registration flows that aren't
modal dialogs at all) haven't been reviewed either. Flagging this
honestly rather than presenting a bug fix as the complete answer to a
broader design request.

## 163. Two forms modernized as the first example, before doing the rest

Picking up the visual modernization the person confirmed after the
scroll-bug fix. Rather than redesign every form blind, built the
shared pattern once and applied it to two forms as a first look,
matching the offer made last turn.

Investigated the existing design system first rather than inventing a
new one: this platform already has a deliberate, specific visual
identity (family-registry-tokens.css's own "ledger book of record"
concept — Fraunces serif for display, Inter for body, IBM Plex Mono
for small data labels, a warm paper background, hairline rules
between entries), already used consistently across every page
already built. Modernizing the forms means extending that same
established language into them, not replacing it with something
generic.

New shared primitives (`components/forms/FormPrimitives.tsx`):
`FormSection` groups related fields under the same small,
tracked-out, mono-font eyebrow label already used elsewhere on this
platform (e.g. "COMMUNITY REGISTER · n" above the Members page's own
title), separated by a hairline rule, the same "related entries
grouped under a heading" convention a ledger book already follows.
`Field` wraps a label, its input, and an optional hint consistently.
`FIELD_INPUT_CLASS` is the one input treatment every form should
share, a slightly more visible focus ring than before, still quiet.

Applied to Register Member (the form in the person's screenshot) and
Create Funeral, the two longest forms in the system. Register
Member's roughly 20 fields now read as five clear groups: Who they
are, Reaching them, Personal details, Family background, In case of
emergency, Funeral contribution. Create Funeral's fields group into
The deceased, Family & rate, Collection window, More details. No
field, value, condition, or validation logic changed, only how the
same fields are presented.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

**Not yet done, deliberately**: every other data-entry form on the
platform (the shorter dialogs, and the full-page forms that aren't
modals at all) still use the old, unsectioned pattern. Waiting on the
person's reaction to these two before applying the same treatment
everywhere, per the plan discussed.

## 164. The form modernization applied to every remaining dialog in the system

Continuing straight from the last turn's two examples, now applied
everywhere else. Not every dialog needed the same treatment, since
several were already short enough (a single field, a confirmation
with a checkbox) that grouping them into sections would have added
structure without adding clarity — judgment applied per dialog rather
than mechanically forcing the same shape onto everything.

**Sectioned with the full `FormSection`/`Field` treatment** (multiple
genuinely distinct groups of fields): Register a New Family (Family,
Family Head, Their login, More about the Family Head), Record Gift
Donation (Donor, Cash, Gift item), and Request a Funeral Opening (The
deceased, Collection window, More details) — matching the same
pattern already applied to Register Member and Create Funeral last
turn.

**Given the lighter, consistent-styling treatment** (real fields, but
few enough that sectioning wouldn't add anything): Rename Family,
Family Rate, Merge Family, Transfer Members, Assign Deceased Rep, and
Record Payment — each now uses the same `Field` wrapper and the same
`FIELD_INPUT_CLASS` focus treatment as every other form, without
being broken into sections they don't actually need.

**Left untouched, correctly**: Deactivate Family and Delete Family
have no real input fields at all, just confirmations, so there was
nothing to modernize. Family History Drawer is read-only.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

**Still open**: the two Mobile Money payment dialogs still use the
older input styling rather than the shared class, and haven't been
touched yet given how short they already are. Full-page forms that
aren't modal dialogs at all (registration and settings pages
reachable directly by URL, not opened as a popup) also haven't been
reviewed under this same lens yet.

## 165. MoMo payment integrity verified end to end, and both flows moved to full pages

"Not until a successful MoMo transaction should the money reflect
paid." Investigated the actual code path in full before touching
anything, rather than assuming. Confirmed, concretely: initiating a
MoMo request only ever creates a PENDING record and sends it to the
provider, it never touches either ledger. The only place a real
payment or gift ever gets recorded is `_finalize_as_successful`,
called only from a signature-verified webhook or a polling fallback,
both of which only fire once the provider itself confirms success. A
misconfigured provider correctly fails the request rather than
silently succeeding. This protection already existed and is already
tested. Confirmed rather than assumed.

The second half of the request, moving both MoMo flows off popups
onto their own pages, is genuinely new work. Built
`/pay-momo/[obligationId]` and `/pay-gift-momo/[funeralId]` as real,
standalone pages, carrying over the exact same safe logic (the same
hooks, the same "wait for the provider" polling, the same OTP step)
with nothing about how or when a payment is recorded changed. Updated
all four places across the platform that used to open the old popups
(the funeral committee's own obligation table, a member's own
outstanding-obligations card, the Front Desk, and the gift ledger
panel) to navigate to these pages instead, then deleted the two old
dialog components once nothing referenced them anymore.

Full frontend verification: TypeScript check, production build (both
new routes confirmed present), vitest, all clean. Backend untouched
this turn, confirmed with a system check and the full `payments`
regression suite (28 tests, unchanged and still passing).

## 166. Critical fix: the frontend forms updated to match last turn's newly-required field

Last turn made `cause_of_death` required at the real API boundary for
opening a funeral, but never updated the two actual forms that call
it. Until this fix, nobody could have created a new funeral through
the app at all, since the backend would have rejected every request
missing a field the UI never asked for. Caught and fixed before
anyone else touched this.

Both Create Funeral and Request a Funeral Opening now ask for cause
of death directly, and both gained a new, related capability: "when
someone dies... their membership account should go inactive." Since
the deceased is overwhelmingly already a registered Member of the
family whose ledger is being opened, both forms now offer a
family-scoped member picker: choosing a name there automatically
fills in the name and gender fields, and is what actually drives the
member's own status changing to Deceased once the funeral goes live.
Naming the deceased by free text, without picking a registered
member, still works exactly as before, unchanged, for a deceased
person who was never registered here.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

## 167. User Management and System Settings, now genuinely separate pages

"The user management should stand on its own and the system settings
should stand on its own." Previously combined into one 716-line page
titled "User Management & System Settings," mixing two genuinely
different concerns: who holds which role, and what a role, once
held, can see and do.

Split cleanly. User Management is now purely about the first
question, assigning, revoking, suspending, reactivating. System
Settings is now purely about the second, and gained real scope in
the process, not just a rename: it already had a Platform Admin
version (restricting what a Platform Admin account can do), and now
also serves Community Admin, Family Head, and Traditional Leader with
the dashboard-visibility controls that used to live buried inside
User Management, moved here where they actually belong. Same page,
different content depending on who's looking, since each tier
already had its own version of "restrict what an account can do"
before this turn, just scattered across the wrong pages.

Added System Settings to the sidebar for all three jurisdiction tiers
that previously had no link to it at all.

One real bug caught and fixed during the split: an early file-splice
edit accidentally deleted a type definition and constant that a kept
function still depended on, caught immediately by the TypeScript
compiler rather than assumed correct.

Full frontend verification: TypeScript check, production build (both
pages confirmed present, sizes shifted in the expected direction),
vitest, all clean.

## 168. A real dashboard-routing gap found and fixed, plus the collector dashboard brought closer to a reference layout

Shown a reference screenshot of a different platform's collector
front-desk dashboard, with the request that collectors and Arrears
Collectors here look and work similarly.

Investigating first turned up a genuine, unrelated bug worth fixing
regardless of the visual request: `Arrears Collector` and every other
role added across recent turns (`Family Arrears Officer`, `Family
Registration Officer`, `Town Registration Officer`, `Town Elders
Arrears Officer`, `Community Registration Desk`) had never been added
to either the frontend's role-to-dashboard-route map or the backend's
own role-to-dashboard-section logic. Each was silently falling
through to the wrong default dashboard entirely, an Arrears Collector
landing on the generic community overview meant for a Chairman or
Secretary, not their own collections and arrears picture. Fixed on
both sides: each newer role now shares the correct existing
dashboard with the role it is closest to in spirit, Arrears Collector
with Collector, the two family officer roles with Family Head, the
two Town Elders officer roles with the Traditional Leader, matching
"all executive personal dashboard is the same dashboard as the
community member dashboard, since they are members" directly.

The collector dashboard itself gained two things from the reference
layout genuinely worth adding: a "Find account" search bar right on
the dashboard, handing a query straight to the Front Desk page rather
than requiring an extra click first, and two new stat tiles, "Whole
office today" (every collector combined, alongside this collector's
own personal numbers) and "Customers owing" (matched to the same list
already shown further down, so the two can never disagree). "Members
who've paid" was already built in an earlier turn and needed no
changes.

7 new tests confirming every one of the newer roles now lands on its
correct dashboard section, plus the two new metrics' presence. Full
regression: `dashboard` (123 tests), full frontend verification
(TypeScript check, production build, vitest), all clean.

## 169. Refresh and Sync brought forward to where collectors actually work

"I want the synchronize and refresh to be like this template, so that
all the collectors and data entries can be done when the network is
unavailable and later can be synchronized when the network is
restored." Investigated first: the underlying offline queue
(`useOfflineSync`) already did exactly this, syncing automatically
the moment connectivity returns, with a manual retry only reachable
from a separate Pending Sync page. What the reference dashboard
showed that this platform didn't yet have was the visibility, Refresh
and Sync sitting right where the work happens, not one navigation
away.

Added a prominent Refresh and Sync Now button pair, plus live status
tiles (waiting to sync, today's payments, members owing), to both the
Front Desk page itself and the Collector's own dashboard. Nothing
about how or when a queued payment actually syncs changed, this is
the same `drainQueue` the automatic sync already used, now also
reachable by hand and visible at a glance rather than buried.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

**The second, much larger part of this request, restyling the whole
platform's visual language to resemble the reference screenshots more
broadly, was not attempted this turn.** This platform has had a
deliberate, consistent visual identity (the "ledger book of record"
concept in family-registry-tokens.css) built and extended across
dozens of turns. A genuine visual pivot to a different design
language is a significant, largely irreversible-feeling decision
touching effectively every page, worth the person's explicit
confirmation on scope before any of it is attempted, not something to
guess at from two screenshots of a different platform.

## 170. A safe, additive polish pass, no visual direction change

"Update the system to make it look great and advanced, but don't do
anything that would cause confusion." Confirming the person's earlier
answer: no switch away from this platform's own established look.
Everything here only changes how existing interactions feel, never
what anything looks like at rest, never a color, a layout, or a piece
of navigation moved.

Three additions, all in the two shared, truly global stylesheets
rather than touched component by component, so they reach the whole
platform at once with no risk of missing a page:

A smooth, quick transition on every interactive element's color,
background, border, and opacity, replacing instant snaps between
states with something that feels considered rather than static.

A visible keyboard-focus ring on buttons and links that had none
before, a real accessibility gap this also happens to close, applied
only to `:focus-visible` so an ordinary mouse click looks and feels
exactly as it always has.

A genuine hover state added to every solid primary and destructive
button across the entire codebase retroactively, without editing a
single component file. Tailwind's own arbitrary-value syntax
compiles `bg-[var(--forest)]` to the same literal class name
everywhere it's used, so one small set of rules in `globals.css`,
targeting that exact class, reaches every one of the hundreds of
buttons already written this way at once.

Verified the compiled CSS output directly to confirm these rules
actually survived Tailwind's build process rather than assuming.
Full frontend verification: TypeScript check, production build,
vitest, all clean.

## 171. A genuine SaaS dashboard direction, confirmed by the person and built on shared foundations

"Can we make it a SaaS dashboard, since we have a platform admin who
manages all the community admins and the system is also a tenant
system?" A real, deliberate reversal of the earlier "don't switch"
answer, with a legitimate reason behind it. Taken at face value and
built properly, not half-attempted.

Given the scale of this platform, the approach was to change the
shared foundation every page already depends on, rather than editing
dozens of pages individually and risking an inconsistent result.

New tokens added alongside the existing ones, not replacing them,
since forest green, gold, and clay red still carry real meaning
(confirm, family-head badge, destructive) a plain palette swap would
have erased: a new indigo primary color, a light neutral background,
white card surfaces, soft shadows, and a consistent border radius.

Every shared component rebuilt on these new tokens: the Sidebar (dark
slate, indigo active state, rounded nav items), DashboardVisuals
(KpiTile and SectionCard now read as real stat cards with soft
shadows, not printed ledger lines), DashboardPageShell, DialogShell
(all nine family dialogs), and FormPrimitives (every form already
using Field/FormSection). Since these are the components most pages
are already built on, this alone reaches a large share of the
platform automatically.

Beyond the shared components, two further changes cascade the new
look platform-wide without touching individual page files one by
one: a bulk replacement of the most common outer-shell color classes
(background, border, text) across all 45 page files that used them,
and a single redefinition of the `.font-display` CSS class itself,
swapping Fraunces (the serif most responsible for the old "printed
ledger" identity) for a bold, tight sans-serif, which every existing
heading across the whole platform picks up automatically since none
of the hundreds of places using it needed to change.

Verified the compiled CSS output directly at each step, confirming
tokens, hover rules, and the font-display change actually reached the
production build rather than assuming. Full frontend verification:
TypeScript check, production build, vitest, all clean throughout.

**What this covers, honestly.** The shared foundation and every page
built on it now reflects the new SaaS look. Pages with heavily
bespoke markup that never used the shared components, or hardcoded a
color inline rather than through a token, were not individually
reviewed this turn.

## 172. The SaaS conversion continued, closing the gaps flagged last turn

Picking up exactly where the honest note at the end of the last turn
left off, the pages and patterns that weren't reached by shared
components alone.

Four more bulk, platform-wide replacements, the same "change the
common pattern once, reach every file that used it" approach as
before, rather than opening dozens of files individually: sharp
corners (`rounded-sm`) rounded to match the new card system, wherever
they appeared across every page and every shared component; the old
recessed "one step off paper" surface color, mapped to the new light
neutral background so it still reads as a secondary surface rather
than disappearing into a white-on-white card; the heavy double-weight
header and table-divider borders, replaced with the single, lighter
border weight every new card and dialog already uses; and the
matching divider color between list rows.

The Login page, checked individually rather than assumed covered,
since it's the first thing anyone using this platform actually sees
and doesn't route through the dashboard's own shared components at
all. Its sign-in buttons and link hovers were still using the old
forest green as a brand color, since login has no money-related
context of its own, that's a leftover from before the new palette
existed, not a deliberate choice worth keeping. Switched to the new
indigo primary color, 17 occurrences on this one file only, `--forest`
stays exactly where it correctly still means "money, confirm"
everywhere else.

Full frontend verification after every step: TypeScript check,
production build, vitest, all clean throughout, not only at the end.

## 173. Sidebar contrast fixed, a real personal-dashboard leak found and closed, Family Head audited

Three distinct things this turn, one urgent visual fix and two
genuine audits that each turned up a real, previously uncaught gap.

**The sidebar font color.** Computed the actual WCAG contrast ratios
rather than eyeballing it: the muted gray used for section labels,
the empty-search state, and the search placeholder measured 3.75:1
against the dark sidebar background, below the 4.5:1 minimum for
normal text, and genuinely hard to read at the small size those
labels use. Brightened to the next step up, which measures 6.96:1,
comfortably passing.

**"Personal account is the same community member dashboard, and they
are not allowed to see funeral fund or execute task."** Investigated
both halves before touching anything. Task execution was already
correctly restricted, an existing, deliberate test suite (25 tests)
already confirms a person in personal context cannot assign a task,
edit a member, or approve a payment reversal, only view things and
mark their own assigned task done, which is self-service, not an
executive action. Funeral fund visibility was a real, different gap:
a Family Head, Secretary, or Treasurer who switched to personal
context still saw their family's fund financial section injected
alongside the plain member view, exactly the leak described. Fixed,
and a new test added specifically for this case, distinct from the
existing financial_overview test, since no existing test covered a
family officer switching context at all.

**"Each family head should have more feature or more quick access
button... to have more control to manage their family."** Audited
rather than assumed fine: traced where the family rate and Deceased
Rep dialogs actually render from, and found they only ever appeared
on the Community Admin's own Families page. A Family Head had no way
at all to recommend their own family's contribution rate or assign a
Deceased Rep for their own family, a genuine, concrete gap matching
what was reported. Same dialogs, same shared store, now also
reachable as two new quick-access buttons right on the Family Head's
own dashboard, where they actually land.

Full verification throughout: TypeScript check, production build,
vitest, and backend regression (accounts' context-switching suite,
families, 78 tests combined) all clean.

## 174. Front Desk and the Collector dashboard redesigned for easier collections

"Redesign the Arrears Collector and the Front Desk Collector for
easily collections... make it look great and user friendly." Since
Arrears Collector already shares the Front Desk page and the
Collector dashboard (both fixed in an earlier turn), this meant going
through the actual collection workflow itself looking for real
friction, not applying color changes on top of it.

Traced the whole flow from search to a recorded payment before
touching anything, and found one genuine, meaningful gap: recording a
payment with the common delivery method (a printed receipt, not SMS)
called its own completion handler immediately, closing the form with
no confirmation at all. A collector handling real cash from a real
person had no clear signal the payment actually went through beyond
the obligations list quietly updating in the background. Fixed on
both payment paths, the active-funeral form and the arrears form
alike: a brief, unmistakable "GH₵X.XX recorded" confirmation with a
checkmark, shown before the form closes.

Beyond that fix, the visual hierarchy across both the obligation
cards and the arrears cards was rebalanced around what actually
matters to someone standing at a desk: the amount owed is now the
largest, boldest, most prominent number on each card rather than a
small caption line, and the primary actions (MoMo, Cash / Other,
Record payment) are now genuinely sized to be tapped quickly rather
than read as secondary. "Search again," the one navigation action a
collector repeats for every single person at the desk, is now a real
button instead of a small underlined link.

The Collector dashboard itself, checked for consistency against the
rest of the new SaaS design rather than assumed already caught by
earlier bulk passes: a few leftover old-style borders and flat panels
(the two navigation links, the sync-status panel, the daily-tape
panel) picked up the same rounded, softly-shadowed card treatment
every other section on that same page already had.

Full frontend verification: TypeScript check, production build,
vitest, all clean throughout.

## 175. Real navigation gaps closed for family executive roles; the harder architectural questions left open, honestly

"Make it clear for each of the family executive roles... to have
access to what they need to execute their tasks." Investigated
every family-related role's actual sidebar access before touching
anything, since this platform has grown several newer roles over
recent turns and each addition risked being forgotten in one more
place.

Two genuine, concrete gaps found and fixed. `family_registration_officer`
had zero role-specific navigation at all, despite already being
permitted to register members at the backend, their entire job was
functionally unreachable through the interface. `family_arrears_officer`
had exactly one link (Arrears Corrections) and no way to reach Front
Desk, the one page that actually records a payment, meaning their
core job of collecting money was equally unreachable. Confirmed this
same gap affected `arrears_collector` itself too, not only the
family-scoped version. All three, plus `town_elders_arrears_officer`,
now have Front Desk in their navigation; the two registration-focused
roles now have Members.

**What was investigated and deliberately not changed.** Considered
moving `family_arrears_officer` and `town_elders_arrears_officer`
onto the Collector-style dashboard instead of the family/Town-Elders
oversight one, since collecting genuinely is their job, not
overseeing. Started the change, then caught a real risk before
finishing it: the Collector dashboard's own data queries are
community-wide (every outstanding member, every active funeral across
the whole community), not family-scoped. Moving these two roles there
without first making that view genuinely family-aware would have
leaked community-wide financial data to a role that should only ever
see their own family's or Town Elders group's picture. Reverted
rather than shipped half-finished; a dedicated family-scoped version
of the collector view is real, separate work.

**What remains genuinely open, described accurately rather than
assumed solved.** "Family Head is for review and approval, and all
activities in the family should submit to them" describes an
approval-workflow architecture, not a navigation fix, some of it
already exists (expense approvals, task approvals both already
surface on the Family Head's own dashboard), but a full audit of
which family-executive actions currently happen immediately versus
which should route through Family Head approval first was not done
this turn. "Family Head should have an analytics view of all
activities in the family" was checked against the platform's actual
audit log and found to be the wrong mechanism entirely, that log only
ever records platform-level events (role changes, funeral-opening
decisions, billing), never day-to-day activity like registrations or
payments, so pointing Family Head at it would not have delivered what
was actually asked for. The existing family dashboard already
aggregates a good share of family activity in one place; whether
that already satisfies the request or needs a dedicated activity feed
is worth confirming directly rather than guessing.

Full backend verification: system check, the newer-roles routing
suite, and a full `dashboard` regression sweep (123 tests), all
clean. Frontend verified earlier in the same turn: TypeScript check,
production build, vitest, all clean.

## 176. Family Arrears Officer's own, properly family-scoped dashboard, built and verified end to end

Continuing directly from last turn's fix and new backend function, the
frontend half: a real page, not a shared component reused from
somewhere else.

Built `/dashboard/family-arrears`, its own route, modeled on the
Collector dashboard's own layout and conventions (the same Find
Account search, the same Today's Tape, the same collection trend
chart) but every number on it comes from the new, dedicated backend
section built last turn, already scoped to this one family
server-side. Nothing on this page needed its own family filter to
add, the backend never sends anything beyond this family's own
arrears in the first place, which is the whole point of building it
as its own function rather than reusing the community-wide one.

Routing wired through on both ends: `dashboardRoutes.ts` on the
frontend now sends `family_arrears_officer` to this new page instead
of the Family Head's oversight dashboard, and the root `/dashboard`
page's existing redirect logic already reads from that same routing
table, so no separate redirect logic needed adding.

The one leftover test from last turn, written against the old
(incorrect) routing decision, updated to assert the new, corrected
behavior instead of deleted, so the historical reasoning for the
change stays visible in the test file itself.

Full verification, backend and frontend both: the newer-roles
dashboard routing suite, a full `dashboard` regression sweep (123
tests), the arrears collector and family-scoped member management
suites (44 tests) all clean; TypeScript check, production build (the
new route confirmed present in the build output directly, not
assumed), vitest, all clean.

## 177. Family Treasurer given real authority over the one thing most squarely their job

Continuing on the same thread as the last two turns: "the family
finance officer should also be in charge of anything related to
money." Investigated what Family Treasurer already could and
couldn't do before assuming a gap existed. Found their access to the
family fund, expenses, and ledger wallets already comprehensive, but
one genuine, concrete gap: a family's own contribution rate, the
single most consequential money decision a family makes, could only
ever be recommended by a Family Head or Community Admin. A Family
Treasurer, the role explicitly meant to handle money, had no way to
propose one at all.

Fixed additively, not as a replacement: Family Treasurer can now
recommend their own family's rate too, alongside the Family Head, who
keeps the same ability unchanged (useful for a smaller family with no
dedicated Treasurer). The existing recommend-then-approve flow is
untouched either way, a Community Admin still has to approve any
recommended rate regardless of who proposed it, so this adds
authority without weakening the oversight already in place. The same
"Set family rate" quick-access button already on the Family Head's
own dashboard now also shows for the Family Treasurer sharing that
same page; assigning a Deceased Rep stays Family Head-only, a
representation decision rather than a financial one.

3 new tests confirming the Treasurer can now recommend a rate, the
Family Head's own ability is unaffected, and an ordinary member still
cannot, this is additive authority, not a general loosening. Full
regression: `families` (52 tests), `family_funds` and the core
`dashboard` suite (52 tests), all clean. Full frontend verification:
TypeScript check, production build, vitest, all clean.

## 177. "Family head should have the analytics view of all the activities in the family" — a real activity feed, not just counts

Picking up the piece flagged as genuinely open two turns ago, after
confirming the platform's own audit log was the wrong mechanism for
this: that log only ever records platform-level events, never a
day-to-day registration or a payment.

Built the thing actually asked for instead: a real, chronological
"who did what when" for one family, merging four different kinds of
activity that were previously only visible as separate aggregate
counts elsewhere on the same dashboard — member registrations,
payments collected, family expenses recorded (with their approval
status), and completed tasks — into one time-ordered feed, newest
first. Nothing new is logged anywhere to build this; every entry is
read directly from data these roles already write in the ordinary
course of their work, which is also why it required no migration.

Family Head only, deliberately, matching "abusuapanin also oversees
all activities," a design principle already established in this
platform's own family_funds code before this turn — this is oversight
of everyone else's work, not something a Secretary or Treasurer
reviewing their own slice of it needs.

3 new backend tests: confirms all four activity types actually appear
together, confirms a different family's activity never leaks in, and
confirms the two other family-officer roles correctly don't get this
section at all. Full regression: the whole role-differentiated family
dashboard suite (15 tests) and a full `dashboard` sweep (126 tests),
all clean. Frontend verified: TypeScript check, production build,
vitest, all clean.

## 178. Family Secretary's expanded authority completed; Family Head restricted from direct data entry

Finishing what was left mid-edit last turn, plus the most consequential
part of the request: acting on the repeated, still-ambiguous
instruction rather than leaving it unresolved a second time.

**Frontend finished**: the role-assignment panel (arrears officer,
registration officer, and the rest) now actually renders for Family
Secretary, not only Family Head — the backend authority for this was
already built and tested last turn, this closes the gap between what
the API allowed and what the interface actually showed. The Suspend
/ Reactivate control stays Family Head-only, deliberately — that
authority was never part of what was asked for Family Secretary, and
showing a button that would only fail on click helps nobody.

**The ambiguous instruction, resolved**: sent identically a second
time, "the family head... but he can[not] enter data or make
entries... each user role will play its role, not other user's
role." Read together with the exact same principle already built and
tested elsewhere on this platform for expenses ("the Family Head
reviews, approves, or rejects purchases — recording a new one is for
Family Secretary or Treasurer"), Family Head is now blocked from
registering a member directly, the same way, matching the missing
word rather than guessing past it a second time. The Register Member
button is hidden on the Members page for Family Head specifically,
rather than left to fail confusingly on submit.

**A real, second bug caught by this same investigation**: restricting
member registration broke three existing tests that had assumed the
old behavior, one direct, two in bulk CSV upload (which turned out to
call the exact same underlying function per row). All three updated
to assert the new, corrected behavior rather than deleted, keeping
the historical reasoning visible in the test files themselves.

Full backend verification, run deliberately as full app sweeps rather
than only the new tests, precisely because that discipline is what
caught last turn's orphaned-function bug in the first place: `members`
(172 tests), `accounts` (141 tests), `welfare` (already covered last
turn), all clean. Full frontend verification: TypeScript check,
production build, vitest, all clean.

## 179. Chairman given real community-wide authority, matching "upper control over the community ledger and the system"

Traced the same nav-and-permission audit method that found real
family-role gaps two turns ago, applied to Chairman specifically.

A real, previously-unnoticed gap: Chairman had no community-wide
management authority at all, they could only ever be managed BY
Community Admin, never manage anyone themselves. Fixed to match
Community Admin's own authority exactly, every executive and
ordinary member, with the one boundary that never moves: Chairman
still can't touch a Community Admin's own account, the same
restriction Community Admin already applies to itself.

A real, chronological community-wide activity feed, the direct
counterpart to Family Head's own activity feed built two turns ago,
applying the same reasoning: the platform's audit log only tracks
platform-level events, never day-to-day registrations or payments, so
it remains the wrong mechanism here too. Chairman and Community Admin
get it; Secretary doesn't, matching the same "the reviewer gets it,
not every executive" pattern already established for families.

9 new tests (6 for the management authority, 3 for the activity
feed). Full regression: `accounts` (141 tests), `dashboard` (129
tests), all clean. Full frontend verification: TypeScript check,
production build, vitest, all clean.

## 180. The broader consistency sweep, continued: one more real gap found

Systematically checked every remaining community-level role's
navigation against its actual backend authority, the requested sweep
beyond Chairman specifically.

One more real, concrete gap of the exact same shape as
`family_registration_officer`'s a few turns back:
`community_registration_desk` already had backend authority to
register members (`MEMBER_REGISTRATION_ROLES`), but no way at all to
reach the Members page, their entire job was functionally
unreachable. Fixed.

Checked and confirmed correct, not gaps: community Secretary not
having expense-recording authority (`EXPENSE_ROLES`) is a deliberate,
sensible difference from Family Secretary, at the community tier
Treasurer and Financial Secretary already specialize in finance,
whereas a family has fewer roles so Family Secretary reasonably
covers more ground. Financial Secretary and Auditor correctly not
appearing in collector-nomination approval, since neither role is
part of the required approval group for that workflow. All three
role-specific dashboard pages (Notification Officer, Bereaved
Representative, Guest) confirmed to actually exist.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

## 179. Chairman's "upper control," a community-wide activity feed, and the broader sweep's first concrete results

Delivering last turn's verified work, plus continuing the sweep across
the remaining community-level roles the same audit method already
proved out on family roles.

**Chairman**, matching "upper control over the community ledger and
the system": now has the same community-wide management authority
Community Admin already has, every executive and ordinary member,
with the one boundary that never moves, Chairman still can't touch a
Community Admin's own account. And "have all activities analytics
view": a real, chronological community-wide activity feed, the direct
counterpart to Family Head's own feed from two turns ago, member
registrations, payments, expenses, completed tasks, merged into one
timeline. Chairman and Community Admin get it, Secretary doesn't,
matching the same "the reviewer gets it" pattern already established
for families. 9 new tests across both pieces.

**The broader sweep, continued**: checked every remaining
community-level role's actual sidebar access, and found two more real
gaps of the same shape already fixed for family roles. `arrears_collector`
had no way to reach the Members page at all, meaning they couldn't
look someone up before collecting from them, their own core job.
Fixed by adding them to `FUNERAL_BROWSE_ROLES` directly, matching the
Role definition's own comment describing them as "the same job as
Collector, just scoped to arrears" — they now also reach `/funerals`
and `/pending-sync` the same way Collector does.

**A larger gap found, deliberately not attempted this turn**:
`notification_officer` has real backend authority to record funeral
attendance, tested and working, but there is no frontend page for
attendance-taking at all, for anyone, any role. This isn't a
navigation fix like the others, it's a genuine missing feature (a
list, a record form, a summary view), and building it properly is
separate, larger work than the rest of this sweep. Flagged rather
than rushed or silently skipped.

Full verification: `accounts` (141 tests) and `dashboard` (129 tests)
regression sweeps, TypeScript check, production build, vitest, all
clean.

## 180. A correction: last turn's "missing attendance feature" claim was wrong

Before building anything, checked the funeral detail page directly to
plan the new UI — and found `AttendancePanel`, a complete,
already-built, already-correctly-gated attendance feature (a summary
count, a member search to check someone in, a guest-by-name log, all
matching `funeral_logistics.permissions.ATTENDANCE_ROLES` exactly).

Last turn's claim that "there is no frontend page for attendance-
taking at all, for anyone" was wrong. That search used the literal
strings "FuneralAttendance" and "RecordAttendance" and never matched
the component's actual name — a real mistake in how the search was
done, not a real gap in the platform. Worth stating plainly rather
than glossed over.

The one thing that WAS genuinely still missing: `notification_officer`
had no way to navigate to `/funerals` at all, meaning the page this
already-built panel lives on was unreachable for them specifically,
even though their backend authority to use it was already correct and
already tested. Fixed by adding them to `FUNERAL_BROWSE_ROLES`. Member
search, which the panel's own "check in a member" feature depends on,
needed no change — reading and searching members is already open to
any authenticated person in the community, confirmed directly rather
than assumed.

Full frontend verification: TypeScript check, production build,
vitest, all clean.

## 181. Re-verification found two more real bugs — a significant one and a smaller, self-caused one

Doing exactly what was asked: going back through this session's own
claims and checking each one directly rather than trusting memory or
an earlier script, the same way last turn's mistake was caught.

**The significant one.** `ALL_COMMUNITY_ROLES`, the array behind
Tasks, Welfare & Contributions, My Receipts, and Support, was missing
all six roles built across the last several turns: Arrears Collector,
Family Arrears Officer, Family Registration Officer, Town Registration
Officer, Town Elders Arrears Officer, and Community Registration Desk.
None of these six could see their own tasks, their own welfare
obligations, their own receipts, or reach Support, despite every one
of them being, themselves, an ordinary community member with those
same needs. Confirmed the backend already supports all six correctly
(task viewing/completion and welfare access are both role-agnostic,
scoped to the person's own account, not gated by role at all) — this
was purely a frontend visibility gap, now fixed.

**The smaller, self-caused one.** Checking Chairman's own new
authority from the last turn more carefully: the frontend already
showed Chairman a working-looking Suspend/Reactivate button (since
two of the three functions behind it had been updated), but the third,
`_can_manage_this_members_role`, the one that actually authorizes
suspending an account, had been missed. Clicking it would have failed
with an unauthorized error despite looking perfectly usable. Fixed,
matching the same boundary Community Admin's own equivalent branch
already has.

New tests for both: the Chairman suspend fix has a dedicated test;
the `ALL_COMMUNITY_ROLES` fix is structural rather than logic-bearing,
so its own correctness is confirmed by the full regression sweep
passing rather than a new isolated test.

Full verification: `members` (173 tests, including the new Chairman
test), TypeScript check, production build, vitest, all clean.

## 181. Each funeral's closing report — built, tested, and now on screen

"Each funeral should have its own fund [so] the family knows the
expenditure of that particular funeral, so accountability wouldn't be
a problem... after every funeral each ledger should know the amount
they received, and those who didn't pay." Investigated before
building anything: per-funeral financial isolation already existed at
the database level, every obligation, payment, and expense was
already scoped to one specific funeral. What didn't exist was a
single, clear place that actually showed it, combining total received
with the specific list of who still owes.

Built `funeral_closing_report`, one function pulling together total
expected, total received, total outstanding, what was spent on
funeral logistics, gifts received (kept separate, never mixed into
the contribution figures), and the exact list of who hasn't paid with
their balance. Getting there caught several of my own mistakes along
the way, a guessed function name, treating a computed property as a
database field, a wrong function signature, each surfaced immediately
as a real test failure rather than a silent wrong answer. Along the
way, ran directly into two of the platform's own real rules while
testing (an expense can't be approved by whoever recorded it; a
member can't pay a newer funeral while owing an older one) and built
the tests around them rather than fighting them.

Wired into a new API action returning either the data directly or a
real, downloadable PDF, matching the exact pattern this platform
already uses for donation statements. A new panel on a closed
funeral's own detail page shows the same numbers on screen, with a
one-click PDF download for whoever needs the paper version.

7 backend tests (including one verifying isolation directly, a
payment toward a different funeral, confirmed not to count toward
this one's total) plus 2 endpoint tests confirming both the JSON and
the actual PDF bytes come back correctly. Full regression:
`funerals`, `funeral_logistics` (60 tests), all clean. Full frontend
verification: TypeScript check, production build, vitest, all clean.

## 182. Continuing the consistency sweep — a permission chain closed for Chairman

Picking back up the broader sweep across community-level roles.
Checking Secretary's dashboard routing led to re-examining Chairman's
"upper control" authority from two turns ago, and found it was only
half-applied.

That authority actually runs through two separate gates: the view's
own permission class (checked first, before a request reaches any
service code at all) and the service layer's own inner authorization
check. Two turns ago, only the service-layer check
(`_can_manage_this_members_role`) had been updated for Chairman. The
view-level gate (`CanAssignExecutiveRoles`) had not, meaning Chairman
could see a working-looking Suspend button on User Management that
would have failed the moment they actually clicked it, rejected before
the request ever reached the part that was already fixed.

Also revealed something about the existing test suite worth naming
plainly: it already had a test confirming Chairman could suspend an
account, but only by calling the service function directly, which
never exercises the view's permission class at all. That's exactly
the kind of blind spot that let this half-fix go undetected. Added a
new test that goes through the real HTTP endpoint end to end,
specifically to close that gap in coverage, not just in the
permission itself.

Full regression: `members` (174 tests, one new one added), all clean.

## 182. The closing report now shows each ledger's own amount received, not just one merged total

Extending last turn's closing report, matching its own wording more
precisely: "after every funeral each ledger should be able to know
the amount they received" names ledgers, plural. The report's single
`total_received` figure was accurate but merged, it never said which
of this platform's seven distinct ledgers (Family, Community, Guest,
Town Leaders, Town Elders' own mandatory contribution, Asupedeɛ,
In-Law) contributed what.

Checked first whether that per-ledger split already existed anywhere
rather than building a new one: it does, `funeral_full_ledger_breakdown`
already computes exactly this for a funeral's live, still-open view,
already tested, already correct. Folded that existing, unmodified
result directly into `funeral_closing_report`, on screen and in the
PDF both, rather than recomputing the same numbers a second way.

1 new backend test confirming the breakdown appears and each ledger's
own figure is genuinely correct, not just present. Full regression
across `funerals`, `reports`, and `gifts` (192 tests) to make sure
reusing this shared function didn't disturb where it already runs
elsewhere, all clean. Full frontend verification: TypeScript check,
production build, vitest, all clean.

## 183. A full system health check — every backend app, every role, both verified clean

"Update the system and make sure all features and dashboards are
working correctly." Taken as what it actually asks for: a genuine,
comprehensive verification pass, not a guess at what might be broken.

Ran the complete test suite for every one of this platform's 21
backend apps, not a sampled subset, in five batches to stay within
tool limits: `ai_features`, `communication`, `contribution_rules` (92
tests); `family_funds`, `funeral_logistics`, `messaging` (91);
`nsaabodeeq`, `payments`, `realtime`, `support` (60); `tasks`,
`tenants`, `audit_log`, `notifications` (297); `families` (55) — all
clean. Combined with `funerals`, `dashboard`, `members`, `accounts`,
`welfare`, `reports`, and `gifts`, already verified clean earlier in
recent turns, every single backend app now has a confirmed, passing
test run.

Beyond running tests, checked the dashboard routing matrix directly
rather than trusting that it was still complete after so many roles
have been added over many turns: every one of the 21 defined roles
has exactly one entry in the frontend's role-to-dashboard-route table
(no duplicates, nothing missing), and every one is explicitly
accounted for in the backend's own dashboard-section logic, with
Guest correctly landing in the one safe, intentional default rather
than by accident.

Full frontend verification: TypeScript check, production build (no
warnings, not just no errors), vitest, all clean.

**Nothing needed fixing this time** — worth stating plainly rather
than manufacturing a change to report. This was a genuine health
check, and the honest result is that the system checked out clean
across the board.

## 184. Donation money no longer netted against expenses; expenses can now record which ledger actually funded them

Working through a dense, multi-part message, taking each concrete
piece in turn rather than guessing at the whole thing at once.

**"You don't have to subtract the funeral expenses from the donation
collected."** Found a real, existing bug this pointed at directly:
`funeral_financial_overview`'s "Net cash position", shown prominently
on a funeral's own detail page, computed contributions plus gift cash
minus expenses as one merged figure. Gifts are a genuinely separate
ledger from the mandatory contribution ledger that actually funds a
funeral's expenses, netting them together implied donation money was
being spent down by expenses, which it never was. Fixed: net cash
position is now contributions minus expenses only; gift cash stays
its own, separate, untouched figure in the same response, exactly as
it should. The one existing test asserting the old, incorrect
calculation updated to assert the correct one.

**"The family financial secretary should create new expenses for the
family, so he can enter where the money they received to finance the
funeral is coming from."** A real, concrete gap: `FamilyFuneralExpense`
had no field at all for recording which ledger actually paid for a
given purchase. Added `funding_source` (contributions, gifts, or
other), threaded through the recording function, the API view, and
the serializer, with a safe default so it migrates cleanly onto the
already-deployed table. 2 new tests confirm it records correctly and
stays optional.

Full backend verification: `funeral_logistics` and `family_funds`
combined (75 tests), all clean.

**What this message also asked for, not yet touched**: summing all
four (or seven) ledgers into one grand total visible to both the
community finance officer and the family secretary together, and a
dedicated financial account the community finance officer opens per
family. Both are real, separate pieces of work, not small additions
to what's already built, and deserve their own dedicated turn rather
than being rushed in alongside this.

## 185. Both finance officers see money received per funeral; a real subscription plan tier for communities

Three distinct pieces, all backend, all tested.

**The community finance officer** (Treasurer, Financial Secretary,
Auditor): their dashboard was entirely time-based (today,
month-to-date) with no funeral-by-funeral view at all. Now includes a
list of every funeral that has actually collected anything, active or
closed, with expected, received, and outstanding totals per funeral —
"they are always involved in any funeral in the community, so they
should know all the money they received on each funeral." Pending
approval and cancelled funerals deliberately left out, since neither
ever had real obligations to collect against.

**The family finance officer** (Family Treasurer): the same list, but
narrowed strictly to funerals where the deceased is from their own
family — "only works when the funeral is from his family... each
family has their own finance officer." One shared helper with an
optional family filter, not two parallel implementations that could
ever drift apart. The isolation, a Family Treasurer never seeing
another family's funerals, verified by a dedicated test rather than
assumed.

**Subscription plan tiers**: checked first and found that the three
marketed plans (Single Funeral, Community, Multi-Community) existed
only as "coming soon" lead capture, explicitly documented as "aren't
real, payable products yet" — no way to actually put a community on
one. Added a real `subscription_plan` field to Community (distinct
from `access_plan`, which is about how long access lasts, not which
tier of service) and a Platform-Admin-only function to promote or
demote a community between tiers, recorded in the audit log. Two
real mistakes caught during this by verifying rather than assuming: a
guessed audit function name and a guessed category value, both
corrected against what actually exists.

Also confirmed, rather than rebuilt: suspend (`deactivate_community`),
reactivate, edit, delete, billing records, waiving a bill, and
subscription reminders all already existed for Platform Admin.

11 new tests across all three pieces. Full regression: `dashboard`
plus relevant `tenants` suites (159 tests), all clean.

**Not yet done, honestly**: none of this has a frontend yet. The
per-funeral lists aren't displayed on either finance dashboard, and
the plan-change function has no API endpoint or Communities-console
control wired to it. Backend complete and tested; the visible half is
next.

## 186. The visible half: all three finance/plan pieces now on screen

Finishing what last turn built and tested on the backend.

**Community finance officer's dashboard** now shows "Money received,
per funeral" — one row per funeral, newest first, received and
outstanding totals side by side, each deceased name linking straight
to that funeral's own page. **Family Treasurer's dashboard** shows
the same, already narrowed server-side to funerals where the
deceased is from their own family.

**Platform Admin's Communities console** gained a "Subscription
plan" control per community — the three marketed tiers as buttons,
the current one highlighted, one click to promote or demote. Wired
end to end: a new API endpoint matching the existing suspend/
reactivate pattern and capability check, the field added to the
community response, and 3 HTTP-level tests confirming a Platform
Admin can change it, a Community Admin is rejected, and an invalid
plan returns 400.

One real bug caught by those tests rather than by eye: the new view
referenced `ValidationError` unqualified, but that file imports it
under an alias — a NameError at runtime, fixed against what the file
actually imports.

Full verification: subscription plan suite (9 tests) clean;
TypeScript check, production build, vitest all clean.

## 187. Plans as real, editable records; the platform console redesigned; a role-by-role audit with four real gaps fixed

**Recovered first.** The sandbox reset between sessions, and the
previous session's plan-model backend work had been verified but
never packaged. Rebuilt it from the exact scripts that had been run,
against the last delivered zip, and re-verified with the same tests
before anything new was added. Checkpoint zips are now written
mid-turn, not only at the end, so this cannot recur.

**Subscription plans, "this is how I want the subscription plan to
be."** Plans were a fixed three-value enum with no prices, limits or
feature lists — nothing to edit, nothing to add. Now a real
`SubscriptionPlan` record: code, name, description, yearly and
monthly price, member limit, community limit, trial days, feature
list, active flag. Seeded from the exact three plans the public
homepage already describes (same names, taglines, features), prices
deliberately left unset rather than invented, since the platform has
no published pricing. `Community.subscription_plan` converted from
the enum to a foreign key in a three-step migration (add, seed and
map, swap) verified against the real upgrade path: communities
already on the old enum keep their plan through the conversion.
Platform Admin can create and edit plans (never the code, which
communities refer to); every change is audit-logged. A new `/plans`
page shows one card per plan with "+ New plan" and "Edit", and the
Communities console's plan selector now reads live plans instead of
a hardcoded three. 16 tests.

**Platform console overview**, "should look like the [reference]
template": KPI tiles (communities, members, users, subscription
revenue, active funerals), subscriptions by status, communities by
plan, and recent registrations, side by side.

**The role audit.** Built a matrix of all 21 roles: dashboard route,
the section its page reads, and every sidebar link it can actually
see. Every role has a route whose page reads exactly what the
backend sends. Four real gaps against roles' actual jobs, fixed:
Town Elders Arrears Officer and Town Registration Officer were shut
out of the Town Elders ledger, the one ledger they work on (backend
403 and no link); the Traditional Leader had no Members or Funerals
link despite managing Town Elders; the Bereaved Rep's dashboard
linked to funerals they had no navigation to; and the rule letting
a collector request reversal of their own mistaken payment was
hardcoded to `collector`, so the three arrears roles who also record
payments could not flag their own errors — extended to every
collecting role, with the `collected_by` check still the real
safeguard. 3 new tests.

Full verification: affected backend suites (55 tests) clean;
TypeScript check, production build, vitest clean.

## 188. Quick demo login covers all 22 roles, and the seed gives every one of them a working dashboard

"Add all the user roles to the quick login demo and seed it to be
working." The seed already created a `demo_<role>` account for every
role, but only 16 of the 22 had a button on the login page, and the
six newer roles would have landed on empty dashboards: the three
arrears roles work on closed-funeral balances, and the demo had no
closed funeral; the two Town-Elders officers work the Town Elders
ledger, and the demo had no town elders; the Deceased Rep was never
actually assigned to a family, so their dashboard had nothing to show.

Login page: all 22 roles now listed. Seed: a second, already-closed
funeral (Bretuo family) with real unpaid balances — 22 obligations
left owing on purpose, only the one member who later pays the active
funeral settles it first, since the platform's older-debt-first rule
would otherwise block that later payment; two genuine Town Elders on
the ledger; the demo Deceased Rep assigned to Asona; the demo
community placed on the Community plan. Still idempotent — run twice
on a fresh database, identical result.

One real seed bug found by actually running it rather than reading
it: the closed funeral is Bretuo's, and Bretuo had never been given
an approved contribution rate, so funeral creation aborted the whole
seed midway on a fresh database. Fixed.

Verified end to end on a fresh database: every one of the 22 demo
accounts logs in through the quick-login endpoint (all 200), and
every one's dashboard builds with real data — Family Arrears Officer
sees 20 members to follow up, Arrears Collector 22 customers owing,
Treasurer two funerals with money received, Chairman a 25-entry
activity feed, Deceased Rep their family's funeral, the Town Elders
ledger two elders. Existing demo-login and super-admin boundary suites
(21 tests) clean; TypeScript check and production build clean.

Deploy note: the seed is idempotent and safe to run on the live demo
community with `python manage.py seed_demo_data`; it only adds what
is missing.

## 189. The Member Registry: analytics, full profiles, move-to-family, and soft delete — each role inside its own jurisdiction

"Each user role should play and have access to its own tasks only.
The Town Registration Officer should know the total number of
registered members, update members' details, delete, edit, move a
member from one family to another... totals by gender, age, family by
family... click on someone's profile and see all their details, the
family they come from, how much they owe."

Checked the existing permissions first rather than assuming: the Town
Registration Officer was already in COMMUNITY_WIDE_MEMBER_ROLES — able
to edit any member in the community — with one deliberate, earlier
restriction kept intact: they may only *register* Town Elders. So the
registry follows the rules already in force, no scope change needed.

**Analytics** (`/members/analytics`, "Member Registry" in the sidebar):
totals by status, gender, age band, defaulter tier, family by family,
town elders, members with a login, registrations per month for the
last year, and the newest registrations. The scope is decided
server-side from the role: a community-wide registrar (Admin,
Chairman, Secretary, Community Registration Desk, Town Registration
Officer) sees the whole community; the Traditional Leader sees Town
Elders; a family officer sees only their own family. One page, no
leakage across jurisdictions — verified by test.

**Full profile** on every member's page: all recorded details, the
family, their login and role, what they owe right now, what they have
paid all time, every funeral obligation with its balance, and recent
payments. A family officer opening another family's member gets a
clean refusal.

**Move to another family**: genuinely new — `family` had never been an
editable field. Community-wide registrars only; a family officer can
never move someone across the line between two families. Past
obligations keep the family they were billed under (rewriting them
would corrupt the ledger); only future funerals bill under the new
family. A current Family Head must be replaced before being moved.

**"Can delete"**: a soft delete, on purpose. A member with payments on
the ledger can never be hard-deleted without corrupting every funeral's
accounts. Deactivating removes them from future funerals and active
counts and suspends their login; every past payment stays exactly
where it was recorded. Reactivation reverses it.

9 new tests; full `members` regression (183 tests) clean; TypeScript
check, production build, vitest clean.

## 190. Registration officers see member information only — never financial oversight

"The registration officer is not allowed to see the financial
oversight; his responsibility is to manage the community's
information."

Audited all three registration roles rather than the one link that
prompted this. One sidebar link was financial (the Town Elders Ledger,
added to the Town Registration Officer in the recent audit) — removed,
and the backend ledger view reverted for that role while the Town
Elders *Arrears* Officer keeps it, since collecting on that ledger is
their job. The bigger problem was the dashboards themselves: all three
registration roles landed on financial overviews — the Town
Registration Officer on the Chief's (collections, ledger totals), the
Community Registration Desk on the community's (today's collections,
outstanding, expenses), the Family Registration Officer on the
family's (statement, payment compliance).

All three now land on their own **Registration** dashboard: what they
have registered (total and this month), the size of their register,
members missing a date of birth, quick links to the Members page,
full registry analytics and inactive members, and the same registry
breakdowns (status, gender, age, defaulter standing, family by family,
registrations per month), scoped to their jurisdiction. No collections,
ledgers, or expenses anywhere on it — a test serialises the entire
payload sent to a Town Registration Officer and asserts none of
"collect", "expense", "ledger", "paid", "owed", "balance" or "amount"
appears in it.

Also removed these roles from the community/family/Town-Elders
dashboard role sets so they can never fall back onto a financial view.
Member profiles still show what a member owes: that is the member's
own record, which the officer explicitly needs.

Affected suites (22 tests) and full `dashboard` regression clean;
TypeScript check, production build, vitest clean.

## 191. Arrears Collector and Contribution Collector are now two genuinely different roles

"Differentiate the arrears officers and the contribution collectors;
they shouldn't have the same features since they're two different
role types. The arrears collector should have more options, and should
be able to clear or collect community members' arrears before they can
pay the current bills."

**What they shared before:** one blanket payment permission (either
could record any payment, open or closed funeral), one dashboard, and
the same arrears form on the Front Desk.

**The line now drawn at the ledger.** A Contribution Collector collects
on funerals that are OPEN. Arrears — what was left owing after a funeral
CLOSED — are collected only by the Arrears Collector (and the family /
Town Elders arrears officers within their scope). A plain Collector who
tries is refused with a message naming who can. A desk-worker
assignment, being an assignment to an open funeral's desk, no longer
reaches closed-funeral arrears either. Members can still pay their own
arrears themselves.

**"More options": the Arrears Desk** (`/arrears-desk`), the Arrears
Collector's own workspace. Look a member up and see every arrear
oldest first with the running total, their current bills, and — made
visible instead of discovered on a refused payment — whether those
bills are payable yet: they are locked until every arrear is cleared,
which is the platform's own older-debt-first rule. Then one lump sum:
the collector enters what the member is paying and the platform splits
it oldest-first across the arrears, never touching a newer debt while
an older one stands; once the arrears are cleared, anything left flows
straight on to the current bill, because that is exactly the moment
it becomes payable. More than everything owed is refused rather than
parked. Every slice is a real payment, so receipts, the audit trail
and live updates apply as always. The desk shows the split before
"Collect" is pressed.

**Their own dashboard** (`/dashboard/arrears`): the community-wide
worklist of everyone owing on a closed funeral, largest total first,
total arrears outstanding, closed funerals still carrying arrears, and
their own collections today and this week. The Contribution Collector
keeps the open-funeral dashboard unchanged. On the Front Desk, a plain
Collector now sees a referral notice for a member with arrears (what is
owed, and that it is collected at the Arrears Desk) instead of a form.

One bug caught by the tests rather than by eye: the lump-sum split
summed the remaining arrears after the loop, by which point the same
objects had already been updated in memory, so "cleared" came out
false even when it was true. Fixed by totalling before applying.

8 new tests; affected suites (51 tests) clean; TypeScript check,
production build, vitest clean. Verified on the seeded demo: the demo
Arrears Collector lands on the new dashboard with 22 members owing,
the demo Collector on the open-funeral one.

## 192. The role model made airtight: an executable access matrix, and a real leak it caught

"Make sure every role has all the features it needs and is restricted
from the features its role type does not have access to — make it
conk."

Reading through permissions again would not have made this solid. An
executable matrix does: `accounts/tests/test_role_access_matrix.py`
creates one user for every one of the 22 roles and, for each protected
area, hits it as every role and asserts against an explicit allowed
set — naming any role that reaches something it must not, or is
refused something it needs. Fourteen areas: the member registry, the
Town Elders ledger, the arrears worklist, the daily collections report,
the expense ledger (read), recording an expense, expense liabilities,
user management, subscription plans, the audit log, recording a
payment on an open funeral, recording one on a closed funeral
(arrears), registering a member, and — for every role — landing on
exactly its own dashboard section and never a management or financial
one it should not see. This is now the role model's definition, and
it runs with the suite.

**A real leak it caught on the first run.** Community expense
liabilities — money owed to suppliers — were readable by every
logged-in role, guests and ordinary members included. The permission
class on those views (`CanRecordExpenses`) waved every GET through and
only checked roles on writes, so the expense ledger, its summary, and
the liabilities were all open to anyone with a login. Fixed with a
distinct read-only `CanViewExpenses` for the recorders plus the two
who oversee spending without recording it (Chairman, Auditor —
exactly the five the sidebar's Expenses and Liabilities links already
showed); recording stays Treasurer/Financial Secretary/Admin. Checked
every other permission class with the same GET-through pattern: the
rest are intentional and documented (anyone in the community may view
members, families, funerals, and contribution rates).

The matrix also corrected my own expectation once: it flagged the
Family Arrears Officer reaching a closed-funeral arrear, and on
inspection the code was right — the debtor was in that officer's own
family, exactly their scope.

Regression across `funeral_logistics`, `reports`, and the role suites
(168 tests) clean. No frontend change was needed: the sidebar already
matched the enforced model.

## 193. System Settings: a fuller catalogue, save-with-confirmation, tasks by role, and the Family Secretary's bounded share

"Each community admin should have more settings options... assign
specific tasks to each user role type... the user should have options
to select or deselect more options before they click save, and ask
for confirmation before saving... each family secretary should also
have these features to manage their family, but shouldn't have more
options, and should have options to assign tasks to family members
only."

**More options.** The restrictable catalogue had not been extended
since several pages were built. Added the Member Registry, Arrears
Desk, Arrears Corrections, Funerals, Members, Families, User
Management, Collector Nominations, and Defaulters — 30 features now,
and the page groups them into Funerals & Collections, Finance &
Oversight, Administration, and Personal & Family so a long list reads
as four short ones.

**Select, then save, then confirm.** Every toggle used to save the
instant it was clicked. Now changes are held locally, highlighted,
with Show all / Hide all / Reset; nothing is sent until Save changes is
pressed, and then a dialog lists exactly what will be hidden and what
shown again for that person before "Yes, save".

**Tasks by role.** A new panel assigns one task to every person
currently holding a chosen role — one real, individually tracked task
each, through the same assignment path as a hand-assigned task — after
a confirmation naming the role and the count. Community-wide for
Admin, Chairman and Secretary; a Family Head or Family Secretary only
ever reaches their own family's members, enforced server-side.

**The Family Secretary's share.** Can now open System Settings and
restrict features for their own family's members, with the same
narrower family-relevant subset a Family Head gets and nothing wider
(a test asserts it is a strict subset with no Audit Log in it); and
can assign tasks — singly or by role — to family members only. One
subtle catch handled: the family boundary on task assignment was
keyed to "family_head" alone, so adding the Secretary to the role set
without widening that check would have made them community-wide.
Widened first, then added.

8 new tests; regression across tasks and accounts (105 tests) clean;
TypeScript check, production build, vitest clean.

## 194. Family roles now carry their family's name — "Asona Family Head", "Bretuo Secretary"

"Since each community will have many families, amend each family
executive role name to represent their family name... so their family
name is attached to their role, to make it unique and easier to
identify the family they are serving."

Done as a display rule, deliberately not a data change: every family
role's VALUE stays exactly what it is (`family_head`, `family_secretary`,
and so on drive every permission, dashboard route, and test in the
platform, and renaming them would be a large, risky migration for a
cosmetic result). What changed is the label a person sees.

`role_label_for(user)` on the backend: a family role reads as the
family it serves — Asona Family Head, Asona Secretary, Asona Treasurer,
Asona Registration Officer, Asona Arrears Officer; the same role in
Bretuo reads as Bretuo. A Collector nominated for one family's
contributions reads as that family's Collector ("Bretuo Collector");
community roles stay generic. Threaded through `/me` (so the sidebar
badge and profile show it), the User Management and System Settings
lists, and the login line on a member's full profile. On the System
Settings page, a Family Head or Secretary assigning a task by role now
sees "Asona treasurer", not "family treasurer", in the picker and the
confirmation. Historical records that carry only a role value (audit
log, messaging) stay generic, since the family at the time is not
stored with them.

5 new tests, including the family-nominated collector and the
end-to-end path to `/me` and the user list. Related suites (33 tests)
clean; TypeScript check, production build, vitest clean. On the seeded
demo, every family officer now reads as an Asona officer.

## 195. User Management: rename positions, edit details, reset passwords, and the Community Admin can restrict themselves (safely)

"Update user management so the admin can rename the user role
position, can edit details, and more... the community admin dashboard
and any feature he is not allowed to use should be restricted, and
ensure he has all the facilities he would need. Also add community
admin to the user settings so that he can also restrict himself."

**Rename role positions.** A community can now give any position the
title it actually uses — "President" for the Chairman, "Abusuapanin"
for a family head — stored on the community, display-only (every
permission still keys off the role value). Family roles keep their
family in front: the Asona head reads "Asona Abusuapanin". A panel on
User Management holds the titles locally until Save, then confirms.
Blank restores the default.

**Edit details and reset passwords.** For anyone you manage: correct
their name, email and phone; or set a new password when someone has
forgotten theirs at the desk. Both confirmed before sending, the reset
recorded in the audit log, and never usable on your own account (that
stays on the profile page).

**The Community Admin restricting themselves.** They now appear in
their own System Settings list as "(you)", with one safeguard the
earlier code had been protecting against: System Settings and User
Management can never be hidden from yourself, because that would be a
lockout with no way back. Enforced server-side and shown as "(always
yours)" in the UI. No other role may restrict itself.

**The boundary around the Community Admin.** Added a row to the
executable access matrix asserting the platform's community console
is Platform-Admin-only; together with the existing rows (plans,
audit log, and every operational area) this defines what a Community
Admin can and cannot reach, and the sidebar's six platform-only links
(communities, plans, platform admins, feature flags, revenue, platform
settings) never render for them.

10 new tests; full `accounts` suite (176 tests) clean; TypeScript
check, production build, vitest clean. Verified on the seeded demo.
...
3. ~~**WebSocket authentication**~~ Fixed. See entry 157 above:
   `realtime/consumers.py` now validates a real JWT at connect time and
   checks the connecting user's community against the funeral's, the
   same isolation the REST API already enforces.
