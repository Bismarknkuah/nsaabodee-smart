...
class MemberTaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser or user.role in COMMUNITY_WIDE_TASK_ROLES:
            return qs
        if user.role in (Role.FAMILY_HEAD, Role.FAMILY_SECRETARY) and own_member and own_member.family_id:
    ...
    @action(detail=False, methods=["post"], url_path="assign-to-role")
    def assign_to_role(self, request):
        """POST {role, title, description?, due_date?, priority?} -> one task per person holding that role, within the caller's own reach."""
        try:
            result = services.assign_task_to_role(
                actor=request.user, role=request.data.get("role", ""), title=request.data.get("title", ""),
                description=request.data.get("description", ""), due_date=request.data.get("due_date") or None,
                priority=request.data.get("priority") or MemberTask.Priority.MEDIUM,
            )
        except DjangoValidationError as exc:
