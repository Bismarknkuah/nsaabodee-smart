...
class TaskAssignmentTests(TestCase):
    ...
    def test_traditional_leader_can_assign_a_task_community_wide(self):
        """'The Traditional Leader should be able to assign tasks to the community executives and the family head.' A real regression risk: being in TASK_ASSIGNMENT_ROLES alone isn't enough — without also being in COMMUNITY_WIDE_TASK_ROLES, every assignment would fail the family-scope check, since a Traditional Leader typically has no family of their own at all."""
        User.objects.create_user(username="chief_tasker", password="x", community=self.bodi, role=Role.TRADITIONAL_LEADER)
        client = self._login("chief_tasker")
        # Asona and Bretuo are two different families — a genuinely
        # community-wide assigner must be able to reach both, not just
        # whichever family they might coincidentally belong to.
        res_a = client.post("/api/tasks/", {"assigned_to_id": str(self.asona_member.id), "title": "Chief's task for Asona"})
        res_b = client.post("/api/tasks/", {"assigned_to_id": str(self.bretuo_member.id), "title": "Chief's task for Bretuo"})
