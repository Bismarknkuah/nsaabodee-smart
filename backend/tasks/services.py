...
def _can_act_on_assignee(actor, assignee) -> bool:
    ...
    if actor is None or actor.is_superuser:
        return True
    if actor.role not in ("family_head", "family_secretary"):
...
def assign_task_to_role(*, actor, role: str, title: str, description: str = "", due_date=None, priority=MemberTask.Priority.MEDIUM) -> dict:
    """
    'Assign specific tasks to each user role type.' One task per person
    currently holding `role`, inside the actor's own reach: community-wide
    for Community Admin / Chairman / Secretary / Traditional Leader; a
    Family Head's or Family Secretary's own family members only. The
    same assign_task underneath, so every task is a real, individually
    tracked one — approval, reassignment, and the activity feed all
    apply exactly as for a task assigned by hand.
    """
    from accounts.models import Role, User
    from .permissions import COMMUNITY_WIDE_TASK_ROLES, TASK_ASSIGNMENT_ROLES
    if not (actor.is_superuser or actor.role in TASK_ASSIGNMENT_ROLES):
        raise ValidationError("Your role cannot assign tasks.")
    if role not in set(Role.values):
        raise ValidationError(f"'{role}' is not a role.")
    if not title.strip():
        raise ValidationError("A task needs a title.")
    holders = User.objects.filter(community=actor.community, role=role, is_active=True, member_profile__isnull=False).select_related("member_profile")
    if not (actor.is_superuser or actor.role in COMMUNITY_WIDE_TASK_ROLES):
        own_family = getattr(getattr(actor, "member_profile", None), "family_id", None)
        if own_family is None:
            raise ValidationError("Your account isn't linked to a family.")
        holders = holders.filter(member_profile__family_id=own_family)
    created = []
    for user in holders:
        created.append(assign_task(
            community=actor.community, assigned_to=user.member_profile, title=title.strip(), description=description,
            due_date=due_date, priority=priority, assigned_by=actor,
        ))
    return {"role": role, "assigned_count": len(created), "task_ids": [str(t.id) for t in created]}
