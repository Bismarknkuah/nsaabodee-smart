...
class FuneralLedgerConsumerTests(TransactionTestCase):
    ...
    async def test_connecting_and_receiving_a_broadcast_payment_event(self):
        ...
        communicator = WebsocketCommunicator(
            application, f"/ws/funerals/{self.funeral.id}/?token={token}",
        ...
        await database_sync_to_async(funeral_services.record_payment)(
            obligation=obligation, amount=Decimal("50"), method="cash", collector=self.admin, collector_name="Test Collector"
        )
    ...
    async def test_connecting_with_no_token_is_rejected(self):
        """The whole point of the fix: a funeral's memorial page is public and its id is right there in the URL, so with no check here anyone could watch every payer's name and amount live."""
        communicator = WebsocketCommunicator(
    ...
    async def test_connecting_with_an_invalid_token_is_rejected(self):
        communicator = WebsocketCommunicator(
            application, f"/ws/funerals/{self.funeral.id}/?token=not-a-real-token",
            headers=[(b"origin", b"http://testserver")],
        )
        connected, _ = await communicator.connect()
        self.assertFalse(connected)

    async def test_a_different_communitys_user_is_rejected(self):
        from channels.db import database_sync_to_async
        other_community = await database_sync_to_async(Community.objects.create)(name="Other Town", slug="other-town-ws")
        other_user = await database_sync_to_async(User.objects.create_user)(
            username="other_ws_user", password="x", community=other_community, role=Role.COMMUNITY_ADMIN
        )
        token = self._token_for(other_user)
        communicator = WebsocketCommunicator(
            application, f"/ws/funerals/{self.funeral.id}/?token={token}",
            headers=[(b"origin", b"http://testserver")],
        )
    ...
    async def test_a_platform_admin_can_connect_to_any_communitys_funeral(self):
        from channels.db import database_sync_to_async
        platform_admin = await database_sync_to_async(User.objects.create_user)(
            username="ws_platform_admin", password="x", role=Role.PLATFORM_ADMIN
        )
    ...
    async def test_two_different_funerals_are_isolated_channel_groups(self):
        ...
        await database_sync_to_async(funeral_services.record_payment)(
            obligation=obligation_a, amount=obligation_a.expected_amount, method="cash", collector_name="Test Collector"
        )

        communicator_a = WebsocketCommunicator(
            application, f"/ws/funerals/{self.funeral.id}/?token={self._token_for(self.admin)}",
            headers=[(b"origin", b"http://testserver")],
        )
        ...
        await database_sync_to_async(funeral_services.record_payment)(
            obligation=obligation_b, amount=Decimal("5"), method="cash", collector_name="Test Collector"
        )
