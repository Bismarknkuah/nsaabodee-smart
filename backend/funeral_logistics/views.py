...
from .permissions import CanRecordAttendance, CanRecordExpenses, CanViewExpenses
...
class FuneralExpenseListCreateView(APIView):
    ...
    def get_permissions(self):
        # Reading the expense ledger is oversight (Chairman and Auditor
        # included); adding to it is recording (Treasurer/Financial
        # Secretary/Admin only).
        extra = CanViewExpenses() if self.request.method == "GET" else CanRecordExpenses()
        return [IsAuthenticated(), extra]
...
class ExpenseSummaryView(APIView):
    ...
    def get(self, request, funeral_id):
        funeral = _get_funeral(request, funeral_id)
        return Response({**services.expense_summary(funeral), **services.expense_fund_summary(funeral)})


class ExpenseFundDepositView(APIView):
    """'Another option to add fund' — a real, explicit deposit event, never inferred from a contribution or gift payment."""
    permission_classes = [IsAuthenticated, CanRecordExpenses]

    def get(self, request, funeral_id):
        funeral = _get_funeral(request, funeral_id)
        deposits = funeral.expense_fund_deposits.select_related("deposited_by")
        return Response([
            {
                "id": str(d.id), "amount": str(d.amount), "note": d.note,
                "deposited_by_username": d.deposited_by.username if d.deposited_by_id else None,
    ...
    def post(self, request, funeral_id):
        ...
        try:
            deposit = services.add_expense_fund_deposit(
                funeral=funeral, amount=amount, note=request.data.get("note", ""), actor=request.user,
            )
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response({
            "id": str(deposit.id), "amount": str(deposit.amount), "note": deposit.note,
            "deposited_by_username": request.user.username, "created_at": deposit.created_at.isoformat(),
