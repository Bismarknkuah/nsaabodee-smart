...
class ExpenseFundTests(TestCase):
    """
    'It doesn't deduct once they are recording purchase but they
    should have another option to add fund and add purchases to
    subtract.' Recording an expense never touches contributions or
    gifts — that was already true. This tests the genuinely new part:
    a real, separate deposit history with its own running balance.
    """

    def setUp(self):
        self.bodi = Community.objects.create(
            name="Bodi Anidasoɔ", slug="bodi-expense-fund",
            default_general_male_amount=Decimal("5"), default_general_female_amount=Decimal("3"),
        )
        self.admin = User.objects.create_user(username="expense_fund_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.treasurer = User.objects.create_user(username="expense_fund_treasurer", password="a-real-password-123", community=self.bodi, role=Role.TREASURER)
        self.member = User.objects.create_user(username="expense_fund_member", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.funeral = funeral_services.create_funeral_event(
            community=self.bodi, deceased_name="Fund Deceased", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
            actor=self.admin, own_family_amount=Decimal("50"),
        )

    def test_a_deposit_never_touches_contributions_or_gifts(self):
        """The whole point — recording a fund deposit is completely separate from every other ledger."""
        from funerals.services import funeral_summary
        before = funeral_summary(self.funeral)
        services.add_expense_fund_deposit(funeral=self.funeral, amount=Decimal("500"), actor=self.admin)
        after = funeral_summary(self.funeral)
        self.assertEqual(before, after)

    def test_fund_balance_is_deposits_minus_real_expenses(self):
        services.add_expense_fund_deposit(funeral=self.funeral, amount=Decimal("500"), actor=self.admin)
        services.record_expense(
            funeral=self.funeral, description="Coffin", category="coffin", amount=Decimal("150"),
            incurred_on="2026-07-02", recorded_by=self.treasurer,
        )
        summary = services.expense_fund_summary(self.funeral)
        self.assertEqual(summary["total_deposited"], "500.00")
        self.assertEqual(summary["total_spent"], "150.00")
        self.assertEqual(summary["balance"], "350.00")

    def test_a_cancelled_expense_does_not_draw_down_the_fund(self):
        services.add_expense_fund_deposit(funeral=self.funeral, amount=Decimal("500"), actor=self.admin)
        expense = services.record_expense(
            funeral=self.funeral, description="Cancelled item", category="other", amount=Decimal("100"),
            incurred_on="2026-07-02", recorded_by=self.treasurer,
        )
        services.decide_expense_status(expense=expense, status="cancelled", actor=self.admin)
        summary = services.expense_fund_summary(self.funeral)
        self.assertEqual(summary["balance"], "500.00")

    def test_a_negative_balance_is_shown_honestly_not_hidden(self):
        """More committed to purchases than deposited to cover them — real information, not clamped to zero."""
        services.record_expense(
            funeral=self.funeral, description="Overspend", category="other", amount=Decimal("200"),
            incurred_on="2026-07-02", recorded_by=self.treasurer,
        )
        summary = services.expense_fund_summary(self.funeral)
    ...
    def test_a_zero_or_negative_deposit_is_rejected(self):
        with self.assertRaises(ValidationError):
            services.add_expense_fund_deposit(funeral=self.funeral, amount=Decimal("0"), actor=self.admin)


class ExpenseFundHttpTests(TestCase):
    def setUp(self):
        self.bodi = Community.objects.create(
            name="Bodi Anidasoɔ", slug="bodi-expense-fund-http",
            default_general_male_amount=Decimal("5"), default_general_female_amount=Decimal("3"),
        )
        self.admin = User.objects.create_user(username="expense_fund_http_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.member = User.objects.create_user(username="expense_fund_http_member", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.funeral = funeral_services.create_funeral_event(
            community=self.bodi, deceased_name="Fund HTTP Deceased", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
            actor=self.admin, own_family_amount=Decimal("50"),
        )
    ...
    def test_admin_can_record_a_deposit_via_http(self):
        client = self._login("expense_fund_http_admin")
        res = client.post(f"/api/funerals/{self.funeral.id}/expenses/fund-deposits/", {"amount": "300", "note": "Initial allocation"})
        self.assertEqual(res.status_code, 201)
        self.assertEqual(res.data["amount"], "300")

    def test_an_ordinary_member_cannot_record_a_deposit(self):
        client = self._login("expense_fund_http_member")
        res = client.post(f"/api/funerals/{self.funeral.id}/expenses/fund-deposits/", {"amount": "300"})
        self.assertEqual(res.status_code, 403)

    def test_the_expense_summary_endpoint_includes_the_fund_balance(self):
        admin_client = self._login("expense_fund_http_admin")
        admin_client.post(f"/api/funerals/{self.funeral.id}/expenses/fund-deposits/", {"amount": "300"})
        res = admin_client.get(f"/api/funerals/{self.funeral.id}/expenses/summary/")
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.data["balance"], "300.00")
