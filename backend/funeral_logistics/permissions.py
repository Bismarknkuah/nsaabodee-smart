...
# Oversight of expenses is wider than recording them: the Chairman ("upper
# control over the community ledger") and the Auditor review what was spent,
# but neither ever records a purchase — that stays with EXPENSE_ROLES.
EXPENSE_VIEWING_ROLES = EXPENSE_ROLES | {Role.CHAIRMAN, Role.AUDITOR}
...
class CanViewExpenses(BasePermission):
    def has_permission(self, request, view):
        if not (request.user and request.user.is_authenticated):
            return False
        return request.user.is_superuser or request.user.role in EXPENSE_VIEWING_ROLES
