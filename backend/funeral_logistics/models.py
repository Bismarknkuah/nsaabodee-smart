...
class FuneralExpenseFund(models.Model):
    """
    'It doesn't deduct once they are recording purchase but they
    should have another option to add fund and add purchases to
    subtract.' Recording an expense already never touches contributions
    or gifts — that was true before this model existed. What this adds
    is a genuine, dedicated running balance FOR expenses specifically:
    a real deposit history the committee can add to on purpose (an
    explicit allocation, a cash advance, a top-up), which approved
    purchases then draw down against — see
    funeral_logistics.services.expense_fund_summary for the actual
    balance calculation (total deposited minus total approved spend).
    A deposit here is deliberately its own, separate, auditable event —
    never inferred from a contribution or gift payment.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="+")
    funeral_event = models.ForeignKey("funerals.FuneralEvent", on_delete=models.CASCADE, related_name="expense_fund_deposits")
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    note = models.CharField(max_length=255, blank=True)
    deposited_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"+{self.amount} to {self.funeral_event.deceased_name}'s expense fund"
