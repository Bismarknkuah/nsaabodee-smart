...
from .models import FuneralAttendance, FuneralExpense, FuneralExpenseFund
...
def add_expense_fund_deposit(*, funeral: FuneralEvent, amount, note: str = "", actor=None):
    """
    'They should have another option to add fund and add purchases to
    subtract.' A genuinely separate, explicit event from recording a
    purchase — an allocation, a cash advance, a top-up — never
    inferred from a contribution or gift payment, and never touching
    those ledgers either way.
    """
    if amount <= 0:
        raise ValidationError("Deposit amount must be greater than zero.")
    return FuneralExpenseFund.objects.create(
        community=funeral.community, funeral_event=funeral, amount=amount, note=note.strip(), deposited_by=actor,
    )


def expense_fund_summary(funeral: FuneralEvent) -> dict:
    """
    The real, dedicated balance this fund actually has — total
    deposited minus every real (non-cancelled) expense recorded
    against this funeral, the same total expense_summary already
    computes. A negative balance is real information, not hidden —
    more has been committed to purchases than has actually been
    deposited to cover them yet.
    """
    total_deposited = sum((d.amount for d in funeral.expense_fund_deposits.all()), Decimal("0"))
    total_spent = Decimal(expense_summary(funeral)["total_expenses"])
    return {
        "total_deposited": str(total_deposited),
        "total_spent": str(total_spent),
        "balance": str(total_deposited - total_spent),
        "deposit_count": funeral.expense_fund_deposits.count(),
    }
...
def community_expenses_overview(community) -> list:
    ...
    return [
        {**expense_summary(funeral), **expense_fund_summary(funeral), "deceased_name": funeral.deceased_name, "deceased_family_name": funeral.deceased_family.name}
...
def funeral_financial_overview(funeral: FuneralEvent) -> dict:
    ...
    return {
        ...
        # "You don't have to subtract the funeral expenses from the
        # donation collected." Gifts are a genuinely separate ledger
        # from the mandatory contribution ledger that actually funds a
        # funeral's expenses — netting them together here implied
        # donation money was being spent down by expenses, which it
        # isn't. Contributions minus expenses only; gift_cash_collected
        # above stays its own, untouched figure.
        "net_cash_position": str(contributions_collected - total_expenses),
    }
