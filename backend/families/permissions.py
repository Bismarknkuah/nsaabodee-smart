...
class CanRecommendFamilyRate(BasePermission):
    ...
    related to money' — Family Treasurer can recommend their own
    family's rate too, the single most consequential money decision a
    family makes, without taking anything away from the Family Head,
    who can still do it directly for a smaller family with no dedicated
    Treasurer. Approval is a separate, stricter permission
    (CanManageFamilies) since only management roles may approve/reject —
    unaffected either way, whoever recommended it.
    """
    ...
    def has_permission(self, request, view):
        if not (request.user and request.user.is_authenticated):
            return False
        from accounts.models import Role
        return request.user.can_manage_families() or request.user.role in (Role.FAMILY_HEAD, Role.FAMILY_TREASURER)
