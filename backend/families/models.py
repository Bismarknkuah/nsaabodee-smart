...
class BereavedRepAssignment(models.Model):
    """
    'Each funeral must have a bereaved rep, and the rep should
    represent the family, so if one family have 2 deceased it means
    that all the two has to be on the bereaved rep dashboard... that
    account should be created by the community admin, secretary or
    chair but when one create she need other one to approved.'
    Deliberately family-scoped, not funeral-scoped — this is the real
    representation this role holds. Whichever active funerals belong
    to this family, current or future, are automatically what this
    person's own dashboard shows (see dashboard.services._bereaved_rep_view
    for the analytics view itself, which reads member.family directly
    rather than anything stored here).

    'Should be temporal' — a real account that genuinely stops working
    once deactivated, not merely hidden in a UI. is_active mirrors the
    same 'starts inactive until a second, different person from the
    same eligible set approves' pattern already established for desk
    assignments and donation-account registrations elsewhere in this
    platform — the creator and the approver must be two distinct
    people, never the same one twice.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    family = models.ForeignKey(Family, on_delete=models.CASCADE, related_name="bereaved_rep_assignments")
    ...
    def __str__(self):
        return f"Bereaved Rep for {self.family.name} — {self.member.full_name}"
