...
class FamilyDataExportView(APIView):
    """GET -> 'let the family head of each family be able to download their data.' A JSON snapshot of this family's own member roster, as a downloadable file."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, family_id):
        ...
        try:
            data = services.export_family_data(family=family, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        payload = json.dumps(data, indent=2)
        response = HttpResponse(payload, content_type="application/json")
        response["Content-Disposition"] = f'attachment; filename="{family.name}-backup.json"'
        return response
