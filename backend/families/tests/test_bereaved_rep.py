...
class CreateBereavedRepServiceTests(TestCase):
    ...
    def test_a_family_head_can_nominate_a_rep_for_their_own_family(self):
        """'The deceased family head will submit a representative to the community committee.'"""
        head_member = member_services.register_member(community=self.bodi, full_name="Asona Head", gender="male", family=self.asona)
        head_user = User.objects.create_user(username="bereaved_asona_head", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        member_services.link_member_to_user(member=head_member, user=head_user, actor=self.admin)

        assignment = services.create_bereaved_rep(family=self.asona, actor=head_user, member=self.candidate, new_username="nominated_rep_1", new_password="a-real-password-123")
        self.assertFalse(assignment.is_active, "A Family Head's nomination starts pending, submitted TO the committee, not self-approved.")

    def test_a_family_head_cannot_nominate_a_rep_for_a_different_family(self):
        bretuo = services.create_family(community=self.bodi, name="Bretuo", actor=self.admin)
        bretuo_head_member = member_services.register_member(community=self.bodi, full_name="Bretuo Head", gender="male", family=bretuo)
        bretuo_head_user = User.objects.create_user(username="bereaved_bretuo_head", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        member_services.link_member_to_user(member=bretuo_head_member, user=bretuo_head_user, actor=self.admin)

        with self.assertRaises(ValidationError):
            services.create_bereaved_rep(family=self.asona, actor=bretuo_head_user, member=self.candidate, new_username="nominated_rep_2", new_password="a-real-password-123")

    def test_a_family_head_cannot_approve_their_own_nomination(self):
        head_member = member_services.register_member(community=self.bodi, full_name="Asona Head 2", gender="male", family=self.asona)
        head_user = User.objects.create_user(username="bereaved_asona_head_2", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        member_services.link_member_to_user(member=head_member, user=head_user, actor=self.admin)

        assignment = services.create_bereaved_rep(family=self.asona, actor=head_user, member=self.candidate, new_username="nominated_rep_3", new_password="a-real-password-123")
        with self.assertRaises(ValidationError):
            services.approve_bereaved_rep(assignment=assignment, actor=head_user)
