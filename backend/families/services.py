...
from .models import BereavedRepAssignment, Family, FamilyAuditLog
...
_BEREAVED_REP_CREATOR_ROLES = {"community_admin", "secretary", "chairman"}


def create_bereaved_rep(*, family: Family, actor, member=None, new_username: str = None, new_password: str = None, new_email: str = ""):
    """
    'That account should be created by the community admin, secretary
    or chair but when one create she need other one to approved.'
    Community Admin's own direct creation is still immediately active
    — that authority already is the approval, the same pattern used
    everywhere else in this platform. Secretary or Chairman creating
    one starts it pending, needing a genuinely different person from
    this same eligible set to approve.

    'The deceased family head will submit a representative to the
    community committee.' A Family Head can also nominate one now —
    but ONLY for their own family (never someone else's, checked
    below), and their nomination always starts pending too, submitted
    "to the committee" rather than self-approved, the same as
    Secretary or Chairman.
    """
    from accounts.models import Role, User

    is_own_family_head = bool(actor) and actor.role == "family_head" and bool(
        getattr(actor, "member_profile", None) and actor.member_profile.family_id == family.id
    )
    if (actor is None or actor.role not in _BEREAVED_REP_CREATOR_ROLES) and not is_own_family_head and not (actor and actor.is_superuser):
        raise ValidationError("Only the Community Admin, Secretary, Chairman, or this family's own Family Head can create a Bereaved Rep account.")
    if family.status != Family.Status.ACTIVE:
        raise ValidationError("Can't assign a Bereaved Rep to an inactive family.")

    if member is None:
        if not (new_username and new_password):
            raise ValidationError("Provide either an existing member or a new username and password.")
        if User.objects.filter(username=new_username).exists():
            raise ValidationError(f"The username '{new_username}' is already taken.")
        from members.services import register_member
        member = register_member(community=family.community, full_name=new_username, gender="male", family=family)
    elif member.family_id != family.id:
        raise ValidationError("The Bereaved Rep must be a member of this specific family.")

    if member.linked_user_id:
        user = member.linked_user
        user.role = Role.BEREAVED_REP
        user.save(update_fields=["role"])
    else:
        if not (new_username and new_password):
            raise ValidationError("This member has no login yet — provide a username and password to create one.")
        if User.objects.filter(username=new_username).exists():
            raise ValidationError(f"The username '{new_username}' is already taken.")
        user = User.objects.create_user(
            username=new_username, password=new_password, email=new_email,
            community=family.community, role=Role.BEREAVED_REP,
        )
        from members.services import link_member_to_user
        link_member_to_user(member=member, user=user, actor=actor)

    is_directly_approved = bool(actor.is_superuser or actor.role == "community_admin")
    assignment = BereavedRepAssignment.objects.create(
        family=family, member=member, user=user, created_by=actor, is_active=is_directly_approved,
...
def approve_bereaved_rep(*, assignment: "BereavedRepAssignment", actor):
    """A genuinely different person from the same eligible set (Community Admin/Secretary/Chairman) than whoever created it."""
    if actor.role not in _BEREAVED_REP_CREATOR_ROLES and not actor.is_superuser:
        raise ValidationError("Only the Community Admin, Secretary, or Chairman can approve a Bereaved Rep account.")
    if assignment.created_by_id == actor.id and not actor.is_superuser:
        raise ValidationError("You created this Bereaved Rep assignment — someone else must approve it.")
...
def deactivate_bereaved_rep(*, assignment: "BereavedRepAssignment", actor):
    """'Should be temporal' — a real, working deactivation, not just a hidden UI state."""
    if actor.role not in _BEREAVED_REP_CREATOR_ROLES and not actor.is_superuser:
        raise ValidationError("Only the Community Admin, Secretary, or Chairman can deactivate a Bereaved Rep account.")
    assignment.is_active = False
    assignment.deactivated_at = timezone.now()
    assignment.deactivated_by = actor
    assignment.save(update_fields=["is_active", "deactivated_at", "deactivated_by"])
    return assignment


def list_pending_bereaved_rep_assignments(community):
    return BereavedRepAssignment.objects.filter(
        family__community=community, is_active=False, deactivated_at__isnull=True,
    ).select_related("family", "member")


def export_family_data(*, family: Family, actor) -> dict:
    """
    'Let the family head of each family be able to download their
    data.' A JSON snapshot of this family's own member roster —
    matching the shape of the Community Admin's own
    tenants.services.export_community_backup, so the same restore path
    (bulk_register_members, matched by membership_number/name) already
    works on a file downloaded from here. is_family_officer gates this
    the same way it already gates the Family Fund — this family's own
    head/secretary/treasurer, or Community Admin+, never any other
    family's officer.
    """
    if not is_family_officer(actor, family):
        raise ValidationError("Only this family's own Head, Secretary, or Treasurer can download its data.")

    from members.models import Member

    members = []
    for m in Member.objects.filter(family=family):
        members.append({
            "membership_number": m.membership_number, "full_name": m.full_name, "family_name": family.name,
    ...
    return {
        "family_name": family.name, "community_name": family.community.name,
        "exported_at": timezone.now().isoformat(), "members": members,
    }
