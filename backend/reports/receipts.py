...
def contribution_receipt_data(payment: ContributionPayment) -> dict:
    ...
    return {
        ...
        # 'The collector name should always be in the receipt.' Uses the
        # real, explicitly-typed collector_name field — the shared-desk
        # login account's own get_full_name() is frequently blank
        # (first_name/last_name are rarely set on these accounts) and,
        # more importantly, is the wrong person to record when a desk
        # assignment is genuinely shared by more than one real person
        # across a long day.
        "collector_name": payment.collector_name or (payment.collected_by.get_full_name() if payment.collected_by else None),
        ...
        # the pay.' Passed through so contribution_receipt_pdf below
        # can embed a genuine, scannable QR image directly on the
        # printed slip, not just show it separately on a web page.
...
def gift_receipt_data(donation: GiftDonation) -> dict:
    ...
    return {
        ...
        "collector_name": donation.collector_name or (donation.collected_by.get_full_name() if donation.collected_by else None),
