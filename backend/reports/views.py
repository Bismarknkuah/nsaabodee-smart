...
class TownEldersLedgerReportView(APIView):
    ...
    def get(self, request):
        if not (
            request.user.is_superuser
            or request.user.role in ("traditional_leader", "community_admin", "chairman", "secretary")
        ):
            return Response({"detail": "Only the Traditional Leader, Community Admin, Chairman, or Secretary can view the Town Elders ledger."}, status=403)
