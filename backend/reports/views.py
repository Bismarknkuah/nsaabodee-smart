...
class OutstandingMembersReportView(APIView):
    """
    'Must NOT... access sensitive personal financial information
    unless explicitly authorized by community policy' — the same rule
    that made the Traditional Leader's own dashboard aggregate-only
    (see dashboard.services._traditional_leader_view) applies here
    too. Reaching this same data through the separate Reports page was
    a real gap: individual member names and personal debt amounts,
    exactly what that rule exists to keep from this role.
    """
    permission_classes = [IsAuthenticated, CanViewReports]

    def get(self, request):
        report = services.outstanding_members_report(community=request.user.community)
        if request.user.role == "traditional_leader" and not request.user.is_superuser:
            from decimal import Decimal
            return Response({
                "outstanding_member_count": report["outstanding_member_count"],
                "total_owed": str(sum((Decimal(m["total_owed"]) for m in report["members"]), Decimal("0"))),
            })
        return Response(report)


class TownEldersLedgerReportView(APIView):
    """
    'The town leader/king is the head of the community's elders
    ledger.' Deliberately a narrow, explicit exception to the
    aggregate-only rule OutstandingMembersReportView enforces above —
    the Traditional Leader gets full member-level detail specifically
    for THIS ledger, since they're the one actually running it, but
    still get the aggregate-only treatment everywhere else on the
    platform; this view is the one, deliberate carve-out, not a
    general loosening of that rule.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        if not (
            request.user.is_superuser
            # The Town Elders Arrears Officer collects on exactly this
            # ledger, so they can read it. The Town Registration Officer
            # deliberately cannot: "the registration officer is not allowed
            # to see the financial oversight" — their job is the elders'
            # information, not their money.
            or request.user.role in ("traditional_leader", "community_admin", "chairman", "secretary", "town_elders_arrears_officer")
        ):
            return Response({"detail": "Only the Traditional Leader, Community Admin, Chairman, Secretary, or the Town Elders Arrears Officer can view the Town Elders ledger."}, status=403)
        return Response(services.town_elders_ledger_report(community=request.user.community))
...
class ContributionReceiptQrCodeView(APIView):
    """
    GET -> 'the receipt should have a bar scan code so that they can
    scan to confirm their payment.' A separate endpoint rather than
    folding this into the existing plain-text /text/ endpoint above,
    which three different screens already call expecting a plain
    string back — changing that shape risks breaking all three.
    """
    permission_classes = [IsAuthenticated, CanViewReceipts]

    def get(self, request, payment_id):
        from members.services import generate_qr_code_base64_for
        payment = _get_payment(request, payment_id)
        return Response({
            "qr_code_base64": generate_qr_code_base64_for(payment.qr_payload),
            "verify_url": payment.qr_payload,
        })
...
class SendPaymentTrackingSmsView(APIView):
    """POST -> 'either to print receipt, or to send SMS message which... there should be a link that the person can click on to track his payment.' The other delivery option, alongside marking a physical receipt printed."""
    permission_classes = [IsAuthenticated, CanViewReceipts]
    ...
    def post(self, request, payment_id):
        ...
        payment = _get_payment(request, payment_id)
        try:
            result = services.send_payment_tracking_sms(payment=payment, actor=request.user)
...
class ReceiptVerificationView(APIView):
    ...
    mean the collector.' Deliberately IsAuthenticated only, not
    CanViewReceipts — this is what the printed receipt's own QR code
    lands on (see ContributionPayment.qr_payload), matching the same
    "a real, scannable URL behind a login, not a role-restricted
    page" choice already made for Member.qr_payload. Confirming a
    receipt is genuine is a lighter action than viewing/downloading it.
    """
    permission_classes = [IsAuthenticated]

    def get(self, request, payment_id):
        payment = _get_payment(request, payment_id)
        return Response(services.verify_receipt(payment=payment))


class ReceiptQrCodeView(APIView):
    """GET -> the printed receipt's own QR code image, base64-encoded — scanning it lands on ReceiptVerificationView above."""
    permission_classes = [IsAuthenticated, CanViewReceipts]

    def get(self, request, payment_id):
        payment = _get_payment(request, payment_id)
        return Response({"qr_code_base64": services.generate_payment_qr_code_base64(payment)})
...
class MembersExportView(APIView):
    """
    'All user role types should have analytics views based on what
    they have access to... all data should be downloaded or
    printable.' Deliberately just IsAuthenticated, not CanViewReports
    — that permission excludes Family Head/Secretary/Treasurer, but
    "family executive can only sort or download data of family
    members only" means they need this too. services.members_export_rows
    reuses members.services.search_members directly, so the exact same
    scoping the Members page itself already enforces (family-only for
    family executives, community-wide for everyone else) applies here
    automatically — there's no separate, parallel scoping check to
    keep in sync or accidentally get wrong.
    """
    permission_classes = [IsAuthenticated]

    def get(self, request):
        # Deliberately 'export_format', not 'format' — DRF reserves
        # ?format= for its own content-negotiation (URL_FORMAT_OVERRIDE),
        # and since no renderer is registered for "csv" or "pdf", using
        # the reserved name here caused every request to fail content
        # negotiation and 404 before authentication even ran, no matter
        # how the request was otherwise correct.
        export_format = request.query_params.get("export_format", "csv")
        rows = services.members_export_rows(community=request.user.community, actor=request.user)

        is_family_scoped = (
            not request.user.is_superuser
            and request.user.role in ("family_head", "family_secretary", "family_treasurer")
        )
        scope_label = "Your family only" if is_family_scoped else "Community-wide"

        if export_format == "pdf":
            pdf_bytes = pdf_module.members_list_pdf(rows, request.user.community.name if request.user.community else "", scope_label)
            response = HttpResponse(pdf_bytes, content_type="application/pdf")
            response["Content-Disposition"] = 'inline; filename="members.pdf"'
            return response
        ...
        writer = csv.DictWriter(buffer, fieldnames=[
            "membership_number", "full_name", "family_name", "gender", "status", "phone", "email",
            "date_of_birth", "occupation", "address", "ghana_card_number",
            "mother_name", "father_name", "hometown", "marital_status", "spouse_name",
            "emergency_contact_name", "emergency_contact_phone", "defaulter_tier",
        ])
        writer.writeheader()
