...
class FourLedgerFamilyStatementTests(TestCase):
    ...
    def test_outsider_member_payment_shows_up_in_community_ledger_for_this_family(self):
        ...
        funeral_services.record_payment(obligation=obligation, amount=Decimal("3"), method="cash", collector_name="Test Collector")
    ...
    def test_guest_and_town_leader_ledgers_appear_separately_in_family_statement(self):
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Guest", amount_cash=Decimal("40"), collector_name="Test Collector")
...
class SixLedgerBreakdownTests(TestCase):
    """The full-ledger breakdown now covers all six of the document's own ledgers, not just the original four."""

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="sixledger-bodi", default_general_male_amount=Decimal("5"))
        self.admin = User.objects.create_user(username="sixledger_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.funeral = funeral_services.create_funeral_event(
    ...
    def test_the_breakdown_includes_asupede_and_in_law_alongside_the_original_five(self):
        breakdown = services.funeral_full_ledger_breakdown(self.funeral)
        self.assertIn("asupede_ledger", breakdown)
        self.assertIn("in_law_ledger", breakdown)
        self.assertIn("family_ledger", breakdown)
        self.assertIn("community_ledger", breakdown)
        self.assertIn("guest_ledger", breakdown)
        self.assertIn("town_leaders_ledger", breakdown)
        self.assertIn("town_elders_contribution_ledger", breakdown)

    def test_an_inactive_asupede_ledger_reports_zero_without_erroring(self):
        breakdown = services.funeral_full_ledger_breakdown(self.funeral)
        self.assertFalse(breakdown["asupede_ledger"]["active"])
        self.assertEqual(breakdown["asupede_ledger"]["expected_total"], "0")

    def test_activating_asupede_is_reflected_in_the_combined_breakdown(self):
        funeral_services.activate_asupede(funeral=self.funeral, amount=Decimal("20"), actor=self.admin)
        breakdown = services.funeral_full_ledger_breakdown(self.funeral)
        self.assertTrue(breakdown["asupede_ledger"]["active"])
        self.assertEqual(Decimal(breakdown["asupede_ledger"]["amount"]), Decimal("20"))

    def test_an_in_law_request_is_reflected_in_the_combined_breakdown(self):
        ...
        funeral_services.request_in_law_contribution(
            funeral=self.funeral, in_law_family=bretuo, relationship="test", requested_amount=Decimal("300"), actor=self.admin,
        )
        breakdown = services.funeral_full_ledger_breakdown(self.funeral)
        self.assertEqual(breakdown["in_law_ledger"]["request_count"], 1)
        self.assertEqual(breakdown["in_law_ledger"]["pending_count"], 1)
