...
class PdfGenerationTests(TestCase):
    ...
    def test_contribution_receipt_pdf_handles_missing_collector_and_family(self):
        ...
        payment = funeral_services.record_payment(obligation=obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
    ...
    def test_gift_receipt_pdf_cash_only(self):
        donation = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Donor", amount_cash=Decimal("100"), collector_name="Test Collector")
    ...
    def test_collections_report_pdf_with_activity(self):
        ...
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A", amount_cash=Decimal("20"), payment_method="mobile_money", collector_name="Test Collector")
    ...
    def test_receipt_text_uses_the_real_typed_collector_name_not_the_login_account(self):
        """
        'The collector name should always be in the receipt.' A real
        bug: this used to read payment.collected_by.get_full_name() —
        the login account's Django name, frequently blank — instead of
        the actual collector_name typed at the moment of collection.
        """
        from funerals.models import ContributionObligation
        collector_account = User.objects.create_user(username="unnamed_collector_account", password="x", community=self.bodi, role=Role.COLLECTOR)
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        payment = funeral_services.record_payment(
            obligation=obligation, amount=Decimal("50"), method="cash",
            collector=collector_account, collector_name="Ama Serwaa",
        )
        text = receipts.contribution_receipt_text(payment, self.bodi.name)
        self.assertIn("Collector: Ama Serwaa", text)
        self.assertNotIn("Collector: -", text)

    def test_gift_receipt_text_uses_the_real_typed_collector_name(self):
        collector_account = User.objects.create_user(username="unnamed_gift_collector", password="x", community=self.bodi, role=Role.COLLECTOR)
        donation = gift_services.record_gift_donation(
            funeral=self.funeral, donor_name="A Donor", amount_cash=Decimal("20"), payment_method="cash",
            collected_by=collector_account, collector_name="Kofi Boateng",
        )
        text = receipts.gift_receipt_text(donation, self.bodi.name)
        self.assertIn("Collector: Kofi Boateng", text)
