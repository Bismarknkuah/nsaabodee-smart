...
class FuneralDailyBreakdownTests(TestCase):
    ...
    def test_a_payment_shows_up_on_the_correct_day_only(self):
        payment = funeral_services.record_payment(obligation=self.obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
    ...
    def test_gifts_and_contributions_combine_into_the_day_total(self):
        ...
        payment.save(update_fields=["paid_at"])
        gift = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Guest", amount_cash=Decimal("20"), collector_name="Test Collector")
