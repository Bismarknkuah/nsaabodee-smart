...
class CollectionsReportTests(TestCase):
    ...
    def test_daily_report_totals_contributions_by_method(self):
        ...
        funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector=self.collector, collector_name="Test Collector")
    ...
    def test_gift_cash_counted_but_item_only_gift_excluded_from_cash_total(self):
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A", amount_cash=Decimal("40"),
                                            payment_method="mobile_money", collected_by=self.collector, collector_name="Test Collector")
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="B", gift_item="Goat",
                                            estimated_item_value=Decimal("300"), collected_by=self.collector, collector_name="Test Collector")
    ...
    def test_combined_cash_position_sums_both_ledgers_without_altering_them(self):
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A", amount_cash=Decimal("40"), payment_method="cash", collector_name="Test Collector")
    ...
    def test_report_scoped_to_one_collector_excludes_others(self):
        ...
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        funeral_services.record_payment(obligation=obligation, amount=Decimal("20"), method="cash", collector=self.collector, collector_name="Test Collector")
        ...
        funeral_services.record_payment(obligation=obligation2, amount=Decimal("50"), method="cash", collector=other_collector, collector_name="Test Collector")
...
class ReceiptTests(TestCase):
    ...
    def test_contribution_receipt_data_has_every_required_field(self):
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        payment = funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector=self.admin, collector_name="Test Collector")
    ...
    def test_contribution_receipt_text_is_printable_plain_text(self):
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        payment = funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
...
class MyReceiptsAndDeliveryChannelTests(TestCase):
    ...
    def test_momo_payment_is_classified_electronic(self):
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        payment = funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="mobile_money", collector_name="Test Collector")
    ...
    def test_gift_cash_physical_gift_item_electronic(self):
        cash_gift = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A", amount_cash=Decimal("20"), payment_method="cash", collector_name="Test Collector")
        item_gift = gift_services.record_gift_donation(funeral=self.funeral, donor_name="B", gift_item="Yam",
                                                         estimated_item_value=Decimal("30"), collector_name="Test Collector")
    ...
    def test_linked_user_sees_both_their_payments_and_gifts_they_gave(self):
        ...
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        funeral_services.record_payment(obligation=obligation, amount=Decimal("20"), method="cash", collector_name="Test Collector")
...
class PrintTrackingTests(TestCase):
    ...
    def test_electronic_payment_never_appears_in_unprinted_list(self):
        ...
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.member)
        funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="mobile_money", collector_name="Test Collector")

        unprinted = services.unprinted_receipts(community=self.bodi)
        self.assertEqual(len(unprinted["unprinted_contribution_payments"]), 0)

    def test_gift_cash_donation_tracked_separately_from_contribution_payments(self):
        donation = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Donor", amount_cash=Decimal("40"), collector_name="Test Collector")
        unprinted = services.unprinted_receipts(community=self.bodi)
    ...
    def test_item_only_gift_never_appears_in_unprinted_list(self):
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A", gift_item="A cow", estimated_item_value=Decimal("800"), collector_name="Test Collector")
        unprinted = services.unprinted_receipts(community=self.bodi)
        self.assertEqual(len(unprinted["unprinted_gift_donations"]), 0)


class SendPaymentTrackingSmsTests(TestCase):
    """'Either to print receipt, or to send SMS message which... there should be a link that the person can click on to track his payment.'"""

    def setUp(self):
        self.bodi = Community.objects.create(
            name="Bodi Anidasoɔ", slug="sptms-bodi",
            default_general_male_amount=Decimal("5"), default_general_female_amount=Decimal("3"),
        )
        self.admin = User.objects.create_user(username="sptms_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        family_services.recommend_family_rate(family=self.asona, amount=Decimal("50"), actor=self.admin)
        family_services.approve_family_rate(family=self.asona, actor=self.admin)
        self.member = member_services.register_member(community=self.bodi, full_name="Kojo", gender="male", family=self.asona, phone="+233200000001")
        self.funeral = funeral_services.create_funeral_event(
            community=self.bodi, deceased_name="Yaw Asona", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
        )
    ...
    def _mock_success(self):
        ...
        mock_response.text = '{"sid": "SMxxx"}'
        return mock_response

    @override_settings(TWILIO_ACCOUNT_SID="test_sid", TWILIO_AUTH_TOKEN="test_token", TWILIO_FROM_NUMBER="+15005550006")
    def test_sending_the_sms_succeeds_and_includes_the_tracking_link(self):
        with patch("requests.post", return_value=self._mock_success()) as mock_post:
            result = services.send_payment_tracking_sms(payment=self.payment, actor=self.admin)
        self.assertEqual(result["status"], "sent")
        sent_message = mock_post.call_args.kwargs["data"]["Body"]
        self.assertIn(f"/verify-receipt/{self.payment.id}", sent_message)
        self.assertIn(self.payment.receipt_number, sent_message)

    def test_a_member_with_no_phone_number_is_rejected_clearly(self):
        no_phone_member = member_services.register_member(community=self.bodi, full_name="No Phone", gender="female", family=self.asona)
        from funerals.models import ContributionObligation
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=no_phone_member)
        payment = funeral_services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
        with self.assertRaises(ValidationError):
            services.send_payment_tracking_sms(payment=payment, actor=self.admin)

    def test_sms_not_configured_is_reported_clearly_not_as_a_crash(self):
        # No override_settings here — Twilio is genuinely unconfigured, matching production defaults.
        with self.assertRaises(ValidationError):
            services.send_payment_tracking_sms(payment=self.payment, actor=self.admin)

    @override_settings(TWILIO_ACCOUNT_SID="test_sid", TWILIO_AUTH_TOKEN="test_token", TWILIO_FROM_NUMBER="+15005550006")
    def test_a_twilio_failure_response_is_reflected_in_the_result_not_silently_swallowed(self):
        mock_response = Mock()
    ...
    @override_settings(TWILIO_ACCOUNT_SID="test_sid", TWILIO_AUTH_TOKEN="test_token", TWILIO_FROM_NUMBER="+15005550006")
    def test_send_tracking_sms_over_http(self):
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "sptms_admin", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        with patch("requests.post", return_value=self._mock_success()):
            res = client.post(f"/api/receipts/contribution-payments/{self.payment.id}/send-tracking-sms/")
