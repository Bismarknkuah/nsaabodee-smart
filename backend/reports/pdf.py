...
def contribution_receipt_pdf(data: dict, community_name: str) -> bytes:
    ...
    # scan it should confirm the amount they paid and who received the
    # pay.' Generated fresh here (not passed in as an image) so this
    # function stays a pure presentation layer over qr_payload, the
    # one already-correct fact — same "generate straight from the
    # payload, right where it's drawn" approach reportlab itself
    # expects for inline images.
    import qrcode
    from reportlab.lib.utils import ImageReader
    qr_buffer = io.BytesIO()
    qrcode.make(data["qr_payload"]).save(qr_buffer, format="PNG")
    qr_buffer.seek(0)
    qr_size = 22 * mm
    c.drawImage(ImageReader(qr_buffer), (width - qr_size) / 2, y - qr_size, width=qr_size, height=qr_size)
    y -= qr_size + 4 * mm
    c.setFont("Helvetica", 6)
    c.setFillColor(colors.grey)
    c.drawCentredString(width / 2, y, "Scan to verify this receipt")
    y -= 6 * mm
...
def members_list_pdf(members: list, community_name: str, scope_label: str) -> bytes:
    """
    'All user role types should have analytics views... all data
    should be downloaded or printable.' `members` is a list of dicts
    with the exact keys this function reads below — the caller (see
    reports.services.members_export_rows) decides the querying and
    scoping (community-wide vs. one family only); this file only lays
    out numbers already decided elsewhere, the same division of
    responsibility every other function in this file already follows.
    """
    buffer, doc, elements = _statement_doc(community_name, "Members Register", scope_label)
    rows = [["#", "Name", "Family", "Gender", "Status", "Phone"]]
    for i, m in enumerate(members, start=1):
        rows.append([str(i), m["full_name"], m["family_name"] or "—", m["gender"].title(), m["status"].title(), m["phone"] or "—"])
    table = _styled_table(rows)
    table.setStyle(TableStyle([("ALIGN", (1, 0), (-1, -1), "LEFT")]))  # override _styled_table's money-table default (right-aligned)
    elements.append(table)
    elements.append(Spacer(1, 6 * mm))
    elements.append(Paragraph(f"{len(members)} member(s) listed.", _LABEL_STYLE))
    doc.build(elements)
    return buffer.getvalue()


def funeral_closing_report_pdf(*, community_name: str, report: dict) -> bytes:
    """
    ...
    amount they received, and those who didn't pay.' A single,
    printable document for one funeral, `report` is exactly what
    funerals.services.funeral_closing_report() returns.
    """
    buffer, doc, elements = _statement_doc(
        community_name, f"Closing Report — {report['deceased_name']}'s Funeral",
        subtitle=f"{report['deceased_family']} family",
    )

    totals_rows = [
        ["", "Amount"],
        ["Total expected", _money(report["total_expected"])],
        ["Total received", _money(report["total_received"])],
        ["Total outstanding", _money(report["total_outstanding"])],
        ["Spent on funeral logistics", _money(report["total_spent_on_logistics"])],
        ["Gifts received (separate ledger)", _money(report["total_gifts_received"])],
    ]
    elements.append(_styled_table(totals_rows))
    elements.append(Spacer(1, 8 * mm))

    # 'After every funeral each ledger should be able to know the
    # amount they received.' Each of the seven ledgers this funeral
    # actually keeps, side by side, rather than only the one merged
    # total_received figure above.
    lb = report.get("ledger_breakdown")
    if lb:
        elements.append(Paragraph("Amount received, by ledger", _STYLES["Heading3"]))
        ledger_rows = [["Ledger", "Received"]]
        ledger_rows.append(["Family Ledger", _money(lb["family_ledger"]["collected_total"])])
        ledger_rows.append(["Community Ledger", _money(lb["community_ledger"]["collected_total"])])
        ledger_rows.append(["Town Elders Ledger (mandatory)", _money(lb["town_elders_contribution_ledger"]["collected_total"])])
        ledger_rows.append(["Guest Ledger (gifts)", _money(lb["guest_ledger"]["total_value"])])
        ledger_rows.append(["Town Leaders Ledger (gifts)", _money(lb["town_leaders_ledger"]["total_value"])])
        ledger_rows.append(["Asupedeɛ Ledger", _money(lb["asupede_ledger"].get("collected_total", "0"))])
        ledger_rows.append(["In-Law Ledger", _money(lb["in_law_ledger"].get("collected_total", "0"))])
        elements.append(_styled_table(ledger_rows))
        elements.append(Spacer(1, 8 * mm))

    elements.append(Paragraph("Members who have not yet paid", _STYLES["Heading3"]))
    if report["who_did_not_pay"]:
        rows = [["Member", "Family", "Expected", "Paid", "Balance"]]
        for e in report["who_did_not_pay"]:
            rows.append([e["member_name"], e["family_name"] or "-", _money(e["expected_amount"]), _money(e["amount_paid"]), _money(e["balance"])])
        elements.append(_styled_table(rows))
    else:
        elements.append(Paragraph("Everyone obligated on this funeral has paid in full.", _LABEL_STYLE))
