...
class DashboardServiceTests(TestCase):
    ...
    def test_financial_officer_trend_never_includes_gift_cash(self):
        ...
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Guest", amount_cash=Decimal("500"), collector_name="Test Collector")
        result = build_dashboard(self.treasurer)
    ...
    def test_financial_officer_view_includes_real_pending_approval_counts(self):
        ...
        payment = funeral_services.record_payment(obligation=self._obligation_for(self.funeral, self.member), amount=Decimal("20"), method="cash", collector_name="Test Collector")
...
class PlatformOverviewControlCenterTests(TestCase):
    """'The platform admin should have more control to manage all the community platform.' Metrics that already existed elsewhere (billing, subscription attention, support tickets) but were never surfaced on the landing page."""

    def setUp(self):
        self.platform_admin = User.objects.create_user(username="poc_platform_admin", password="x", role=Role.PLATFORM_ADMIN)
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="poc-bodi")
        self.bodi_admin = User.objects.create_user(username="poc_bodi_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)

    def test_overview_includes_unpaid_billing_and_subscription_attention_counts(self):
        from tenants.models import PlatformBillingRecord
        from datetime import timedelta
        from django.utils import timezone

        PlatformBillingRecord.objects.create(community=self.bodi, description="Setup fee", amount=Decimal("150"), created_by=self.platform_admin)
        self.bodi.access_plan = Community.AccessPlan.TIME_LIMITED
        self.bodi.access_expires_at = timezone.now() - timedelta(days=1)
        self.bodi.save()

        result = build_dashboard(self.platform_admin)
        overview = result["sections"]["platform_overview"]
        self.assertEqual(overview["unpaid_billing_count"], 1)
        self.assertEqual(overview["unpaid_billing_total"], "150.00")
        self.assertEqual(overview["communities_needing_subscription_attention_count"], 1)

    def test_overview_includes_open_support_ticket_count(self):
        from support import services as support_services
        support_services.submit_ticket(submitted_by=self.bodi_admin, subject="Test issue", description="Something's wrong")

        result = build_dashboard(self.platform_admin)
        overview = result["sections"]["platform_overview"]
        self.assertEqual(overview["open_support_ticket_count"], 1)

    def test_a_resolved_ticket_does_not_count_as_open(self):
        from support import services as support_services
        from support.models import SupportTicket

        ticket = support_services.submit_ticket(submitted_by=self.bodi_admin, subject="Resolved issue", description="Fixed already")
        support_services.update_ticket_status(ticket=ticket, status=SupportTicket.Status.RESOLVED, actor=self.platform_admin)

        result = build_dashboard(self.platform_admin)
        overview = result["sections"]["platform_overview"]
        self.assertEqual(overview["open_support_ticket_count"], 0)

    def test_a_paid_billing_record_does_not_count_as_unpaid(self):
        from tenants.models import PlatformBillingRecord

        PlatformBillingRecord.objects.create(
            community=self.bodi, description="Paid fee", amount=Decimal("100"),
            status=PlatformBillingRecord.Status.PAID, created_by=self.platform_admin,
        )
        result = build_dashboard(self.platform_admin)
        overview = result["sections"]["platform_overview"]
        self.assertEqual(overview["unpaid_billing_count"], 0)
        self.assertEqual(overview["unpaid_billing_total"], "0")
