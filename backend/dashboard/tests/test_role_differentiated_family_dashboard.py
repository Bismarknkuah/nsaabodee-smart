...
class RoleDifferentiatedFamilyDashboardTests(TestCase):
    ...
    def test_family_head_gets_an_activity_feed_covering_registration_payment_expense_and_task(self):
        """
        'Family head should have the analytics view of all the
        activities in the family.' A real, chronological log, not just
        the aggregate counts already tested above — one entry per kind
        of activity, confirming all four sources actually feed it.
        """
        from datetime import date
        from tasks import services as task_services

        new_member = Member.objects.create(community=self.bodi, family=self.asona, full_name="Newly Registered", gender="female")
        obligation = self.funeral.obligations.get(member=self.treasurer_member)
        funeral_services.record_payment(obligation=obligation, amount=obligation.balance, method="cash", collector_name="Front Desk")
        fund_services.record_funeral_expense(
            family=self.asona, funeral_event=self.funeral, item_name="Chairs", seller_name="Rental Co",
            amount=Decimal("80"), date_purchased="2026-07-06", recorded_by=self.secretary_user,
        )
        task = task_services.assign_task(
            community=self.bodi, assigned_to=self.treasurer_member, title="Set up canopy",
            assigned_by=self.head_user, due_date=date.today(),
        )
        task_services.update_task_status(task=task, status="pending_approval", actor=self.treasurer_user)
        task_services.decide_task_completion(task=task, approved=True, actor=self.head_user)
        ...
        self.assertTrue(any(t.startswith("expense_") for t in types_present))
        self.assertIn("task_completed", types_present)
        self.assertTrue(any(new_member.full_name in e["description"] for e in feed))

    def test_activity_feed_never_includes_a_different_familys_activity(self):
        bretuo = family_services.create_family(community=self.bodi, name="Bretuo", actor=self.admin)
        Member.objects.create(community=self.bodi, family=bretuo, full_name="Bretuo Person", gender="male")
        overview = build_dashboard(self.head_user)["sections"]["family_overview"]
        descriptions = " ".join(e["description"] for e in overview["activity_feed"])
        self.assertNotIn("Bretuo Person", descriptions)

    def test_treasurer_and_secretary_do_NOT_get_the_activity_feed(self):
        """Head-only oversight of everyone else's work — not something a Secretary or Treasurer reviewing their own slice needs."""
        for user in (self.treasurer_user, self.secretary_user):
            overview = build_dashboard(user)["sections"]["family_overview"]
            self.assertNotIn("activity_feed", overview)

    def test_family_treasurer_sees_money_received_per_funeral_for_their_own_familys_funerals(self):
        """'The family finance officer... has to know anything about transactions for a funeral in their family.'"""
        overview = build_dashboard(self.treasurer_user)["sections"]["family_overview"]
        rows = overview["money_received_per_funeral"]
        self.assertTrue(any(r["deceased_name"] == "Deceased" for r in rows))
        self.assertTrue(all(r["family_name"] == "Asona" for r in rows))

    def test_family_treasurer_never_sees_another_familys_funerals(self):
        """'Only works when the funeral is from his family' — the isolation that actually matters, verified directly."""
        bretuo = family_services.create_family(community=self.bodi, name="Bretuo", actor=self.admin)
        family_services.recommend_family_rate(family=bretuo, amount=Decimal("40"), actor=self.admin)
        family_services.approve_family_rate(family=bretuo, actor=self.admin)
        funeral_services.create_funeral_event(
            community=self.bodi, deceased_name="Bretuo Deceased", deceased_gender="female",
            deceased_family=bretuo, date_of_death="2026-08-01", collection_start_date="2026-08-01",
        )
        overview = build_dashboard(self.treasurer_user)["sections"]["family_overview"]
        names = {r["deceased_name"] for r in overview["money_received_per_funeral"]}
        self.assertIn("Deceased", names)
        self.assertNotIn("Bretuo Deceased", names)
