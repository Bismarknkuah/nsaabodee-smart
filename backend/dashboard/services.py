...
_COMMUNITY_OVERVIEW_ROLES = {Role.COMMUNITY_ADMIN, Role.CHAIRMAN, Role.SECRETARY, Role.COMMUNITY_REGISTRATION_DESK}
_FINANCIAL_OFFICER_ROLES = {Role.TREASURER, Role.FINANCIAL_SECRETARY, Role.AUDITOR}
_FAMILY_OFFICER_ROLES = {Role.FAMILY_HEAD, Role.FAMILY_SECRETARY, Role.FAMILY_TREASURER, Role.FAMILY_ARREARS_OFFICER, Role.FAMILY_REGISTRATION_OFFICER}
...
# Collector's own dashboard and its collections/arrears data —
# the same underlying concept, just scoped differently.
_COLLECTOR_ROLES = {Role.COLLECTOR, Role.ARREARS_COLLECTOR}
...
_TOWN_ELDERS_OFFICER_ROLES = {Role.TOWN_REGISTRATION_OFFICER, Role.TOWN_ELDERS_ARREARS_OFFICER}
...
def build_dashboard(user) -> dict:
    ...
    if user.is_superuser or user.role == Role.PLATFORM_ADMIN:
        from tenants.models import Announcement, Community, PlanInterestSubmission, PlatformBillingRecord
    ...
    if user.role == Role.TRADITIONAL_LEADER or user.role in _TOWN_ELDERS_OFFICER_ROLES:
