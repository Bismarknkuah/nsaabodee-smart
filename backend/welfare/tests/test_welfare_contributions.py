...
class FamilyInitiatedCampaignApprovalTests(TestCase):
    ...
    def test_a_family_treasurer_can_also_initiate_a_family_campaign(self):
        """'The finance officer will handle... anything related to money in the family, including creating family welfare.'"""
        campaign = welfare_services.initiate_family_campaign(category=self.category, family=self.asona, title="Asona Welfare Drive", actor=self.treasurer_user)
        self.assertEqual(campaign.status, ContributionCampaign.Status.PENDING_APPROVAL)
        self.assertEqual(campaign.initiated_by_id, self.treasurer_user.id)

    def test_a_family_treasurer_can_only_initiate_for_their_own_family(self):
        with self.assertRaises(ValidationError):
            welfare_services.initiate_family_campaign(category=self.category, family=self.bretuo, title="Should Fail", actor=self.treasurer_user)

    def test_a_family_secretary_still_cannot_initiate_a_family_campaign(self):
        """Only Head and Treasurer, not every family officer — the Secretary's own expanded role this turn is member registration, role assignment, and expenses, not welfare campaigns."""
        with self.assertRaises(ValidationError):
            welfare_services.initiate_family_campaign(category=self.category, family=self.asona, title="Should Fail", actor=self.secretary_user)
