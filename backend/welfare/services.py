...
def initiate_family_campaign(*, category: ContributionCategory, family, title: str, amount: Decimal = None, due_date=None, actor) -> ContributionCampaign:
    ...
    'The finance officer will handle... anything related to money in
    the family, including creating family welfare.' Family Treasurer
    added as a second allowed initiator, alongside Family Head — the
    same "two other family executives must still approve" requirement
    applies either way, this only changes who is allowed to start the
    request in the first place.
    """
    if actor.role not in ("family_head", "family_treasurer"):
        raise ValidationError("Only a Family Head or Family Treasurer can initiate a family's own contribution campaign.")
