...
class FamilyPositionRate(models.Model):
    """
    Configurable own-family contribution rate per FamilyPosition (see
    members.models.Member.FamilyPosition) — 'the exact amounts must
    remain configurable... do not implement the Family Ledger as
    every family member pays GHS X.' A dedicated model rather than a
    dozen more individual DecimalFields on Community/FuneralEvent
    (which is how the coarser, older family_head/senior/junior/woman
    tiers are still stored) — the richer taxonomy has enough distinct
    positions, and room to grow further, that bolting on that many
    fields would make Community/FuneralEvent unwieldy. One row per
    (community, position); a position with no configured row falls
    back to the community's existing coarser tier via
    funerals.models.FuneralEvent.rate_for — see
    contribution_rules.services.family_position_rate_for.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="family_position_rates")
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["community", "position"], name="one_rate_per_position_per_community")
        ]
