...
class FuneralExpenseWorkflowTests(TestCase):
    ...
    def test_an_expense_records_which_ledger_actually_funded_it(self):
        """'So he can enter where the money they received to finance the funeral is coming from.'"""
        expense = fund_services.record_funeral_expense(
            family=self.asona, funeral_event=self.funeral, item_name="Chairs", seller_name="Rental Co.",
            amount=Decimal("300"), date_purchased="2026-07-02", recorded_by=self.secretary_user,
            funding_source=FamilyFuneralExpense.FundingSource.GIFTS,
        )
        self.assertEqual(expense.funding_source, FamilyFuneralExpense.FundingSource.GIFTS)

    def test_funding_source_is_optional_and_defaults_to_blank(self):
        expense = fund_services.record_funeral_expense(
            family=self.asona, funeral_event=self.funeral, item_name="Tent", seller_name="Rental Co.",
            amount=Decimal("100"), date_purchased="2026-07-02", recorded_by=self.secretary_user,
        )
        self.assertEqual(expense.funding_source, "")
