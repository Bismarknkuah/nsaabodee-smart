...
class FamilyFuneralExpense(models.Model):
    ...
    class FundingSource(models.TextChoices):
        # 'So he can enter where the money they received to finance the
        # funeral is coming from.' Kept to the three sources that
        # actually pay for a funeral in practice, rather than a
        # separate option per individual ledger — the mandatory
        # contribution ledger (family + community rates combined, the
        # actual fund a funeral committee draws from) is one real
        # source regardless of which of the two rates any given payer
        # happened to fall under.
        CONTRIBUTIONS = "contributions", "Community/Family Contributions"
        GIFTS = "gifts", "Gifts & Donations"
        OTHER = "other", "Other (family's own money, etc.)"
    ...
    # default="" so this migrates safely onto the existing, already-
    # deployed table — every real, new expense goes through
    # record_funeral_expense, which always sets a real value; "" only
    # ever applies to rows that existed before this field did.
    funding_source = models.CharField(max_length=20, choices=FundingSource.choices, default="", blank=True)
