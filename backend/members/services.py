...
def register_member(
    *, community, full_name, gender, family=None, date_of_birth=None, occupation="",
    ...
    # members... the family head will have all analytics and can also
    # assign task to members or executive and also make approval, but
    # he can[not] enter data or make entries... each user role will
    # play its role, not other user's role.' Read together with the
    # word actually already missing here in what was sent twice
    # verbatim, and matching the exact principle this codebase already
    # applies to expenses ("the Family Head reviews, approves, or
    # rejects purchases — recording a new one is for Family Secretary
    # or Treasurer") — Family Head is blocked from registering a
    # member directly here, the same way. Community Admin, Chairman,
    # Secretary, and Collector remain entirely unaffected — this only
    # narrows the one role whose own authority already came from the
    # family-scoped check just above, it grants nothing new.
    if registered_by is not None and registered_by.role == "family_head":
        raise ValidationError("The Family Head reviews and approves the family roster — registering a new member is for Family Secretary or the Family Registration Officer.")
...
FAMILY_SCOPED_MEMBER_ROLES = {"family_head", "family_secretary", "family_treasurer", "family_registration_officer", "family_arrears_officer"}
...
def generate_qr_code_base64_for(payload: str) -> str:
    """
    Generic QR code generator — any scannable payload (a URL, most
    often) in, a base64 PNG out. Shared by both a member's digital
    membership card (below) and a printed payment receipt's own
    verification code (see reports/receipts.py), rather than each
    reimplementing the same three lines of qrcode/Pillow plumbing.
    """
    import qrcode
...
def generate_qr_code_base64(member: Member) -> str:
    return generate_qr_code_base64_for(member.qr_payload)
...
ASSIGNABLE_COMMUNITY_ROLES = [
    ...
    "treasurer", "financial_secretary", "auditor", "collector", "arrears_collector",
    "family_head", "family_secretary", "family_treasurer",
    "family_registration_officer", "community_registration_desk",
...
# and assign task to... same as each family head should be able to add
# new family executive role, and the town leader should also have
# these features... the arrears collector should be available to each
# of the family head to assign to someone." Family Head's own,
# narrower slice of ASSIGNABLE_COMMUNITY_ROLES, scoped to their own
# family's members only, checked in assign_role_to_member below.
FAMILY_HEAD_ASSIGNABLE_ROLES = ["family_secretary", "family_treasurer", "family_arrears_officer", "family_registration_officer"]

# The Traditional Leader's own slice, scoped to members who are
# genuinely Town Elders (Member.is_town_leader), the same population
# Town Registration Officer is already scoped to.
TRADITIONAL_LEADER_ASSIGNABLE_ROLES = ["town_registration_officer", "town_elders_arrears_officer"]
...
def suspend_member_account(*, member: Member, actor) -> "User":
    """
    'Suspend, or delete executive role.' Distinct from revoke: this
    only ever flips is_active off, the exact same flag Django's own
    login already checks (a suspended account cannot sign in at all),
    without touching their role. Reactivating later restores them
    exactly as they were, still holding whatever role they had, rather
    than needing to be reassigned from scratch the way a full revoke
    would require.
    """
    if not member.linked_user_id:
        raise ValidationError(f"{member.full_name} doesn't have a login to suspend.")
    if not _can_manage_this_members_role(actor=actor, member=member):
        raise ValidationError(f"'{actor.username}' isn't authorized to manage {member.full_name}'s account.")
    user = member.linked_user
    if not user.is_active:
        raise ValidationError(f"{member.full_name}'s account is already suspended.")
    user.is_active = False
    user.save(update_fields=["is_active"])

    from audit_log.services import record_event
    record_event(
        category="role", action="account_suspended", actor=actor, community=getattr(actor, "community", None),
        target_type="User", target_id=user.id, target_label=user.username,
        description=f"{member.full_name}'s account was suspended by {actor.username}.",
        metadata={"member_id": str(member.id)},
    )
    return user


def reactivate_member_account(*, member: Member, actor) -> "User":
    """The other half of suspend_member_account — restores exactly the role they already had, nothing reassigned."""
    if not member.linked_user_id:
        raise ValidationError(f"{member.full_name} doesn't have a login to reactivate.")
    if not _can_manage_this_members_role(actor=actor, member=member):
        raise ValidationError(f"'{actor.username}' isn't authorized to manage {member.full_name}'s account.")
    user = member.linked_user
    if user.is_active:
        raise ValidationError(f"{member.full_name}'s account is already active.")
    user.is_active = True
    user.save(update_fields=["is_active"])

    from audit_log.services import record_event
    record_event(
        category="role", action="account_reactivated", actor=actor, community=getattr(actor, "community", None),
        target_type="User", target_id=user.id, target_label=user.username,
        description=f"{member.full_name}'s account was reactivated by {actor.username}.",
        metadata={"member_id": str(member.id)},
    )
    return user


def _can_manage_this_members_role(*, actor, member: Member) -> bool:
    """Shared by suspend/reactivate above — the same three-tier scoping assign_role_to_member itself enforces, factored out so both agree on who's authorized rather than drifting apart."""
    # 'The community chair is to have upper control over the community
    # ledger and the system.' Chairman added as a peer to Community
    # Admin here too — this was a real, caught-late gap: the frontend
    # already showed Chairman a working-looking Suspend/Reactivate
    # button (since can_restrict_features_for and list_manageable_users
    # were both already updated for Chairman), but this separate
    # function, the one actually enforcing suspend/reactivate itself,
    # had not been, so clicking it would have failed with an
    # unauthorized error despite looking perfectly usable.
    if actor.role in ("community_admin", "chairman"):
        return member.community_id == actor.community_id
    if actor.role == "family_head":
        own_member = getattr(actor, "member_profile", None)
        return bool(own_member and own_member.family_id and member.family_id == own_member.family_id)
    if actor.role == "traditional_leader":
        return member.community_id == actor.community_id and member.is_town_leader
    return False
...
def assign_role_to_member(*, member: Member, role: str, actor, username: str = None, password: str = None):
    ...
    options as he supervises and manages the community system.' Widened
    since first written: a Family Head can assign their own family's
    narrower slice of executive roles to their own family's members,
    and a Traditional Leader can do the same for the Town Elders group,
    matching the exact same "family head should be able to add new
    family executive role, and the town leader should also have these
    features" ask. Community Admin's own authority (any of
    ASSIGNABLE_COMMUNITY_ROLES, community-wide) is unchanged.
    ...
    if actor.role == "community_admin":
        ...
        own_member = getattr(actor, "member_profile", None)
        if own_member is None or own_member.family_id is None:
            raise ValidationError("Your own account isn't linked to a family.")
        if member.family_id != own_member.family_id:
            raise ValidationError("You can only assign roles to members of your own family.")
        if role not in FAMILY_HEAD_ASSIGNABLE_ROLES:
            raise ValidationError(f"'{role}' isn't a role a Family Head or Family Secretary can assign.")
    elif actor.role == "traditional_leader":
        if member.community_id != actor.community_id:
            raise ValidationError("You can only assign roles to members of your own community.")
        if not member.is_town_leader:
            raise ValidationError("You can only assign this to a member of the Town Elders group.")
        if role not in TRADITIONAL_LEADER_ASSIGNABLE_ROLES:
            raise ValidationError(f"'{role}' isn't a role a Traditional Leader can assign.")
    else:
        raise ValidationError("Only the Community Admin, a Family Head, a Family Secretary, or the Traditional Leader can assign roles.")
...
COLLECTOR_NOMINATION_ROLES = {
    ...
    # family user role to his family members, like assigning or
    # adding the family arrears collector, family desk collector,
    # family donation collector.' Family Secretary given the same
    # nominating authority as Family Head here — both are scoped to
    # their own family regardless of which of the two actually starts
    # the nomination (see the "family"/"donation" scoping check just
    # below this dict).
    "family": {"family_head", "family_secretary"},
    "donation": {"family_head", "family_secretary"},
...
# ---------------------------------------------------------------------------
# Member Registry — "his major role is to have access of the members data:
# total numbers of the community members by gender, age, family by family,
# and more... click on someone's profile and see all their details, the
# family they come from, how much they owe."
#
# Every function below takes the ACTOR and derives its jurisdiction from
# the role sets this module already enforces everywhere else, so a
# community-wide registrar (Community Registration Desk, Town Registration
# Officer, Admin, Chairman, Secretary) sees the whole community, the
# Traditional Leader sees Town Elders, and a family officer sees only their
# own family — "each user role plays its role, not other users' role."
# ---------------------------------------------------------------------------

_REGISTRY_COMMUNITY_WIDE_ROLES = {"community_admin", "chairman", "secretary", "community_registration_desk", "town_registration_officer"}
_REGISTRY_MANAGE_ROLES = {"community_admin", "chairman", "secretary", "community_registration_desk", "town_registration_officer"}


def member_registry_scope(actor):
    """The members this actor is allowed to see in the registry — a queryset, or None if they have no registry access at all."""
    from django.db.models import Q
    if actor.is_superuser or actor.role in _REGISTRY_COMMUNITY_WIDE_ROLES:
        return Member.objects.filter(community=actor.community)
    if actor.role == "traditional_leader":
        return Member.objects.filter(community=actor.community, is_town_leader=True)
    if actor.role in FAMILY_SCOPED_MEMBER_ROLES:
        own = getattr(actor, "member_profile", None)
        if own is None or own.family_id is None:
            return Member.objects.none()
        return Member.objects.filter(community=actor.community, family_id=own.family_id)
    return None


def member_registry_analytics(*, actor) -> dict:
    """Totals by status, gender, age band, family, town-elder flag, defaulter tier, plus registrations per month for the last 12 months."""
    from datetime import date
    from django.db.models import Count
    from django.db.models.functions import TruncMonth

    scope = member_registry_scope(actor)
    if scope is None:
        raise ValidationError("Your role does not include the member registry.")
    today = date.today()

    def _count_by(field):
        return {row[field] or "unknown": row["n"] for row in scope.values(field).annotate(n=Count("id")).order_by()}

    age_bands = {"under_18": 0, "18_35": 0, "36_50": 0, "51_65": 0, "over_65": 0, "unknown": 0}
    for dob in scope.values_list("date_of_birth", flat=True):
        if dob is None:
            age_bands["unknown"] += 1
            continue
        age = today.year - dob.year - ((today.month, today.day) < (dob.month, dob.day))
        key = "under_18" if age < 18 else "18_35" if age <= 35 else "36_50" if age <= 50 else "51_65" if age <= 65 else "over_65"
        age_bands[key] += 1

    by_family = [
        {"family_id": str(r["family_id"]) if r["family_id"] else None, "family_name": r["family__name"] or "No family",
         "total": r["n"], "active": r["active"], "deceased": r["deceased"]}
        for r in scope.values("family_id", "family__name").annotate(
            n=Count("id"), active=Count("id", filter=Q(status="active")), deceased=Count("id", filter=Q(status="deceased")),
        ).order_by("-n", "family__name")
    ]
    since = date(today.year - 1, today.month, 1)
    by_month = [
        {"month": r["m"].strftime("%Y-%m"), "count": r["n"]}
        for r in scope.filter(created_at__date__gte=since).annotate(m=TruncMonth("created_at")).values("m").annotate(n=Count("id")).order_by("m")
    ]
    return {
        "scope": "community" if (actor.is_superuser or actor.role in _REGISTRY_COMMUNITY_WIDE_ROLES) else "town_elders" if actor.role == "traditional_leader" else "family",
        "total": scope.count(),
        "by_status": _count_by("status"),
        "by_gender": _count_by("gender"),
        "by_age_band": age_bands,
        "by_family": by_family,
        "town_elders": scope.filter(is_town_leader=True).count(),
        "by_defaulter_tier": _count_by("defaulter_tier"),
        "with_login": scope.filter(linked_user__isnull=False).count(),
        "registrations_by_month": by_month,
        "recent_registrations": [
            {"id": str(m.id), "full_name": m.full_name, "family_name": m.family.name if m.family_id else None, "created_at": m.created_at.isoformat()}
            for m in scope.select_related("family").order_by("-created_at")[:10]
        ],
    }
...
def _role_label(user):
    ...
    return role_label_for(user)


def member_profile(*, member: Member, actor) -> dict:
    """Everything about one member in one place: identity, family, login, every obligation with its balance, total owed, recent payments."""
    from django.db.models import Sum
    from funerals.models import ContributionObligation, ContributionPayment

    scope = member_registry_scope(actor)
    if scope is None or not scope.filter(id=member.id).exists():
        raise ValidationError("This member is outside your jurisdiction.")
    ...
    total_owed = sum((o.balance for o in obligations), Decimal("0"))
    paid_all_time = ContributionPayment.objects.filter(obligation__member=member).aggregate(t=Sum("amount"))["t"] or Decimal("0")
    payments = ContributionPayment.objects.filter(obligation__member=member).select_related("obligation__funeral_event").order_by("-paid_at")[:10]
    linked = member.linked_user
    return {
        "member": {
            "id": str(member.id), "membership_number": member.membership_number, "full_name": member.full_name, "gender": member.gender,
            "date_of_birth": member.date_of_birth.isoformat() if member.date_of_birth else None, "phone": member.phone, "email": member.email,
            "address": member.address, "hometown": member.hometown, "occupation": member.occupation, "marital_status": member.marital_status,
            "spouse_name": member.spouse_name, "mother_name": member.mother_name, "father_name": member.father_name,
            "emergency_contact_name": member.emergency_contact_name, "emergency_contact_phone": member.emergency_contact_phone,
            "status": member.status, "defaulter_tier": member.defaulter_tier, "missed_contributions_count": member.missed_contributions_count,
            "is_town_leader": member.is_town_leader, "town_elder_title": member.town_elder_title, "family_seniority": member.family_seniority,
            "family_position": member.family_position, "created_at": member.created_at.isoformat(),
            "registered_by": member.registered_by.username if member.registered_by_id else None,
        },
        "family": {"id": str(member.family_id), "name": member.family.name} if member.family_id else None,
        "login": {"username": linked.username, "role": linked.role, "role_label": _role_label(linked), "is_active": linked.is_active} if linked else None,
        "total_owed": str(total_owed),
        "paid_all_time": str(paid_all_time),
        "obligations": [
            {"funeral_id": str(o.funeral_event_id), "deceased_name": o.funeral_event.deceased_name, "funeral_status": o.funeral_event.status,
             "rate_type": o.rate_type, "expected_amount": str(o.expected_amount), "amount_paid": str(o.amount_paid),
             "balance": str(o.balance), "payment_status": o.payment_status}
            for o in obligations
        ],
        "recent_payments": [
            {"paid_at": p.paid_at.isoformat(), "amount": str(p.amount), "method": p.method,
             "deceased_name": p.obligation.funeral_event.deceased_name, "collector_name": p.collector_name}
            for p in payments
        ],
    }


def _require_registry_manager(actor, what: str):
    if not (actor.is_superuser or actor.role in _REGISTRY_MANAGE_ROLES):
        raise ValidationError(f"Only a community-wide registrar (Community Admin, Chairman, Secretary, Community Registration Desk, or Town Registration Officer) can {what}.")


@transaction.atomic
def move_member_to_family(*, member: Member, family, actor) -> Member:
    """
    'Move member from one family to another.' Community-wide registrars
    only — a family officer can act inside their own family but never
    move someone across the line between two families; that is a
    community-level decision. Existing obligations keep their original
    rate type and amounts: they were billed under the family the member
    belonged to at the time, and rewriting history would corrupt the
    ledger. Only future funerals bill under the new family.
    """
    _require_registry_manager(actor, "move a member between families")
    if family.community_id != member.community_id:
        raise ValidationError("A member can only be moved to a family in the same community.")
    if member.family_id == family.id:
        return member
    if member.family_id and member.family.family_head_id == member.id:
        raise ValidationError("This member is the Family Head of their current family — assign a new head there first.")
    member.family = family
    member.save(update_fields=["family", "updated_at"])
    return member


def deactivate_member(*, member: Member, actor, reason: str = "") -> Member:
    """
    'Can delete.' A soft delete, on purpose: a member with payments on
    the ledger can never be hard-deleted without corrupting every
    funeral's accounts. Marking them inactive removes them from every
    future funeral's obligations and from active counts, and their
    linked login (if any) is suspended, while every past payment stays
    exactly where it was recorded.
    """
    _require_registry_manager(actor, "deactivate a member")
    if member.status == Member.Status.DECEASED:
        raise ValidationError("A deceased member's record is closed and cannot be deactivated.")
    member.status = Member.Status.INACTIVE
    member.save(update_fields=["status", "updated_at"])
    if member.linked_user_id and member.linked_user.is_active:
        member.linked_user.is_active = False
        member.linked_user.save(update_fields=["is_active"])
    return member


def reactivate_member(*, member: Member, actor) -> Member:
    _require_registry_manager(actor, "reactivate a member")
    if member.status != Member.Status.INACTIVE:
        raise ValidationError("Only an inactive member can be reactivated.")
    member.status = Member.Status.ACTIVE
    member.save(update_fields=["status", "updated_at"])
    if member.linked_user_id and not member.linked_user.is_active:
        member.linked_user.is_active = True
        member.linked_user.save(update_fields=["is_active"])
    return member
