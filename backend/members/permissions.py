...
class CanAssignExecutiveRoles(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # community ledger and the system.' Given the same
        # community-wide authority Community Admin already has here —
        # matches list_manageable_users/can_restrict_features_for,
        # which already treat Chairman as a peer to Community Admin;
        # leaving Chairman out of this specific gate would have meant
        # they could see the assign-role/suspend/reactivate buttons on
        # User Management without ever actually being able to use
        # them.
        return request.user.is_superuser or request.user.role in ("community_admin", "chairman", "family_head", "traditional_leader")
