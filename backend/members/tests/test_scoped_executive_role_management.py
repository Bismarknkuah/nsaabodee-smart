...
class SuspendReactivateAccountTests(TestCase):
    ...
    def test_a_chairman_can_suspend_a_community_wide_executive(self):
        """
        'The community chair is to have upper control over the
        community ledger and the system.' Caught by this turn's
        re-verification pass — the frontend already showed Chairman a
        working-looking Suspend button before this function itself was
        updated to actually authorize it.
        """
        chairman_user = User.objects.create_user(username="sra_chairman", password="x", community=self.bodi, role=Role.CHAIRMAN)
        user = member_services.suspend_member_account(member=self.member, actor=chairman_user)
        self.assertFalse(user.is_active)
    ...
    def test_chairman_can_suspend_an_account_over_http_end_to_end(self):
        """
        'The community chair is to have upper control over the
        community ledger and the system.' Deliberately over HTTP, not
        the service function directly — this specific chain (the
        view's own permission class, then the service's inner
        authorization check) had two separate gates, and only one had
        actually been updated for Chairman before this turn.
        """
        chairman_user = User.objects.create_user(username="sra_chairman", password="a-real-password-123", community=self.bodi, role=Role.CHAIRMAN)
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "sra_chairman", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")

        res = client.post(f"/api/members/{self.member.id}/suspend-account/")
        self.assertEqual(res.status_code, 200, res.data)
        self.collector_user.refresh_from_db()
        self.assertFalse(self.collector_user.is_active)
...
class FamilySecretaryRoleAssignmentAndNominationTests(TestCase):
    """
    'The family secretary... should be able to assign all type of
    family user role to his family members, like assigning or adding
    the family arrears collector, family desk collector, family
    donation collector.' Same authority as Family Head, given to
    Family Secretary too — both scoped to their own family only.
    """

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="fsra-bodi")
        self.admin = User.objects.create_user(username="fsra_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.bretuo = family_services.create_family(community=self.bodi, name="Bretuo", actor=self.admin)

        self.secretary_user = User.objects.create_user(username="fsra_secretary", password="x", community=self.bodi, role=Role.FAMILY_SECRETARY)
        secretary_member = member_services.register_member(community=self.bodi, full_name="The Secretary", gender="male", family=self.asona)
        member_services.link_member_to_user(member=secretary_member, user=self.secretary_user, actor=self.admin)

        self.asona_member = member_services.register_member(community=self.bodi, full_name="Asona Member", gender="male", family=self.asona)
        self.bretuo_member = member_services.register_member(community=self.bodi, full_name="Bretuo Member", gender="male", family=self.bretuo)

    def test_a_family_secretary_can_assign_family_arrears_officer_to_their_own_member(self):
        user = member_services.assign_role_to_member(
            member=self.asona_member, role="family_arrears_officer", actor=self.secretary_user,
            username="fsra_arrears_officer", password="a-real-password-123",
        )
        self.assertEqual(user.role, "family_arrears_officer")

    def test_a_family_secretary_cannot_assign_a_role_to_a_different_familys_member(self):
        with self.assertRaises(ValidationError):
            member_services.assign_role_to_member(
                member=self.bretuo_member, role="family_arrears_officer", actor=self.secretary_user,
                username="fsra_other", password="a-real-password-123",
            )

    def test_a_family_secretary_can_nominate_a_family_collector(self):
        nomination = member_services.nominate_collector(member=self.asona_member, collector_type="family", actor=self.secretary_user)
        self.assertEqual(nomination.scoped_family_id, self.asona.id)

    def test_a_family_secretary_can_nominate_a_donation_collector(self):
        nomination = member_services.nominate_collector(member=self.asona_member, collector_type="donation", actor=self.secretary_user)
        self.assertEqual(nomination.scoped_family_id, self.asona.id)

    def test_a_family_secretary_cannot_nominate_a_family_collector_from_a_different_family(self):
        with self.assertRaises(ValidationError):
            member_services.nominate_collector(member=self.bretuo_member, collector_type="family", actor=self.secretary_user)
