...
class FamilyHeadRegistrationJurisdictionTests(TestCase):
    ...
    def test_family_head_can_no_longer_register_a_member_even_into_their_own_family(self):
        """
        'The family secretary will be responsible for all adding
        members... [Family Head] can[not] enter data or make
        entries... each user role will play its role, not other
        user's role.' Corrected from this test's own original name —
        matches the exact principle already applied to expenses
        ("the Family Head reviews, approves, or rejects purchases —
        recording a new one is for Family Secretary or Treasurer").
        """
