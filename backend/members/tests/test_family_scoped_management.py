...
class FamilyScopedMemberManagementTests(TestCase):
    ...
    def test_a_family_arrears_officer_only_sees_their_own_familys_roster(self):
        """
        'We have a family arrears collector who is responsible for
        managing and collecting his family arrears only.' A genuine,
        previously-uncaught gap: this role was missing from
        FAMILY_SCOPED_MEMBER_ROLES entirely, meaning their search saw
        every member across the whole community, not just their own
        family — fixed here, and locked in by this test.
        """
        arrears_member = member_services.register_member(community=self.bodi, full_name="Asona Arrears Officer", gender="male", family=self.asona)
        arrears_user = User.objects.create_user(username="asona_arrears_officer", password="x", community=self.bodi, role=Role.FAMILY_ARREARS_OFFICER)
        member_services.link_member_to_user(member=arrears_member, user=arrears_user, actor=self.admin)

        client = self._login("asona_arrears_officer")
        res = client.get("/api/members/")
        self.assertEqual(res.status_code, 200)
        names = {m["full_name"] for m in res.data["results"]}
        self.assertIn("Asona Secretary Person", names)
