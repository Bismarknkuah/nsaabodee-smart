...
class MemberRegistrationTests(TestCase):
    ...
    def test_registration_via_http_now_accepts_email_and_date_of_birth(self):
        """
        'The system should require more information.' email was on the
        Member model and accepted by the service function, but missing
        from the serializer entirely — a real gap found while adding
        this to the frontend form, not a pre-existing test.
        """
        from rest_framework.test import APIClient
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "admin", "password": "x"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.post("/api/members/", {
            "full_name": "Ama Serwaa", "gender": "female", "family_id": str(self.asona.id),
            "email": "ama@example.com", "date_of_birth": "1990-05-15",
            "occupation": "Teacher", "address": "12 Liberation Rd",
            "emergency_contact_name": "Kofi Serwaa", "emergency_contact_phone": "0244000111",
        })
        self.assertEqual(res.status_code, 201)
        self.assertEqual(res.data["email"], "ama@example.com")
        self.assertEqual(res.data["date_of_birth"], "1990-05-15")
...
class DefaulterEscalationTests(TestCase):
    ...
    def test_paying_in_full_before_close_prevents_a_miss(self):
        ...
        funeral_services.record_payment(obligation=obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
