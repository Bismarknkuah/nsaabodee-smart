...
from .permissions import CanAssignExecutiveRoles, CanManageMembers, IsSameCommunity, IsSameFamilyOrCommunityWide
...
class MemberViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=["post"], url_path="assign-role", permission_classes=[IsAuthenticated, CanAssignExecutiveRoles, IsSameCommunity, IsSameFamilyOrCommunityWide, RequiresExecutiveContext])
    ...
    @action(detail=True, methods=["post"], url_path="revoke-role", permission_classes=[IsAuthenticated, CanAssignExecutiveRoles, IsSameCommunity, IsSameFamilyOrCommunityWide, RequiresExecutiveContext])
    ...
    @action(detail=True, methods=["post"], url_path="suspend-account", permission_classes=[IsAuthenticated, CanAssignExecutiveRoles, IsSameCommunity, IsSameFamilyOrCommunityWide, RequiresExecutiveContext])
    def suspend_account(self, request, id=None):
        """'Suspend, or delete executive role.' Unlike revoke, this never touches their role — reactivating later restores them exactly as they were."""
        member = self.get_object()
        try:
            user = services.suspend_member_account(member=member, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response({"member_id": str(member.id), "role": user.role, "username": user.username, "is_active": user.is_active})

    @action(detail=True, methods=["post"], url_path="reactivate-account", permission_classes=[IsAuthenticated, CanAssignExecutiveRoles, IsSameCommunity, IsSameFamilyOrCommunityWide, RequiresExecutiveContext])
    def reactivate_account(self, request, id=None):
        member = self.get_object()
        try:
            user = services.reactivate_member_account(member=member, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response({"member_id": str(member.id), "role": user.role, "username": user.username, "is_active": user.is_active})

    # ---- Member Registry: analytics, full profile, move-family, deactivate ----
    # "His major role is to have access of the members data... click on
    # someone's profile and see all details, the family, how much they
    # owe." Jurisdiction is decided inside the services, from the role.

    @action(detail=False, methods=["get"], url_path="registry-analytics", permission_classes=[IsAuthenticated])
    def registry_analytics(self, request):
        try:
            return Response(services.member_registry_analytics(actor=request.user))
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)

    @action(detail=True, methods=["get"], url_path="profile", permission_classes=[IsAuthenticated, IsSameCommunity])
    def profile(self, request, id=None):
        member = self.get_object()
        try:
            return Response(services.member_profile(member=member, actor=request.user))
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)

    @action(detail=True, methods=["post"], url_path="move-family", permission_classes=[IsAuthenticated, IsSameCommunity])
    def move_family(self, request, id=None):
        from families.models import Family
        member = self.get_object()
        family = get_object_or_404(Family, id=request.data.get("family_id"), community=request.user.community)
        try:
            updated = services.move_member_to_family(member=member, family=family, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response(MemberSerializer(updated).data)

    @action(detail=True, methods=["post"], url_path="deactivate", permission_classes=[IsAuthenticated, IsSameCommunity])
    def deactivate(self, request, id=None):
        member = self.get_object()
        try:
            updated = services.deactivate_member(member=member, actor=request.user, reason=request.data.get("reason", ""))
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response(MemberSerializer(updated).data)

    @action(detail=True, methods=["post"], url_path="reactivate", permission_classes=[IsAuthenticated, IsSameCommunity])
    def reactivate(self, request, id=None):
        member = self.get_object()
        try:
            updated = services.reactivate_member(member=member, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response(MemberSerializer(updated).data)
...
class MemberArrearsView(APIView):
    """
    GET -> 'when the person standing in front of the table mentions
    their name or code, their arrears they own should show.' Every
    outstanding obligation this member has, across every funeral
    regardless of status — what an Arrears Collector needs to see
    the moment someone is identified, no separate search per funeral.
    """
    permission_classes = [IsAuthenticated]

    def get(self, request, member_id):
        from funerals.serializers import MemberArrearsObligationSerializer
        from funerals.services import lookup_member_arrears

        member = get_object_or_404(Member, id=member_id, community=request.user.community)
        arrears = lookup_member_arrears(member=member)
        return Response({
            "member_id": str(member.id), "member_name": member.full_name, "membership_number": member.membership_number,
            "arrears": MemberArrearsObligationSerializer(arrears, many=True).data,
        })
