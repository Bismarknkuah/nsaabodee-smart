...
class MomoPaymentRequest(models.Model):
    ...
    Supports THREE independent targets, exactly mirroring the
    platform's own ledgers:
      - `obligation` — a mandatory Ledger 1 contribution (own-family,
        general, or town-elder rate).
      - `funeral_event` (with `donor_name`/`received_by_member`) — a
        ...
        it).
      - `deposit_member` — 'members can have their own wallet where
        they can deposit money and can use to pay for their
        contribution.' A self-service top-up of the payer's own
        MemberWallet, not tied to any specific obligation or funeral at
        all — the money simply becomes spendable balance, applied to
        whatever obligation the member chooses later, via the existing
        apply_wallet_to_obligation.
      Exactly one of `obligation`, `funeral_event`, or `deposit_member`
      must be set — see `clean()`.
    ...
    class TargetType(models.TextChoices):
        ...
        WALLET_DEPOSIT = "wallet_deposit", "Member Wallet Deposit"
    ...
    # Only used when target_type == WALLET_DEPOSIT — 'members can have
    # their own wallet where they can deposit money.' The one thing
    # this target needs that the other two don't: whose MemberWallet
    # actually gets credited once the charge clears.
    deposit_member = models.ForeignKey(
        "members.Member", null=True, blank=True, on_delete=models.CASCADE, related_name="momo_wallet_deposit_requests"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=(
                    models.Q(target_type="contribution", obligation__isnull=False, funeral_event__isnull=True, deposit_member__isnull=True)
                    | models.Q(target_type="gift", obligation__isnull=True, funeral_event__isnull=False, deposit_member__isnull=True)
                    | models.Q(target_type="wallet_deposit", obligation__isnull=True, funeral_event__isnull=True, deposit_member__isnull=False)
                ),
