...
from .serializers import InitiateMomoGiftPaymentSerializer, InitiateMomoPaymentSerializer, InitiateMomoWalletDepositSerializer, MomoPaymentRequestSerializer, SubmitMomoOtpSerializer
...
class InitiateMomoWalletDepositView(APIView):
    """POST {phone_number, amount} -> a pending MomoPaymentRequest that tops up the caller's own MemberWallet once it clears."""
    permission_classes = [IsAuthenticated]

    def post(self, request):
        serializer = InitiateMomoWalletDepositSerializer(data=request.data, context={"request": request})
        serializer.is_valid(raise_exception=True)
        momo_request = serializer.save()
        return Response(MomoPaymentRequestSerializer(momo_request).data, status=status.HTTP_201_CREATED)
