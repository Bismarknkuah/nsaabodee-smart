...
class MomoPaymentRequestSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "deposit_member", "reference_id", "phone_number", "amount", "status", "created_at", "updated_at",
        ]
        read_only_fields = ["id", "target_type", "reference_id", "status", "created_at", "updated_at"]


class InitiateMomoWalletDepositSerializer(serializers.Serializer):
    """'Members can have their own wallet where they can deposit money.' A self-service top-up of the payer's own MemberWallet via MoMo."""
    ...
    def save(self, **kwargs):
        ...
        if own_member is None:
            raise serializers.ValidationError("Only an account linked to a Member profile can deposit into a wallet.")
        try:
            return services.initiate_momo_wallet_deposit(
                member=own_member,
