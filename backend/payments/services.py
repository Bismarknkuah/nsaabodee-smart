...
def initiate_momo_wallet_deposit(*, member: Member, phone_number: str, amount: Decimal, initiated_by=None) -> MomoPaymentRequest:
    """
    'Members can have their own wallet where they can deposit money
    and can use to pay for their contribution.' A self-service top-up
    of the member's own MemberWallet — not tied to any specific
    obligation or funeral at all. The money simply becomes spendable
    balance, applied to whatever obligation the member chooses later
    via the existing apply_wallet_to_obligation, exactly the same
    "prefer Paystack" channel every other charge on this platform
    ...
    request = MomoPaymentRequest.objects.create(
        ...
        target_type=MomoPaymentRequest.TargetType.WALLET_DEPOSIT,
        deposit_member=member,
        phone_number=phone_number,
        amount=amount,
        initiated_by=initiated_by,
    )
    _send_request_to_pay(request, f"Wallet top-up — {member.full_name}")
    return request
...
def _finalize_as_successful(request: MomoPaymentRequest, *, provider_response: str) -> MomoPaymentRequest:
    ...
    if request.target_type == MomoPaymentRequest.TargetType.CONTRIBUTION:
        ...
        record_payment(
            ...
            method="mobile_money",
            # No human collector is involved in this flow at all — the
            # payer confirms directly with their own phone, and the
            # provider's webhook/poll is what finalizes it. An honest
            # label for that, not a fabricated person's name, matching
            # 'any collector has to input their own name' — this simply
            # isn't that scenario.
            collector_name="Mobile Money (self-service)",
            client_op_id=str(request.reference_id),
        )
    elif request.target_type == MomoPaymentRequest.TargetType.GIFT:
        ...
        credit_wallet(
            member=request.deposit_member,
            amount=request.amount,
            note=f"Mobile Money top-up — MoMo reference {request.reference_id}",
