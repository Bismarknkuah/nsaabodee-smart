...
def record_gift_donation(
    *, funeral: FuneralEvent, donor_name: str, donor_phone: str = "", donor_member: Member | None = None,
    ...
    relationship_to_recipient: str = "", collector_name: str = "", delegation_size: int | None = None,
):
    """
    'Any collector have to input in their names for them to know who
    they paid to.' Required whenever a real collector is actually
    involved (collected_by is set) — a shared desk assignment doesn't
    always identify who physically took this specific gift. Not
    required when there's no collector at all (a system-recorded or
    direct/online gift, where the concept doesn't apply).
    """
    ...
    if collected_by is not None and not collector_name.strip():
        raise ValidationError("The collector's own name is required when recording a gift.")
...
def register_donation_account_holder(*, funeral: FuneralEvent, member: Member, actor=None) -> DonationAccountRegistration:
    """
    UPDATED RULE, superseding the original "no executive user role"
    restriction: 'The Deceased Rep is not allowed to receive donations.
    Community members and executives with a personal dashboard (since
    their personal dashboard is the same as the community member
    accounts) are set to receive donations.' Only Bereaved/Deceased Rep
    is excluded now — every other role, executive or not, can be
    registered, since an executive's Personal Dashboard is explicitly
    meant to be identical to an ordinary member's, donation eligibility
    included. Bereaved/Deceased Rep stays excluded for a different,
    unrelated reason: they already represent the bereaved family
    officially on the funeral committee, so personally registering as
    a donation recipient too would blur that representative role with
    a private financial one.

    Unless the Family Head of the member's own family is the one
    ...
    if member.linked_user_id and member.linked_user.role == Role.BEREAVED_REP:
        raise ValidationError("A Deceased Rep can't be registered to receive donations — they already represent the bereaved family officially.")
