...
class GiftDonation(models.Model):
    ...
    # present a contribution on behalf of Town A... the system should
    # be able to record: funeral, guest town, representative, number
    # of guests, contribution... without necessarily creating 15
    # separate financial obligations.' Null/1 means an ordinary,
    # individual gift (the default, and every existing donation before
    # this field existed) — a number greater than 1 means donor_name
    # is the delegation's own representative, giving on behalf of that
    # many people, still recorded as this ONE GiftDonation row.
    delegation_size = models.PositiveIntegerField(null=True, blank=True)
    ...
    # they paid to.' Same reasoning as ContributionPayment.collector_name
    # — a real, required name independent of whichever account happened
    # to be logged in when a shared desk assignment collected this.
    ...
    def clean(self):
        ...
        if self.delegation_size is not None and self.delegation_size < 1:
            raise ValidationError("A delegation size, when given, must represent at least one person.")
