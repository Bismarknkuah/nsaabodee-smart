...
class DonationAccountRegistrationListCreateView(APIView):
    ...
    def get(self, request, funeral_id):
        funeral = _get_funeral(request, funeral_id)
        if request.query_params.get("include_pending") == "true":
            # For the registration panel itself, so a pending request
            # is visibly distinct from an active one rather than simply
            # invisible until approved — the actual gift-recording flow
            # (record_gift_donation, and every other caller of
            # list_donation_account_holders) is untouched and still only
            # ever sees active ones.
            registrations = DonationAccountRegistration.objects.filter(funeral_event=funeral).select_related("member")
        else:
