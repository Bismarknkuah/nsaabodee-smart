...
class GiftDonationTests(TestCase):
    ...
    def test_record_item_only_gift_requires_estimated_value(self):
        with self.assertRaises(ValidationError):
            services.record_gift_donation(funeral=self.funeral, donor_name="Kojo", gift_item="A cow", collector_name="Test Collector")
    ...
    def test_donation_needs_cash_or_item(self):
        with self.assertRaises(ValidationError):
            services.record_gift_donation(funeral=self.funeral, donor_name="Nobody Gave Anything", collector_name="Test Collector")
    ...
    def test_duplicate_offline_gift_is_idempotent_on_client_op_id(self):
        ...
        d1 = services.record_gift_donation(funeral=self.funeral, donor_name="Ama", amount_cash=Decimal("30"), client_op_id=op_id, collector_name="Test Collector")
        d2 = services.record_gift_donation(funeral=self.funeral, donor_name="Ama", amount_cash=Decimal("30"), client_op_id=op_id, collector_name="Test Collector")
        self.assertEqual(d1.id, d2.id)
        self.assertEqual(GiftDonation.objects.filter(funeral_event=self.funeral).count(), 1)

    def test_gift_summary_totals(self):
        services.record_gift_donation(funeral=self.funeral, donor_name="A", amount_cash=Decimal("100"), collector_name="Test Collector")
        services.record_gift_donation(funeral=self.funeral, donor_name="B", gift_item="Goat", estimated_item_value=Decimal("300"), collector_name="Test Collector")
    ...
    def test_gifts_never_touch_the_mandatory_contribution_ledger(self):
        ...
        services.record_gift_donation(funeral=self.funeral, donor_name="Big Donor", amount_cash=Decimal("5000"), collector_name="Test Collector")
        services.record_gift_donation(funeral=self.funeral, donor_name="Another Donor", gift_item="A car", estimated_item_value=Decimal("50000"), collector_name="Test Collector")
    ...
    def test_donor_member_from_another_community_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            services.record_gift_donation(funeral=self.funeral, donor_name="Stranger", donor_member=outsider, amount_cash=Decimal("10"), collector_name="Test Collector")
