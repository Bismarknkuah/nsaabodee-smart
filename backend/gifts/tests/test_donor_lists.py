...
class DonorListAndRelationshipTests(TestCase):
    ...
    def test_all_receivers_donation_lists_keeps_receivers_separate(self):
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="Donor A", amount_cash=Decimal("30"), received_by_member=self.adwoa, collector_name="Test Collector")
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="Donor B", amount_cash=Decimal("50"), received_by_member=self.yaw, collector_name="Test Collector")
...
class GiftReceiptAppreciationMessageTests(TestCase):
    ...
    def test_receipt_data_includes_deceased_date_of_birth(self):
        donation = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Donor", amount_cash=Decimal("20"), collector_name="Test Collector")
