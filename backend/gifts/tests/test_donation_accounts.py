...
class DonationAccountRegistrationTests(TestCase):
    """
    UPDATED RULE: 'The Deceased Rep is not allowed to receive
    donations. Community members and executives with a personal
    dashboard... are set to receive donations.' Only Bereaved/Deceased
    Rep is excluded now, not every executive role.
    """
    ...
    def test_an_executive_role_CAN_now_be_registered_to_receive_donations(self):
        """The rule changed: 'executives with a personal dashboard... are set to receive donations, since their personal dashboard is the same as the community member accounts.'"""
        registration = gift_services.register_donation_account_holder(funeral=self.funeral, member=self.head_member, actor=self.head_user)
        self.assertIsNotNone(registration)

    def test_a_deceased_rep_cannot_be_registered_to_receive_donations(self):
        """'The Deceased Rep is not allowed to receive donations.' They already represent the bereaved family officially — a private financial role on top of that would blur the two."""
        rep_member = member_services.register_member(community=self.bodi, full_name="Rep Member", gender="male", family=self.asona)
        rep_user = User.objects.create_user(username="donation_test_rep", password="x", community=self.bodi, role=Role.BEREAVED_REP)
        member_services.link_member_to_user(member=rep_member, user=rep_user, actor=self.admin)
        with self.assertRaises(ValidationError):
            gift_services.register_donation_account_holder(funeral=self.funeral, member=rep_member, actor=self.head_user)
    ...
    def test_gift_without_a_receiver_specified_still_works_as_before(self):
        ...
        donation = gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Guest", amount_cash=Decimal("50"), collector_name="Test Collector")
    ...
    def test_donations_received_by_member_aggregates_correctly_including_momo(self):
        ...
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="Unattributed Guest", amount_cash=Decimal("999"), collector_name="Test Collector")
...
class DonationAccountApprovalHttpTests(TestCase):
    ...
    def test_include_pending_shows_both_but_the_default_view_stays_active_only(self):
        """The registration panel itself needs pending visible, distinct from active — but every other caller (recording an actual gift) must be unaffected."""
        collector_client = self._login("donation_http_collector")
        reg_res = collector_client.post(f"/api/funerals/{self.funeral.id}/donation-accounts/", {"member_id": str(self.receiver.id)})
        self.assertEqual(reg_res.status_code, 201)

        default_res = collector_client.get(f"/api/funerals/{self.funeral.id}/donation-accounts/")
        self.assertEqual(len(default_res.data), 0)

        with_pending_res = collector_client.get(f"/api/funerals/{self.funeral.id}/donation-accounts/?include_pending=true")
        self.assertEqual(len(with_pending_res.data), 1)
        self.assertFalse(with_pending_res.data[0]["is_active"])
...
class DonationVisibilityPermissionTests(TestCase):
    ...
    def setUp(self):
        ...
        gift_services.record_gift_donation(funeral=self.funeral, donor_name="A Guest", amount_cash=Decimal("50"), collector_name="Test Collector")
