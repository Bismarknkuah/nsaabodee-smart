...
from .models import ArrearsCorrectionRequest, AsupedeObligation, AsupedePayment, ContributionObligation, ContributionPayment, FuneralCommitteePosition, FuneralDeskAssignment, FuneralEvent, InLawContributionRequest, InLawObligation, InLawPayment, MemorialTribute, PaymentReversal
...
class FuneralEventCreateSerializer(serializers.Serializer):
    ...
    # provided.' Required here, at the real, user-facing boundary —
    # see create_funeral_event's own docstring for why the service
    # function underneath stays optional.
    cause_of_death = serializers.CharField(max_length=255)
    # 'When someone dies... their membership account should go
    # inactive.' Optional — not every deceased person is already a
    # registered Member — but when it is given, this is what actually
    # drives that transition.
    deceased_member_id = serializers.UUIDField(required=False, allow_null=True)
    ...
    # community price can be set again.' A per-funeral override for
    # exactly that situation — everyone else keeps the community's
    # standing default automatically.
    town_leader_amount = serializers.DecimalField(max_digits=10, decimal_places=2, required=False, allow_null=True)
    ...
    def save(self, **kwargs):
        ...
        if data.get("deceased_member_id"):
            from members.models import Member
            try:
                deceased_member = Member.objects.get(id=data["deceased_member_id"], community=request.user.community)
            except Member.DoesNotExist:
                raise serializers.ValidationError({"deceased_member_id": "Member not found in this community."})
        ...
        try:
            return services.create_funeral_event(
                ...
                town_leader_amount=data.get("town_leader_amount"),
                actor=request.user,
            )
...
class RequestFuneralEventSerializer(serializers.Serializer):
    ...
    # All optional — a Family Head can still request an opening without
    # nominating a Deceased Rep yet, and use the separate flow later.
    rep_member_id = serializers.UUIDField(required=False, allow_null=True)
    rep_new_username = serializers.CharField(required=False, allow_blank=True, max_length=150)
    rep_new_password = serializers.CharField(required=False, allow_blank=True, write_only=True)
    ...
    def save(self, **kwargs):
        ...
        rep_member = None
        if data.get("rep_member_id"):
            try:
                rep_member = Member.objects.get(id=data["rep_member_id"], community=user.community)
            except Member.DoesNotExist:
                raise serializers.ValidationError({"rep_member_id": "Member not found in this community."})

        deceased_member = None
        if data.get("deceased_member_id"):
            try:
                deceased_member = Member.objects.get(id=data["deceased_member_id"], community=user.community)
            except Member.DoesNotExist:
                raise serializers.ValidationError({"deceased_member_id": "Member not found in this community."})
        ...
        try:
            return services.request_funeral_event(
                ...
                rep_new_username=data.get("rep_new_username") or None,
                rep_new_password=data.get("rep_new_password") or None,
                actor=user,
            )
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages)
...
class MemberArrearsObligationSerializer(serializers.ModelSerializer):
    """'Their arrears they own should show.' Names the funeral and its status directly, since that's exactly what an Arrears Collector needs to see at the table — not just an obligation id."""
    balance = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    payment_status = serializers.CharField(read_only=True)
    funeral_deceased_name = serializers.CharField(source="funeral_event.deceased_name", read_only=True)
    funeral_status = serializers.CharField(source="funeral_event.status", read_only=True)
    funeral_collection_start_date = serializers.DateField(source="funeral_event.collection_start_date", read_only=True)

    class Meta:
        model = ContributionObligation
        fields = [
            "id", "funeral_event", "funeral_deceased_name", "funeral_status", "funeral_collection_start_date",
            "expected_amount", "amount_paid", "balance", "payment_status",
        ]
...
class RecordPaymentSerializer(serializers.Serializer):
    ...
    credit_overpayment_to_wallet = serializers.BooleanField(required=False, default=False)
    ...
    def save(self, **kwargs):
        ...
        try:
            return services.record_payment(
                ...
                collector_name=self.validated_data["collector_name"],
                credit_overpayment_to_wallet=self.validated_data.get("credit_overpayment_to_wallet", False),
            )
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages)
...
class ActivateAsupedeSerializer(serializers.Serializer):
    """'The funeral should therefore have enough information for the system to identify the relevant collection period.' Window fields are both optional — see AsupedeObligation's own docstring."""
    amount = serializers.DecimalField(max_digits=10, decimal_places=2)
    collection_start = serializers.DateTimeField(required=False, allow_null=True)
    collection_end = serializers.DateTimeField(required=False, allow_null=True)

    def save(self, **kwargs):
        request = self.context["request"]
        funeral = self.context["funeral"]
        try:
            return services.activate_asupede(
                funeral=funeral,
                amount=self.validated_data["amount"],
                collection_start=self.validated_data.get("collection_start"),
                collection_end=self.validated_data.get("collection_end"),
                actor=request.user,
            )
        except DjangoValidationError as exc:
...
class AsupedeObligationSerializer(serializers.ModelSerializer):
    member_name = serializers.CharField(source="member.full_name", read_only=True)
    balance = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    overpaid_amount = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    payment_status = serializers.CharField(read_only=True)

    class Meta:
        model = AsupedeObligation
        fields = ["id", "member", "member_name", "expected_amount", "amount_paid", "balance", "overpaid_amount", "payment_status"]


class RecordAsupedePaymentSerializer(serializers.Serializer):
    amount = serializers.DecimalField(max_digits=10, decimal_places=2)
    method = serializers.ChoiceField(choices=[("cash", "Cash"), ("mobile_money", "Mobile Money"), ("bank", "Bank"), ("other", "Other")])
    client_op_id = serializers.UUIDField(required=False, allow_null=True)
    collector_name = serializers.CharField(max_length=255)
    ...
    def save(self, **kwargs):
        ...
        obligation = self.context["obligation"]
        try:
            return services.record_asupede_payment(
                obligation=obligation,
                amount=self.validated_data["amount"],
                method=self.validated_data["method"],
                collector=request.user,
...
class AsupedePaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = AsupedePayment
        fields = ["id", "obligation", "amount", "method", "receipt_number", "collected_by", "paid_at"]
...
class ArrearsCorrectionRequestSerializer(serializers.ModelSerializer):
    obligation_member_name = serializers.CharField(source="obligation.member.full_name", read_only=True)
    funeral_deceased_name = serializers.CharField(source="obligation.funeral_event.deceased_name", read_only=True)
    requested_by_username = serializers.CharField(source="requested_by.username", read_only=True)
    decided_by_username = serializers.CharField(source="decided_by.username", read_only=True, default=None)

    class Meta:
        model = ArrearsCorrectionRequest
        fields = [
            "id", "obligation", "obligation_member_name", "funeral_deceased_name",
            "amount", "method", "reason", "status",
            "requested_by", "requested_by_username", "requested_at",
            "decided_by", "decided_by_username", "decided_at", "decision_notes", "resulting_payment",
        ]
        read_only_fields = fields


class RequestArrearsCorrectionSerializer(serializers.Serializer):
    """'If they paid but the system didn't reflect' — the reason is required, not optional, since it becomes part of the permanent audit trail."""
    amount = serializers.DecimalField(max_digits=10, decimal_places=2)
    method = serializers.ChoiceField(choices=ContributionPayment.Method.choices)
    reason = serializers.CharField()
    ...
    def save(self, **kwargs):
        ...
        actor = self.context["request"].user
        try:
            return services.request_arrears_correction(obligation=obligation, actor=actor, **self.validated_data)
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class DecideArrearsCorrectionSerializer(serializers.Serializer):
    notes = serializers.CharField(required=False, allow_blank=True, default="")

    def save(self, **kwargs):
        correction = self.context["correction"]
        actor = self.context["request"].user
        approve = self.context["approve"]
        try:
            if approve:
                return services.approve_arrears_correction(correction=correction, actor=actor, notes=self.validated_data.get("notes", ""))
            return services.reject_arrears_correction(correction=correction, actor=actor, notes=self.validated_data.get("notes", ""))
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class RecordPaymentsAcrossActiveFuneralsSerializer(serializers.Serializer):
    """
    'The system should be more efficient and friendly, so it can be
    faster.' One search, one form, every outstanding obligation across
    every active funeral settled in one submission — instead of a
    collector repeating the same search-and-submit per funeral.
    """
    member_id = serializers.UUIDField()
    method = serializers.ChoiceField(choices=ContributionPayment.Method.choices)
    collector_name = serializers.CharField(max_length=255)
    ...
    def save(self, **kwargs):
        ...
        data = self.validated_data
        try:
            member = Member.objects.get(id=data["member_id"], community=request.user.community)
        except Member.DoesNotExist:
            raise serializers.ValidationError({"member_id": "Member not found in this community."})
        if data["method"] == ContributionPayment.Method.WALLET:
            raise serializers.ValidationError({"method": "'Wallet credit' isn't selectable here — apply an existing wallet balance from a specific obligation instead."})
        try:
            return services.record_payments_across_active_funerals(
                member=member, method=data["method"], collector_name=data["collector_name"], collector=request.user,
            )
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class RequestInLawContributionSerializer(serializers.Serializer):
    ...
    in_law_family_id = serializers.UUIDField()
    ...
    def save(self, **kwargs):
        ...
        funeral = self.context["funeral"]
        try:
            in_law_family = Family.objects.get(id=self.validated_data["in_law_family_id"], community=request.user.community)
        except Family.DoesNotExist:
            raise serializers.ValidationError({"in_law_family_id": "Family not found in this community."})
        try:
            return services.request_in_law_contribution(
                funeral=funeral, in_law_family=in_law_family, relationship=self.validated_data["relationship"],
                requested_amount=self.validated_data["requested_amount"], reason=self.validated_data.get("reason", ""),
                actor=request.user,
            )
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))
...
class InLawObligationSerializer(serializers.ModelSerializer):
    in_law_family_name = serializers.CharField(source="in_law_family.name", read_only=True)
    balance = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    overpaid_amount = serializers.DecimalField(max_digits=10, decimal_places=2, read_only=True)
    payment_status = serializers.CharField(read_only=True)

    class Meta:
        model = InLawObligation
        fields = ["id", "request", "in_law_family", "in_law_family_name", "expected_amount", "amount_paid", "balance", "overpaid_amount", "payment_status"]


class InLawContributionRequestSerializer(serializers.ModelSerializer):
    in_law_family_name = serializers.CharField(source="in_law_family.name", read_only=True)
    ...
    # financial obligation.' Nested directly here — null until the
    # request is approved — so the frontend can discover the
    # obligation to record a payment against without a second,
    # separate "list obligations" endpoint just to look one up.
    obligation = InLawObligationSerializer(read_only=True, default=None)

    class Meta:
        model = InLawContributionRequest
        fields = [
            "id", "funeral_event", "in_law_family", "in_law_family_name", "relationship",
            "requested_amount", "reason", "status", "requested_by_username", "created_at", "decided_at",
            "obligation",
        ]


class RecordInLawPaymentSerializer(serializers.Serializer):
    amount = serializers.DecimalField(max_digits=10, decimal_places=2)
    method = serializers.ChoiceField(choices=[("cash", "Cash"), ("mobile_money", "Mobile Money"), ("bank", "Bank"), ("other", "Other")])
    collector_name = serializers.CharField(max_length=255)
    paid_by_member_id = serializers.UUIDField(required=False, allow_null=True)
    client_op_id = serializers.UUIDField(required=False, allow_null=True)
    ...
    def save(self, **kwargs):
        ...
        obligation = self.context["obligation"]
        paid_by_member = None
        member_id = self.validated_data.get("paid_by_member_id")
        if member_id:
            try:
                paid_by_member = Member.objects.get(id=member_id, community=request.user.community)
            except Member.DoesNotExist:
                raise serializers.ValidationError({"paid_by_member_id": "Member not found in this community."})
        try:
            return services.record_in_law_payment(
                obligation=obligation, amount=self.validated_data["amount"], method=self.validated_data["method"],
                collector=request.user, collector_name=self.validated_data["collector_name"],
                paid_by_member=paid_by_member, client_op_id=self.validated_data.get("client_op_id"),
            )
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class InLawPaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = InLawPayment
        fields = ["id", "obligation", "amount", "method", "receipt_number", "collected_by", "paid_by_member", "paid_at"]
