...
# role types.' A Contribution Collector collects on funerals that are
# OPEN. An Arrears Collector collects what was left owing after a
# funeral CLOSED — and, because arrears must be cleared before a
# current bill can be paid, may then also take the current bill.
CONTRIBUTION_COLLECTING_ROLES = {Role.COLLECTOR, Role.ARREARS_COLLECTOR}
ARREARS_COLLECTING_ROLES = {Role.ARREARS_COLLECTOR}


def can_collect_on(user, obligation) -> bool:
    """Role-level check for one obligation: closed funeral -> arrears roles only (never a plain Collector); open funeral -> either collector type."""
    if user.is_superuser or user.role == Role.COMMUNITY_ADMIN:
        return True
    from funerals.models import FuneralEvent
    if obligation.funeral_event.status == FuneralEvent.Status.ACTIVE:
        return user.role in CONTRIBUTION_COLLECTING_ROLES
    return user.role in ARREARS_COLLECTING_ROLES


def is_family_arrears_officer_for(user, obligation) -> bool:
    """
    'Each family should also have family arrears officer.' Scoped
    exactly like Family Registration Officer's own scope: this role can
    only collect arrears from a member of their OWN family, never
    community-wide the way Collector and Arrears Collector can.
    """
    if user.role != Role.FAMILY_ARREARS_OFFICER:
        return False
    own_member = getattr(user, "member_profile", None)
    if own_member is None or own_member.family_id is None:
        return False
    return obligation.member.family_id == own_member.family_id


def is_town_elders_arrears_officer_for(user, obligation) -> bool:
    """
    'The town elders should also have arrears.' Scoped to the Town
    Elders group itself, the same way Town Registration Officer already
    is: this role can only collect arrears from a member who is
    genuinely a Town Elder (Member.is_town_leader), never community
    -wide.
    """
    if user.role != Role.TOWN_ELDERS_ARREARS_OFFICER:
        return False
    return bool(obligation.member.is_town_leader)
