...
from .models import ArrearsCorrectionRequest, AsupedeObligation, AsupedePayment, ContributionObligation, ContributionPayment, FuneralApproval, FuneralCommitteePosition, FuneralDeskAssignment, FuneralEvent, FuneralMemberRateOverride, LedgerWallet, LedgerWalletTransaction, MemorialPage, MemorialTribute, PaymentReversal


@transaction.atomic
def create_funeral_event(
    *, community, deceased_name, deceased_gender, deceased_family,
    date_of_death, collection_start_date, cause_of_death="", burial_date=None, funeral_date=None,
    ...
    provided.' Enforced as required at the serializer layer
    (CreateFuneralSerializer, RequestFuneralOpeningSerializer), which
    is what every real, user-facing way of opening a funeral actually
    goes through. Left optional here, with the same safe "" default
    the model field itself has, since this service function is also
    the shared test fixture helper dozens of existing test suites
    across the whole backend already call directly without it —
    enforcing it this deep would have meant updating every one of
    those call sites for a rule that only actually matters at the
    point a real person opens a real funeral.
    """
    ...
    if deceased_member is not None and deceased_member.community_id != community.id:
        raise ValidationError("The deceased member must belong to this community.")
    ...
    funeral = FuneralEvent.objects.create(
        ...
        cause_of_death=(cause_of_death or "").strip(),
    ...
    # inactive.' The one place this transition actually happens —
    # deliberately not left to a human to remember to do separately,
    # since forgetting it would leave a dead member still showing as
    # active everywhere (still owing contributions on future funerals,
    # still appearing in registers) even though a funeral has already
    # been held for them.
    if deceased_member is not None and deceased_member.status != Member.Status.DECEASED:
        deceased_member.status = Member.Status.DECEASED
        deceased_member.save(update_fields=["status"])
...
def request_funeral_event(
    *, community, deceased_name, deceased_gender, deceased_family,
    date_of_death, collection_start_date, cause_of_death="", deceased_member=None, burial_date=None, funeral_date=None,
    collection_end_date=None, own_family_amount=None, general_male_amount=None,
    general_female_amount=None, actor=None, deceased_date_of_birth=None, town_leader_amount=None,
...
def approve_funeral_opening(*, funeral: FuneralEvent, approver) -> FuneralEvent:
    ...
    if funeral.status == FuneralEvent.Status.PENDING_APPROVAL:
        ...
        if distinct_approvers >= funeral.community.required_funeral_approvals:
            ...
            # 'When someone dies... their membership account should go
            # inactive.' Deliberately held off until the funeral is
            # actually approved and live, not at request time — a
            # pending request can still be rejected, and a rejected
            # funeral should never have left the deceased's own
            # membership status changed behind it.
            if funeral.deceased_member_id and funeral.deceased_member.status != Member.Status.DECEASED:
                funeral.deceased_member.status = Member.Status.DECEASED
                funeral.deceased_member.save(update_fields=["status"])
...
def request_payment_reversal(*, payment: ContributionPayment, reason: str, actor) -> PaymentReversal:
    ...
    unilaterally.
    """
    # Every role that records payments (Collector, Arrears Collector, and
    # the family/Town-Elders arrears officers) gets the same "flag my own
    # error" ability — the `collected_by` check is what actually keeps
    # this safe, not which collecting role the person happens to hold.
    collecting_roles = {"collector", "arrears_collector", "family_arrears_officer", "town_elders_arrears_officer"}
    is_collector_correcting_own_payment = actor.role in collecting_roles and payment.collected_by_id == actor.id
...
# 'The arrears collector should edit but have to be approved by the
# community treasurer before it reflects.' Deliberately narrow, unlike
# the broader reversal approval set above: this is specifically the
# Treasurer's own call, not shared with Secretary or Chairman, since
# it is the Treasurer who is asked by name.
ARREARS_CORRECTION_REQUEST_ROLES = {"arrears_collector", "collector", "family_arrears_officer", "town_elders_arrears_officer"}
ARREARS_CORRECTION_APPROVAL_ROLES = {"treasurer", "community_admin"}


def request_arrears_correction(*, obligation: ContributionObligation, amount: Decimal, method: str, reason: str, actor) -> ArrearsCorrectionRequest:
    """
    'If they paid but the system didn't reflect' is the request step.
    Nothing about the obligation's balance changes yet. Reuses the same
    two-person safeguard PaymentReversal already established: whoever
    notices the missing payment flags it, a different, specifically
    named authority (the Treasurer) decides whether it is genuine.
    """
    if not (actor.is_superuser or actor.role in ARREARS_CORRECTION_REQUEST_ROLES):
        raise ValidationError("Only an Arrears Collector, Collector, Family Arrears Officer, Town Elders Arrears Officer, or Super/Platform Admin can request an arrears correction.")
    if not reason.strip():
        raise ValidationError("A reason is required. This becomes part of the permanent audit trail.")
    if amount <= 0:
        raise ValidationError("The correction amount must be greater than zero.")
    return ArrearsCorrectionRequest.objects.create(
        obligation=obligation, amount=amount, method=method, reason=reason.strip(), requested_by=actor,
    )


@transaction.atomic
def approve_arrears_correction(*, correction: ArrearsCorrectionRequest, actor, notes: str = "") -> ArrearsCorrectionRequest:
    """
    The only place a correction actually takes effect. Records a real
    ContributionPayment through record_payment itself, so idempotency,
    the debt-priority rule, and suspicious-transaction flagging all
    still apply exactly as they do for any other payment. A different
    person than whoever requested it, matching the same two-person
    principle used everywhere else on this platform.
    """
    if not (actor.is_superuser or actor.role in ARREARS_CORRECTION_APPROVAL_ROLES):
        raise ValidationError("Only the Community Treasurer or Community Admin can approve an arrears correction.")
    if correction.status != ArrearsCorrectionRequest.Status.PENDING:
        raise ValidationError("This correction request has already been decided.")
    if correction.requested_by_id == actor.id:
        raise ValidationError("A different authorized person must approve this. The same person cannot request and approve their own correction.")

    payment = record_payment(
        obligation=correction.obligation, amount=correction.amount, method=correction.method,
        collector=correction.requested_by, collector_name=correction.requested_by.username,
    )

    correction.status = ArrearsCorrectionRequest.Status.APPROVED
    correction.decided_by = actor
    correction.decided_at = timezone.now()
    correction.decision_notes = notes
    correction.resulting_payment = payment
    correction.save()

    from audit_log.services import record_event
    record_event(
        category="arrears_correction", action="arrears_correction_approved", actor=actor, community=correction.obligation.community,
        target_type="ArrearsCorrectionRequest", target_id=correction.id, target_label=f"Correction of {correction.amount} approved",
        description=f"Approved an arrears correction of {correction.amount} for {correction.obligation.member.full_name}, requested by {correction.requested_by.username}. Reason: {correction.reason}",
    )
    return correction


def reject_arrears_correction(*, correction: ArrearsCorrectionRequest, actor, notes: str = "") -> ArrearsCorrectionRequest:
    if not (actor.is_superuser or actor.role in ARREARS_CORRECTION_APPROVAL_ROLES):
        raise ValidationError("Only the Community Treasurer or Community Admin can decide an arrears correction.")
    if correction.status != ArrearsCorrectionRequest.Status.PENDING:
        raise ValidationError("This correction request has already been decided.")
    correction.status = ArrearsCorrectionRequest.Status.REJECTED
    correction.decided_by = actor
    correction.decided_at = timezone.now()
    correction.decision_notes = notes
    correction.save()
...
def list_arrears_correction_requests(*, community, actor) -> list:
    if not (actor.is_superuser or actor.role in ARREARS_CORRECTION_APPROVAL_ROLES or actor.role in ARREARS_CORRECTION_REQUEST_ROLES):
        raise ValidationError("Only an Arrears Collector, Community Treasurer, or Community Admin can view arrears correction requests.")
    return list(
        ArrearsCorrectionRequest.objects.filter(obligation__community=community).select_related("obligation__member", "obligation__funeral_event", "requested_by", "decided_by")
    )
...
def lookup_member_arrears(*, member) -> list:
    """
    'When the person standing in front of the table mentions their name
    or code, their arrears they own should show.' Every one of this
    member's outstanding obligations, across EVERY funeral regardless
    of status — deliberately not limited to closed funerals only, since
    an Arrears Collector should see the member's full outstanding
    picture, not just the closed-funeral slice of it.
    Oldest first, matching the same debt-priority order
    _find_older_unpaid_obligation and record_payments_across_active_funerals
    already use — the oldest is the one that actually has to be settled
    next.
    """
    return list(
        ContributionObligation.objects
        .filter(member=member, community=member.community)
        .filter(amount_paid__lt=F("expected_amount"))
        .select_related("funeral_event")
        .order_by("funeral_event__collection_start_date")
    )
...
def lookup_family_arrears(*, family) -> list:
    """
    'We have a family arrears collector who is responsible for
    managing and collecting his family arrears only... each family
    will have their own family arrears collector.' The bulk,
    whole-family counterpart to lookup_member_arrears above (which is
    per-member, for the Front Desk lookup) — every member of this
    ONE family with an outstanding balance on a CLOSED funeral,
    grouped by member, oldest debt first within each. Deliberately
    closed funerals only, unlike lookup_member_arrears's "every
    status" — arrears specifically means debt left behind after a
    funeral has already ended, matching what ArrearsSection on the
    front desk already treats as arrears for one person at a time.
    """
    from django.db.models import Q

    obligations = (
        ContributionObligation.objects
        .filter(member__family=family, amount_paid__lt=F("expected_amount"))
        .filter(~Q(funeral_event__status=FuneralEvent.Status.ACTIVE))
        .select_related("member", "funeral_event")
        .order_by("member_id", "funeral_event__collection_start_date")
    )

    by_member = {}
    for o in obligations:
        entry = by_member.setdefault(o.member_id, {
            "member_id": str(o.member_id), "member_name": o.member.full_name, "total_owed": Decimal("0"), "obligations": [],
        })
        entry["total_owed"] += o.balance
        entry["obligations"].append({
            "obligation_id": str(o.id), "funeral_event_id": str(o.funeral_event_id),
            "funeral_deceased_name": o.funeral_event.deceased_name, "funeral_status": o.funeral_event.status,
            "balance": str(o.balance),
        })

    return sorted(
        [{**v, "total_owed": str(v["total_owed"])} for v in by_member.values()],
        key=lambda v: Decimal(v["total_owed"]), reverse=True,
    )
...
def funeral_closing_report(funeral: FuneralEvent) -> dict:
    """
    ...
    amount they received, and those who didn't pay.' Every number
    here was already isolated per funeral before this function existed
    (ContributionObligation, ContributionPayment, and FuneralExpense
    are all funeral-scoped at the database level) — this just gathers
    what was already separate into one place, for one funeral, rather
    than asking someone to read three different screens.
    """
    from funeral_logistics.models import FuneralExpense
    from gifts.services import gift_summary
    from reports.services import funeral_full_ledger_breakdown

    summary = funeral_summary(funeral)
    ledger_breakdown = funeral_full_ledger_breakdown(funeral)
    unpaid_obligations = (
        funeral.obligations.filter(amount_paid__lt=F("expected_amount"))
        .select_related("member", "member__family")
        .order_by("member__family__name", "member__full_name")
    )
    total_expected = sum((Decimal(b["expected_total"]) for b in (summary["own_family"], summary["general"], summary["town_elder"])), Decimal("0"))
    total_collected = sum((Decimal(b["collected_total"]) for b in (summary["own_family"], summary["general"], summary["town_elder"])), Decimal("0"))

    spent_expenses = FuneralExpense.objects.filter(funeral_event=funeral, status__in=[FuneralExpense.Status.PAID, FuneralExpense.Status.PARTIAL])
    total_spent = sum((e.amount for e in spent_expenses), Decimal("0"))
    total_gifts_received = Decimal(gift_summary(funeral)["total_combined_value"])
    ...
    return {
        ...
        "total_received": str(total_collected),
        "total_outstanding": str(total_expected - total_collected),
        "total_spent_on_logistics": str(total_spent),
        "total_gifts_received": str(total_gifts_received),
        # 'After every funeral each ledger should be able to know the
        # amount they received.' Each of the seven ledgers this
        # funeral actually keeps, side by side, rather than the single
        # merged total_received above — the same already-tested split
        # reports.services.funeral_full_ledger_breakdown uses for the
        # live, still-open view of a funeral, unchanged here.
...
# ---------------------------------------------------------------------------
# The Arrears Collector's own tools — "the arrears collector should have
# more options, and should be able to clear or collect community members'
# arrears before they can pay the current bills."
# ---------------------------------------------------------------------------

def lookup_community_arrears(*, community) -> list:
    """
    The community-wide counterpart to lookup_family_arrears: every member
    in the community with a balance still owing on a CLOSED funeral,
    grouped by member, oldest debt first within each, largest total owed
    first across members. This is the Arrears Collector's worklist.
    """
    from django.db.models import Q
    obligations = (
        ContributionObligation.objects
        .filter(community=community, amount_paid__lt=F("expected_amount"))
        .filter(~Q(funeral_event__status=FuneralEvent.Status.ACTIVE))
        .select_related("member", "member__family", "funeral_event")
        .order_by("member_id", "funeral_event__collection_start_date")
    )
    by_member = {}
    for o in obligations:
        row = by_member.setdefault(o.member_id, {
            "member_id": str(o.member_id), "member_name": o.member.full_name,
            "family_name": o.member.family.name if o.member.family_id else None,
            "phone": o.member.phone, "funerals": [], "total_owed": Decimal("0"),
        })
        row["funerals"].append({
            "obligation_id": str(o.id), "funeral_id": str(o.funeral_event_id), "deceased_name": o.funeral_event.deceased_name,
            "collection_start_date": o.funeral_event.collection_start_date.isoformat() if o.funeral_event.collection_start_date else None,
            "expected_amount": str(o.expected_amount), "amount_paid": str(o.amount_paid), "balance": str(o.balance),
        })
        row["total_owed"] += o.balance
    rows = sorted(by_member.values(), key=lambda r: r["total_owed"], reverse=True)
    for r in rows:
        r["total_owed"] = str(r["total_owed"])
        r["funeral_count"] = len(r["funerals"])
        r["oldest_deceased_name"] = r["funerals"][0]["deceased_name"]
    return rows


def arrears_position(*, member) -> dict:
    """
    One member's full picture for the Arrears Desk: their arrears
    (closed funerals, oldest first), the total, and their current bills
    (open funerals) — plus whether those current bills are payable yet.
    They are not until every arrear is cleared: that is the platform's
    own older-debt-first rule, made visible instead of only discovered
    when a payment is refused.
    """
    every = lookup_member_arrears(member=member)
    arrears = [o for o in every if o.funeral_event.status != FuneralEvent.Status.ACTIVE]
    current = [o for o in every if o.funeral_event.status == FuneralEvent.Status.ACTIVE]

    def _row(o):
        return {
            "obligation_id": str(o.id), "funeral_id": str(o.funeral_event_id), "deceased_name": o.funeral_event.deceased_name,
            "funeral_status": o.funeral_event.status, "rate_type": o.rate_type,
            "collection_start_date": o.funeral_event.collection_start_date.isoformat() if o.funeral_event.collection_start_date else None,
            "expected_amount": str(o.expected_amount), "amount_paid": str(o.amount_paid), "balance": str(o.balance),
        }
    total_arrears = sum((o.balance for o in arrears), Decimal("0"))
    total_current = sum((o.balance for o in current), Decimal("0"))
    return {
        "member_id": str(member.id), "member_name": member.full_name,
        "family_name": member.family.name if member.family_id else None,
        "arrears": [_row(o) for o in arrears], "total_arrears": str(total_arrears),
        "current_bills": [_row(o) for o in current], "total_current_bills": str(total_current),
        "current_bills_payable": total_arrears == 0,
        "total_owed": str(total_arrears + total_current),
    }


@transaction.atomic
def collect_arrears(*, member, amount: Decimal, collector, method: str = "cash", collector_name: str = "", client_op_id=None) -> dict:
    """
    One lump sum, cleared oldest-first: the Arrears Collector takes a
    single amount from the member and the platform splits it across
    their arrears in the order the debts actually fall due — never
    letting a newer balance be touched while an older one still stands.
    If every arrear is cleared and money remains, it flows on to the
    member's current bills (oldest open funeral first), because that is
    exactly the moment those become payable. Anything beyond everything
    owed is refused rather than silently parked. Each slice is a real
    record_payment, so receipts, the audit trail, live updates and the
    older-debt rule all apply exactly as for any other payment.
    """
    if amount <= 0:
        raise ValidationError("Amount must be greater than zero.")
    position = lookup_member_arrears(member=member)
    arrears = [o for o in position if o.funeral_event.status != FuneralEvent.Status.ACTIVE]
    current = [o for o in position if o.funeral_event.status == FuneralEvent.Status.ACTIVE]
    total_owed = sum((o.balance for o in position), Decimal("0"))
    if total_owed == 0:
        raise ValidationError(f"{member.full_name} owes nothing — there is no arrear or current bill to collect.")
    if amount > total_owed:
        raise ValidationError(f"{member.full_name} owes {total_owed} in total; {amount} is more than that. Collect at most {total_owed}.")

    total_arrears_before = sum((o.balance for o in arrears), Decimal("0"))
    remaining = amount
    payments, applied_to_arrears, applied_to_current = [], Decimal("0"), Decimal("0")
    for o in arrears + current:
        if remaining <= 0:
            break
        slice_amount = min(remaining, o.balance)
        if slice_amount <= 0:
            continue
        payment = record_payment(
            obligation=o, amount=slice_amount, method=method, collector=collector, collector_name=collector_name,
            client_op_id=f"{client_op_id}:{o.id}" if client_op_id else None,
        )
        payments.append({"payment_id": str(payment.id), "obligation_id": str(o.id), "deceased_name": o.funeral_event.deceased_name, "amount": str(slice_amount), "funeral_status": o.funeral_event.status})
        if o.funeral_event.status == FuneralEvent.Status.ACTIVE:
            applied_to_current += slice_amount
        else:
            applied_to_arrears += slice_amount
        remaining -= slice_amount

    remaining_arrears = total_arrears_before - applied_to_arrears
    return {
        "payments": payments,
        "applied_to_arrears": str(applied_to_arrears),
        "applied_to_current_bills": str(applied_to_current),
        "remaining_arrears": str(remaining_arrears),
        "arrears_cleared": remaining_arrears == 0,
    }
