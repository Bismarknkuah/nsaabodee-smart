...
class FuneralEvent(models.Model):
    ...
    # inactive.' Nullable, not every deceased person was already a
    # registered Member here (a child, a visiting relative, someone
    # who died before this platform existed) — deceased_name/gender
    # above stay the source of truth for the funeral itself regardless
    # of whether this link exists. When it is set, creating the
    # funeral automatically moves that Member to Status.DECEASED (see
    # create_funeral_event), which is also what makes the deceased
    # members registry (members.services.list_deceased_members)
    # possible at all.
    deceased_member = models.ForeignKey(
        "members.Member", null=True, blank=True, on_delete=models.SET_NULL, related_name="funerals_as_deceased",
    )
    # 'When opening for a funeral the cause of death has to be
    # provided.' Free text rather than a fixed choice list, since a
    # cause of death is rarely a single clean category. Enforced as
    # required in create_funeral_event and request_funeral_opening
    # (every way a funeral actually gets created), not at the model
    # level with blank=False — this field is being added to an
    # existing, already-deployed table with real rows in it, and a
    # non-nullable field with no default would fail that migration.
    # default="" only ever applies to funerals that already existed
    # before this field did.
    cause_of_death = models.CharField(max_length=255, default="", blank=True)
    ...
    # Per-title snapshots, mirroring the Community-level defaults above
    # exactly — nullable, falling back to town_leader_amount when unset,
    # so a funeral created before this feature (or for a community that
    # never configured per-title rates) behaves exactly as before.
    town_elder_chief_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    town_elder_queen_mother_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    town_elder_linguist_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    town_elder_other_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)

    # Asupedeɛ — 'a distinct funeral contribution category... must
    # remain identifiable... because it has a different purpose,
    # collection period, collection process, financial classification,
    # reporting category.' Null amount means Asupedeɛ simply isn't
    # active for this funeral at all — most funerals, and any
    # community that never configures a default, have none. The
    # collection window is deliberately optional even once an amount
    # is set: leaving both blank means payable any time the funeral is
    # open (no window restriction), matching 'the exact time should be
    # configurable rather than hardoded' — this platform never invents
    # a specific burial-morning window (e.g. "6am-10am") that wasn't
    # actually specified.
    asupede_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    asupede_collection_start = models.DateTimeField(null=True, blank=True)
    asupede_collection_end = models.DateTimeField(null=True, blank=True)
    ...
    def rate_for(self, member) -> tuple[str, "models.DecimalField"]:
        ...
        funeral — resolved in priority order:
          ...
          1. Town leader/elder (chief, queen mother, linguist, or
             other town executive) — their own distinct "town_elder"
             rate_type, regardless of which family they belong to.
             'Do not assume that every town elder has the same
             contribution amount' — resolved by
             town_elder_amount_for() below: an individual override on
             the member takes absolute priority, then this funeral's
             own per-title rate, then the flat town_leader_amount
             fallback — never a single universal figure unless a
             community genuinely never configured anything more
             specific.
        ...
        if member.is_town_leader:
            return "town_elder", self.town_elder_amount_for(member)
        ...
        if member.family_id == self.deceased_family_id:
            ...
            # 'The exact contribution hierarchy and amounts must remain
            # configurable... do not implement the Family Ledger as
            # every family member pays GHS X.' family_position is
            # optional and additive — only actually used when BOTH the
            # member has one recorded AND the community has configured
            # a rate for it; otherwise falls through to the original
            # gender/family_seniority tiers below exactly as before.
            if member.family_position:
                from contribution_rules.services import family_position_rate_for
                position_rate = family_position_rate_for(self.community, member.family_position)
                if position_rate is not None:
                    return "own_family", position_rate
    ...
    def town_elder_amount_for(self, member) -> "models.DecimalField":
        """
        'Town elder contribution can also be individual... Elder A ->
        GHS X, Elder B -> GHS Y' AND 'the expected contribution may
        differ according to the elder's official position' — two
        separate, stackable rules, resolved in this order:
          1. member.town_elder_individual_rate, if set — one specific
             person's own standing rate, wins over everything else.
          2. This funeral's own per-title snapshot (town_elder_chief_amount
             etc.), if set for this member's title.
          3. self.town_leader_amount — the flat fallback every funeral
             already has, unchanged for any community that never
             configures anything more specific.
        """
        if member.town_elder_individual_rate is not None:
            return member.town_elder_individual_rate

        title = member.town_elder_title
        per_title_field = {
            "chief": self.town_elder_chief_amount,
            "queen_mother": self.town_elder_queen_mother_amount,
            "linguist": self.town_elder_linguist_amount,
            "other": self.town_elder_other_amount,
        }.get(title)
        if per_title_field is not None:
            return per_title_field

        return self.town_leader_amount
...
class FuneralDeskAssignmentApproval(models.Model):
    """
    ...
    community chairman and the secretary to approve.' Unlike a
    Community Admin's own direct assignment (immediately active — that
    authority already IS the approval), a Treasurer-initiated one
    needs sign-off from two SPECIFIC, NAMED roles — not just any two
    people from a broader set. One row per person who has actually
    approved; the unique constraint means the same person approving
    twice is a no-op, never a double-count, and
    funerals.services.approve_desk_assignment checks that both
    specific required roles are genuinely represented before
    activating — not just any two rows.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    desk_assignment = models.ForeignKey(FuneralDeskAssignment, on_delete=models.CASCADE, related_name="approvals")
    approved_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="+")
...
class ContributionObligation(models.Model):
    ...
    class RateType(models.TextChoices):
        ...
        TOWN_ELDER = "town_elder", "Town Elder Rate"
...
class ContributionPayment(models.Model):
    ...
    # they paid to.' collected_by (above) is the logged-in account —
    # but a desk assignment can genuinely be shared by more than one
    # real person working the same table across a long day, so the
    # login alone doesn't always identify who actually took this
    # specific payment. A real, required name typed at the moment of
    # collection, independent of whichever account happened to be
    # logged in.
    ...
    def qr_payload(self) -> str:
        """
        'All receipt printed should have the QR scanner so when they
        scan it should confirm the amount they paid and who received
        the pay.' The same "a real, scannable URL, not a custom
        app-only URI scheme" choice already made for Member.qr_payload
        — lands on a login-protected verification page showing the
        amount and the collector, exactly what the request asks
        someone scanning a printed receipt to be able to confirm.
        """
        from django.conf import settings
        return f"{settings.FRONTEND_BASE_URL}/verify-receipt/{self.id}"


class ArrearsCorrectionRequest(models.Model):
    """
    ...
    Follows the same two-person safeguard as PaymentReversal above.
    The Arrears Collector never edits ContributionObligation.amount_paid
    directly. A request names the amount they say was actually paid and
    why it never got recorded, and stays inert until a Community
    Treasurer approves it. Only on approval does a real
    ContributionPayment row get created through the normal
    record_payment path, so every existing safeguard still applies:
    idempotency, the debt-priority rule, and suspicious-transaction
    flagging. resulting_payment stays null until then, which is how the
    system tells a request that changed nothing yet from one that
    genuinely did.
    """
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    obligation = models.ForeignKey("ContributionObligation", on_delete=models.CASCADE, related_name="arrears_correction_requests")
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    method = models.CharField(max_length=20, choices=ContributionPayment.Method.choices)
    reason = models.TextField()
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.PENDING)
    requested_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="+")
    requested_at = models.DateTimeField(auto_now_add=True)
    ...
    decided_at = models.DateTimeField(null=True)
    decision_notes = models.TextField(blank=True)
    resulting_payment = models.ForeignKey(ContributionPayment, null=True, on_delete=models.SET_NULL, related_name="+")

    class Meta:
        ordering = ["-requested_at"]

    def __str__(self):
        return f"Arrears correction for {self.obligation_id} ({self.status})"
...
class LedgerWallet(models.Model):
    """
    'Each contribution will be saved [in] the community wallet, and
    each family should have their wallet being managed by the family
    treasurer... and the town elder should also have their wallet as
    well.' A real, held balance for each of this platform's three
    mandatory-contribution ledgers — general (community-wide),
    own_family (per family), and town_elder — mirroring
    ContributionObligation.RateType exactly, since that's what decides
    which wallet a given payment credits.

    Deliberately its OWN model, never folded into family_funds.FamilyFund
    — that model is explicitly "a family's own, entirely private
    contribution scheme," structurally isolated from every community
    -wide ledger on purpose. This wallet holds a genuinely different
    kind of money (mandatory funeral contributions, not voluntary
    family savings), and mixing the two under one model would be
    exactly the kind of confusion this whole feature exists to avoid.

    One row per (community, scope, family) — family is only ever set
    for scope=own_family; community and town_elder scopes are
    community-wide, so exactly one row of each exists per community.
    Created on first use rather than for every community/family up
    front, matching MemberWallet's own "created on first use" pattern.
    """

    class Scope(models.TextChoices):
        COMMUNITY = "general", "Community Ledger"
        FAMILY = "own_family", "Family Ledger"
        TOWN_ELDER = "town_elder", "Town Elders Ledger"
        ASUPEDE = "asupede", "Asupedeɛ"

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="ledger_wallets")
    scope = models.CharField(max_length=20, choices=Scope.choices)
    # Only ever set when scope="own_family" — a community or town_elder
    # scoped wallet is community-wide, never tied to one specific family.
    family = models.ForeignKey("families.Family", null=True, blank=True, on_delete=models.CASCADE, related_name="ledger_wallet")
    balance = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["community", "scope", "family"], name="one_ledger_wallet_per_community_scope_family"),
            models.CheckConstraint(
                condition=(
                    models.Q(scope__in=["own_family", "asupede"], family__isnull=False)
                    | (~models.Q(scope__in=["own_family", "asupede"]) & models.Q(family__isnull=True))
                ),
    ...
    def __str__(self):
        if self.scope == self.Scope.FAMILY:
            return f"{self.family.name}'s Ledger Wallet — {self.balance}"
        return f"{self.community.name}'s {self.get_scope_display()} Wallet — {self.balance}"


class LedgerWalletTransaction(models.Model):
    """
    A full, auditable ledger of every credit (a real ContributionPayment
    landing) and debit (an approved withdrawal/expense against it) —
    'balance' alone is never enough of a record for real money moving
    through a community-wide account, the same reasoning behind
    MemberWallet.WalletTransaction and every other ledger on this
    platform.
    """

    class Kind(models.TextChoices):
        CREDIT = "credit", "Credit (a contribution payment landed)"
        DEBIT = "debit", "Debit (an approved withdrawal or expense)"

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    wallet = models.ForeignKey(LedgerWallet, on_delete=models.CASCADE, related_name="transactions")
    kind = models.CharField(max_length=10, choices=Kind.choices)
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    # Traceability straight back to the payment that caused this credit —
    # never guessing which payment funded which line in the wallet's history.
    source_payment = models.ForeignKey(
        "funerals.ContributionPayment", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
...
class AsupedeObligation(models.Model):
    """
    Asupedeɛ — 'a small communal burial contribution/pre-burial
    offering, collected on the morning of the burial while the
    deceased is lying in state... to support the immediate family with
    the final burial preparations.' Deliberately its OWN model, never
    a fourth ContributionObligation.RateType: Asupedeɛ is ADDITIVE, not
    mutually exclusive with a member's ordinary family/general/town
    -elder obligation on the SAME funeral ('a family member could have
    Family Contribution = GHS 500 and separately Asupedeɛ = GHS 20 —
    both must be recorded independently') — ContributionObligation's
    own one-obligation-per-member-per-funeral constraint exists
    precisely because rate_for() already picks exactly one mutually
    -exclusive rate_type per member, so bolting a fifth, additive
    obligation onto that model would mean either breaking that
    constraint (a real risk to the hundreds of existing `.get(funeral_event=,
    member=)` lookups across this codebase that depend on it) or
    silently conflating two obligations that must stay independently
    traceable. Same precedent as gifts.GiftDonation already being its
    own, separate model from this ledger rather than a bolted-on
    rate_type.

    Only ever created for a funeral that has actually activated
    Asupedeɛ (see funerals.services.activate_asupede) — most funerals,
    and any community that never configures it, have none at all.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="+")
    funeral_event = models.ForeignKey(FuneralEvent, on_delete=models.CASCADE, related_name="asupede_obligations")
    member = models.ForeignKey("members.Member", on_delete=models.CASCADE, related_name="asupede_obligations")
    expected_amount = models.DecimalField(max_digits=10, decimal_places=2)
    amount_paid = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["funeral_event", "member"], name="one_asupede_obligation_per_member_per_funeral")
        ]
        indexes = [models.Index(fields=["community", "funeral_event"])]

    @property
    def balance(self):
        """'Where the community treats the Asupedeɛ amount as a minimum, the contributor may give more' — never negative; excess is overpaid_amount, exactly like the mandatory ledger."""
        return max(self.expected_amount - self.amount_paid, Decimal("0"))
    ...
    def __str__(self):
        return f"{self.member.full_name}'s Asupedeɛ for {self.funeral_event.deceased_name}'s funeral"


class AsupedePayment(models.Model):
    """One instalment against an AsupedeObligation — mirrors ContributionPayment's own shape exactly, kept as its own table for the same reason the obligation above is."""
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    obligation = models.ForeignKey(AsupedeObligation, on_delete=models.CASCADE, related_name="payments")
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    method = models.CharField(max_length=20, choices=Method.choices)
    receipt_number = models.CharField(max_length=50, unique=True)
    ...
    def __str__(self):
        return f"{self.amount} via {self.method} — Asupedeɛ receipt {self.receipt_number}"


class InLawContributionRequest(models.Model):
    """
    'Do NOT automatically create a financial obligation solely
    because a relationship exists. The deceased family should be
    able to initiate an In-Law Contribution Request... only after
    approval should the system generate the actual financial
    obligation.' Deliberately a request-first, approval-gated
    workflow — the opposite of every other ledger on this platform,
    which auto-enrolls every eligible member the moment a funeral is
    created. Targets a whole FAMILY (the connected in-law family),
    not an individual member — the relationship this represents is
    inherently a connection between two families through a marriage,
    and the amount is a single, negotiated figure for that family as
    a whole, not a per-member rate lookup.
    """
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="+")
    funeral_event = models.ForeignKey(FuneralEvent, on_delete=models.CASCADE, related_name="in_law_requests")
    # The connected in-law family this request targets — deliberately
    # never the deceased's own family (see clean()/services validation).
    in_law_family = models.ForeignKey("families.Family", on_delete=models.CASCADE, related_name="+")
    # 'Relevant relationship' — free text, the same "a real-world
    # relationship rarely maps cleanly onto a fixed list" reasoning
    # already applied to gifts.GiftDonation.relationship_to_recipient.
    ...
    def __str__(self):
        return f"In-law request — {self.in_law_family.name} for {self.funeral_event.deceased_name}'s funeral ({self.status})"


class InLawObligation(models.Model):
    """The actual financial obligation — only ever created the moment an InLawContributionRequest is approved, never before."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    request = models.OneToOneField(InLawContributionRequest, on_delete=models.CASCADE, related_name="obligation")
    community = models.ForeignKey("tenants.Community", on_delete=models.CASCADE, related_name="+")
    funeral_event = models.ForeignKey(FuneralEvent, on_delete=models.CASCADE, related_name="in_law_obligations")
    in_law_family = models.ForeignKey("families.Family", on_delete=models.CASCADE, related_name="+")
    expected_amount = models.DecimalField(max_digits=10, decimal_places=2)
    amount_paid = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.in_law_family.name}'s In-Law contribution for {self.funeral_event.deceased_name}'s funeral"


class InLawPayment(models.Model):
    """One instalment against an InLawObligation — mirrors ContributionPayment/AsupedePayment's own shape."""
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    obligation = models.ForeignKey(InLawObligation, on_delete=models.CASCADE, related_name="payments")
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    method = models.CharField(max_length=20, choices=Method.choices)
    receipt_number = models.CharField(max_length=50, unique=True)
    collected_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, on_delete=models.SET_NULL, related_name="+")
    collector_name = models.CharField(max_length=255, blank=True)
    # Which member of the in-law family actually paid — a family-level
    # obligation, but a real, individual person still hands over the
    # money, and that's worth recording.
    paid_by_member = models.ForeignKey("members.Member", null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    client_op_id = models.UUIDField(unique=True, null=True, blank=True)
    paid_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.amount} via {self.method} — In-Law receipt {self.receipt_number}"
