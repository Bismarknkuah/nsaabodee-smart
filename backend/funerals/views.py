...
from .serializers import (
    ...
    ArrearsCorrectionRequestSerializer,
    RequestArrearsCorrectionSerializer,
    DecideArrearsCorrectionSerializer,
...
class FuneralEventViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=["get"], url_path="closing-report")
    def closing_report(self, request, id=None):
        """
        'Each funeral should have its own fund [so] the family knows
        the expenditure of that particular funeral, so accountability
        wouldn't be a problem... after every funeral each ledger
        should know the amount they received, and those who didn't
        pay.' Same visibility as this funeral's own detail page
        (FUNERAL_BROWSE_ROLES, already enforced by this ViewSet's own
        queryset) — no separate permission class needed here.
        """
        funeral = self.get_object()
        report = services.funeral_closing_report(funeral)
        if request.query_params.get("export") == "pdf":
            from django.http import HttpResponse
            from reports.pdf import funeral_closing_report_pdf
            pdf_bytes = funeral_closing_report_pdf(community_name=funeral.community.name, report=report)
            response = HttpResponse(pdf_bytes, content_type="application/pdf")
            response["Content-Disposition"] = f'inline; filename="closing-report-{funeral.deceased_name}.pdf"'
            return response
    ...
    def record_payment(self, request, id=None, obligation_id=None):
        ...
        from .permissions import can_collect_on, is_family_arrears_officer_for, is_town_elders_arrears_officer_for
        own_member = getattr(user, "member_profile", None)
        ...
        # A desk worker is assigned to an OPEN funeral's desk, so that grant
        # (like the plain Collector role) only ever covers active funerals;
        # closed-funeral arrears are the arrears roles' work.
        is_active_funeral = funeral.status == FuneralEvent.Status.ACTIVE
        if not (
            can_collect_on(user, obligation) or is_own_obligation
            or (is_active_funeral and is_desk_worker_for(user, funeral, "contributions"))
            or is_family_arrears_officer_for(user, obligation) or is_town_elders_arrears_officer_for(user, obligation)
        ):
            detail = (
                "This is an arrears balance on a closed funeral — only an Arrears Collector (or the family's or Town Elders' arrears officer) can collect it."
                if not is_active_funeral else
                "You're not assigned to this funeral's contributions desk, and this isn't your own contribution."
            )
...
class RequestArrearsCorrectionView(APIView):
    """'If they paid but the system didn't reflect' is the request step. Community-scoped so nobody can reach into another community's obligations."""
    permission_classes = [IsAuthenticated]

    def post(self, request, obligation_id):
        obligation = get_object_or_404(ContributionObligation, id=obligation_id)
        if not request.user.is_superuser and obligation.funeral_event.community_id != request.user.community_id:
            return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
        serializer = RequestArrearsCorrectionSerializer(data=request.data, context={"obligation": obligation, "request": request})
        serializer.is_valid(raise_exception=True)
        correction = serializer.save()
        return Response(ArrearsCorrectionRequestSerializer(correction).data, status=status.HTTP_201_CREATED)


class ListArrearsCorrectionRequestsView(APIView):
    """Every arrears correction request for the acting user's own community, pending and decided alike."""
    permission_classes = [IsAuthenticated]

    def get(self, request):
        try:
            corrections = services.list_arrears_correction_requests(community=request.user.community, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response(ArrearsCorrectionRequestSerializer(corrections, many=True).data)


class _DecideArrearsCorrectionView(APIView):
    permission_classes = [IsAuthenticated, RequiresExecutiveContext]
    approve: bool

    def post(self, request, correction_id):
        from .models import ArrearsCorrectionRequest
        correction = get_object_or_404(ArrearsCorrectionRequest, id=correction_id)
        if not request.user.is_superuser and correction.obligation.funeral_event.community_id != request.user.community_id:
            return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
        serializer = DecideArrearsCorrectionSerializer(
            data=request.data, context={"correction": correction, "request": request, "approve": self.approve},
        )
        serializer.is_valid(raise_exception=True)
        updated = serializer.save()
        return Response(ArrearsCorrectionRequestSerializer(updated).data)


class ApproveArrearsCorrectionView(_DecideArrearsCorrectionView):
    approve = True


class RejectArrearsCorrectionView(_DecideArrearsCorrectionView):
    approve = False
...
# ---- Arrears Desk: the Arrears Collector's own endpoints ----
_ARREARS_DESK_ROLES = {"arrears_collector", "community_admin", "treasurer", "financial_secretary"}


def _arrears_desk_allowed(user) -> bool:
    return user.is_superuser or user.role in _ARREARS_DESK_ROLES


class ArrearsWorklistView(APIView):
    """GET -> every member in the community owing on a closed funeral, largest total first."""
    permission_classes = [IsAuthenticated]

    def get(self, request):
        if not _arrears_desk_allowed(request.user):
            return Response({"detail": "Only the Arrears Collector (or a finance officer) can see the community arrears worklist."}, status=status.HTTP_403_FORBIDDEN)
        return Response(services.lookup_community_arrears(community=request.user.community))


class ArrearsPositionView(APIView):
    """GET -> one member's arrears (oldest first), current bills, and whether those are payable yet."""
    permission_classes = [IsAuthenticated]

    def get(self, request, member_id):
        from members.models import Member
        from .permissions import is_family_arrears_officer_for, is_town_elders_arrears_officer_for
        member = get_object_or_404(Member, id=member_id, community=request.user.community)
        scoped_ok = False
        if request.user.role in ("family_arrears_officer", "town_elders_arrears_officer"):
            first = ContributionObligation.objects.filter(member=member).first()
            scoped_ok = bool(first) and (is_family_arrears_officer_for(request.user, first) or is_town_elders_arrears_officer_for(request.user, first))
        if not (_arrears_desk_allowed(request.user) or scoped_ok):
            return Response({"detail": "Only an arrears officer can open the Arrears Desk for a member."}, status=status.HTTP_403_FORBIDDEN)
        return Response(services.arrears_position(member=member))


class CollectArrearsView(APIView):
    """POST {amount, method, collector_name} -> one lump sum, cleared oldest-first across the member's arrears, then current bills."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, member_id):
        ...
        if request.user.role in ("family_arrears_officer", "town_elders_arrears_officer"):
            ...
            scoped_ok = bool(first) and (is_family_arrears_officer_for(request.user, first) or is_town_elders_arrears_officer_for(request.user, first))
        if not (_arrears_desk_allowed(request.user) or scoped_ok):
            return Response({"detail": "Only an arrears officer can collect arrears."}, status=status.HTTP_403_FORBIDDEN)
        try:
            amount = Decimal(str(request.data.get("amount", "0")))
        except Exception:
            return Response({"detail": "A valid amount is required."}, status=status.HTTP_400_BAD_REQUEST)
        try:
            result = services.collect_arrears(
                member=member, amount=amount, collector=request.user, method=request.data.get("method", "cash"),
                collector_name=request.data.get("collector_name", "") or request.user.get_full_name() or request.user.username,
                client_op_id=request.data.get("client_op_id"),
            )
        except DjangoValidationError as exc:
