...
class FuneralEventViewSet(viewsets.ModelViewSet):
    ...
    def record_payment(self, request, id=None, obligation_id=None):
        ...
        from .permissions import PAYMENT_COLLECTING_ROLES, is_family_arrears_officer_for, is_town_elders_arrears_officer_for
        own_member = getattr(user, "member_profile", None)
        ...
        if not (
            user.is_superuser or user.role in PAYMENT_COLLECTING_ROLES
            or is_desk_worker_for(user, funeral, "contributions") or is_own_obligation
