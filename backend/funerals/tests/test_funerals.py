...
class FuneralLedgerTests(TestCase):
    ...
    def test_record_payment_updates_running_total_and_status(self):
        ...
        services.record_payment(obligation=obligation, amount=Decimal("20"), method="cash", collector=self.admin, collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.payment_status, "partial")
        self.assertEqual(obligation.balance, Decimal("30"))

        services.record_payment(obligation=obligation, amount=Decimal("30"), method="mobile_money", collector=self.admin, collector_name="Test Collector")
        obligation.refresh_from_db()
    ...
    def test_paying_more_than_the_required_amount_is_accepted_not_blocked(self):
        ...
        services.record_payment(obligation=obligation, amount=Decimal("100"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
    ...
    def test_duplicate_offline_payment_is_idempotent_on_client_op_id(self):
        ...
        p1 = services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", client_op_id=op_id, collector_name="Test Collector")
        p2 = services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", client_op_id=op_id, collector_name="Test Collector")
...
class OverpaymentTests(TestCase):
    ...
    def test_paying_more_than_the_general_rate_is_accepted_not_rejected(self):
        ...
        services.record_payment(obligation=obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
    ...
    def test_a_second_separate_payment_after_already_fully_paid_is_now_rejected(self):
        """
        'One person is not allowed to pay twice of each funeral, but can
        decide to pay more than the required amount.' This supersedes
        the platform's earlier behavior (which used to accept a further,
        separate "generous" payment even after an obligation was
        already fully settled) — overpaying is still fully allowed, but
        only within a single payment, decided upfront, never as a
        second, separate top-up after the fact.
        """
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.outsider)
        services.record_payment(obligation=obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.payment_status, "paid")

        with self.assertRaises(ValidationError):
            services.record_payment(obligation=obligation, amount=Decimal("20"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.amount_paid, Decimal("5"))  # untouched by the rejected second attempt

    def test_overpaying_within_a_single_payment_is_still_fully_allowed(self):
        """The other half of the same rule — deciding upfront to pay more than required, in ONE payment, is never blocked."""
        obligation = ContributionObligation.objects.get(funeral_event=self.funeral, member=self.outsider)
        services.record_payment(obligation=obligation, amount=Decimal("25"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
    ...
    def test_partial_underpayment_is_still_supported_unchanged(self):
        ...
        services.record_payment(obligation=obligation, amount=Decimal("2"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.payment_status, "partial")
        self.assertEqual(obligation.balance, Decimal("3"))

    def test_multiple_partial_payments_that_together_settle_it_are_still_allowed(self):
        """The double-payment guard must never block legitimate installments — only a payment attempted AFTER the balance already reached zero."""
        from funerals.models import ContributionObligation as CO
        obligation = CO.objects.get(funeral_event=self.funeral, member=self.outsider)
        services.record_payment(obligation=obligation, amount=Decimal("2"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.payment_status, "partial")
        services.record_payment(obligation=obligation, amount=Decimal("3"), method="cash", collector_name="Test Collector")
        obligation.refresh_from_db()
        self.assertEqual(obligation.payment_status, "paid")
        self.assertEqual(obligation.amount_paid, Decimal("5"))
...
class TieredFamilyRateTests(TestCase):
    ...
    def test_the_full_tiered_pricing_model(self):
        ...
        self.assertEqual(self._obligation_for(funeral, self.town_leader).rate_type, "town_elder")
