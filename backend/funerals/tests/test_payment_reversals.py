...
class PaymentReversalServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.payment = services.record_payment(obligation=self.obligation, amount=Decimal("20"), method="cash", collector=self.collector, collector_name="Test Collector")
    ...
    def test_a_collector_CAN_now_request_a_reversal_for_their_own_payment(self):
        """'Collectors should have access to edit, especially when they make mistakes.' self.payment was collected by self.collector."""
        reversal = services.request_payment_reversal(payment=self.payment, reason="I entered the wrong amount", actor=self.collector)
        self.assertEqual(reversal.requested_by, self.collector)

    def test_a_collector_cannot_request_a_reversal_for_someone_elses_payment(self):
        """Deliberately narrow: this is about correcting your OWN mistake, not general reversal authority over any payment community-wide."""
        other_collector = User.objects.create_user(username="a_different_collector", password="x", community=self.bodi, role=Role.COLLECTOR)
        with self.assertRaises(ValidationError):
            services.request_payment_reversal(payment=self.payment, reason="Not my payment", actor=other_collector)
...
class PaymentReversalHttpTests(TestCase):
    def setUp(self):
        ...
        self.payment = services.record_payment(obligation=obligation, amount=Decimal("20"), method="cash", collector_name="Test Collector")
