...
class DebtPriorityTests(TestCase):
    ...
    def test_paying_toward_a_newer_obligation_is_blocked_while_an_older_one_is_unpaid(self):
        with self.assertRaises(ValidationError):
            funeral_services.record_payment(obligation=self.newer_obligation, amount=Decimal("3"), method="cash", collector_name="Test Collector")
    ...
    def test_paying_the_older_debt_itself_is_never_blocked(self):
        payment = funeral_services.record_payment(obligation=self.older_obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
        self.assertIsNotNone(payment)

    def test_once_the_older_debt_is_fully_settled_the_newer_payment_goes_through(self):
        funeral_services.record_payment(obligation=self.older_obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
        payment = funeral_services.record_payment(obligation=self.newer_obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
        self.assertIsNotNone(payment)

    def test_a_partial_payment_on_the_older_debt_still_blocks_the_newer_one(self):
        funeral_services.record_payment(obligation=self.older_obligation, amount=Decimal("2"), method="cash", collector_name="Test Collector")  # partial (owes 5 total)
        with self.assertRaises(ValidationError):
            funeral_services.record_payment(obligation=self.newer_obligation, amount=Decimal("5"), method="cash", collector_name="Test Collector")
    ...
    def test_two_same_day_funerals_never_block_each_other(self):
        ...
        payment = funeral_services.record_payment(obligation=same_day_obligation, amount=Decimal("3"), method="cash", collector_name="Test Collector")
        self.assertIsNotNone(payment)
    ...
    def test_no_settlement_notification_when_there_was_never_a_newer_obligation_to_block(self):
        ...
        funeral_services.record_payment(obligation=lone_obligation, amount=Decimal("50"), method="cash", collector_name="Test Collector")
