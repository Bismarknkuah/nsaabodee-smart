...
class FuneralOpeningApprovalServiceTests(TestCase):
    ...
    def test_requesting_a_funeral_can_nominate_a_rep_in_the_same_call(self):
        """'The ledger opening has to ask for that person to be nominated.'"""
        from families.models import BereavedRepAssignment

        # assign_family_head alone never creates a login — link one here
        # since this test needs a genuine, authorized Family Head actor.
        head_user = User.objects.create_user(username="nominating_head", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        member_services.link_member_to_user(member=self.head_member, user=head_user, actor=self.admin)

        funeral_services.request_funeral_event(
            community=self.bodi, deceased_name="Nomination Deceased", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
            actor=head_user,
            rep_new_username="nominated_at_request", rep_new_password="a-real-password-123",
        )
        assignment = BereavedRepAssignment.objects.get(family=self.asona)
        self.assertEqual(assignment.user.username, "nominated_at_request")
        self.assertFalse(assignment.is_active, "A Family Head's own nomination still starts pending, submitted to the committee.")

    def test_requesting_a_funeral_without_nominating_anyone_still_works(self):
        """Deliberately optional — nomination can still happen separately later via the Families page."""
        from families.models import BereavedRepAssignment

        funeral_services.request_funeral_event(
            community=self.bodi, deceased_name="No Rep Yet Deceased", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
            actor=self.head_member.linked_user,
        )
        self.assertFalse(BereavedRepAssignment.objects.filter(family=self.asona).exists())
...
class FuneralOpeningApprovalHttpTests(TestCase):
    ...
    def test_family_head_can_request_an_opening_for_his_own_family(self):
        ...
        res = client.post("/api/funerals/request/", {
            ...
            "date_of_death": "2026-07-01", "cause_of_death": "Natural causes", "collection_start_date": "2026-07-01",
        })
