...
class PaymentRecordingPerformanceTests(TestCase):
    ...
    def test_recording_a_single_payment_uses_a_bounded_number_of_queries(self):
        ...
        # Observed at 7 queries in practice before LedgerWallet routing
        # existed; now 14-17 (bounded, not growing with member count —
        # confirmed directly: a second payment against an
        # already-existing wallet uses FEWER queries than the first,
        # which has to create one) since every payment now also credits
        # the matching community/family/town-elder ledger wallet — see
        # funerals.services.credit_ledger_wallet_for_payment. Asserting
        # <=25 (an upper bound, not an exact match) so a legitimate
        # small addition later doesn't break this test, while still
        # catching a genuine N+1 regression (which would blow this
        # budget by many multiples, not by a fixed handful).
        ...
        self.assertLessEqual(len(ctx.captured_queries), 25, "More queries than expected for a single payment — check for an N+1.")
