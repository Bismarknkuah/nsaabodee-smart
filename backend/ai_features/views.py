...
class MeetingSummaryView(APIView):
    """Community Admin/Chairman/Secretary only — an unrestricted AI-provider call had no role check at all before this."""
    permission_classes = [IsAuthenticated]

    def post(self, request):
        if not (request.user.is_superuser or request.user.role in ("community_admin", "chairman", "secretary")):
            return Response(
                {"detail": "Only Community Admin, Chairman, or Secretary can generate a meeting summary."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class SuspiciousTransactionFlagViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Read: any authenticated member of the community (matches the
    broad Finance Oversight nav access). Review (dismiss/confirm) is
    narrower — the same "reviews, doesn't just audit" split already
    used for payment reversals: Auditor can see every flag but never
    decides one, matching their role everywhere else on this platform.
    """
    ...
    def partial_update(self, request, *args, **kwargs):
        if not (
            ...
            or request.user.role in ("community_admin", "chairman", "secretary", "treasurer", "financial_secretary")
        ):
            return Response(
                {"detail": "Only Community Admin, Chairman, Secretary, Treasurer, or Financial Secretary can review a flag."},
                status=status.HTTP_403_FORBIDDEN,
            )
