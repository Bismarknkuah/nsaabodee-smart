...
class PredictionAndInactiveMembersTests(TestCase):
    ...
    def test_prediction_uses_historical_collection_rate(self):
        ...
        funeral_services.record_payment(obligation=obligation, amount=Decimal("25"), method="cash", collector_name="Test Collector")  # half of 50
...
class SuspiciousTransactionTests(TestCase):
    ...
    def test_no_flag_without_enough_history(self):
        ...
        funeral_services.record_payment(obligation=obligation, amount=Decimal("500"), method="cash", collector=self.collector, collector_name="Test Collector")
    ...
    def test_outlier_amount_flagged_once_a_baseline_exists(self):
        ...
        for i, amount in enumerate([Decimal("10"), Decimal("11"), Decimal("9"), Decimal("10"), Decimal("12")]):
            ...
            from funerals.models import ContributionObligation
            obligation = ContributionObligation.objects.get(funeral_event=funeral, member=member)
            funeral_services.record_payment(obligation=obligation, amount=amount, method="cash", collector=self.collector, collector_name="Test Collector")
        ...
        funeral_services.record_payment(obligation=outlier_obligation, amount=Decimal("500"), method="cash", collector=self.collector, collector_name="Test Collector")

        self.assertTrue(
            SuspiciousTransactionFlag.objects.filter(reason=SuspiciousTransactionFlag.Reason.AMOUNT_OUTLIER).exists()
        )

    def _create_an_outlier_flag(self):
        for i, amount in enumerate([Decimal("10"), Decimal("11"), Decimal("9"), Decimal("10"), Decimal("12")]):
            member = member_services.register_member(community=self.bodi, full_name=f"Flagtest Member {i}", gender="male", family=self.asona)
            funeral = funeral_services.create_funeral_event(
                community=self.bodi, deceased_name=f"Flagtest Deceased {i}", deceased_gender="male",
                deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
                own_family_amount=amount,
            )
            from funerals.models import ContributionObligation
        ...
        outlier_member = member_services.register_member(community=self.bodi, full_name="Flagtest Outlier", gender="male", family=self.asona)
        outlier_funeral = funeral_services.create_funeral_event(
            community=self.bodi, deceased_name="Flagtest Outlier Deceased", deceased_gender="male",
            deceased_family=self.asona, date_of_death="2026-07-01", collection_start_date="2026-07-01",
        )
        from funerals.models import ContributionObligation
        outlier_obligation = ContributionObligation.objects.get(funeral_event=outlier_funeral, member=outlier_member)
        funeral_services.record_payment(obligation=outlier_obligation, amount=Decimal("500"), method="cash", collector=self.collector, collector_name="Test Collector")
        return SuspiciousTransactionFlag.objects.filter(reason=SuspiciousTransactionFlag.Reason.AMOUNT_OUTLIER).first()

    def test_an_ordinary_member_cannot_review_a_flag(self):
        """'Confirm/dismiss is narrower than view' — was a real backend gap: no role check existed at all before this."""
        from rest_framework.test import APIClient
        member = member_services.register_member(community=self.bodi, full_name="Plain Member", gender="male", family=self.asona)
        member_user = User.objects.create_user(username="plain_member", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        member_services.link_member_to_user(member=member, user=member_user, actor=self.admin)
        flag = self._create_an_outlier_flag()
        self.assertIsNotNone(flag)

        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "plain_member", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.patch(f"/api/suspicious-transactions/{flag.id}/", {"review_status": "dismissed"})
        self.assertEqual(res.status_code, 403)
        flag.refresh_from_db()
        self.assertEqual(flag.review_status, "unreviewed")

    def test_a_treasurer_can_review_a_flag(self):
        from rest_framework.test import APIClient
        User.objects.create_user(username="flag_treasurer", password="a-real-password-123", community=self.bodi, role=Role.TREASURER)
        flag = self._create_an_outlier_flag()
        self.assertIsNotNone(flag)

        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "flag_treasurer", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.patch(f"/api/suspicious-transactions/{flag.id}/", {"review_status": "dismissed"})
...
class MeetingSummaryTests(TestCase):
    ...
    def test_an_ordinary_member_cannot_generate_a_summary(self):
        """This AI-provider-calling endpoint had no role check at all before this — any authenticated user could trigger it."""
        from rest_framework.test import APIClient
        admin = User.objects.create_user(username="meeting_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        member = member_services.register_member(community=self.bodi, full_name="Meeting Member", gender="male", family=family_services.create_family(community=self.bodi, name="Meeting Family", actor=admin))
        member_user = User.objects.create_user(username="meeting_member", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        member_services.link_member_to_user(member=member, user=member_user, actor=admin)

        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "meeting_member", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.post("/api/ai/meeting-summary/", {"transcript": "We discussed the budget."})
        self.assertEqual(res.status_code, 403)

    @override_settings(ANTHROPIC_API_KEY="test-key")
    def test_a_chairman_can_generate_a_summary(self):
        from rest_framework.test import APIClient
        User.objects.create_user(username="meeting_chairman", password="a-real-password-123", community=self.bodi, role=Role.CHAIRMAN)
        mock_summarize = MagicMock(return_value={"summary": "The committee approved the new catering budget.", "decisions": [], "action_items": []})
        with patch("ai_features.llm_provider.MeetingSummaryProvider.summarize", mock_summarize):
            client = APIClient()
            login = client.post("/api/auth/login/", {"username": "meeting_chairman", "password": "a-real-password-123"})
            client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
            res = client.post("/api/ai/meeting-summary/", {"transcript": "We discussed the budget."})
