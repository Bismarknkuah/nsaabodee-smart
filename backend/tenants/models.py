...
class SubscriptionPlan(models.Model):
    """
    A subscription tier as a real, editable record — 'this is how I
    want the subscription plan to be': name, description, yearly and
    monthly price, member limit, trial length, and a feature list, with
    Platform Admin able to add or edit plans rather than being stuck
    with a fixed set. `code` is the stable identifier the rest of the
    system refers to (the same three codes the homepage's own plan
    cards and PlanInterestSubmission already use); `features` is
    descriptive — what the plan is sold as including — not an
    enforcement mechanism, which is a separate concern.
    """
    ...
    # Nullable rather than defaulting to 0.00 — the platform has no
    # published pricing yet (the homepage plan cards are lead capture),
    # and "GHS 0.00/yr" would read as a real, free price, not an unset one.
    ...
    max_members = models.PositiveIntegerField(null=True, blank=True, help_text="Null means unlimited.")
...
class Community(models.Model):
    ...
    # community itself — distinct from its first Community Admin's own
    # account (that admin could change; this describes the actual town
    # and its traditional leadership, which doesn't).
    district = models.CharField(max_length=255, blank=True)
    traditional_authority_name = models.CharField(max_length=255, blank=True, help_text="e.g. the reigning chief's name and title")
    contact_phone = models.CharField(max_length=20, blank=True)
    contact_email = models.EmailField(blank=True)
    address = models.CharField(max_length=255, blank=True)
    estimated_population = models.PositiveIntegerField(null=True, blank=True)
    registration_notes = models.TextField(blank=True, help_text="Why this community is signing up, or anything the Platform Admin should know")
    ...
    # 'This is how I want the subscription plan to be' — a real,
    # editable plan record (name, description, prices, limits, feature
    # list), not a fixed enum. Nullable so a community can exist before
    # a plan is chosen; PROTECT so a plan with communities on it can
    # never be deleted out from under them.
    # 'The admin can rename the user role position' — a community's own
    # titles for roles ("President" for chairman, "Abusuapanin" for
    # family head). Display only: {role: title}. The role value itself
    # never changes.
    role_titles = models.JSONField(default=dict, blank=True)
    subscription_plan = models.ForeignKey(
        "tenants.SubscriptionPlan", null=True, blank=True, on_delete=models.PROTECT, related_name="communities",
    )
    # 'The platform admin should have a way to remind communities to
    # pay their subscription fees.' Tracked here so the Platform Admin
    # can see at a glance whether — and when — a community was last
    # reminded, rather than sending reminders blind with no memory of
    # who's already been nudged.
    last_subscription_reminder_at = models.DateTimeField(null=True, blank=True)
    last_subscription_reminder_by = models.ForeignKey(
    ...
    # elder's official position.' Additive, never breaking: each is
    # nullable and falls back to default_town_leader_amount above when
    # unset, so an existing community that never configures these
    # keeps behaving exactly as before — a single flat rate for every
    # elder. See members.models.Member.TownElderTitle for the four
    # titles these correspond to, and funerals.models.FuneralEvent
    # for the matching per-funeral override fields.
    default_town_elder_chief_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    default_town_elder_queen_mother_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    default_town_elder_linguist_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    default_town_elder_other_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)

    # 'The Asupedeɛ amount must be configurable. Do not hardcode an
    # amount.' Null means this community doesn't use Asupedeɛ at all —
    # activate_asupede() (funerals.services) is the only way it
    # actually turns on for a given funeral, so setting this default
    # alone never silently creates obligations on its own.
    default_asupede_amount = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
...
class HomepageImage(models.Model):
    ...
    'Should be able to upload videos as well.' A real file upload, not
    a link to somewhere else — video is a genuine, separate FileField
    alongside image, mutually exclusive: a given slide is either a
    photo or a video, never both, enforced in clean(). Kept as a raw
    file, played back through a plain HTML5 <video> tag — no
    transcoding, streaming, or adaptive bitrate, which is a
    fundamentally larger undertaking than this hero carousel needs;
    honest about being a file upload, not a video-hosting platform.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    def clean(self):
        ...
        if not self.image and not self.video:
            raise DjangoValidationError("A homepage slide needs either an image or a video.")
        if self.image and self.video:
            raise DjangoValidationError("A homepage slide is either a photo or a video, not both — add a second slide instead.")
...
class Announcement(models.Model):
    ...
    'They should be able to post funeral announcements, and add photos
    and videos.' `funeral_event` is optional — set it to tag this
    announcement as specifically about one funeral (shown alongside
    that funeral's own deceased name/date on the notice board), left
    null for a general community notice. `video` is a genuine file
    upload (the same real, admin-controlled storage HomepageImage
    already uses) — `video_url` remains as an alternative for linking
    to an already-hosted video (YouTube, Vimeo) instead of uploading
    one directly; an announcement can use either, never needs both.
    """
    ...
    # Nullable, SET_NULL — a funeral being deleted (a genuinely rare,
    # platform-admin-only action) should never take down an
    # already-approved public announcement with it; the announcement
    # just quietly stops being tagged to that funeral.
    funeral_event = models.ForeignKey("funerals.FuneralEvent", null=True, blank=True, on_delete=models.SET_NULL, related_name="announcements")
    ...
    video_url = models.URLField(blank=True, help_text="A link to a hosted video (YouTube, Vimeo, or similar) — an alternative to uploading a video file directly.")
...
class CommunityBackupRecord(models.Model):
    """
    'Each community admin will have a section... where they can just
    import a Google Drive link they want to back up their system data
    to, and should also have options to retrieve them back.' No OAuth
    integration with Google's own API — this platform never
    authenticates as the community's Drive account, it only ever reads
    a file the person themselves already made "anyone with the link"
    shareable. A record of every export generated and every restore
    attempted, so a Community Admin can see their own backup history
    rather than backups happening invisibly with no trace.
    """
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    community = models.ForeignKey(Community, on_delete=models.CASCADE, related_name="backup_records")
    kind = models.CharField(max_length=10, choices=Kind.choices)
    # Only ever populated for a restore — the Drive link that was
    # actually read from, kept for the record even though the link
    # itself might later be deleted or made private again.
    ...
    def __str__(self):
        return f"{self.get_kind_display()} — {self.community.name} ({self.member_count} members, {self.family_count} families)"
