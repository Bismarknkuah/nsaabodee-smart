...
from .models import SubscriptionPlan, Announcement, AnnouncementReviewLog, Community, CommunityBackupRecord, CommunityPayoutAccount, FeatureFlag, HomepageImage, PlanInterestSubmission, PlatformBillingRecord
    ...
    *, community_name: str, admin_username: str, admin_password: str, admin_email: str = "",
    ...
    try:
        community = Community.objects.create(
            ...
            # Every new community starts on the ordinary single-community
            # tier — the same default the enum this replaced used to
            # apply — and Platform Admin promotes or demotes from there.
            subscription_plan=SubscriptionPlan.objects.filter(code="community", is_active=True).first(),
        )
...
def set_community_subscription_plan(*, community: Community, plan: str, actor) -> Community:
    """
    'The platform admin should be able to promote or demote a
    community to any of the subscription plans.' Platform Admin only.
    `plan` is the plan's code (or its id). Recorded in the audit log so
    a tier change is never a silent, unexplained one.
    """
    if not (actor.is_superuser or actor.role == "platform_admin"):
        raise ValidationError("Only a Platform Admin can change a community's subscription plan.")
    target = SubscriptionPlan.objects.filter(code=plan, is_active=True).first()
    if target is None:
        try:
            target = SubscriptionPlan.objects.filter(id=plan, is_active=True).first()
        except (ValueError, ValidationError):
            target = None
    if target is None:
        raise ValidationError(f"'{plan}' is not a recognised, active subscription plan.")
    previous = community.subscription_plan.code if community.subscription_plan_id else None
    if previous == target.code:
        return community
    community.subscription_plan = target
    community.save(update_fields=["subscription_plan"])
    from audit_log.services import record_event
    record_event(
        category="community", action="subscription_plan_changed", actor=actor, community=community,
        target_type="Community", target_id=community.id, target_label=community.name,
        description=f"Subscription plan changed from {previous or 'none'} to {target.code}.",
    )
    return community


_PLAN_EDITABLE_FIELDS = {"name", "description", "price_yearly", "price_monthly", "max_members", "max_communities", "trial_days", "features", "is_active", "sort_order"}


def _require_platform_admin(actor, what: str):
    if not (actor.is_superuser or actor.role == "platform_admin"):
        raise ValidationError(f"Only a Platform Admin can {what}.")


def create_subscription_plan(*, actor, code: str, name: str, **fields) -> SubscriptionPlan:
    """'+ New plan' — a genuinely new tier, not one of a fixed three."""
    _require_platform_admin(actor, "create a subscription plan")
    code = (code or "").strip().lower().replace(" ", "_")
    if not code:
        raise ValidationError("A plan needs a short code (e.g. 'community').")
    if SubscriptionPlan.objects.filter(code=code).exists():
        raise ValidationError(f"A plan with the code '{code}' already exists.")
    unknown = set(fields) - _PLAN_EDITABLE_FIELDS
    if unknown:
        raise ValidationError(f"Unknown plan field(s): {', '.join(sorted(unknown))}.")
    if not isinstance(fields.get("features", []), list):
        raise ValidationError("features must be a list of short descriptions.")
    plan = SubscriptionPlan.objects.create(code=code, name=name.strip(), **fields)
    from audit_log.services import record_event
    record_event(category="billing", action="subscription_plan_created", actor=actor, target_type="SubscriptionPlan", target_id=plan.id, target_label=plan.name, description=f"Plan '{plan.name}' ({plan.code}) created.")
    return plan


def update_subscription_plan(*, plan: SubscriptionPlan, actor, **fields) -> SubscriptionPlan:
    """'Edit' on a plan card. The code itself never changes — it's what every community on the plan refers to."""
    _require_platform_admin(actor, "edit a subscription plan")
    unknown = set(fields) - _PLAN_EDITABLE_FIELDS
    if unknown:
        raise ValidationError(f"Unknown plan field(s): {', '.join(sorted(unknown))}.")
    if "features" in fields and not isinstance(fields["features"], list):
        raise ValidationError("features must be a list of short descriptions.")
    for key, value in fields.items():
        setattr(plan, key, value.strip() if isinstance(value, str) else value)
    plan.save()
    from audit_log.services import record_event
    record_event(category="billing", action="subscription_plan_updated", actor=actor, target_type="SubscriptionPlan", target_id=plan.id, target_label=plan.name, description=f"Plan '{plan.name}' updated: {', '.join(sorted(fields))}.")
    return plan
