...
class AnnouncementDecisionHttpStatusTests(TestCase):
    """
    A real bug found during a full-system audit: ResubmitAnnouncementView,
    ApproveAnnouncementView, and RejectAnnouncementView all delegated
    their authorization check to the serializer's own exception
    handling, which DRF always converts into a generic 400 — the
    same "wrong role, not bad input" distinction already established
    elsewhere on this platform (e.g. TownElderPerTitleRatesView,
    DecideWelfareRequestView) was missing here.
    """

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="adhs-bodi")
        self.other = Community.objects.create(name="Other Town", slug="adhs-other")
        self.bodi_admin = User.objects.create_user(username="adhs_bodi_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.other_admin = User.objects.create_user(username="adhs_other_admin", password="a-real-password-123", community=self.other, role=Role.COMMUNITY_ADMIN)
        self.plain_member = User.objects.create_user(username="adhs_plain", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        self.platform_admin = User.objects.create_user(username="adhs_platform_admin", password="a-real-password-123", role=Role.PLATFORM_ADMIN)

        self.rejected_announcement = Announcement.objects.create(
            community=self.bodi, title="T", content="C", status=Announcement.Status.REJECTED, submitted_by=self.bodi_admin,
        )
        self.pending_announcement = Announcement.objects.create(
            community=self.bodi, title="T2", content="C2", status=Announcement.Status.PENDING, submitted_by=self.bodi_admin,
        )
    ...
    def test_a_different_communitys_admin_gets_403_resubmitting(self):
        res = self._login("adhs_other_admin").post(f"/api/tenants/announcements/{self.rejected_announcement.id}/resubmit/", {"title": "Hacked"}, format="json")
        self.assertEqual(res.status_code, 403)

    def test_the_owning_communitys_admin_can_actually_resubmit(self):
        res = self._login("adhs_bodi_admin").post(f"/api/tenants/announcements/{self.rejected_announcement.id}/resubmit/", {"title": "Fixed"}, format="json")
        self.assertEqual(res.status_code, 200, res.data)

    def test_a_plain_community_member_gets_403_approving(self):
        res = self._login("adhs_plain").post(f"/api/tenants/announcements/{self.pending_announcement.id}/approve/", {}, format="json")
        self.assertEqual(res.status_code, 403)

    def test_a_community_admin_gets_403_approving_not_just_any_authenticated_user(self):
        res = self._login("adhs_bodi_admin").post(f"/api/tenants/announcements/{self.pending_announcement.id}/approve/", {}, format="json")
        self.assertEqual(res.status_code, 403)

    def test_the_platform_admin_can_actually_approve(self):
        res = self._login("adhs_platform_admin").post(f"/api/tenants/announcements/{self.pending_announcement.id}/approve/", {}, format="json")
        self.assertEqual(res.status_code, 200, res.data)

    def test_a_plain_member_gets_403_rejecting(self):
        res = self._login("adhs_plain").post(f"/api/tenants/announcements/{self.pending_announcement.id}/reject/", {"reason": "test"}, format="json")
        self.assertEqual(res.status_code, 403)

    def test_a_genuine_state_error_still_returns_400_not_403(self):
        """The fix must not overcorrect — an already-decided announcement is a real 400, not an authorization question."""
        already_approved = Announcement.objects.create(
            community=self.bodi, title="T3", content="C3", status=Announcement.Status.APPROVED, submitted_by=self.bodi_admin,
        )
        res = self._login("adhs_platform_admin").post(f"/api/tenants/announcements/{already_approved.id}/approve/", {}, format="json")
...
class FuneralAnnouncementAndVideoTests(TestCase):
    """'They should be able to post funeral announcements, and add photos and videos.'"""

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="faav-bodi")
        self.other_community = Community.objects.create(name="Other Town", slug="faav-other")
        self.bodi_admin = User.objects.create_user(username="faav_bodi_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        ...
        self.funeral = funeral_services.create_funeral_event(
            ...
            actor=self.bodi_admin, own_family_amount=Decimal("50"),
        )

    def test_an_announcement_can_be_tagged_to_a_specific_funeral(self):
        announcement = services.submit_announcement(
            community=self.bodi, title="Funeral of Someone Asona", content="Details of the funeral",
            actor=self.bodi_admin, funeral_event=self.funeral,
        )
        self.assertEqual(announcement.funeral_event, self.funeral)

    def test_an_announcement_can_include_an_uploaded_video(self):
        announcement = services.submit_announcement(
            community=self.bodi, title="Funeral Video", content="Watch the tribute",
            actor=self.bodi_admin, video=_fake_video(),
        )
        self.assertTrue(announcement.video.name)

    def test_an_announcement_still_works_with_neither_video_nor_funeral(self):
        """Both fields are optional — every existing announcement flow is completely unaffected."""
        announcement = services.submit_announcement(community=self.bodi, title="General Notice", content="Meeting on Sunday", actor=self.bodi_admin)
        self.assertIsNone(announcement.funeral_event)
        self.assertFalse(announcement.video)

    def test_tagging_a_different_communitys_funeral_is_rejected(self):
        from decimal import Decimal
        from families import services as family_services
        from funerals import services as funeral_services

        other_admin = User.objects.create_user(username="faav_other_admin", password="x", community=self.other_community, role=Role.COMMUNITY_ADMIN)
        other_family = family_services.create_family(community=self.other_community, name="Bretuo", actor=other_admin)
        other_funeral = funeral_services.create_funeral_event(
            community=self.other_community, deceased_name="Someone Else", deceased_gender="male",
            deceased_family=other_family, date_of_death="2026-07-01", collection_start_date="2026-07-01",
            actor=other_admin, own_family_amount=Decimal("50"),
        )
        with self.assertRaises(ValidationError):
            services.submit_announcement(community=self.bodi, title="Cross-community", content="test", actor=self.bodi_admin, funeral_event=other_funeral)

    def test_full_http_submission_with_funeral_tag_and_video(self):
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "faav_bodi_admin", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.post(
            f"/api/tenants/communities/{self.bodi.id}/announcements/submit/",
            {"title": "Funeral Announcement", "content": "Details", "funeral_event_id": str(self.funeral.id), "video": _fake_video()},
            format="multipart",
        )
        self.assertEqual(res.status_code, 201, res.data)
        self.assertEqual(res.data["funeral_deceased_name"], "Someone Asona")
        self.assertIsNotNone(res.data["video_file_url"])

    def test_http_submission_with_an_unknown_funeral_id_is_rejected_cleanly(self):
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "faav_bodi_admin", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        import uuid
        res = client.post(
            f"/api/tenants/communities/{self.bodi.id}/announcements/submit/",
            {"title": "Bad Ref", "content": "Details", "funeral_event_id": str(uuid.uuid4())},
            format="multipart",
        )
        self.assertEqual(res.status_code, 400)
