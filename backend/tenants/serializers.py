...
class CommunitySerializer(serializers.ModelSerializer):
    # The plan's stable code, as the frontend already consumes it — the
    # FK conversion underneath is invisible to every existing caller.
    subscription_plan = serializers.SerializerMethodField()
    subscription_plan_name = serializers.SerializerMethodField()

    def get_subscription_plan(self, obj):
        return obj.subscription_plan.code if obj.subscription_plan_id else None

    def get_subscription_plan_name(self, obj):
        return obj.subscription_plan.name if obj.subscription_plan_id else None

    is_access_expired = serializers.BooleanField(read_only=True)
    access_days_remaining = serializers.IntegerField(read_only=True)
    last_subscription_reminder_by_username = serializers.CharField(source="last_subscription_reminder_by.username", read_only=True, default=None)

    class Meta:
        model = Community
        fields = [
            "id", "name", "slug", "region", "is_active", "default_general_male_amount", "default_general_female_amount",
            "created_at", "access_plan", "subscription_plan", "subscription_plan_name", "access_expires_at", "is_access_expired", "access_days_remaining",
            "logo", "primary_color", "secondary_color", "tagline", "required_funeral_approvals",
            "last_subscription_reminder_at", "last_subscription_reminder_by_username",
            "district", "traditional_authority_name", "contact_phone", "contact_email",
            "address", "estimated_population", "registration_notes",
        ]
        read_only_fields = ["id", "slug", "is_active", "created_at", "is_access_expired", "access_days_remaining", "last_subscription_reminder_at"]
...
class UpdateCommunitySerializer(serializers.Serializer):
    ...
    district = serializers.CharField(max_length=255, required=False, allow_blank=True)
    traditional_authority_name = serializers.CharField(max_length=255, required=False, allow_blank=True)
    contact_phone = serializers.CharField(max_length=20, required=False, allow_blank=True)
    contact_email = serializers.EmailField(required=False, allow_blank=True)
    address = serializers.CharField(max_length=255, required=False, allow_blank=True)
    estimated_population = serializers.IntegerField(required=False, allow_null=True, min_value=1)
    registration_notes = serializers.CharField(required=False, allow_blank=True)
...
class OnboardCommunitySerializer(serializers.Serializer):
    ...
    # information' — genuine identity/contact details about the
    # community itself, all still optional at the API layer (a
    # Platform Admin who genuinely has none of this yet shouldn't be
    # blocked from onboarding someone) — the frontend form is what
    # actually asks for these up front.
    district = serializers.CharField(max_length=255, required=False, allow_blank=True, default="")
    traditional_authority_name = serializers.CharField(max_length=255, required=False, allow_blank=True, default="")
    ...
    estimated_population = serializers.IntegerField(required=False, allow_null=True, default=None, min_value=1)
    registration_notes = serializers.CharField(required=False, allow_blank=True, default="")
...
class HomepageImageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = HomepageImage
        fields = ["id", "image_url", "video_url", "caption", "subcaption", "display_order", "is_active", "created_at"]
        read_only_fields = ["id", "image_url", "video_url", "created_at"]
...
class UploadHomepageImageSerializer(serializers.Serializer):
    """'The homepage live pictures... should be uploaded by the super admin... should be able to upload videos as well.'"""
    ...
    def save(self, **kwargs):
        ...
        try:
            ...
            raise serializers.ValidationError(exc.message_dict if hasattr(exc, "message_dict") else (exc.messages if hasattr(exc, "messages") else str(exc)))
...
class AnnouncementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Announcement
        fields = [
            "id", "community", "community_name", "funeral_event", "funeral_deceased_name",
            "title", "content", "image_url", "video_file_url", "video_url", "status",
...
class SubmitAnnouncementSerializer(serializers.Serializer):
    """'Has to be submitted by the community admin... they should be able to post funeral announcements, and add photos and videos.'"""
    title = serializers.CharField(max_length=255)
    content = serializers.CharField()
    image = serializers.ImageField(required=False, allow_null=True)
    video = serializers.FileField(required=False, allow_null=True)
    video_url = serializers.URLField(required=False, allow_blank=True, default="")
    funeral_event_id = serializers.UUIDField(required=False, allow_null=True, default=None)
    ...
    def save(self, **kwargs):
        ...
        funeral_event_id = data.pop("funeral_event_id", None)
        funeral_event = None
        if funeral_event_id:
            try:
                funeral_event = FuneralEvent.objects.get(id=funeral_event_id, community=community)
            except FuneralEvent.DoesNotExist:
                raise serializers.ValidationError({"funeral_event_id": "That funeral wasn't found in this community."})
        try:
            return services.submit_announcement(community=community, actor=actor, funeral_event=funeral_event, **data)
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))
...
class CommunityBackupRecordSerializer(serializers.ModelSerializer):
    performed_by_username = serializers.CharField(source="performed_by.username", read_only=True, default=None)

    class Meta:
        model = CommunityBackupRecord
        fields = ["id", "kind", "drive_link", "member_count", "family_count", "performed_by_username", "created_at"]


class RestoreCommunityBackupSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        ...
        actor = self.context["request"].user
        try:
            return services.restore_community_backup_from_drive_link(community=community, actor=actor, **self.validated_data)
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class RestoreCommunityBackupFromFileSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        ...
        actor = self.context["request"].user
        try:
            return services.restore_community_backup_from_uploaded_file(community=community, actor=actor, uploaded_file=self.validated_data["file"])
        except DjangoValidationError as exc:
            raise serializers.ValidationError(exc.messages if hasattr(exc, "messages") else str(exc))


class SubscriptionPlanSerializer(serializers.ModelSerializer):
    """One plan card. community_count is how many active communities are currently on it."""
    ...
    class Meta:
        ...
        fields = [
            "id", "code", "name", "description", "price_yearly", "price_monthly", "max_members", "max_communities",
            "trial_days", "features", "is_active", "sort_order", "community_count", "created_at", "updated_at",
        ]
        read_only_fields = ["id", "code", "community_count", "created_at", "updated_at"]
