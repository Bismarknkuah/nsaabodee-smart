...
class SubscriptionPlanListCreateView(_PlatformAdminOnly):
    """GET -> every plan (active and retired); POST -> '+ New plan'."""
    ...
    def post(self, request):
        ...
        if denied:
            return denied
        from .serializers import SubscriptionPlanSerializer
        data = {k: v for k, v in request.data.items() if k not in ("code", "name")}
        try:
            plan = services.create_subscription_plan(actor=request.user, code=request.data.get("code", ""), name=request.data.get("name", ""), **data)
...
class SubscriptionPlanDetailView(_PlatformAdminOnly):
    """PATCH -> 'Edit' on a plan card."""
    ...
    def patch(self, request, plan_id):
        ...
        try:
            plan = services.update_subscription_plan(plan=plan, actor=request.user, **{k: v for k, v in request.data.items()})
        except DjangoValidationError as exc:
            return Response({"detail": exc.messages if hasattr(exc, "messages") else str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response(SubscriptionPlanSerializer(plan).data)


class CommunitySubscriptionPlanView(_PlatformAdminOnly):
    """
    POST {plan} -> 'the platform admin should be able to promote or
    demote a community to any of the subscription plans.' Same
    manage_communities capability as suspend/reactivate above.
    """
    ...
    def post(self, request, community_id):
        ...
        community = get_object_or_404(Community, id=community_id)
        try:
            updated = services.set_community_subscription_plan(community=community, plan=request.data.get("plan", ""), actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": exc.messages if hasattr(exc, "messages") else str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response(CommunitySerializer(updated).data)
