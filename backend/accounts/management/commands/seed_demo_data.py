...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Every demo community sits on a real plan, and has a Town Elders
        # rate — without one, the two Town-Elders officer roles' ledger
        # would have nothing on it.
        from tenants.models import SubscriptionPlan
        changed = []
        if community.subscription_plan_id is None:
            community.subscription_plan = SubscriptionPlan.objects.filter(code="community").first()
            changed.append("subscription_plan")
        if not community.default_town_leader_amount:
            community.default_town_leader_amount = Decimal("20")
            changed.append("default_town_leader_amount")
        if changed:
            community.save(update_fields=changed)
        ...
        bretuo = self._get_or_create_family(family_services, community, "Bretuo", admin_user)
        # Bretuo needs an approved standing rate too — the closed
        # funeral seeded below is one of theirs, and a funeral can't
        # open for a family with no rate.
        if bretuo.standing_family_rate is None:
            family_services.recommend_family_rate(family=bretuo, amount=Decimal("40"), actor=admin_user)
            family_services.approve_family_rate(family=bretuo, actor=admin_user)
        ...
        # Two genuine Town Elders, so the Town Elders ledger — and the
        # Chief's, Town Registration Officer's and Town Elders Arrears
        # Officer's dashboards built on it — show real rows.
        from members.models import Member
        elders = []
        for full_name in ("Nana Demo Elder I", "Nana Demo Elder II"):
            elder = Member.objects.filter(community=community, full_name=full_name).first()
            if elder is None:
                elder = member_services.register_member(community=community, full_name=full_name, gender="male", family=bretuo, is_town_leader=True)
            elders.append(elder)

        # An older, already-CLOSED funeral with real unpaid balances —
        # what the Arrears Collector, Family Arrears Officer and Town
        # Elders Arrears Officer actually work on. Only the one member
        # who later pays the active funeral settles this one first
        # (the platform's older-debt-first rule would otherwise block
        # that later payment); everyone else is left owing on purpose.
        from funerals import services as funeral_services
        from funerals.models import ContributionObligation, FuneralEvent
        closed = FuneralEvent.objects.filter(community=community, deceased_name="Demo Deceased (Closed)").first()
        if closed is None:
            closed = funeral_services.create_funeral_event(
                community=community, deceased_name="Demo Deceased (Closed)", deceased_gender="female",
                deceased_family=bretuo, date_of_death=date.today() - timedelta(days=40),
                collection_start_date=date.today() - timedelta(days=38),
            )
            settled = ContributionObligation.objects.filter(funeral_event=closed, member=role_members[Role.COMMUNITY_MEMBER]).first()
            if settled and settled.amount_paid == 0:
                funeral_services.record_payment(obligation=settled, amount=settled.expected_amount, method="cash", collector=role_users[Role.COLLECTOR], collector_name="Test Collector")
            funeral_services.close_funeral_event(funeral=closed, actor=admin_user)
        ...
        # The demo Deceased Rep genuinely assigned to Asona (the family
        # whose funeral is in progress) — a bereaved_rep with no
        # assignment has an empty dashboard.
        from families.models import BereavedRepAssignment
        if not BereavedRepAssignment.objects.filter(member=role_members[Role.BEREAVED_REP], is_active=True).exists():
            family_services.create_bereaved_rep(family=asona, actor=admin_user, member=role_members[Role.BEREAVED_REP])
