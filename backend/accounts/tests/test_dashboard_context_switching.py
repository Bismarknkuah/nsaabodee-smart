...
class ExecutiveActionsBlockedInPersonalContextTests(TestCase):
    ...
    def test_a_desk_assigned_ordinary_member_can_still_record_payments_regardless_of_context(self):
        ...
        res = client.post(f"/api/funerals/{funeral.id}/obligations/{obligation.id}/record-payment/", {"amount": "50", "method": "cash", "collector_name": "Test Collector"})
    ...
    def test_a_family_head_switching_to_personal_does_not_see_the_family_fund_either(self):
        """'Personal account is the same community member dashboard, and they are not allowed to see funeral fund.' family_fund_overview is a genuinely different section from financial_overview above — this was a real, separate gap, not the same one caught twice."""
        head_member = member_services.register_member(community=self.bodi, full_name="Enforce Family Head", gender="male", family=self.asona)
        head = User.objects.create_user(username="enforce_family_head", password="a-real-password-123", community=self.bodi, role=Role.FAMILY_HEAD)
        member_services.link_member_to_user(member=head_member, user=head, actor=self.admin)
        self.asona.family_head = head_member
        self.asona.save(update_fields=["family_head"])

        client = self._login("enforce_family_head")
        client.post("/api/auth/switch-context/", {"context": "personal"})
        dashboard_res = client.get("/api/dashboard/")
        self.assertEqual(dashboard_res.status_code, 200)
        self.assertIn("member_overview", dashboard_res.data["sections"])
        self.assertNotIn("family_fund_overview", dashboard_res.data["sections"])
        self.assertNotIn("family_overview", dashboard_res.data["sections"])
...
class ExecutiveDashboardNeverShowsDonationsReceivedTests(TestCase):
    ...
    def test_bereaved_rep_can_switch_dashboard_context(self):
        """'The Deceased Rep will also have to execute an executive role type.' Deliberately checked separately from EXECUTIVE_ROLES — see the comment on can_switch_dashboard_context."""
        rep_member = member_services.register_member(community=self.bodi, full_name="Rep Person", gender="male", family=self.asona)
        rep_user = User.objects.create_user(username="context_bereaved_rep", password="x", community=self.bodi, role=Role.BEREAVED_REP)
        member_services.link_member_to_user(member=rep_member, user=rep_user, actor=self.admin)

        self.assertTrue(rep_user.can_switch_dashboard_context())
        from accounts import services as account_services
        updated = account_services.switch_dashboard_context(user=rep_user, context="personal")
        self.assertEqual(updated.active_context, "personal")

    def test_bereaved_rep_remains_eligible_to_receive_donations_despite_switching_context(self):
        """The whole reason this had to be checked separately from EXECUTIVE_ROLES — a Deceased Rep's core purpose is representing their bereaved family, which includes receiving condolence gifts on their behalf."""
        from accounts.models import EXECUTIVE_ROLES
        self.assertNotIn(Role.BEREAVED_REP, EXECUTIVE_ROLES)
