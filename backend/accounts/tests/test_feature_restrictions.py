...
class CommunityAdminRestrictionTests(TestCase):
    ...
    def test_community_admin_can_now_restrict_themselves_but_never_out_of_settings(self):
        """'Add community admin to the user settings so that he can also restrict himself' — allowed, except the two features that would lock them out."""
        from django.core.exceptions import ValidationError
        self.assertTrue(can_restrict_features_for(self.admin, self.admin))
        with self.assertRaises(ValidationError):
            services.set_disabled_features(target=self.admin, features=["/system-settings"], actor=self.admin)

    def test_community_admin_can_now_restrict_a_plain_community_member(self):
        """'All the community members and personal dashboard should all have the same features and should be managed by the community admin.' Widened from the earlier executives-only scope."""
        ...
        self.assertTrue(can_restrict_features_for(self.admin, member_user))

    def test_community_admin_still_cannot_restrict_a_different_communitys_plain_member(self):
        """The widened scope stays bounded by community — never crosses tenant lines."""
        other_community = Community.objects.create(name="Other Town", slug="carc-other")
        other_member_user = User.objects.create_user(username="car_other_plain_member", password="x", community=other_community, role=Role.COMMUNITY_MEMBER)
        self.assertFalse(can_restrict_features_for(self.admin, other_member_user))
    ...
    def test_list_manageable_users_includes_executives_and_the_admin_themselves_but_never_a_different_community(self):
        ...
        self.assertIn("car_admin", usernames)  # themselves — so they can restrict their own dashboard
        self.assertNotIn("car_other_treasurer", usernames)


class ChairmanCommunityWideManagementTests(TestCase):
    """
    'The community chair is to have upper control over the community
    ledger and the system.' The same community-wide management
    authority Community Admin already has, given to Chairman too —
    every executive and ordinary member except another Community
    Admin, matching Community Admin's own boundary exactly.
    """

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="ccm-bodi")
        self.other_community = Community.objects.create(name="Other Town", slug="ccm-other-town")
        self.chairman = User.objects.create_user(username="ccm_chairman", password="a-real-password-123", community=self.bodi, role=Role.CHAIRMAN)
        self.treasurer = User.objects.create_user(username="ccm_treasurer", password="x", community=self.bodi, role=Role.TREASURER)
        self.secretary = User.objects.create_user(username="ccm_secretary", password="x", community=self.bodi, role=Role.SECRETARY)
        self.community_admin = User.objects.create_user(username="ccm_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.other_community_treasurer = User.objects.create_user(username="ccm_other_treasurer", password="x", community=self.other_community, role=Role.TREASURER)

    def test_chairman_can_restrict_a_treasurer_in_their_own_community(self):
        self.assertTrue(can_restrict_features_for(self.chairman, self.treasurer))

    def test_chairman_can_restrict_the_secretary(self):
        self.assertTrue(can_restrict_features_for(self.chairman, self.secretary))

    def test_chairman_cannot_restrict_the_community_admin(self):
        """The one boundary that never moves — Community Admin remains the ultimate authority, the same as it already is for itself."""
        self.assertFalse(can_restrict_features_for(self.chairman, self.community_admin))

    def test_chairman_cannot_restrict_an_executive_in_a_different_community(self):
        self.assertFalse(can_restrict_features_for(self.chairman, self.other_community_treasurer))

    def test_chairman_cannot_restrict_themselves(self):
        self.assertFalse(can_restrict_features_for(self.chairman, self.chairman))

    def test_list_manageable_users_for_chairman_includes_every_executive_and_member(self):
        candidates = services.list_manageable_users(actor=self.chairman)
        usernames = {u.username for u in candidates}
        self.assertIn("ccm_treasurer", usernames)
        self.assertIn("ccm_secretary", usernames)
        self.assertNotIn("ccm_admin", usernames)
...
class BulkApplyDisabledFeaturesTests(TestCase):
    """'The community admin can should have options where he can apply changes for some family and can also set for all members.'"""

    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="baf-bodi")
        self.admin = User.objects.create_user(username="baf_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.bretuo = family_services.create_family(community=self.bodi, name="Bretuo", actor=self.admin)

        self.asona_member_1 = User.objects.create_user(username="baf_asona_1", password="x", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        Member.objects.create(community=self.bodi, family=self.asona, full_name="Asona One", gender="male", linked_user=self.asona_member_1)
        self.asona_member_2 = User.objects.create_user(username="baf_asona_2", password="x", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        Member.objects.create(community=self.bodi, family=self.asona, full_name="Asona Two", gender="female", linked_user=self.asona_member_2)

        self.bretuo_member = User.objects.create_user(username="baf_bretuo_1", password="x", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        Member.objects.create(community=self.bodi, family=self.bretuo, full_name="Bretuo One", gender="male", linked_user=self.bretuo_member)

    def test_setting_for_a_whole_family_only_affects_that_familys_members(self):
        updated_count = services.set_disabled_features_for_family(family=self.asona, features=["/reports"], actor=self.admin)
        self.assertEqual(updated_count, 2)
        self.asona_member_1.refresh_from_db()
        self.asona_member_2.refresh_from_db()
        self.bretuo_member.refresh_from_db()
        self.assertEqual(self.asona_member_1.disabled_features, ["/reports"])
        self.assertEqual(self.asona_member_2.disabled_features, ["/reports"])
        self.assertEqual(self.bretuo_member.disabled_features, [])

    def test_setting_for_all_members_affects_every_family(self):
        updated_count = services.set_disabled_features_for_all_members(community=self.bodi, features=["/welfare-contributions"], actor=self.admin)
        self.assertEqual(updated_count, 3)
        for user in (self.asona_member_1, self.asona_member_2, self.bretuo_member):
            user.refresh_from_db()
            self.assertEqual(user.disabled_features, ["/welfare-contributions"])

    def test_setting_for_all_members_never_touches_executives(self):
        secretary = User.objects.create_user(username="baf_secretary", password="x", community=self.bodi, role=Role.SECRETARY)
        services.set_disabled_features_for_all_members(community=self.bodi, features=["/reports"], actor=self.admin)
        secretary.refresh_from_db()
        self.assertEqual(secretary.disabled_features, [])

    def test_a_family_head_can_bulk_apply_to_their_own_family_only(self):
        family_head_user = User.objects.create_user(username="baf_family_head", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        Member.objects.create(community=self.bodi, family=self.asona, full_name="The Head", gender="male", linked_user=family_head_user)
        updated_count = services.set_disabled_features_for_family(family=self.asona, features=["/family-fund"], actor=family_head_user)
        # 2 pre-existing Asona members + the family head themselves is excluded from their own restriction
        self.assertEqual(updated_count, 2)

    def test_a_family_head_cannot_bulk_apply_to_a_different_family(self):
        family_head_user = User.objects.create_user(username="baf_family_head_2", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        Member.objects.create(community=self.bodi, family=self.asona, full_name="The Other Head", gender="male", linked_user=family_head_user)
        updated_count = services.set_disabled_features_for_family(family=self.bretuo, features=["/family-fund"], actor=family_head_user)
        self.assertEqual(updated_count, 0)
        self.bretuo_member.refresh_from_db()
        self.assertEqual(self.bretuo_member.disabled_features, [])

    def test_a_plain_member_cannot_bulk_apply_to_all_members(self):
        with self.assertRaises(ValidationError):
            services.set_disabled_features_for_all_members(community=self.bodi, features=["/reports"], actor=self.asona_member_1)

    def test_a_family_head_cannot_bulk_apply_to_all_members(self):
        family_head_user = User.objects.create_user(username="baf_family_head_3", password="x", community=self.bodi, role=Role.FAMILY_HEAD)
        with self.assertRaises(ValidationError):
            services.set_disabled_features_for_all_members(community=self.bodi, features=["/reports"], actor=family_head_user)


class BulkApplyHttpTests(TestCase):
    def setUp(self):
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="bah-bodi")
        self.admin = User.objects.create_user(username="bah_admin", password="a-real-password-123", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.asona = family_services.create_family(community=self.bodi, name="Asona", actor=self.admin)
        self.member_user = User.objects.create_user(username="bah_member", password="x", community=self.bodi, role=Role.COMMUNITY_MEMBER)
        Member.objects.create(community=self.bodi, family=self.asona, full_name="Member", gender="male", linked_user=self.member_user)

    def _login(self):
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "bah_admin", "password": "a-real-password-123"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        return client

    def test_bulk_apply_for_family_over_http(self):
        res = self._login().patch(
            "/api/accounts/manageable-users/disabled-features/family/",
            {"family_id": str(self.asona.id), "disabled_features": ["/reports"]},
            format="json",
        )
        self.assertEqual(res.status_code, 200, res.data)
        self.assertEqual(res.data["updated_count"], 1)

    def test_bulk_apply_for_all_members_over_http(self):
        res = self._login().patch(
            "/api/accounts/manageable-users/disabled-features/all-members/",
            {"disabled_features": ["/welfare-contributions"]},
            format="json",
        )
        self.assertEqual(res.status_code, 200, res.data)
        self.assertEqual(res.data["updated_count"], 1)

    def test_bulk_apply_for_all_members_by_a_non_admin_gets_403(self):
        client = APIClient()
        login = client.post("/api/auth/login/", {"username": "bah_member", "password": "x"})
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
        res = client.patch(
            "/api/accounts/manageable-users/disabled-features/all-members/",
