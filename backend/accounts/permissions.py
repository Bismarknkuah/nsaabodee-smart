...
_COMMUNITY_EXECUTIVE_ROLES = {
    ...
    "collector", "arrears_collector", "traditional_leader", "notification_officer",
    "family_head", "family_secretary", "family_treasurer", "family_arrears_officer",
    "family_registration_officer", "town_registration_officer", "community_registration_desk",
    "town_elders_arrears_officer",
}
_FAMILY_EXECUTIVE_ROLES = {"family_secretary", "family_treasurer", "family_registration_officer", "family_arrears_officer"}
...
def can_restrict_features_for(actor, target) -> bool:
    ...
    'All the community members and personal dashboard should all have
    the same features and should be managed by the community admin.'
    Community Admin's scope covers every executive role WITHIN their
    own community, and now also every plain Community Member — the
    two tiers below stay otherwise unchanged.

    Two distinct tiers of authority, deliberately never overlapping:
    - Community Admin can restrict any executive role, or any plain
      Community Member, WITHIN their own community (Family Head
      included, since a family officer is still part of the community
      the Admin runs) — never another Community Admin, never
      themselves.
    ...
    if actor.id == target.id:
        ...
        # restrict himself.' Only the Community Admin, and never the two
        # features that would lock them out (enforced in set_disabled_features).
        return actor.role == "community_admin"
    if target.is_superuser:
        return False

    # 'The community chair is to have upper control over the community
    # ledger and the system.' Chairman given the same community-wide
    # management authority as Community Admin here — the target set
    # (_COMMUNITY_EXECUTIVE_ROLES) already excludes community_admin
    # itself, so this never lets a Chairman restrict or manage a
    # Community Admin's own account, only every other executive and
    # ordinary member, the same boundary Community Admin itself
    # already respects.
    if actor.role in ("community_admin", "chairman"):
        if target.community_id != actor.community_id:
            return False
        return target.role in _COMMUNITY_EXECUTIVE_ROLES or target.role == "community_member"

    if actor.role in ("family_head", "family_secretary"):
