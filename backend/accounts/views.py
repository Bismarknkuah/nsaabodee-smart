...
class ManageableUsersView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            "feature_groups": services.feature_groups_for(services.restrictable_features_for(request.user)),
            "role_titles": dict(getattr(request.user.community, "role_titles", None) or {}) if request.user.community_id else {},
            "self_restriction_protected": sorted(services.SELF_RESTRICTION_PROTECTED),
            "own_family_id": str(own_family.id) if own_family else None,
            "own_family_name": own_family.name if own_family else None,
            "users": [
                {
                    "id": str(u.id), "username": u.username, "role": u.role, "role_label": services.role_label_for(u),
                    "is_self": u.id == request.user.id, "email": u.email, "phone_number": u.phone_number,
                    "full_name": u.member_profile.full_name if getattr(u, "member_profile", None) else None,
                    "community_name": u.community.name if u.community else None,
                    "disabled_features": u.disabled_features, "is_active": u.is_active,
                    "member_id": str(u.member_profile.id) if getattr(u, "member_profile", None) else None,
                }
                for u in users
            ],
        })
...
class SetDisabledFeaturesForFamilyView(APIView):
    """PATCH {family_id, disabled_features} -> 'the community admin... can apply changes for some family.' Applies the same restriction to every member of one family at once."""
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request):
        ...
        disabled_features = request.data.get("disabled_features")
        if not family_id or not isinstance(disabled_features, list):
            return Response({"detail": "'family_id' and a 'disabled_features' list are required."}, status=status.HTTP_400_BAD_REQUEST)
        family = get_object_or_404(Family, id=family_id)
        try:
            updated_count = services.set_disabled_features_for_family(family=family, features=disabled_features, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response({"updated_count": updated_count})


class SetDisabledFeaturesForAllMembersView(APIView):
    """PATCH {disabled_features} -> 'can also set for all members.' Applies the same restriction to every plain Community Member in the acting Community Admin's own community at once."""
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request):
        ...
        if not isinstance(disabled_features, list):
            return Response({"detail": "A 'disabled_features' list is required."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            updated_count = services.set_disabled_features_for_all_members(community=request.user.community, features=disabled_features, actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response({"updated_count": updated_count})
...
class RoleTitlesView(APIView):
    """GET -> this community's custom role titles; PATCH {titles: {role: title}} -> rename positions (Community Admin). Empty title restores the default."""
    permission_classes = [IsAuthenticated]

    def get(self, request):
        return Response({"role_titles": dict(getattr(request.user.community, "role_titles", None) or {}) if request.user.community_id else {}})

    def patch(self, request):
        if not request.user.community_id:
            return Response({"detail": "No community."}, status=status.HTTP_400_BAD_REQUEST)
        try:
            titles = services.set_role_titles(community=request.user.community, titles=request.data.get("titles", {}), actor=request.user)
        except DjangoValidationError as exc:
            return Response({"detail": exc.messages if hasattr(exc, "messages") else str(exc)}, status=status.HTTP_403_FORBIDDEN)
        return Response({"role_titles": titles})


class ManagedUserDetailsView(APIView):
    """PATCH {email?, phone_number?, full_name?} -> 'can edit details' for someone you manage."""
    permission_classes = [IsAuthenticated]

    def patch(self, request, user_id):
        target = get_object_or_404(User, id=user_id)
        try:
            services.update_user_details(actor=request.user, target=target, email=request.data.get("email"), phone_number=request.data.get("phone_number"), full_name=request.data.get("full_name"))
        except DjangoValidationError as exc:
            return Response({"detail": exc.messages if hasattr(exc, "messages") else str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        return Response({"id": str(target.id), "email": target.email, "phone_number": target.phone_number,
                         "full_name": target.member_profile.full_name if getattr(target, "member_profile", None) else None})


class ManagedUserResetPasswordView(APIView):
    """POST {new_password} -> set a new password for someone you manage; audit-logged."""
    permission_classes = [IsAuthenticated]

    def post(self, request, user_id):
        target = get_object_or_404(User, id=user_id)
        try:
            services.admin_reset_password(actor=request.user, target=target, new_password=request.data.get("new_password", ""))
        except DjangoValidationError as exc:
            return Response({"detail": exc.messages if hasattr(exc, "messages") else str(exc)}, status=status.HTTP_400_BAD_REQUEST)
