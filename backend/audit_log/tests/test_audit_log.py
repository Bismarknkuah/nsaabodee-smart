...
class PlatformAdminExcludesMemberLevelActivityTests(TestCase):
    """
    ...
    logs of the community admins only, not members.'
    """

    def setUp(self):
        self.platform_admin = User.objects.create_user(username="pafm_platform_admin", password="x", role=Role.PLATFORM_ADMIN)
        self.bodi = Community.objects.create(name="Bodi Anidasoɔ", slug="pafm-bodi")
        self.community_admin = User.objects.create_user(username="pafm_community_admin", password="x", community=self.bodi, role=Role.COMMUNITY_ADMIN)
        self.family_head = User.objects.create_user(username="pafm_family_head", password="x", community=self.bodi, role=Role.FAMILY_HEAD)

        from audit_log.services import record_event
        record_event(category="community", action="community_admin_did_something", actor=self.community_admin, community=self.bodi, description="A community-admin-level action.")
        record_event(category="finance", action="family_head_did_something", actor=self.family_head, community=self.bodi, description="An ordinary Family Head's own action.")
        record_event(category="community", action="system_generated", actor=None, community=self.bodi, description="A system-generated entry with no actor.")

    def test_platform_admin_sees_the_community_admins_entry(self):
        entries = audit_services.list_audit_log(actor=self.platform_admin)
        actions = {e.action for e in entries}
        self.assertIn("community_admin_did_something", actions)

    def test_platform_admin_does_not_see_the_family_heads_entry(self):
        """The core new restriction — an ordinary executive's own action stays out of the Platform Admin's view entirely."""
        entries = audit_services.list_audit_log(actor=self.platform_admin)
        actions = {e.action for e in entries}
        self.assertNotIn("family_head_did_something", actions)

    def test_platform_admin_still_sees_system_generated_entries_with_no_actor(self):
        entries = audit_services.list_audit_log(actor=self.platform_admin)
        actions = {e.action for e in entries}
        self.assertIn("system_generated", actions)

    def test_the_community_admin_themselves_still_sees_every_entry_in_their_own_community(self):
        """The other half of the rule — nothing changed about what a Community Admin can see in their own community."""
        entries = audit_services.list_audit_log(actor=self.community_admin)
        actions = {e.action for e in entries}
        self.assertEqual(actions, {"community_admin_did_something", "family_head_did_something", "system_generated"})

    def test_a_superuser_still_sees_everything_regardless_of_actor_role(self):
        """The genuine debugging-account exception this platform already makes everywhere else."""
        superuser = User.objects.create_superuser(username="pafm_superuser", password="x")
        entries = audit_services.list_audit_log(actor=superuser)
        actions = {e.action for e in entries}
        self.assertIn("family_head_did_something", actions)
