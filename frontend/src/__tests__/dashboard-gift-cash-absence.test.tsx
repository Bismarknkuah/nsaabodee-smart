...
const REAL_COMMUNITY_OVERVIEW_WITHOUT_GIFT_CASH = {
  ...
  task_summary: { pending: 3, in_progress: 1, pending_approval: 0, done: 5, overdue: 0 },
};
...
describe("Financial dashboard — Treasurer/Auditor/Financial Secretary's actual data shape", () => {
  it("renders without throwing when gift_cash is genuinely absent (it never exists for this role)", async () => {
    ...
    expect(await screen.findByText(/Money in today/)).toBeTruthy();
  });
});

const REAL_COLLECTIONS_REPORT_WITHOUT_GIFT_CASH = {
  start_date: "2026-07-23",
  end_date: "2026-07-23",
  collector_id: null,
  contributions: { count: 1, total: "50", by_method: { cash: "50", mobile_money: "0", bank: "0", other: "0" } },
  combined_cash_position_by_method: { cash: "50", mobile_money: "0", bank: "0", other: "0" },
  receipts_issued: 1,
  // gift_cash deliberately absent — exactly what Treasurer/Chairman/Secretary/Financial Secretary/Auditor actually receive on /reports too, not just on the dashboard.
};

describe("Reports page — the same committee-role data shape, on a completely different page", () => {
  it("renders without throwing when gift_cash is genuinely absent", async () => {
    vi.resetModules();
    vi.doMock("@/lib/api/reports", () => ({
      reportsApi: {
        daily: async () => REAL_COLLECTIONS_REPORT_WITHOUT_GIFT_CASH,
        weekly: async () => REAL_COLLECTIONS_REPORT_WITHOUT_GIFT_CASH,
        monthly: async () => REAL_COLLECTIONS_REPORT_WITHOUT_GIFT_CASH,
        annual: async () => REAL_COLLECTIONS_REPORT_WITHOUT_GIFT_CASH,
        outstandingMembers: async () => ({ outstanding_member_count: 3, total_owed: "0" }),
        expenseStatement: async () => ({ start_date: "2026-07-01", end_date: "2026-07-23", expense_count: 0, total: "0", by_category: {} }),
        openCollectionsPdf: () => {},
      },
    }));
    vi.doMock("@/lib/hooks/useFamilies", () => ({ useFamilies: () => ({ data: [] }) }));
    ...
    expect(await screen.findByText(/1 payment\(s\)/)).toBeTruthy();
  });
});
