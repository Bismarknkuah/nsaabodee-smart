...
export function useAllDonationAccountsIncludingPending(funeralId: string) {
  return useQuery({
    queryKey: ["donation-accounts-all", funeralId],
    queryFn: () => giftsApi.listAllDonationAccountsIncludingPending(funeralId),
    enabled: Boolean(funeralId),
  });
}
