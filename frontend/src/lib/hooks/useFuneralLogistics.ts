...
export function useFundDeposits(funeralId: string) {
  return useQuery({ queryKey: ["expense-fund-deposits", funeralId], queryFn: () => funeralLogisticsApi.listFundDeposits(funeralId), enabled: Boolean(funeralId) });
}

export function useAddFundDeposit(funeralId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (input: { amount: string; note?: string }) => funeralLogisticsApi.addFundDeposit(funeralId, input),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["expense-fund-deposits", funeralId] });
