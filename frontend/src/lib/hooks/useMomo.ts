...
/** 'Members can have their own wallet where they can deposit money.' A self-service top-up via Mobile Money. */
export function useInitiateMomoWalletDeposit() {
  return useMutation({
    mutationFn: ({ phoneNumber, amount }: { phoneNumber: string; amount: string }) => momoApi.walletDeposit(phoneNumber, amount),
  });
}
...
export function useMomoWalletDepositQueryInvalidation(memberId: string | null) {
  const qc = useQueryClient();
  return () => {
    if (memberId) qc.invalidateQueries({ queryKey: ["member-wallet", memberId] });
  };
}
