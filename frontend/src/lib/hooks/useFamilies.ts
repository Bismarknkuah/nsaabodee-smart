...
export function useCreateBereavedRep(familyId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (input: { member_id?: string; new_username?: string; new_password?: string; new_email?: string }) =>
      familiesApi.createBereavedRep(familyId, input),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["bereaved-rep-pending"] }),
  });
}

export function usePendingBereavedRepApprovals(enabled = true) {
  return useQuery({ queryKey: ["bereaved-rep-pending"], queryFn: familiesApi.pendingBereavedRepApprovals, retry: false, enabled });
}

export function useApproveBereavedRep() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (assignmentId: string) => familiesApi.approveBereavedRep(assignmentId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["bereaved-rep-pending"] }),
  });
}

export function useDeactivateBereavedRep() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (assignmentId: string) => familiesApi.deactivateBereavedRep(assignmentId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["bereaved-rep-pending"] }),
  });
}
