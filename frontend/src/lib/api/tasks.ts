...
export const tasksApi = {
  ...
  /** "Assign specific tasks to each user role type" — one task per person holding the role, within the caller's own reach. */
  assignToRole: (input: { role: string; title: string; description?: string; priority?: TaskPriority; due_date?: string }) =>
    request<{ role: string; assigned_count: number; task_ids: string[] }>(`/tasks/assign-to-role/`, { method: "POST", body: JSON.stringify(input) }),
