...
export interface Community {
  ...
  /** The plan's stable code, or null if none assigned yet. */
...
/** "This is how I want the subscription plan to be" — a real, editable plan record. */
...
export type SubscriptionPlanInput = Partial<Pick<SubscriptionPlan, "name" | "description" | "price_yearly" | "price_monthly" | "max_members" | "max_communities" | "trial_days" | "features" | "is_active" | "sort_order">>;
...
export const tenantsApi = {
  ...
  setSubscriptionPlan: (communityId: string, plan: string) =>
    request<Community>(`/tenants/communities/${communityId}/subscription-plan/`, { method: "POST", body: JSON.stringify({ plan }) }),
  listSubscriptionPlans: () => request<SubscriptionPlan[]>(`/tenants/subscription-plans/`),
  createSubscriptionPlan: (input: SubscriptionPlanInput & { code: string; name: string }) =>
    request<SubscriptionPlan>(`/tenants/subscription-plans/`, { method: "POST", body: JSON.stringify(input) }),
  updateSubscriptionPlan: (planId: string, input: SubscriptionPlanInput) =>
    request<SubscriptionPlan>(`/tenants/subscription-plans/${planId}/`, { method: "PATCH", body: JSON.stringify(input) }),
  ...
  /** "They can manage the platform admins on the community admins." One query, every community. */
  listAllCommunityAdmins: () =>
    request<{ id: string; username: string; email: string; community_id: string | null; community_name: string | null }[]>(`/tenants/community-admins/all/`),
  ...
  getPlatformAdminCapabilities: () =>
    request<{
      capability_definitions: Record<string, string>;
      admins: { id: string; username: string; is_you: boolean; capabilities: Record<string, boolean> }[];
    }>(`/tenants/platform-admins/capabilities/`),
  updatePlatformAdminCapabilities: (targetId: string, capabilities: Record<string, boolean>) =>
    request<{ id: string; username: string; capabilities: Record<string, boolean> }>(`/tenants/platform-admins/capabilities/`, {
      method: "PATCH",
      body: JSON.stringify({ target_id: targetId, capabilities }),
    }),
  ...
  // --- Data backup — 'import a Google Drive link to back up their
  // system data, and should also have options to retrieve them back.' ---

  /** Downloads the community's own member/family backup as a JSON file. Triggers a real browser download rather than returning JSON, since authFetch's normal request() helper always expects a JSON body. */
  downloadMyCommunityBackup: async () => {
    const res = await authFetch("/tenants/my-community/backup/export/");
    if (!res.ok) throw new Error("Could not generate a backup.");
  ...
  restoreMyCommunityBackup: (driveLink: string) =>
    request<{ families_restored: number; members_created: number; members_updated: number; member_errors: { row: number; full_name: string; error: string }[] }>(
      `/tenants/my-community/backup/restore/`,
      { method: "POST", body: JSON.stringify({ drive_link: driveLink }) }
    ),

  /** "Should also be able to upload from my computer to synchronize." A backup file already sitting on the Community Admin's own device, instead of a Drive link. */
  restoreMyCommunityBackupFromFile: async (file: File) => {
    const form = new FormData();
    form.set("file", file);
    const res = await authFetch("/tenants/my-community/backup/restore-from-file/", { method: "POST", body: form });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(body.detail?.toString() ?? "Could not restore from that file.");
    }
    return res.json() as Promise<{ families_restored: number; members_created: number; members_updated: number; member_errors: { row: number; full_name: string; error: string }[] }>;
  },

  myCommunityBackupHistory: () =>
    request<{ id: string; kind: "export" | "restore"; drive_link: string; member_count: number; family_count: number; performed_by_username: string | null; created_at: string }[]>(
      `/tenants/my-community/backup/history/`
    ),

  // --- Billing & subscriptions — 'the platform admin should have more control to manage all the community platform.' ---

  communitiesNeedingSubscriptionAttention: () => request<Community[]>(`/tenants/communities/needing-subscription-attention/`),

  sendSubscriptionReminder: (communityId: string, message?: string) =>
    request<{ sent_to_admin_count: number; community: Community }>(
      `/tenants/communities/${communityId}/send-subscription-reminder/`,
