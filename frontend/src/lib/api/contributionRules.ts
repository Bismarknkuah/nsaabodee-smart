...
export const contributionRulesApi = {
  ...
  /** 'The king sets the price for each of them, so the town leader/king is the head of the community's elders ledger.' The chief's own, separate rate-setting authority. */
  setTownElderRate: (amount: string) =>
    request<{ default_town_leader_amount: string }>(`/contribution-rules/town-elder-rate/`, {
      method: "POST",
