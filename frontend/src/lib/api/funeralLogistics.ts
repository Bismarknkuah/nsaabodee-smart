...
export const funeralLogisticsApi = {
  ...
  /** 'Another option to add fund' — a real, separate deposit event, never inferred from a contribution or gift payment. */
  listFundDeposits: (funeralId: string) => request<ExpenseFundDeposit[]>(`/funerals/${funeralId}/expenses/fund-deposits/`),
  addFundDeposit: (funeralId: string, input: { amount: string; note?: string }) =>
    request<ExpenseFundDeposit>(`/funerals/${funeralId}/expenses/fund-deposits/`, {
      method: "POST",
  ...
  expensesOverview: () =>
    request<{
      ...
      by_category: Record<string, string>; total_deposited: string; total_spent: string; balance: string; deposit_count: number;
    }[]>(`/expenses/overview/`),
