import type { BereavedRepAssignment, Family, FamilyAuditLogEntry } from "@/types/family";
...
export const familiesApi = {
  ...
  createBereavedRep: (familyId: string, input: { member_id?: string; new_username?: string; new_password?: string; new_email?: string }) =>
    request<BereavedRepAssignment>(`/families/${familyId}/bereaved-rep/`, { method: "POST", body: JSON.stringify(input) }),

  pendingBereavedRepApprovals: () => request<BereavedRepAssignment[]>(`/bereaved-rep-assignments/pending/`),

  approveBereavedRep: (assignmentId: string) =>
    request<BereavedRepAssignment>(`/bereaved-rep-assignments/${assignmentId}/approve/`, { method: "POST" }),

  deactivateBereavedRep: (assignmentId: string) =>
    request<BereavedRepAssignment>(`/bereaved-rep-assignments/${assignmentId}/deactivate/`, { method: "POST" }),

  /** "Let the family head of each family be able to download their data." */
  downloadFamilyBackup: async (familyId: string, familyName: string) => {
    const res = await authFetch(`/families/${familyId}/backup/export/`);
    if (!res.ok) throw new Error("Could not generate a backup for this family.");
    const blob = await res.blob();
    const disposition = res.headers.get("Content-Disposition") ?? "";
    const filenameMatch = disposition.match(/filename="?([^"]+)"?/);
    const filename = filenameMatch?.[1] ?? `${familyName}-backup.json`;
