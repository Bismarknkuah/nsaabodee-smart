...
export const accountsApi = {
  ...
  manageableUsers: async () => {
    ...
    return res.json() as Promise<{
      ...
      self_restriction_protected?: string[];
      own_family_id: string | null;
      own_family_name: string | null;
      users: { id: string; username: string; role: string; role_label?: string; is_self?: boolean; email?: string; phone_number?: string | null; full_name?: string | null; community_name: string | null; disabled_features: string[]; is_active: boolean; member_id: string | null }[];
    }>;
  },
  /** "The admin can rename the user role position." Empty title restores the default. */
  setRoleTitles: async (titles: Record<string, string>) => {
    const res = await authFetch(`/accounts/role-titles/`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ titles }) });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "Couldn't rename positions.");
    return res.json() as Promise<{ role_titles: Record<string, string> }>;
  },
  /** "Can edit details." */
  updateManagedUserDetails: async (userId: string, input: { email?: string; phone_number?: string; full_name?: string }) => {
    const res = await authFetch(`/accounts/manageable-users/${userId}/details/`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "Couldn't save details.");
    return res.json() as Promise<{ id: string; email: string; phone_number: string | null; full_name: string | null }>;
  },
  resetManagedUserPassword: async (userId: string, newPassword: string) => {
    const res = await authFetch(`/accounts/manageable-users/${userId}/reset-password/`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ new_password: newPassword }) });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "Couldn't reset the password.");
  ...
  /** "The community admin can should have options where he can apply changes for some family." Same authority rule as setDisabledFeatures, applied to a whole family at once. */
  setDisabledFeaturesForFamily: async (familyId: string, disabledFeatures: string[]) => {
    const res = await authFetch(`/accounts/manageable-users/disabled-features/family/`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ family_id: familyId, disabled_features: disabledFeatures }),
    });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "Couldn't update this family's features.");
    return res.json() as Promise<{ updated_count: number }>;
  },

  /** "Can also set for all members." Community Admin only — every plain Community Member in their own community, in one action. */
  setDisabledFeaturesForAllMembers: async (disabledFeatures: string[]) => {
    const res = await authFetch(`/accounts/manageable-users/disabled-features/all-members/`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ disabled_features: disabledFeatures }),
    });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "Couldn't update every member's features.");
    return res.json() as Promise<{ updated_count: number }>;
  },
};
