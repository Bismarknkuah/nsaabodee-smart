...
export const reportsApi = {
  ...
  sendPaymentTrackingSms: async (paymentId: string) => {
    const res = await authFetch(`/receipts/contribution-payments/${paymentId}/send-tracking-sms/`, { method: "POST" });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(body.detail?.toString() ?? "Could not send the tracking SMS.");
    }
    return res.json() as Promise<{ status: string; phone: string }>;
  },

  /** "The receipt should have a bar scan code so that they can scan to confirm their payment." */
  contributionReceiptQrCode: async (paymentId: string) => {
    const res = await authFetch(`/receipts/contribution-payments/${paymentId}/qr-code/`);
    if (!res.ok) throw new Error("Could not generate the receipt's QR code.");
    return res.json() as Promise<{ qr_code_base64: string; verify_url: string }>;
  },
