    ...
    <div className="nb-home">
      ...
      <nav className="sticky top-0 z-20 flex items-center justify-between border-b border-black/5 bg-white px-6 py-4 sm:px-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="#contact" className="rounded-lg bg-[var(--nb-navy)] px-5 py-2.5 text-sm font-medium text-white hover:bg-[var(--nb-navy-deep)]">
            Contact Us
          </a>
        </div>
      </nav>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 sm:px-10 sm:py-24 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              <Link href="/login" className="rounded-lg bg-[var(--nb-gold)] px-7 py-3 font-medium text-[var(--nb-navy-deep)] hover:brightness-110">
                Sign in to your community
              </Link>
              <a href="#how-it-works" className="rounded-lg border border-white/25 px-7 py-3 font-medium text-white hover:border-[var(--nb-gold)]">
                See how it works
              </a>
            </div>
          </div>
      ...
      <section id="services" className="bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-6xl px-6 sm:px-10">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {SERVICES.map((s) => (
              <div key={s.title} className="nb-card rounded-lg border border-black/5 bg-[var(--nb-cream)] p-7" style={{ borderTop: "3px solid var(--nb-navy)" }}>
                <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--nb-navy)] text-[var(--nb-gold)]">
                  <s.Icon />
                </div>
      ...
      <section id="about" className="py-20 sm:py-28">
        <div className="mx-auto max-w-6xl px-6 sm:px-10">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2">
            {SEGMENTS.map((seg) => (
              <div key={seg.title} className="nb-card rounded-lg border border-black/5 bg-white p-7">
                <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--nb-gold-soft)] text-[var(--nb-navy)]">
                  <seg.Icon />
                </div>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 sm:px-10 sm:py-28">
        ...
        <div className="mt-12 space-y-3">
          {FAQS.map((faq, i) => (
            <div key={faq.q} className="rounded-lg border border-black/10 bg-white">
              <button
  ...
  return (
    <section className="bg-white py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-6 sm:px-10">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((a) => (
            <div key={a.id} className="nb-card rounded-lg border border-black/5 bg-[var(--nb-cream)] p-7" style={{ borderTop: "3px solid var(--nb-gold)" }}>
              {a.image_url && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={a.image_url} alt="" className="mb-4 h-36 w-full rounded-lg object-cover" />
              )}
...
function HeroImageRotator() {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-white/10 shadow-2xl">
      {current?.video_url ? (
        <video key={current.id} src={current.video_url} autoPlay muted loop playsInline className="h-[420px] w-full object-cover sm:h-[480px]" />
      ) : current?.image_url ? (
        // eslint-disable-next-line @next/next/no-img-element
...
function PlanCard({ plan }: { plan: (typeof PLANS)[number] }) {
  ...
  return (
    <div className={`rounded-lg border p-8 ${plan.featured ? "border-[var(--nb-gold)] bg-white shadow-lg" : "border-white/10 bg-white/[0.03]"}`}>
      {plan.featured && (
      ...
      {submitted ? (
        <p className="mt-6 rounded-lg bg-[var(--nb-gold-soft)] px-4 py-3 text-center text-sm text-[var(--nb-navy)]">
          Thanks. We&apos;ll be in touch once this plan is available.
        </p>
      ) : !expanded ? (
        <button
          onClick={() => setExpanded(true)}
          className={`mt-6 w-full rounded-lg border py-2.5 text-sm font-medium transition ${
            plan.featured
        ...
        <form onSubmit={submit} className="mt-6 space-y-2">
          <input
            ...
            className="w-full rounded-lg border border-black/10 bg-white px-3 py-2 text-sm text-[var(--nb-ink)] outline-none focus:border-[var(--nb-navy)]"
          />
          <input
          ...
          <button
            ...
            className="w-full rounded-lg bg-[var(--nb-gold)] py-2.5 text-sm font-medium text-[var(--nb-navy-deep)] disabled:opacity-60"
          >
