      ...
      {overview && (
        <div className="lg:col-span-2 mx-auto w-full max-w-xl">
          {overview.active_funerals.length === 0 ? (
            <p className="text-center text-sm text-[var(--text-soft)]">There's no funeral currently active in this community.</p>
          ) : (
            <ul className="divide-y divide-[var(--rule)] border-y border-[var(--border)]">
              {overview.active_funerals.map((f) => (
                <li key={f.id} className="py-3 text-center text-sm">
                  ...
                  <span className="block text-[var(--text-soft)]">{f.deceased_family_name}</span>
                </li>
              ))}
            </ul>
          )}
