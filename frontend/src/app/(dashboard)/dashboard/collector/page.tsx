...
export default function CollectorDashboardPage() {
  ...
  return (
    <DashboardPageShell folio="Folio IV" register="Collector's Daily Log" title="Today's Collections" subtitle="Your cash position, and the funerals you can collect for.">
      ...
      */}
      <form onSubmit={findAccount} className="lg:col-span-2 flex flex-col gap-3 border-2 border-[var(--forest)] bg-white p-4 sm:flex-row">
        <input
          ...
          className="flex-1 rounded-sm border border-[var(--border)] px-4 py-3 text-base outline-none focus:border-[var(--forest)]"
        />
        <button type="submit" className="rounded-sm bg-[var(--forest)] px-6 py-3 font-display text-lg text-white">
          Find account
        </button>
      </form>

      <div className="lg:col-span-2 flex flex-col gap-3 sm:flex-row">
        <Link href="/front-desk" className="flex-1 border border-[var(--ink)] px-5 py-3 text-center font-medium">
          Open Front Desk
        </Link>
        <Link href="/pending-sync" className="flex-1 border border-[var(--ink)] px-5 py-3 text-center font-medium">
          Pending Sync
        </Link>
      </div>
      ...
      */}
      <div className="lg:col-span-2 border border-[var(--border)] bg-white p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => refreshQueue()} className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--forest)]">
              Refresh
            </button>
            <button
              onClick={() => drainQueue()}
        ...
        <div className="mt-3 grid grid-cols-2 gap-px border border-[var(--border)] bg-[var(--rule)] sm:grid-cols-3">
      ...
      {overview && (
        <>
          <div className="border border-[var(--border)] bg-white p-5 font-mono text-sm">
