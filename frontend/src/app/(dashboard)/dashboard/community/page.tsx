    ...
    <DashboardPageShell folio="Folio II" register="Community Register" title="Community Operations" subtitle="Community Admin, Chairman, and Secretary — the full operational picture.">
      ...
      {overview && (
        <>
          ...
          {isCommunityAdmin && pendingDeskAssignments && pendingDeskAssignments.length > 0 && (
            <SectionCard title="Desk assignment approvals" eyebrow="Activated only once you approve" accent="gold">
              <p className="text-xs text-[var(--ink-soft)]">
                A family head assigned these people to a front desk or as a collector for their
                funeral — nobody can actually work the desk until you approve.
              </p>
              <ul className="mt-3 space-y-2">
                {pendingDeskAssignments.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-3 rounded-sm bg-white px-3 py-2">
                    <span className="text-sm">
                      {a.username} <span className="text-[var(--ink-soft)]">— {a.deceased_name}&apos;s funeral</span>
                    </span>
                    <button
          ...
          {isBereavedRepApprover && pendingBereavedReps && pendingBereavedReps.length > 0 && (
            <SectionCard title="Bereaved Rep approvals" eyebrow="Activated only once a different approver signs off" accent="gold">
              <p className="text-xs text-[var(--ink-soft)]">
                Someone else — Community Admin, Secretary, or Chairman — created this account.
                A genuinely different one of those three has to approve it before it works.
              </p>
              <ul className="mt-3 space-y-2">
                {pendingBereavedReps.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-3 rounded-sm bg-white px-3 py-2">
                    <span className="text-sm">
                      {a.username} <span className="text-[var(--ink-soft)]">— represents {a.family_name}</span>
                    </span>
                    <button
