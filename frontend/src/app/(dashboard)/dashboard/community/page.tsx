...
/** A quiet visual cue for each kind of activity feed entry — a colored left border, matching the same palette the family dashboard's own activity feed already uses. */
const ACTIVITY_ACCENT: Record<string, string> = {
  member_registered: "var(--primary)",
  payment_recorded: "var(--forest)",
  expense_paid: "var(--forest)",
  expense_pending_approval: "var(--gold)",
  expense_partial: "var(--gold)",
  expense_credit: "var(--clay-red)",
  expense_cancelled: "var(--text-soft)",
...
interface CommunityOverview {
  ...
  // Chairman and Community Admin only — 'the community chair is to have upper control over the community ledger and the system... have all activities analytics view.'
    ...
    <DashboardPageShell folio="Folio II" register="Community Register" title="Community Operations" subtitle="Community Admin, Chairman, and Secretary — the full operational picture.">
      ...
      {overview && (
        <>
            ...
            <div className="lg:col-span-2 border-2 border-[var(--gold)] bg-[var(--gold-soft)] p-4">
              ...
              <ul className="mt-3 divide-y divide-[var(--border-soft)]">
                {pendingDeskAssignments?.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-3 py-2">
                    <span className="text-sm">
                      Desk assignment: {a.username} <span className="text-[var(--text-soft)]">, {a.deceased_name}&apos;s funeral</span>
                    </span>
                    <button
                      onClick={() => approveDeskAssignment.mutate(a.id)}
                      disabled={approveDeskAssignment.isPending}
                      className="shrink-0 rounded-lg border border-[var(--forest)] bg-white px-2 py-1 text-xs text-[var(--forest)] disabled:opacity-50"
                    >
                      Approve
                    </button>
                  </li>
                ...
                {pendingBereavedReps?.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-3 py-2">
                    <span className="text-sm">
                      Deceased Rep: {a.username} <span className="text-[var(--text-soft)]">, represents {a.family_name}</span>
                    </span>
                    <button
          ...
          <div className="lg:col-span-2 flex flex-wrap items-center gap-x-6 gap-y-2 border-y border-[var(--border)] py-3 font-mono text-sm">
          ...
          <div className="grid gap-4 lg:col-span-2 lg:grid-cols-2">
            ...
            <SectionCard title="Task oversight" eyebrow="Every task assigned community-wide" accent="violet">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                ...
                <div>
                  <p className="text-xs text-[var(--text-soft)]">Awaiting approval</p>
            ...
            {/*
              ...
              community ledger and the system... have all activities
              analytics view." Real, chronological "who did what
              when" across the whole community, not just aggregate
              counts — the same concept already built for Family
              Head, scaled up.
            */}
            {overview.activity_feed && overview.activity_feed.length > 0 && (
              <SectionCard title="Community activity" eyebrow="Everything, in order — newest first" accent="forest">
            ...
            <SectionCard title="Active funerals" eyebrow={`${overview.recent_active_funerals.length} currently open`} accent="clay">
              {overview.recent_active_funerals.length === 0 ? (
                <p className="text-sm text-[var(--text-soft)]">No funeral is currently open.</p>
              ) : (
