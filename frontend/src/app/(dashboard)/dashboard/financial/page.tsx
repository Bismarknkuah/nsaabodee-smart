...
export default function FinancialDashboardPage() {
  ...
  return (
    <DashboardPageShell folio="Folio III" register="Financial Register" title="Financial Oversight" subtitle="Treasurer, Financial Secretary, and Auditor — reconciliation and approvals.">
      ...
      <div className="lg:col-span-2 grid divide-y divide-[var(--border-soft)] border border-[var(--border)] sm:grid-cols-2 sm:divide-x sm:divide-y-0">
        <div className="p-5">
          ...
          <p className="mt-1 text-xs text-[var(--text-soft)]">{formatCedis(overview.month_to_date.contributions.total)} so far this month</p>
        </div>
        <div className="p-5">
          ...
          <p className="mt-1 text-xs text-[var(--text-soft)]">{overview.outstanding_members.outstanding_member_count} members still owing on open funerals</p>
        </div>
      </div>
          ...
          <tbody>
            {methods.map((m) => (
              <tr key={m.label} className="border-b border-[var(--border)]">
                <td className="py-2.5 font-mono text-xs uppercase tracking-wide text-[var(--text-soft)]">{m.label}</td>
      ...
      {/*
        ...
        they received on each funeral.' Everything above is
        time-based — this is the one funeral-by-funeral picture.
      */}
      {overview.money_received_per_funeral && overview.money_received_per_funeral.length > 0 && (
        <SectionCard title="Money received, per funeral" eyebrow={`${overview.money_received_per_funeral.length} funeral(s), newest first`} accent="gold">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-[var(--text-soft)]">
                <th className="py-2 font-medium">Funeral</th>
            ...
            <tbody className="divide-y divide-[var(--border-soft)]">
              {overview.money_received_per_funeral.map((f) => (
                <tr key={f.funeral_id}>
                  <td className="py-2.5">
                    <Link href={`/funerals/${f.funeral_id}`} className="font-medium hover:underline">{f.deceased_name}</Link>
                    <p className="text-xs text-[var(--text-soft)]">{f.family_name} · {f.status === "closed" ? "Closed" : "Active"}</p>
                  </td>
                  <td className="py-2.5 text-right font-mono" style={{ color: "var(--forest)" }}>{formatCedis(f.received_total)}</td>
                  <td className="py-2.5 text-right font-mono" style={{ color: Number(f.outstanding_total) > 0 ? "var(--clay-red)" : "var(--text-soft)" }}>{formatCedis(f.outstanding_total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {overview.paid_members.length > 0 && (
        <SectionCard title="Who's paid" eyebrow={`${overview.paid_members.length} member(s) settled on open funerals`} accent="forest">
          <ul className="max-h-72 divide-y divide-[var(--border-soft)] overflow-y-auto">
          ...
          <ul className="space-y-2">
            {overview.pending_reversal_requests.map((r) => (
              <li key={r.id} className="flex items-center justify-between text-sm">
                <span>
                  {formatCedis(r.payment__amount)} <span className="text-[var(--text-soft)]">— {r.reason}</span>
                </span>
                <span className="font-mono text-xs text-[var(--text-soft)]">by {r.requested_by__username}</span>
              </li>
            ))}
          </ul>
      ...
      {overview.suspicious_transaction_summary && (
        <SectionCard title="Under review" eyebrow="What an Auditor actually reviews, not acts on" accent="clay">
          <div className="grid grid-cols-3 gap-3">
            <div>
              <p className="text-xs text-[var(--text-soft)]">Unreviewed flags</p>
            ...
            <div>
              <p className="text-xs text-[var(--text-soft)]">Dismissed</p>
          ...
          {overview.recent_reversal_history && overview.recent_reversal_history.length > 0 && (
            <div className="mt-4 border-t border-[var(--border)] pt-3">
              <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Recent reversal decisions</p>
              <ul className="mt-2 space-y-1.5">
                {overview.recent_reversal_history.map((r) => (
                  <li key={r.id} className="flex justify-between text-sm">
                    <span>{formatCedis(r.payment__amount)} <span className="text-[var(--text-soft)]">— {r.reason}</span></span>
