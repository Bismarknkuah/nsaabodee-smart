...
export default function BereavedDashboardPage() {
  ...
  return (
    <DashboardPageShell folio="Folio VIII" register="Bereavement Statement" title="Your Family's Funeral" subtitle="A plain account of where things stand, for the family you represent.">
      ...
      {data && (
        <div className="lg:col-span-2 mx-auto w-full max-w-2xl border border-[var(--border)] bg-white p-8">
          {funerals.length === 0 ? (
            <p className="text-center text-sm text-[var(--text-soft)]">There is no active funeral for your family right now.</p>
          ) : (
            <>
              {familySummary && familySummary.active_funeral_count > 1 && (
                <div className="mb-6 border-b border-[var(--border)] bg-[var(--card)] pb-4">
                  <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--text-soft)]">Across {familySummary.active_funeral_count} active funerals</p>
                  ...
                  <div key={f.funeral_id} className="py-6 first:pt-0 last:pb-0">
                    ...
                    <div className="mt-3">
                      <div className="flex items-baseline justify-between text-sm">
                        <span className="text-[var(--text-soft)]">Collected</span>
                      ...
                      {f.outstanding_count > 0 && (
                        <p className="mt-1 text-xs text-[var(--text-soft)]">{f.outstanding_count} member(s) still owing.</p>
                      )}
                    </div>

                    <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm sm:grid-cols-4">
                      <div>
                        <dt className="text-xs text-[var(--text-soft)]">Contributions</dt>
                        <dd className="font-mono">{formatCedis(f.overview.contributions_collected)}</dd>
                      </div>
                      <div>
                        <dt className="text-xs text-[var(--text-soft)]">Gift cash</dt>
                        <dd className="font-mono">{formatCedis(f.overview.gift_cash_collected)}</dd>
                      </div>
                      <div>
                        <dt className="text-xs text-[var(--text-soft)]">Expenses</dt>
                        <dd className="font-mono">{formatCedis(f.overview.total_expenses)}</dd>
                      </div>
                      <div>
                        <dt className="text-xs text-[var(--text-soft)]">Net position</dt>
              ...
              {familySummary && familySummary.member_compliance.length > 0 && (
                <div className="mt-6 border-t border-[var(--border)] pt-4">
                  <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--text-soft)]">Your own family, member by member</p>
                    ...
                    <tbody>
                      {familySummary.member_compliance.map((m) => (
                        <tr key={m.member_id} className="border-b border-[var(--border)] last:border-0">
                          ...
                          <td className="py-1.5 text-right font-mono text-xs text-[var(--text-soft)]">
