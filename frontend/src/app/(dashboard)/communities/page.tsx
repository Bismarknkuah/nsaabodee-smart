  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        <ul className="divide-y divide-[var(--rule)] border-y border-[var(--border)]">
          {communities?.map((c) => (
            <li key={c.id}>
              <button
              ...
              >
                <div>
                  <p className="font-medium">
                    {c.name}
                    {!c.is_active && (
                      <span className="ml-2 rounded-full bg-[var(--surface)] px-2 py-0.5 text-xs text-[var(--text-soft)]">deactivated</span>
                    )}
                    {c.access_expires_at && (
...
function SubscriptionAttentionSection() {
  ...
  return (
    <section className="rounded-sm border border-[var(--clay-red)]/40 bg-[var(--clay-red)]/5 p-5">
      ...
      <div className="mt-3 space-y-2">
        {communities.map((c) => (
          <div key={c.id} className="rounded-sm bg-white p-3 text-sm">
            <div className="flex items-center justify-between gap-3">
              ...
              {sendingId === c.id ? (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button onClick={() => remind.mutate({ id: c.id })} disabled={remind.isPending} className="rounded-sm bg-[var(--forest)] px-3 py-1.5 text-xs font-medium text-white disabled:opacity-60">
                    {remind.isPending ? "Sending…" : "Confirm"}
                  </button>
                  <button onClick={() => setSendingId(null)} className="text-xs text-[var(--text-soft)]">Cancel</button>
                </div>
              ) : (
                <button onClick={() => setSendingId(c.id)} className="whitespace-nowrap rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium">
                  Send reminder
                </button>
              )}
            </div>
    ...
    <div className="mb-4 space-y-4 rounded-sm bg-[var(--surface)] p-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-1.5 text-sm" />
        </div>
        <div>
          <label className="text-xs text-[var(--text-soft)]">Region</label>
          <input value={region} onChange={(e) => setRegion(e.target.value)} className="mt-1 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-1.5 text-sm" />
        </div>
        <div>
          <label className="text-xs text-[var(--text-soft)]">General rate — male (GH₵)</label>
          <input type="number" min="0" step="0.01" value={maleRate} onChange={(e) => setMaleRate(e.target.value)} className="mt-1 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-1.5 text-sm" />
        </div>
        <div>
          <label className="text-xs text-[var(--text-soft)]">General rate — female (GH₵)</label>
          <input type="number" min="0" step="0.01" value={femaleRate} onChange={(e) => setFemaleRate(e.target.value)} className="mt-1 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-1.5 text-sm" />
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {community.is_active ? (
          <button onClick={() => deactivate.mutate()} className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--clay-red)] hover:text-[var(--clay-red)]">
            Deactivate
          </button>
        ) : (
          <button onClick={() => reactivate.mutate()} className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)]">
            Reactivate
          </button>
        )}
      ...
      <div className="border-t border-[var(--border)] pt-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <input
            ...
            className="w-20 rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-xs"
          />
          <button
            onClick={() => extendAccess.mutate()}
            disabled={extendAccess.isPending || !extendDays}
            className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)] disabled:opacity-60"
          >
            + Add days
          </button>
          {community.access_expires_at && (
            <button
              ...
              className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)]"
            >
              Make permanent
            </button>
          )}
          {community.access_expires_at && !community.is_access_expired && (
            <button
              ...
              className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--clay-red)] hover:border-[var(--clay-red)]"
            >
      ...
      {resetting && (
        <span className="ml-2 inline-flex items-center gap-1">
          <input
            ...
            className="w-28 rounded-sm border border-[var(--border)] px-2 py-0.5 text-xs"
            onKeyDown={(e) => {
...
function AddAdminForm({ communityId, onDone }: { communityId: string; onDone: () => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); add.mutate(); }} className="mt-2 flex items-end gap-2">
      <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="username" className="rounded-sm border border-[var(--border)] bg-white px-2 py-1 text-xs" />
      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="password (8+ chars)" className="rounded-sm border border-[var(--border)] bg-white px-2 py-1 text-xs" />
...
function AnnouncementReviewSection() {
  ...
  return (
    <section className="rounded-sm border border-[var(--border)] bg-white p-6">
          ...
          <div key={a.id} className="rounded-sm bg-[var(--surface)] p-4">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 rounded-sm border border-[var(--border)] px-2 py-1.5 text-xs"
              />
              <button
...
function HomepageImagesSection() {
  ...
  return (
    <section className="rounded-sm border border-[var(--border)] bg-white p-6">
      ...
      >
        <select value={mediaKind} onChange={(e) => { setMediaKind(e.target.value as typeof mediaKind); setFile(null); }} className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
        ...
        <input
          ...
          className="text-sm"
        />
        <input value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Caption (optional)" className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm" />
        <input value={subcaption} onChange={(e) => setSubcaption(e.target.value)} placeholder="Subcaption (optional)" className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm" />
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {images?.map((img) => (
          <div key={img.id} className="overflow-hidden rounded-sm border border-[var(--border)]">
            {img.video_url ? (
            ...
            <div className="p-1.5 text-xs">
              {editingId === img.id ? (
                <div className="space-y-1">
                  <input value={editCaption} onChange={(e) => setEditCaption(e.target.value)} placeholder="Caption" className="w-full rounded-sm border border-[var(--border)] px-1.5 py-1 text-xs" />
                  <input value={editSubcaption} onChange={(e) => setEditSubcaption(e.target.value)} placeholder="Subcaption" className="w-full rounded-sm border border-[var(--border)] px-1.5 py-1 text-xs" />
    ...
    <div className="border-t border-[var(--border)] pt-3">
      ...
      {showAdd && (
        <form onSubmit={(e) => { e.preventDefault(); add.mutate(); }} className="mt-2 space-y-2 rounded-sm bg-white p-2">
          <select value={accountType} onChange={(e) => setAccountType(e.target.value as typeof accountType)} className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs">
            <option value="mobile_money">Mobile Money</option>
            <option value="bank">Bank Account</option>
          </select>
          <input value={provider} onChange={(e) => setProvider(e.target.value)} placeholder="Provider (e.g. MTN Mobile Money)" className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs" />
          <input value={number} onChange={(e) => setNumber(e.target.value)} placeholder="Account / phone number" className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs" />
          <input value={holder} onChange={(e) => setHolder(e.target.value)} placeholder="Account holder name" className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs" />
    ...
    <div className="border-t border-[var(--border)] pt-3">
      ...
      {showAdd && (
        <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="mt-2 space-y-2 rounded-sm bg-white p-2">
          <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description (e.g. Monthly subscription — July 2026)" className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs" />
          <input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Amount (GHS)" className="w-full rounded-sm border border-[var(--border)] px-2 py-1 text-xs" />
...
function CreateCommunityDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-sm bg-[var(--surface)] p-6 text-[var(--text)] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="mt-4 space-y-3">
          <input value={communityName} onChange={(e) => setCommunityName(e.target.value)} placeholder="Community name" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <input value={region} onChange={(e) => setRegion(e.target.value)} placeholder="Region (optional)" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <input value={district} onChange={(e) => setDistrict(e.target.value)} placeholder="District" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <input value={traditionalAuthorityName} onChange={(e) => setTraditionalAuthorityName(e.target.value)} placeholder="Traditional authority (e.g. the chief's name and title)" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <div className="grid grid-cols-2 gap-2">
            <input value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="Contact phone" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
            <input type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="Contact email" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          </div>
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <input type="number" min="1" value={estimatedPopulation} onChange={(e) => setEstimatedPopulation(e.target.value)} placeholder="Estimated population (optional)" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          <textarea value={registrationNotes} onChange={(e) => setRegistrationNotes(e.target.value)} placeholder="Notes — why they're signing up, or anything worth knowing (optional)" rows={2} className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />

          <div className="border-t border-[var(--border)] pt-3">
            <input value={adminUsername} onChange={(e) => setAdminUsername(e.target.value)} placeholder="First admin's username" className="w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
            <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Password (8+ chars)" className="mt-2 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm" />
          </div>

          <div className="rounded-sm bg-white p-3">
            <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Access</p>
            <select value={accessType} onChange={(e) => setAccessType(e.target.value as typeof accessType)} className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm">
            ...
            {accessType !== "ongoing" && (
              <>
                <div className="mt-2 flex items-center gap-2">
                  <input type="number" min="1" value={accessDays} onChange={(e) => setAccessDays(e.target.value)} className="w-20 rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm" />
                ...
                <div className="mt-3 border-t border-[var(--border)] pt-3">
                  ...
                  <select value={payoutType} onChange={(e) => setPayoutType(e.target.value as typeof payoutType)} className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm">
                    <option value="mobile_money">Mobile Money</option>
                    <option value="bank">Bank Account</option>
                  </select>
                  <input value={payoutProvider} onChange={(e) => setPayoutProvider(e.target.value)} placeholder="Provider (e.g. MTN Mobile Money)" className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm" />
                  <input value={payoutNumber} onChange={(e) => setPayoutNumber(e.target.value)} placeholder="Account / phone number" className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm" />
                  <input value={payoutHolder} onChange={(e) => setPayoutHolder(e.target.value)} placeholder="Account holder name" className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm" />
                </div>
              </>
            )}
          </div>
