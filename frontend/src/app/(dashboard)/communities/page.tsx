              ...
              {sendingId === c.id ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="rounded-sm border border-[var(--rule)] px-2 py-1 text-xs"
                  />
                  ...
                  <button onClick={() => setSendingId(null)} className="text-xs text-[var(--ink-soft)]">Cancel</button>
                </div>
              ) : (
                <button onClick={() => setSendingId(c.id)} className="whitespace-nowrap rounded-sm border border-[var(--rule)] px-3 py-1.5 text-xs font-medium">
                  Send reminder
                </button>
              )}
            </div>
...
function HomepageImagesSection() {
  ...
  return (
    <section className="rounded-sm border border-[var(--rule)] bg-white p-6">
      ...
      >
        <select value={mediaKind} onChange={(e) => { setMediaKind(e.target.value as typeof mediaKind); setFile(null); }} className="rounded-sm border border-[var(--rule)] bg-white px-2 py-1.5 text-sm">
            ...
            <div className="p-1.5 text-xs">
              {editingId === img.id ? (
                <div className="space-y-1">
                  <input value={editCaption} onChange={(e) => setEditCaption(e.target.value)} placeholder="Caption" className="w-full rounded-sm border border-[var(--rule)] px-1.5 py-1 text-xs" />
                  <input value={editSubcaption} onChange={(e) => setEditSubcaption(e.target.value)} placeholder="Subcaption" className="w-full rounded-sm border border-[var(--rule)] px-1.5 py-1 text-xs" />
                  <div className="flex gap-2">
                    <button onClick={() => update.mutate(img.id)} disabled={update.isPending} className="text-[var(--forest)] hover:underline">Save</button>
                    <button onClick={() => setEditingId(null)} className="text-[var(--ink-soft)] hover:underline">Cancel</button>
                  </div>
                </div>
              ) : (
                <>
...
function CreateCommunityDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-sm bg-[var(--surface)] p-6 text-[var(--ink)] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="mt-4 space-y-3">
          ...
          <input value={district} onChange={(e) => setDistrict(e.target.value)} placeholder="District" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          <input value={traditionalAuthorityName} onChange={(e) => setTraditionalAuthorityName(e.target.value)} placeholder="Traditional authority (e.g. the chief's name and title)" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          <div className="grid grid-cols-2 gap-2">
            <input value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="Contact phone" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
            <input type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="Contact email" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          </div>
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          <input type="number" min="1" value={estimatedPopulation} onChange={(e) => setEstimatedPopulation(e.target.value)} placeholder="Estimated population (optional)" className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          <textarea value={registrationNotes} onChange={(e) => setRegistrationNotes(e.target.value)} placeholder="Notes — why they're signing up, or anything worth knowing (optional)" rows={2} className="w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          ...
          <div className="border-t border-[var(--rule)] pt-3">
            ...
            <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Password (8+ chars)" className="mt-2 w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
          </div>
