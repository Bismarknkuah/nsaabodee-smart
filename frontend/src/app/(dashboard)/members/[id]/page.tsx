...
export default function MemberDetailPage() {
  ...
  const { linkUser, assignRole, revokeRole, transferToTownElder } = useMemberActions();
  ...
  // executive should be able to transfer you' — matches the backend's
  // exact authorized set (community_admin, chairman, secretary).
  const canTransferToTownElder = Boolean(
    currentUser?.is_superuser || ["community_admin", "chairman", "secretary"].includes(currentUser?.role ?? "")
  );
  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="grid gap-6 px-8 py-8 md:grid-cols-2">
        ...
        <section className="rounded-lg border border-[var(--border)] bg-white p-6 md:col-span-2">
          <h2 className="font-display text-lg">App account</h2>
          {member.linked_username ? (
            <p className="mt-2 text-sm text-[var(--text-soft)]">
              Linked to the login <span className="font-mono font-medium text-[var(--text)]">{member.linked_username}</span>.
            ...
            <div className="mt-2">
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  className="w-64 rounded-lg border border-[var(--border)] px-3 py-2 text-sm"
                />
                <button
            ...
            <div className="mt-3 flex flex-wrap items-end gap-2">
              ...
              {!member.linked_username && (
                <>
                  <input
                    ...
                    className="w-48 rounded-lg border border-[var(--border)] px-3 py-2 text-sm"
                  />
                  <input
                    type="password"
              ...
              {member.linked_role && member.linked_role !== "community_member" && (
                <button
                  ...
                  className="rounded-lg border border-[var(--clay-red)] px-4 py-2 text-sm font-medium text-[var(--clay-red)] disabled:opacity-60"
        ...
        {canTransferToTownElder && !member.is_town_leader && (
          <TownElderTransferPanel memberId={member.id} onTransfer={(title) => transferToTownElder.mutate({ id: member.id, title })} />
        )}
        {member.is_town_leader && (
          <div className="rounded-lg border-2 border-[var(--gold)] bg-white p-4">
            <p className="text-sm font-medium">Town Elder — {TITLE_LABEL[member.town_elder_title ?? ""] ?? member.town_elder_title}</p>
            <p className="mt-1 text-xs text-[var(--text-soft)]">Pays the Town Elders' own rate, on the separate Town Elders ledger.</p>
          </div>
        )}

        <section className="flex flex-col items-center rounded-lg border border-[var(--border)] bg-white p-6">
          <h2 className="font-display text-lg">Digital membership card</h2>
          {card && (
            <div className="mt-4 w-full max-w-xs rounded-lg border border-[var(--border)] bg-[var(--bg)] p-5 text-center">
              {card.photo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={card.photo_url} alt="" className="mx-auto h-20 w-20 rounded-full object-cover" />
              ) : (
                <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-white font-display text-2xl text-[var(--text-soft)]">
                  {card.full_name.charAt(0)}
                </div>
              )}
              <p className="font-display mt-3 text-lg">{card.full_name}</p>
              <p className="font-mono text-xs text-[var(--text-soft)]">{card.membership_number}</p>
              <p className="text-xs text-[var(--text-soft)]">{card.family_name ?? "No family"}</p>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
              ...
              />
              <p className="mt-2 font-mono text-xs text-[var(--text-soft)]">Nsaabodeɛ Smart · Bodi Anidasoɔ</p>
            </div>
          )}
        </section>
      </main>
      <MemberProfilePanel memberId={id} />
    </div>
  );
}
...
function TownElderTransferPanel({ memberId, onTransfer }: { memberId: string; onTransfer: (title: "chief" | "queen_mother" | "linguist" | "other") => void }) {
  const [title, setTitle] = useState<"chief" | "queen_mother" | "linguist" | "other">("chief");
  return (
    <section className="rounded-lg border border-[var(--border)] bg-white p-4">
      <h2 className="font-display text-lg">Transfer to Town Elders Ledger</h2>
      <p className="mt-1 text-sm text-[var(--text-soft)]">
        Moves this member onto the separate Town Elders ledger, at the elders' own (higher) rate — never retroactive to any funeral already open.
      </p>
      <div className="mt-3 flex items-end gap-2">
        <select value={title} onChange={(e) => setTitle(e.target.value as typeof title)} className="rounded-lg border border-[var(--border)] px-3 py-2 text-sm">
          {Object.entries(TITLE_LABEL).map(([value, label]) => (
            <option key={value} value={value}>{label}</option>
          ))}
        </select>
        <button
          onClick={() => onTransfer(title)}
          className="rounded-lg bg-[var(--gold)] px-4 py-2 text-sm font-medium text-white"
        >
          Transfer
        </button>
      </div>
