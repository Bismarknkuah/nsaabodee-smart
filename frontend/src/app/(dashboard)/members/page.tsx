...
export default function MembersPage() {
  ...
  // 'The family head... can[not] enter data or make entries...
  // registering a new member is for Family Secretary or the Family
  // Registration Officer.' Hidden here rather than left to error on
  // submit — a button a Family Head can see but can never actually
  // use isn't genuinely more helpful than not showing it at all.
  const isFamilyHead = useAuthStore((s) => s.user?.role) === "family_head";
  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
        ...
        <div className="mx-auto flex max-w-6xl items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <button
              ...
              className="border border-[var(--border)] px-3 py-2 text-sm font-medium hover:border-[var(--ink)] disabled:opacity-60"
            >
      ...
      <div className="mx-auto max-w-6xl px-6 py-5 sm:px-10">
        <div className="flex flex-wrap items-end gap-4">
          <input
            ...
            className="w-full max-w-md border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 text-sm outline-none focus:border-[var(--forest)] sm:w-80"
          />
          <div>
            <label className="block text-[10px] font-medium uppercase tracking-wide text-[var(--text-soft)]">Gender</label>
            <select
              value={gender}
              onChange={(e) => setGender(e.target.value as "" | "male" | "female")}
              className="mt-1 rounded-lg border border-[var(--border)] bg-white px-3 py-1.5 text-sm"
            >
          ...
          <div>
            <label className="block text-[10px] font-medium uppercase tracking-wide text-[var(--text-soft)]">Sort by</label>
            <select
              value={sortBy}
        ...
        {uploadResult && (
          <div className="mt-4 border border-[var(--border)] bg-white p-4">
            ...
            <button onClick={() => setUploadResult(null)} className="mt-2 text-xs text-[var(--text-soft)] hover:underline">
              Dismiss
            </button>
          </div>
        )}
      ...
      <main className="mx-auto max-w-6xl px-6 pb-16 sm:px-10">
        {isLoading && <p className="text-sm text-[var(--text-soft)]">Loading members…</p>}

        {showFuzzy && fuzzyResults && fuzzyResults.length > 0 && (
          <div className="mb-4 border border-dashed border-[var(--border)] bg-white p-4">
            <p className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">
            ...
            <ul className="mt-2 space-y-1">
              {fuzzyResults.map((r) => (
                <li key={r.member_id}>
                  ...
                  >
                    {r.full_name} <span className="font-mono text-xs text-[var(--text-soft)]">({r.membership_number})</span>
                  </Link>
                </li>
              ))}
            </ul>
          ...
          {members?.map((m, i) => (
            <li key={m.id}>
              <Link href={`/members/${m.id}`} className="flex items-center gap-4 py-3.5 hover:bg-white">
                <span className="w-8 shrink-0 font-mono text-xs text-[var(--text-soft)]">{String(i + 1).padStart(3, "0")}</span>
                {m.photo_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={m.photo_url} alt="" className="h-10 w-10 rounded-full object-cover" />
                ) : (
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--bg)] font-display text-sm text-[var(--text-soft)]">
