  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <header className="border-b-2 border-[var(--ink)] px-8 py-6">
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Platform-wide</p>
      ...
      <main className="mx-auto max-w-3xl px-8 py-8">
        ...
        {board?.length === 0 && <p className="mt-2 text-sm text-[var(--text-soft)]">Nothing posted yet.</p>}
        <ul className="mt-4 space-y-4">
          {board?.map((a) => (
            <li key={a.id} className="rounded-sm border border-[var(--border)] bg-white p-6">
              ...
              <p className="mt-3 text-xs text-[var(--text-soft)]">Posted {new Date(a.submitted_at).toLocaleDateString()}</p>
            </li>
          ))}
        </ul>
      </main>
...
function SubmitAnnouncementSection({ communityId }: { communityId: string }) {
  ...
  // and videos." A real video file upload, alongside the existing
  // video-link option — either one, never both.
  const [video, setVideo] = useState<File | null>(null);
  const [funeralEventId, setFuneralEventId] = useState("");
  ...
  const { data: funerals } = useFunerals("active");

  const submit = useMutation({
    mutationFn: () => announcementsApi.submit(communityId, {
      title, content, image: image ?? undefined, video: video ?? undefined, video_url: video ? undefined : (videoUrl || undefined),
      funeral_event_id: funeralEventId || undefined, homepage_feature_requested: homepageFeatureRequested,
    }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["own-announcements", communityId] });
      setTitle(""); setContent(""); setVideoUrl(""); setImage(null); setVideo(null); setFuneralEventId(""); setHomepageFeatureRequested(false);
    },
  });
  ...
  return (
    <section className="rounded-sm border border-[var(--border)] bg-white p-6">
      ...
      <form onSubmit={(e) => { e.preventDefault(); submit.mutate(); }} className="mt-4 space-y-3">
        ...
        />
        <div>
          <label className="text-sm font-medium">Is this about a specific funeral?</label>
          <select
            value={funeralEventId} onChange={(e) => setFuneralEventId(e.target.value)}
          ...
          >
            <option value="">Not tied to a funeral — a general notice</option>
            {funerals?.map((f) => (
        ...
        <div>
          <label className="text-sm font-medium">Video (optional) — upload a file, or link to one instead</label>
          <div className="mt-1 flex flex-wrap gap-2">
            <input type="file" accept="video/*" onChange={(e) => { setVideo(e.target.files?.[0] ?? null); setVideoUrl(""); }} className="text-sm" />
            <input
              value={videoUrl} onChange={(e) => { setVideoUrl(e.target.value); setVideo(null); }} placeholder="Or a link (YouTube, Vimeo)"
              disabled={Boolean(video)}
              className="flex-1 rounded-sm border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)] disabled:bg-[var(--surface)] disabled:text-[var(--text-soft)]"
            />
          </div>
        </div>
      ...
      {pending.length > 0 && (
        <div className="mt-6 border-t border-[var(--border)] pt-4">
          <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Awaiting review</p>
          {pending.map((a) => (
          ...
          {rejected.map((a) => (
            <div key={a.id} className="mt-2 rounded-sm bg-[var(--clay-red-soft,#f6e6e3)] p-3">
              ...
              <textarea
                ...
                rows={2}
                className="mt-2 w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-xs"
              />
              <button
      ...
      {approved.length > 0 && (
        <p className="mt-6 border-t border-[var(--border)] pt-4 text-xs text-[var(--text-soft)]">
