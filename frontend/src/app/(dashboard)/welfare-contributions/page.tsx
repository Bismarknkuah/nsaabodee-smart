    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-3 rounded-sm bg-white p-6 shadow-xl">
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name, e.g. Monthly Welfare Contribution"
          className="w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm" />
        <textarea value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder="Purpose (optional)" rows={2}
          className="w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm" />
        <div className="flex gap-3">
          <select value={amountType} onChange={(e) => setAmountType(e.target.value as ContributionAmountType)} className="flex-1 rounded-sm border border-[var(--border)] px-3 py-2 text-sm">
          ...
          {amountType === "fixed" && (
            <input type="number" min="0.01" step="0.01" value={fixedAmount} onChange={(e) => setFixedAmount(e.target.value)} placeholder="Amount"
              className="flex-1 rounded-sm border border-[var(--border)] px-3 py-2 text-sm" />
          )}
        </div>
        <select value={frequency} onChange={(e) => setFrequency(e.target.value as ContributionFrequency)} className="w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm">
        ...
        <div>
          ...
          <input type="number" min="1" max="10" value={requiredApprovals} onChange={(e) => setRequiredApprovals(e.target.value)}
            className="mt-1 w-24 rounded-sm border border-[var(--border)] px-3 py-2 text-sm" />
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-3 rounded-sm bg-white p-6 shadow-xl">
        ...
        <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm">
          <option value="">Select a category…</option>
          {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>

        {scope === "family" && (
          <div className="w-full rounded-sm border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--text-soft)]">
