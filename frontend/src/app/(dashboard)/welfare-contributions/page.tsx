  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <header className="border-b border-[var(--border)] bg-[var(--card)] px-8 py-6">
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Beyond Funerals</p>
      ...
      <main className="grid gap-6 px-8 py-8 lg:grid-cols-2">
        <section className="rounded-lg border border-[var(--border)] bg-white p-5">
          ...
          <p className="mt-1 text-sm text-[var(--text-soft)]">The reusable types a campaign is started under.</p>
          <ul className="mt-4 divide-y divide-[var(--border-soft)]">
            {categories?.map((c) => (
              <li key={c.id} className="py-3">
                <div className="flex items-center justify-between">
                  <p className="font-medium">{c.name}</p>
                  <span className="font-mono text-xs text-[var(--text-soft)]">{FREQUENCY_LABEL[c.frequency]}</span>
                </div>
                ...
                {c.purpose && <p className="mt-1 text-xs italic text-[var(--text-soft)]">{c.purpose}</p>}
              </li>
            ))}
            {categories?.length === 0 && <p className="py-4 text-sm text-[var(--text-soft)]">No categories yet.</p>}
          </ul>
        </section>

        <section className="rounded-lg border border-[var(--border)] bg-white p-5">
          <h2 className="font-display text-xl">Campaigns</h2>
          <p className="mt-1 text-sm text-[var(--text-soft)]">Community-wide, plus your own family&apos;s.</p>
          <ul className="mt-4 divide-y divide-[var(--border-soft)]">
            {campaigns?.map((c) => (
              <li key={c.id} className="py-3">
                ...
                {c.status === "pending_approval" && c.family && (
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => decideCampaign.mutate({ campaignId: c.id, approve: true })} className="rounded-lg border border-[var(--forest)] px-2 py-1 text-xs text-[var(--forest)]">
                      Approve
                    </button>
                    <button onClick={() => decideCampaign.mutate({ campaignId: c.id, approve: false })} className="rounded-lg border border-[var(--clay-red)] px-2 py-1 text-xs text-[var(--clay-red)]">
                      Reject
                    </button>
                  </div>
                )}
                {c.status === "family_approved" && user?.role === "community_admin" && (
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => adminApproveCampaign.mutate({ campaignId: c.id, approve: true })} className="rounded-lg border border-[var(--gold)] px-2 py-1 text-xs text-[var(--gold)]">
                      Give final approval — bill {c.family_name}&apos;s members
                    </button>
                    <button onClick={() => adminApproveCampaign.mutate({ campaignId: c.id, approve: false })} className="rounded-lg border border-[var(--clay-red)] px-2 py-1 text-xs text-[var(--clay-red)]">
                      Reject
                    </button>
                  </div>
                )}
            ...
            {campaigns?.length === 0 && <p className="py-4 text-sm text-[var(--text-soft)]">No campaigns yet.</p>}
          </ul>
        </section>
      </main>
...
function NewCategoryDialog({ onClose, onCreate }: { onClose: () => void; onCreate: ReturnType<typeof useWelfareActions>["createCategory"] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-3 rounded-lg bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl">New contribution category</h2>
          <button type="button" onClick={onClose} className="text-[var(--text-soft)]">✕</button>
        </div>
        ...
        <div className="flex gap-3">
          <select value={amountType} onChange={(e) => setAmountType(e.target.value as ContributionAmountType)} className="flex-1 rounded-lg border border-[var(--border)] px-3 py-2 text-sm">
        ...
        <select value={frequency} onChange={(e) => setFrequency(e.target.value as ContributionFrequency)} className="w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm">
        ...
        <div>
          ...
          <input type="number" min="1" max="10" value={requiredApprovals} onChange={(e) => setRequiredApprovals(e.target.value)}
            className="mt-1 w-24 rounded-lg border border-[var(--border)] px-3 py-2 text-sm" />
        </div>
        {onCreate.isError && <p className="text-sm text-[var(--clay-red)]">{onCreate.error.message}</p>}
        <button type="submit" disabled={onCreate.isPending || !name.trim()} className="w-full rounded-lg bg-[var(--violet)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-3 rounded-lg bg-white p-6 shadow-xl">
        ...
        {canStartCommunityWide && isFamilyHead && (
          <div className="flex gap-1 rounded-full bg-[var(--bg)] p-1 text-xs">
            {(["community", "family"] as const).map((s) => (
              <button key={s} type="button" onClick={() => setScope(s)}
                className={`flex-1 rounded-full px-3 py-1.5 font-medium ${scope === s ? "bg-[var(--ink)] text-white" : "text-[var(--text-soft)]"}`}>
        ...
        <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm">
          <option value="">Select a category…</option>
          {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>

        {scope === "family" && (
          <div className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-2 text-sm text-[var(--text-soft)]">
        ...
        <button type="submit" disabled={mutation.isPending || !categoryId || !title.trim() || (scope === "family" && !familyId)}
          className="w-full rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[var(--text-soft)]">✕</button>
        </div>
        <p className="mt-1 text-xs text-[var(--text-soft)]">{campaign.category_name} · {campaign.family_name ?? "Community-wide"}</p>

        {isLoading && <p className="mt-4 text-sm text-[var(--text-soft)]">Loading…</p>}
        {campaign.status !== "active" && (
          <p className="mt-4 text-sm text-[var(--text-soft)]">No members are billed until this campaign is active.</p>
        )}
          ...
          {obligations?.map((o) => (
            <li key={o.id} className="flex items-center justify-between gap-2 py-2">
              <div>
                ...
                <p className="text-xs text-[var(--text-soft)]">{o.payment_status} · owes {formatCedis(o.balance)}</p>
              </div>
              {o.payment_status !== "paid" && (
                <button
                  ...
                  className="shrink-0 rounded-lg border border-[var(--forest)] px-2 py-1 text-xs text-[var(--forest)]"
                >
                  Record payment
                </button>
              )}
