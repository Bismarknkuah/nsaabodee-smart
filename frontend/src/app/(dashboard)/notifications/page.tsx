...
const STATUS_STYLE: Record<string, string> = {
  ...
  skipped_not_configured: "bg-[var(--surface)] text-[var(--text-soft)]",
  skipped_no_address: "bg-[var(--surface)] text-[var(--text-soft)]",
    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        <div className="mb-4 flex flex-wrap gap-2">
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
            <option value="">All types</option>
            {Object.entries(CATEGORY_LABEL).map(([value, label]) => (
              <option key={value} value={value}>{label}</option>
            ))}
          </select>
          <select value={readFilter} onChange={(e) => setReadFilter(e.target.value as typeof readFilter)} className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
            <option value="">Read and unread</option>
            <option value="false">Unread only</option>
            <option value="true">Read only</option>
          </select>
          <select value={ordering} onChange={(e) => setOrdering(e.target.value as typeof ordering)} className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
