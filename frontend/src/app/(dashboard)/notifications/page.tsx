...
const STATUS_STYLE: Record<string, string> = {
  ...
  skipped_not_configured: "bg-[var(--bg)] text-[var(--text-soft)]",
  skipped_no_address: "bg-[var(--bg)] text-[var(--text-soft)]",
    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        <div className="mb-4 flex flex-wrap gap-2">
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-lg border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
            <option value="">All types</option>
            {Object.entries(CATEGORY_LABEL).map(([value, label]) => (
              <option key={value} value={value}>{label}</option>
            ))}
          </select>
          <select value={readFilter} onChange={(e) => setReadFilter(e.target.value as typeof readFilter)} className="rounded-lg border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
            <option value="">Read and unread</option>
            <option value="false">Unread only</option>
            <option value="true">Read only</option>
          </select>
          <select value={ordering} onChange={(e) => setOrdering(e.target.value as typeof ordering)} className="rounded-lg border border-[var(--border)] bg-white px-2 py-1.5 text-sm">
            ...
            <li key={n.id}>
              ...
              >
                <div className="flex gap-3">
                  <span className="font-mono text-xs text-[var(--text-soft)]">{String(i + 1).padStart(3, "0")}</span>
                  <div>
                    <p className="text-sm">{n.message}</p>
                    <p className="font-mono mt-1 text-xs text-[var(--text-soft)]">
...
function DeliveryAttempts({ notificationId }: { notificationId: string }) {
  ...
  return (
    <div className="mb-4 rounded-lg bg-[var(--bg)] p-3">
      {isLoading && <p className="text-xs text-[var(--text-soft)]">Loading delivery attempts…</p>}
      {data?.length === 0 && <p className="text-xs text-[var(--text-soft)]">No delivery attempts recorded.</p>}
