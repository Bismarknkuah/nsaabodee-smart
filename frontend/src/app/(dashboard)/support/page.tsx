    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <header className="border-b-2 border-[var(--ink)] px-8 py-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Your account</p>
      ...
      <main className="px-8 py-8">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[20rem_1fr]">
          <div className="border border-[var(--border)] bg-white">
            ...
            {tickets?.length === 0 && <p className="p-4 text-sm text-[var(--text-soft)]">No tickets yet.</p>}
            {tickets?.map((t) => (
              <button
                key={t.id}
          ...
          <div>
            {activeTicket ? (
              ...
              <p className="text-sm text-[var(--text-soft)]">Select a ticket to view its conversation.</p>
            )}
          </div>
        </div>
      </main>
  ...
  return (
    ...
    >
      <input
        ...
        className="w-full border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 text-sm outline-none focus:border-[var(--forest)]"
      />
      <textarea
        value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the problem or question…" rows={4}
        className="mt-3 w-full border border-[var(--border)] p-2 text-sm outline-none focus:border-[var(--forest)]"
      />
      <div className="mt-3 flex items-center gap-3">
        <select value={priority} onChange={(e) => setPriority(e.target.value)} className="border border-[var(--border)] px-2 py-1.5 text-sm">
  ...
  return (
    <div className="flex h-[32rem] flex-col border border-[var(--border)] bg-white">
      ...
      <div ref={scrollRef} className="flex-1 space-y-2 overflow-y-auto p-4">
        ...
        {messages?.length === 0 && <p className="text-sm text-[var(--text-soft)]">No replies yet.</p>}
      </div>
      <form
      ...
      >
        <input
          ...
          className="flex-1 border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
        />
