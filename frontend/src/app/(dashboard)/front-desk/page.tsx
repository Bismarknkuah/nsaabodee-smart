          ...
          <div className="flex items-center gap-2">
            ...
            */}
            <button
              ...
              className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--forest)]"
            >
              Refresh
            </button>
            <button
          ...
          <div className="mx-auto max-w-lg">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-sm border border-[var(--border)] bg-white px-4 py-3 text-base outline-none focus:border-[var(--forest)]"
              />
              {qrScanningSupported && (
                <button
                  type="button"
                  onClick={() => setScannerOpen(true)}
                  className="shrink-0 rounded-sm border border-[var(--border)] px-3 text-sm font-medium hover:border-[var(--forest)]"
                >
                  Scan
                </button>
              )}
            ...
            <ul className="mt-3 divide-y divide-[var(--rule)] rounded-sm border border-[var(--border)] bg-white">
              {results?.map((m) => (
                <li key={m.id}>
                  <button
...
function ActiveFuneralsSummary() {
  ...
  return (
    <div className="mb-4 rounded-sm border border-[var(--border)] bg-white">
      <button
        onClick={() => setOpen((v) => !v)}
  ...
  return (
    <div className="font-body flex min-h-screen items-center justify-center bg-[var(--bg)] text-[var(--text)]">
      <form
        ...
        className="w-full max-w-sm rounded-sm border border-[var(--border)] bg-white p-6"
      >
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Front Desk</p>
        ...
        <div className="mt-4">
          ...
          <input
            ...
            className="mt-1 w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
          />
        </div>
        ...
        */}
        ...
        <button
          type="submit"
          disabled={!name.trim()}
          className="mt-6 w-full rounded-sm bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
        >
          Open desk
        </button>
      </form>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {obligations?.length === 0 && (
        <div className="mt-6 rounded-sm border border-dashed border-[var(--border)] p-6 text-center">
      ...
      <ul className="mt-4 space-y-3">
        {obligations?.map((o) => (
          <li key={o.obligation_id} className="rounded-sm border border-[var(--border)] bg-white p-4">
...
function ArrearsSection({ memberId, memberName, deskSession }: { memberId: string; memberName: string; deskSession: DeskSession }) {
  ...
  return (
    <div className="mt-6 border-t-2 border-[var(--clay-red)]/40 pt-4">
      <h3 className="font-display text-lg" style={{ color: "var(--clay-red)" }}>Arrears from past funerals</h3>
      ...
      <ul className="mt-3 space-y-2">
        {closedArrears.map((a) => (
          <li key={a.id} className="rounded-sm border border-[var(--border)] bg-white p-3">
            <div className="flex items-center justify-between">
              <div>
              ...
              <p className="font-mono text-sm font-medium" style={{ color: "var(--clay-red)" }}>{formatCedis(a.balance)}</p>
            </div>
            <div className="mt-2 flex gap-2">
              <button onClick={() => { setPayingId(a.id); setCorrectingId(null); }} className="text-xs font-medium text-[var(--forest)] hover:underline">
                Record payment
              </button>
    ...
    <form onSubmit={submit} className="mt-2 flex items-end gap-2 border-t border-[var(--border)] pt-2">
      <div>
        ...
        <input
          ...
          className="mt-1 w-28 rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        />
      </div>
      <div>
        ...
        <select
          ...
          className="mt-1 rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        >
      ...
      <button type="submit" disabled={isSaving} className="rounded-sm bg-[var(--forest)] px-3 py-1.5 text-sm font-medium text-white disabled:opacity-60">
    ...
    <form onSubmit={submit} className="mt-2 space-y-2 border-t border-[var(--border)] pt-2">
      ...
      <div className="flex gap-2">
        <input
          type="number" min="0.01" step="0.01" value={amount}
          onChange={(e) => setAmount(e.target.value)}
          className="w-28 rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        />
        <select
          value={method}
          onChange={(e) => setMethod(e.target.value as PaymentMethod)}
          className="rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        >
          <option value="cash">Cash</option>
          <option value="bank">Bank</option>
          <option value="other">Other</option>
      ...
      <textarea
        ...
        rows={2}
        className="w-full rounded-sm border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
      />
      <button type="submit" disabled={isSaving || !reason.trim()} className="rounded-sm bg-[var(--forest)] px-3 py-1.5 text-sm font-medium text-white disabled:opacity-60">
...
function QuickCashForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (deskSession.deliveryMethod === "sms" && method === "cash") {
        setTimeout(onDone, 1500);
      } else {
        onDone();
      }
