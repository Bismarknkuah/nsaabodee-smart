  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {liveObligations && liveObligations.length > 1 && online && (
        <PayAllPanel memberId={memberId} memberName={memberName} obligations={liveObligations} />
      )}
...
function QuickCashForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!online) {
      ...
      try {
        await enqueueOperation({
          ...
          payload: { amount, method, client_op_id: clientOpId, collector_name: collectorName.trim() },
    ...
    try {
      const payment = await funeralsApi.recordPayment(funeralId, obligationId, { amount, method, client_op_id: clientOpId, collector_name: collectorName.trim() });
    ...
    <form onSubmit={submit} className="mt-3 flex items-end gap-2 border-t border-[var(--rule)] pt-3">
      ...
      <div>
        ...
        <input
          ...
          className="mt-1 w-36 rounded-sm border border-[var(--rule)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        />
      </div>
      <button
        type="submit"
        disabled={isSaving || !collectorName.trim()}
 ...
 */
function PayAllPanel({ memberId, memberName, obligations }: { memberId: string; memberName: string; obligations: OutstandingObligation[] }) {
  const currentUser = useAuthStore((s) => s.user);
  ...
  if (isSuccess) {
    return (
      <div className="mb-4 border-2 border-[var(--forest)] bg-white p-4">
        ...
        <p className="mt-1 text-xs text-[var(--ink-soft)]">Receipts for each are also available from My Receipts.</p>
      </div>
    );
  }
      ...
      {!expanded ? (
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!collectorName.trim()) return;
            mutate({ member_id: memberId, method, collector_name: collectorName.trim() });
          }}
        >
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-medium">Your name (as collector)</label>
              <input
                value={collectorName}
                onChange={(e) => setCollectorName(e.target.value)}
                className="mt-1 w-full rounded-sm border border-[var(--rule)] bg-white px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
          ...
          <div className="mt-3 flex justify-end gap-2">
            <button type="button" onClick={() => setExpanded(false)} className="px-3 py-2 text-sm text-[var(--ink-soft)]">
              Cancel
            </button>
            <button
              type="submit"
