...
export default function MessagingPage() {
  ...
  return (
    <div className="font-body flex min-h-screen flex-col bg-[var(--bg)] text-[var(--text)]">
      <header className="border-b border-[var(--border)] bg-[var(--card)] px-8 py-6">
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Channels</p>
      ...
      <div className="flex flex-1">
        <aside className="w-64 shrink-0 border-r border-[var(--border)] bg-white">
          ...
          {channels?.map((c) => (
            <button
              ...
              className={`block w-full border-b border-[var(--border)] px-4 py-3 text-left ${
                c.id === activeChannelId ? "bg-[var(--bg)]" : "hover:bg-[var(--bg)]"
              }`}
            >
              <p className="font-mono text-[10px] font-medium uppercase tracking-wide text-[var(--text-soft)]">{CHANNEL_LABEL[c.channel_type]}</p>
        ...
        <section className="flex flex-1 flex-col">
          {activeChannel ? (
            ...
            !isLoading && <p className="p-6 text-sm text-[var(--text-soft)]">No channel selected.</p>
          )}
        </section>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-6">
        ...
        {messages?.length === 0 && <p className="text-sm text-[var(--text-soft)]">No messages yet — be the first to say something.</p>}
        {messages?.map((m) => (
          <div key={m.id} className={`flex ${m.sender_username === currentUsername ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[70%] px-3 py-2 text-sm ${
                m.sender_username === currentUsername ? "bg-[var(--forest)] text-white" : "border border-[var(--border)] bg-white"
              }`}
            >
              {m.sender_username !== currentUsername && (
                <p className="mb-0.5 font-mono text-[10px] uppercase tracking-wide text-[var(--text-soft)]">
                  {m.sender_username} · {formatRole(m.sender_role)}
                </p>
              )}
              <p>{m.content}</p>
            </div>
