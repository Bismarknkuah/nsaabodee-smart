...
function FundCard({ familyId, fund, onOpen }: { familyId: string; fund: FamilyFund; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="rounded-sm border border-[var(--border)] bg-white p-4 text-left hover:border-[var(--forest)]"
    >
...
function CreateFundDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body w-full max-w-sm rounded-sm bg-[var(--surface)] p-6 text-[var(--text)] shadow-xl">
        ...
        <div className="mt-4 space-y-3">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-sm border border-[var(--border)] bg-white px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
            />
          </div>
          <div>
...
function FundDetailDialog({ familyId, fund, onClose }: { familyId: string; fund: FamilyFund; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-sm bg-[var(--surface)] p-6 text-[var(--text)] shadow-xl">
        ...
        <form onSubmit={submit} className="mt-4 rounded-sm bg-white p-4">
          ...
          <div className="mt-2 grid grid-cols-2 gap-3">
            <div className="col-span-2">
              {memberId ? (
                <div className="flex items-center justify-between rounded-sm border border-[var(--border)] px-3 py-2 text-sm">
                ...
                <>
                  <input
                    ...
                    className="w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
                  />
                  {memberResults && memberResults.length > 0 && (
                    <ul className="mt-1 max-h-28 divide-y divide-[var(--rule)] overflow-y-auto rounded-sm border border-[var(--border)]">
                      {memberResults.map((m) => (
                        <li key={m.id}>
                          <button
                            type="button"
            ...
            <input
              ...
              className="rounded-sm border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
            />
            <select
              value={method}
    ...
    <div className="mb-6 rounded-sm bg-[var(--surface)] p-4">
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <input
          ...
          className="flex-1 rounded-sm border border-[var(--border)] bg-white px-3 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        />
      </div>
      {memberResults && memberResults.length > 0 && (
          ...
          {memberResults.map((m) => (
            <li key={m.id} className="flex items-center justify-between px-3 py-1.5 text-sm">
              ...
              <button
                ...
                className="rounded-sm border border-[var(--border)] px-2 py-1 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)]"
              >
                Assign as {role}
              </button>
            </li>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {usingCustomTitle && (
              <input
                ...
                className="rounded-sm border border-[var(--border)] bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
              />
            )}
            <input
              value={query}
...
function FuneralExpensesPanel({ familyId }: { familyId: string }) {
  ...
  return (
    <section className="mt-8 rounded-sm border border-[var(--border)] bg-white p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
        ...
        <div className="flex shrink-0 gap-2">
          {funeralId && (isFinanceOfficer || canRecord) && (
            <button
              ...
              className="flex items-center rounded-sm border border-[var(--border)] px-4 py-2 text-sm font-medium hover:border-[var(--ink)]"
            >
              Download PDF
            </button>
          )}
      ...
      <select
        ...
        className="mt-3 w-full max-w-sm rounded-sm border border-[var(--border)] px-3 py-2 text-sm"
      >
      ...
      <div className="font-body w-full max-w-sm rounded-sm bg-[var(--surface)] p-6 text-[var(--text)] shadow-xl">
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          ...
          <div>
            ...
            {paidById ? (
              <div className="mt-1 flex items-center justify-between rounded-sm border border-[var(--border)] px-3 py-2 text-sm">
