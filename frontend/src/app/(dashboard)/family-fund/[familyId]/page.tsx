  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        {isError && (
          <div className="rounded-lg border border-dashed border-[var(--clay-red)] p-6 text-center">
        ...
        {funds?.length === 0 && (
          <div className="rounded-lg border border-dashed border-[var(--border)] px-6 py-10 text-center">
            ...
            <p className="mt-1 text-sm text-[var(--text-soft)]">Create one to start collecting.</p>
          </div>
        )}
...
function FundCard({ familyId, fund, onOpen }: { familyId: string; fund: FamilyFund; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="rounded-lg border border-[var(--border)] bg-white p-4 text-left hover:border-[var(--forest)]"
    >
      <p className="font-display text-lg">{fund.name}</p>
      {fund.description && <p className="mt-1 text-sm text-[var(--text-soft)]">{fund.description}</p>}
      {summary && (
        <div className="mt-3 flex items-center gap-6">
          <div>
            <p className="text-xs text-[var(--text-soft)]">Total collected</p>
            <p className="font-mono text-xl font-semibold">{formatCedis(summary.total_collected)}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--text-soft)]">Contributors</p>
...
function CreateFundDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body w-full max-w-sm rounded-lg bg-[var(--bg)] p-6 text-[var(--text)] shadow-xl">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={onClose} className="px-3 py-2 text-sm text-[var(--text-soft)]">Cancel</button>
            <button
...
function FundDetailDialog({ familyId, fund, onClose }: { familyId: string; fund: FamilyFund; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-[var(--bg)] p-6 text-[var(--text)] shadow-xl">
        <div className="flex items-start justify-between gap-4">
          <h2 className="font-display text-xl">{fund.name}</h2>
          <button onClick={onClose} className="text-[var(--text-soft)] hover:text-[var(--text)]" aria-label="Close">✕</button>
        </div>
        ...
        <form onSubmit={submit} className="mt-4 rounded-lg bg-white p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Record a contribution</p>
          <div className="mt-2 grid grid-cols-2 gap-3">
              ...
              {memberId ? (
                ...
                <>
                  <input
                    ...
                    className="w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
                  />
                  {memberResults && memberResults.length > 0 && (
                    <ul className="mt-1 max-h-28 divide-y divide-[var(--border-soft)] overflow-y-auto rounded-lg border border-[var(--border)]">
                      {memberResults.map((m) => (
                        <li key={m.id}>
                          <button
                            type="button"
            ...
            <input
              ...
              className="rounded-lg border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
            />
            <select
              value={method}
          ...
          <button
            ...
            className="mt-3 rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {contributions?.length === 0 && (
                <tr><td colSpan={5} className="px-3 py-4 text-center text-[var(--text-soft)]">No contributions yet.</td></tr>
              )}
              {contributions?.map((c) => (
                <tr key={c.id} className="border-b border-[var(--border)] last:border-b-0">
                  <td className="px-3 py-2 font-medium">{c.member_name}</td>
                  <td className="px-3 py-2 text-[var(--text-soft)]">{c.payment_method}</td>
                  <td className="px-3 py-2 font-mono text-xs text-[var(--text-soft)]">{c.receipt_number}</td>
...
function AssignOfficerPanel({ familyId, family }: { familyId: string; family?: import("@/types/family").Family }) {
  ...
  return (
    <div className="mb-6 rounded-lg bg-[var(--bg)] p-4">
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <input
          ...
          className="flex-1 rounded-lg border border-[var(--border)] bg-white px-3 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
        />
      </div>
      {memberResults && memberResults.length > 0 && (
        <ul className="mt-2 max-h-28 divide-y divide-[var(--border-soft)] overflow-y-auto rounded-lg bg-white">
          {memberResults.map((m) => (
            <li key={m.id} className="flex items-center justify-between px-3 py-1.5 text-sm">
              ...
              <button
                ...
                className="rounded-lg border border-[var(--border)] px-2 py-1 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)]"
              >
                Assign as {role}
              </button>
            </li>
...
function FamilyOfficerPositionsPanel({ familyId, isFamilyHead }: { familyId: string; isFamilyHead: boolean }) {
  ...
  return (
    <div className="mb-6 rounded-lg bg-[var(--bg)] p-4">
      ...
      {isLoading && <p className="mt-2 text-xs text-[var(--text-soft)]">Loading…</p>}
      {positions && positions.length === 0 && <p className="mt-2 text-xs text-[var(--text-soft)]">No positions recorded yet.</p>}
      {positions && positions.length > 0 && (
        <ul className="mt-2 divide-y divide-[var(--border-soft)] rounded-lg bg-white">
          {positions.map((p) => (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {usingCustomTitle && (
              <input
                ...
                className="rounded-lg border border-[var(--border)] bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
              />
            )}
            <input
              value={query}
...
function FuneralExpensesPanel({ familyId }: { familyId: string }) {
  ...
  return (
    <section className="mt-8 rounded-lg border border-[var(--border)] bg-white p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-mono text-[11px] font-medium uppercase tracking-[0.16em] text-[var(--text-soft)]">
        ...
        <div className="flex shrink-0 gap-2">
          {funeralId && (isFinanceOfficer || canRecord) && (
            <button
              ...
              className="flex items-center rounded-lg border border-[var(--border)] px-4 py-2 text-sm font-medium hover:border-[var(--ink)]"
            >
              Download PDF
            </button>
          )}
          {canRecord && (
            <button
              ...
              disabled={!funeralId}
              className="rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
            >
              Record a purchase
            </button>
          )}
      ...
      <select
        ...
        className="mt-3 w-full max-w-sm rounded-lg border border-[var(--border)] px-3 py-2 text-sm"
      >
      ...
      {!funeralId && (
        <p className="mt-3 text-sm text-[var(--text-soft)]">Choose a funeral above to see and record its expenses.</p>
      )}

      {funeralId && (
        <>
          {isError && (
            <div className="mt-4 rounded-lg border border-dashed border-[var(--clay-red)] p-4 text-center text-sm text-[var(--clay-red)]">
              Not permitted to view this family's expenses.
            </div>
          )}

          {summary && (
            <div className="mt-4 grid grid-cols-3 gap-3 rounded-lg bg-[var(--bg)] p-3 text-center text-sm">
              <div>
                <p className="text-xs text-[var(--gold)]">Pending</p>
                <p className="font-mono text-lg font-medium">{formatCedis(summary.pending.total)}</p>
                <p className="text-xs text-[var(--text-soft)]">{summary.pending.count} item(s)</p>
              </div>
              <div>
                ...
                <p className="text-xs text-[var(--text-soft)]">{summary.approved.count} item(s)</p>
              </div>
              <div>
                ...
                <p className="text-xs text-[var(--text-soft)]">{summary.rejected.count} item(s)</p>
              </div>
            </div>
          )}
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-[var(--border)]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading && <tr><td colSpan={7} className="px-3 py-4 text-center text-[var(--text-soft)]">Loading…</td></tr>}
                {expenses?.length === 0 && <tr><td colSpan={7} className="px-3 py-4 text-center text-[var(--text-soft)]">No expenses recorded yet.</td></tr>}
                {expenses?.map((e) => (
                  <tr key={e.id} className="border-b border-[var(--border)] last:border-b-0">
                    ...
                    <td className="px-3 py-2 text-xs text-[var(--text-soft)]">{e.date_purchased}</td>
                    <td className="px-3 py-2 text-[var(--text-soft)]">{e.paid_by_member_name ?? "—"}</td>
                    ...
                    <td className="px-3 py-2">
                      ...
                      {e.status === "rejected" && e.rejection_reason && (
                        <p className="mt-0.5 text-xs text-[var(--text-soft)]">{e.rejection_reason}</p>
                      )}
                      {e.status === "approved" && (
                        <PrintReceiptButton
                        ...
                        {e.status === "pending" && (
                          <div className="flex justify-end gap-1">
                            <button
                              ...
                              className="rounded-lg border border-[var(--forest)] px-2 py-1 text-xs font-medium text-[var(--forest)]"
                            >
                              Approve
                            </button>
                            <button
                              ...
                              className="rounded-lg border border-[var(--clay-red)] px-2 py-1 text-xs font-medium text-[var(--clay-red)]"
                            >
                              Reject
                            </button>
                          </div>
      ...
      <div className="font-body w-full max-w-sm rounded-lg bg-[var(--bg)] p-6 text-[var(--text)] shadow-xl">
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          ...
          <div>
            ...
            {paidById ? (
              <div className="mt-1 flex items-center justify-between rounded-lg border border-[var(--border)] px-3 py-2 text-sm">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={onRecord.isPending} className="rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
...
function FamilyFinancialOverviewCard({ familyId }: { familyId: string }) {
  ...
  return (
    <div className="mb-6 rounded-lg border-2 border-[var(--forest)] bg-white p-4">
      ...
      <div className="mt-2 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div>
          <p className="text-xs text-[var(--text-soft)]">Fund contributions</p>
