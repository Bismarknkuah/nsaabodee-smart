      ...
      <main className="px-8 py-8">
        {showForm && (
          <form
            ...
            className="mb-6 grid grid-cols-3 gap-3 rounded-lg bg-[var(--bg)] p-4"
          >
            <input
            ...
            <button
              ...
              className="col-span-3 rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
            >
        ...
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Platform Administrators</p>
        ...
        <ol className="mt-2 divide-y divide-[var(--border-soft)] border-y border-[var(--border)]">
          {admins?.map((a, i) => (
            <li key={a.id} className="flex items-center gap-3 py-3">
              ...
              <div>
                ...
                {a.email && <p className="text-xs text-[var(--text-soft)]">{a.email}</p>}
              </div>
            </li>
          ))}
        </ol>

        <p className="mt-10 font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">
          Community Administrators · {communityAdmins?.length ?? 0} across every community
        </p>
        {communityAdminsLoading && <p className="mt-2 text-sm text-[var(--text-soft)]">Loading…</p>}
        ...
        <ol className="mt-2 divide-y divide-[var(--border-soft)] border-y border-[var(--border)]">
          ...
          {communityAdmins?.length === 0 && (
            <li className="py-3 text-sm text-[var(--text-soft)]">No Community Admin accounts exist yet.</li>
          )}
        </ol>
      </main>
    </div>
