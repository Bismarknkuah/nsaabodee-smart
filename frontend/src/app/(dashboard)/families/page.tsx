...
const STATUS_STYLE: Record<Family["status"], string> = {
  ...
  deactivated: "bg-[var(--surface)] text-[var(--text-soft)]",
    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="mx-auto flex max-w-6xl items-center gap-4 px-6 py-5 sm:px-10">
        <input
          ...
          className="w-64 border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 text-sm outline-none focus:border-[var(--forest)]"
        />
      ...
      <main className="mx-auto max-w-6xl px-6 pb-16 sm:px-10">
        {isLoading && <p className="text-sm text-[var(--text-soft)]">Loading families…</p>}
        {isError && (
              ...
              <div className="flex shrink-0 flex-wrap gap-2 text-xs">
                {family.status === "active" && canManageFamilies && (
                  <>
                    ...
                    <ActionButton onClick={() => openDialog("bereavedRep", family)}>
                      Deceased Rep
                    </ActionButton>
    ...
    <button
      ...
      className={`border px-3 py-1.5 font-medium transition-colors ${
        tone === "danger"
          ...
          : "border-[var(--border)] text-[var(--text)] hover:border-[var(--forest)] hover:text-[var(--forest)]"
      }`}
    >
      {children}
    </button>
