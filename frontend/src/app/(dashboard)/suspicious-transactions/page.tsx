...
export default function SuspiciousTransactionsPage() {
  ...
  // Matches ai_features.views.SuspiciousTransactionFlagViewSet.partial_update
  // exactly — Auditor sees every flag (broad Finance Oversight nav
  // access) but never decides one, the same "reviews, doesn't
  // execute" split already used for payment reversals.
  const canReview = Boolean(
    currentUser?.is_superuser
    || (currentUser?.role && ["community_admin", "chairman", "secretary", "treasurer", "financial_secretary"].includes(currentUser.role))
  );
      ...
      <main className="px-8 py-8">
        ...
        <div className="mt-6">
          ...
          <ul className="space-y-3">
            {data?.map((flag) => (
              <li key={flag.id} className="border border-[var(--border)] bg-white p-4" style={{ borderLeft: `3px solid ${STATUS_ACCENT[flag.review_status]}` }}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--text-soft)]">{REASON_LABEL[flag.reason]}</p>
                  ...
                  {flag.review_status === "unreviewed" && canReview && (
                    <div className="flex shrink-0 gap-2">
                      <button
                        onClick={() => review.mutate({ id: flag.id, reviewStatus: "dismissed" })}
                        className="border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--ink)]"
                      >
                        Dismiss
                      </button>
                      <button
