  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <header className="border-b border-[var(--border)] bg-[var(--card)] px-8 py-6">
        <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Community Administration</p>
      ...
      <div className="flex flex-wrap items-center gap-4 px-8 py-4">
        <div className="flex border border-[var(--border)] bg-white text-xs">
          {(["daily", "weekly", "monthly", "annual"] as Period[]).map((p) => (
            <button
              key={p}
              onClick={() => setPeriod(p)}
              className={`px-4 py-1.5 font-mono font-medium uppercase tracking-wide ${period === p ? "bg-[var(--ink)] text-white" : "text-[var(--text-soft)] hover:bg-[var(--bg)]"}`}
            >
              {p}
            </button>
          ))}
        ...
        {(period === "daily" || period === "weekly") && (
          <input
            ...
            className="border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
          />
        )}
      </div>
        ...
        <section className="lg:col-span-2">
          <div className="mb-3 flex justify-end">
            <button
              ...
              className="border border-[var(--ink)] px-3 py-1.5 font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text)] hover:bg-[var(--ink)] hover:text-white"
            >
              Download PDF statement
            </button>
          </div>
          {active.isLoading && <p className="text-sm text-[var(--text-soft)]">Loading…</p>}
      ...
      {data && (
        <>
          ...
          {Object.keys(data.by_category).length > 0 ? (
            <table className="mt-3 w-full border-collapse text-sm">
              <tbody>
                {Object.entries(data.by_category).map(([category, amount]) => (
                  <tr key={category} className="border-b border-[var(--border)] last:border-0">
                    <td className="py-2 capitalize text-[var(--text-soft)]">{category.replace("_", " ")}</td>
            ...
            <p className="mt-3 text-sm text-[var(--text-soft)]">Nothing recorded for this period.</p>
          )}
...
function CollectionsReportCard({ report }: { report: CollectionsReport }) {
  const giftCashTotal = Number(report.gift_cash?.total ?? 0);
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      ...
      {/* 'The funeral committee should have access to all the money
          paid except the donations' — gift_cash is deliberately absent
          entirely for committee roles (see the type's own comment);
          this section simply doesn't render for them, rather than
          crashing on a field that was never sent. */}
      {report.gift_cash && (
      ...
      <SectionCard title="Combined cash in hand" eyebrow={`${report.receipts_issued} receipt(s) issued`} accent="gold">
        <KpiTile
          label="Total"
          value={formatCedis((Number(report.contributions.total) + giftCashTotal).toString())}
    ...
    <table className="mt-3 w-full border-collapse text-xs">
      <tbody>
        {Object.entries(breakdown).map(([method, amount]) => (
          <tr key={method} className="border-b border-[var(--border)] last:border-0">
            <td className="py-1.5 capitalize text-[var(--text-soft)]">{method.replace("_", " ")}</td>
    ...
    <div className="lg:col-span-2">
      <SectionCard title="Family statement" eyebrow="The abusuapanin's view" accent="forest">
        <select
          ...
          className="w-full max-w-sm border border-[var(--border)] px-3 py-2 text-sm"
        >
        ...
        {statement && (
          <div className="mt-4 space-y-4">
            <div className="grid grid-cols-2 gap-px border border-[var(--border)] bg-[var(--rule)] sm:grid-cols-4">
            ...
            {statement.guest_ledger && statement.town_leaders_ledger ? (
              <div className="grid grid-cols-2 gap-px border border-[var(--border)] bg-[var(--rule)]">
              ...
              <p className="border border-dashed border-[var(--border)] p-3 text-xs text-[var(--text-soft)]">
            ...
            {statement.donation_receivers && statement.donation_receivers.length > 0 && (
              <div>
                <p className="font-mono text-[10px] font-medium uppercase tracking-[0.16em] text-[var(--text-soft)]">
                  Donation accountability — who received what
                </p>
                <table className="mt-2 w-full border-collapse border border-[var(--border)] text-sm">
                  <tbody>
                    {statement.donation_receivers.map((r) => (
                      <tr key={r.member_id} className="border-b border-[var(--border)] last:border-0">
            ...
            <button
              ...
              className="border border-[var(--border)] px-3 py-1.5 font-mono text-[11px] font-medium uppercase tracking-wide hover:border-[var(--forest)] hover:text-[var(--forest)]"
            >
              Download PDF statement
            </button>
          </div>
...
function OutstandingMembersPanel() {
  ...
  const hasIndividualNames = Array.isArray(data?.members);

  return (
    <SectionCard
      title="Outstanding members"
      eyebrow={hasIndividualNames ? `${data?.members?.length ?? 0} listed` : `${data?.outstanding_member_count ?? 0} total`}
      accent="clay"
    >
      <p className="-mt-2 mb-3 text-sm text-[var(--text-soft)]">
      ...
      {hasIndividualNames ? (
        <ul className="max-h-64 divide-y divide-[var(--border-soft)] overflow-y-auto border-y border-[var(--border)]">
          {data?.members?.map((m) => (
          ...
          {data?.members?.length === 0 && <li className="py-2 text-sm text-[var(--text-soft)]">Nobody owes anything right now.</li>}
        </ul>
      ) : (
        <div className="border-y border-[var(--border)] py-4 text-sm">
          <p>{data?.outstanding_member_count ?? 0} members currently owe money across open funerals.</p>
          {data?.total_owed && <p className="mt-1 font-mono text-[var(--clay-red)]">{formatCedis(data.total_owed)} total</p>}
        </div>
      )}
    </SectionCard>
  );
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between border-b border-[var(--border)] pb-1 text-sm">
