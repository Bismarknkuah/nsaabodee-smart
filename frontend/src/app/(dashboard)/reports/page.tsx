        ...
        <div className="flex border border-[var(--border)] bg-white text-xs">
          {(["daily", "weekly", "monthly", "annual"] as Period[]).map((p) => (
            <button
              key={p}
              onClick={() => setPeriod(p)}
              className={`px-4 py-1.5 font-mono font-medium uppercase tracking-wide ${period === p ? "bg-[var(--ink)] text-white" : "text-[var(--text-soft)] hover:bg-[var(--surface)]"}`}
            >
              {p}
            </button>
          ))}
...
function OutstandingMembersPanel() {
  ...
  return (
    ...
    >
      ...
      {hasIndividualNames ? (
        <ul className="max-h-64 divide-y divide-[var(--rule)] overflow-y-auto border-y border-[var(--border)]">
          {data?.members?.map((m) => (
