...
export default function FuneralsListPage() {
  ...
  // 'POST create (Community Admin+, immediately active...)' — every
  // other role reaching this broadly-shared page (Treasurer, Auditor,
  // Collector, every family officer) always got a 403 on this action.
  const isCommunityAdmin = Boolean(currentUser?.is_superuser || currentUser?.role === "community_admin");
  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="mx-auto flex max-w-6xl gap-1 border-b border-[var(--border)] px-6 py-3 sm:px-10">
        {(["pending_approval", "active", "closed"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-1.5 font-mono text-xs font-medium uppercase tracking-wide ${
              tab === t ? "bg-[var(--ink)] text-white" : "text-[var(--text-soft)] hover:bg-[var(--surface)]"
            }`}
          >
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-6 py-6 sm:grid-cols-2 sm:px-10 xl:grid-cols-3">
        {isLoading && <p className="text-sm text-[var(--text-soft)]">Loading funerals…</p>}
        {!isLoading && funerals?.length === 0 && (
          <div className="col-span-full border border-dashed border-[var(--border)] px-6 py-10 text-center">
...
function PendingFuneralCard({ funeral }: { funeral: FuneralEvent }) {
  ...
  const currentUser = useAuthStore((s) => s.user);
  // '/api/funerals/{id}/approve-opening/ POST (Secretary/Chairman/Admin)'
  const canApproveOpening = Boolean(
        ...
        <div className="min-w-0 flex-1">
          ...
          {progress && (
            <div className="mt-3">
              ...
              {progress.approvals.length > 0 && (
                <ul className="mt-1 text-xs text-[var(--text-soft)]">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              className="rounded-sm border border-[var(--border)] px-3 py-1.5 text-xs font-medium hover:border-[var(--clay-red)] hover:text-[var(--clay-red)]"
            >
              Reject
            </button>
          </div>
...
function FuneralCard({ funeral }: { funeral: FuneralEvent }) {
  ...
  return (
    <Link
      ...
      className="block border border-[var(--border)] bg-white p-5 transition-shadow hover:shadow-md"
    >
      <div className="flex items-start gap-3">
        <span
          aria-hidden
