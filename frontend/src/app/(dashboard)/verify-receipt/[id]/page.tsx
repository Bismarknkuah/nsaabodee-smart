...
export default function VerifyReceiptPage() {
  ...
  return (
    <div className="font-body flex min-h-screen items-center justify-center bg-[var(--bg)] px-6 text-[var(--text)]">
      <div className="w-full max-w-sm border-2 border-[var(--ink)] bg-white p-6 text-center">
        {isLoading && <p className="text-sm text-[var(--text-soft)]">Checking receipt…</p>}
        {error && (
          <>
            <p className="font-display text-2xl text-[var(--clay-red)]">Not verified</p>
            <p className="mt-2 text-sm text-[var(--text-soft)]">{(error as Error).message}</p>
          </>
        )}
        {data && (
          <>
            ...
            <p className="mt-1 text-xs text-[var(--text-soft)]">Receipt {data.receipt_number}</p>

            <dl className="mt-6 space-y-2 border-t border-[var(--border)] pt-4 text-left text-sm">
              <div className="flex justify-between">
                <dt className="text-[var(--text-soft)]">Paid by</dt>
                <dd className="font-medium">{data.member_name}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[var(--text-soft)]">Received by (collector)</dt>
                <dd className="font-medium">{data.collector_name || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[var(--text-soft)]">For</dt>
                <dd className="font-medium">{data.deceased_name}&apos;s funeral</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[var(--text-soft)]">Method</dt>
                <dd className="font-medium capitalize">{data.method.replace("_", " ")}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[var(--text-soft)]">When</dt>
