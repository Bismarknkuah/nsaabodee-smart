    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        {wallets && wallets.length === 0 && (
          <p className="text-sm text-[var(--text-soft)]">No wallet exists yet — one is created automatically the first time a matching payment is recorded.</p>
        )}
  ...
  return (
    <div className="border-2 border-[var(--ink)] bg-white">
      <button onClick={onToggle} className="flex w-full items-center justify-between px-5 py-4 text-left">
        <div>
          <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">{wallet.scope_display}</p>
      ...
      {expanded && (
        <div className="border-t border-[var(--border)] px-5 py-4">
          <p className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Withdraw</p>
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="Amount"
              className="w-32 rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm"
            />
            <input
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder="What this withdrawal is for…"
              className="min-w-[16rem] flex-1 rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm"
            />
            <button
              onClick={() => withdraw.mutate()}
              disabled={withdraw.isPending || !amount || !note.trim()}
              className="rounded-lg bg-[var(--clay-red)] px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50"
            >
          ...
          <p className="mb-2 mt-5 text-xs font-medium uppercase tracking-wide text-[var(--text-soft)]">Transaction history</p>
          <ul className="max-h-72 divide-y divide-[var(--border-soft)] overflow-y-auto border-y border-[var(--border)]">
            {transactions?.map((t) => (
            ...
            {transactions?.length === 0 && <li className="py-2 text-sm text-[var(--text-soft)]">No transactions yet.</li>}
          </ul>
        </div>
      )}
    </div>
