  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        {report && (
          <>
            <div className="grid grid-cols-2 gap-px border border-[var(--border)] bg-[var(--rule)] sm:grid-cols-3">
            ...
            <div className="mt-6 border border-[var(--border)] bg-white p-5">
              <h2 className="font-display text-xl">Paid revenue by community</h2>
              {report.by_community.length === 0 ? (
                <p className="mt-3 text-sm text-[var(--text-soft)]">Nothing paid yet.</p>
              ) : (
                <table className="mt-3 w-full border-collapse text-sm">
                  <tbody>
                    {report.by_community.map((row) => (
                      <tr key={row.community_name} className="border-b border-[var(--border)] last:border-0">
