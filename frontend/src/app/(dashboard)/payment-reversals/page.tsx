...
export default function PaymentReversalsPage() {
  ...
  // Matches funerals.services exactly — REVERSAL_REQUEST_ROLES and
  // APPROVAL_ROLES are genuinely different sets (Auditor can see this
  // whole page via FINANCE_OVERSIGHT nav access but can do neither;
  // Treasurer/Financial Secretary can request but never approve their
  // own or anyone else's). Collector can request too now, but only
  // for a payment they themselves collected — the backend is the real
  // enforcement of that narrower scope; showing the form here just
  // means a Collector isn't blocked from even trying to flag their
  // own mistake.
  const canRequest = Boolean(
    currentUser?.is_superuser
    || (currentUser?.role && ["treasurer", "financial_secretary", "secretary", "chairman", "community_admin", "collector"].includes(currentUser.role))
  );
  const canApprove = Boolean(
    currentUser?.is_superuser
    || (currentUser?.role && ["secretary", "chairman", "community_admin"].includes(currentUser.role))
  );
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 w-full rounded-sm border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]"
              />
              <p className="mt-1 text-xs text-[var(--text-soft)]">This becomes part of the permanent audit trail.</p>
            </div>
        ...
        {decided.length > 0 && (
          <section className="mt-8">
            ...
            <ul className="mt-3 space-y-2">
              {decided.map((r) => (
                <li key={r.id} className="rounded-sm border border-[var(--border)] bg-white p-3 text-sm">
