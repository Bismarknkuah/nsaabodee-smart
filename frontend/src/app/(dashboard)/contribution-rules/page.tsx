...
export default function ContributionRulesPage() {
  ...
  // 'Anyone in the community can view the rules; Community Admin+ or
  // the Secretary can change them.' Matches
  // contribution_rules.permissions.CONTRIBUTION_RULE_MANAGER_ROLES
  // exactly — the page itself is now visible more broadly (Treasurer,
  // Financial Secretary, Auditor all reasonably want to see the rates
  // driving what they collect/audit), but only these three roles ever
  // saw an edit form that could actually succeed.
  const canManageRules = Boolean(
    currentUser?.is_superuser
    || (currentUser?.role && ["community_admin", "secretary", "chairman"].includes(currentUser.role))
  );
  ...
  if (isLoading || !rules) return <div className="min-h-screen bg-[var(--bg)] px-8 py-6 text-sm text-[var(--text-soft)]">Loading rules…</div>;
  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="grid gap-6 px-8 py-8 lg:grid-cols-2">
          ...
          >
            <div>
              ...
              <input
                ...
                className="mt-1 block w-28 rounded-lg border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
              />
            </div>
            <div>
            ...
            <button
              ...
              className="rounded-lg bg-[var(--forest)] px-4 py-1.5 text-sm font-medium text-white disabled:opacity-60"
            >
              Save
            </button>
          </form>
        ...
        {/* Family tier rates */}
        <section className="rounded-lg border-2 border-[var(--gold)] bg-white p-5">
          ...
          >
            <div>
              ...
              <input type="number" min="0.01" step="0.01" value={headAmount} onChange={(e) => setHeadAmount(e.target.value)}
                ...
                className="mt-1 w-full rounded-lg border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]" />
            </div>
            <div>
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full rounded-lg bg-[var(--gold)] px-4 py-1.5 text-sm font-medium text-white disabled:opacity-60"
              >
                Save
              </button>
            </div>
            ...
            {[
              ...
              <div key={f.label}>
                ...
                <input
                  ...
                  className="mt-1 block w-20 rounded-lg border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
                />
              </div>
            ))}
            <button
        ...
        <section className="border border-[var(--border)] bg-white p-5">
          ...
          <p className="mt-1 text-sm text-[var(--text-soft)]">By member status, community-wide.</p>
            ...
            {rules.member_status_exemptions.map((rule) => (
              <li key={rule.status} className="flex items-center justify-between py-2 text-sm">
                <span>
                  ...
                  {rule.is_default && <span className="ml-2 text-xs text-[var(--text-soft)]">(default)</span>}
                </span>
                <label className="flex items-center gap-2">
                  <span className="text-xs text-[var(--text-soft)]">{rule.is_exempt ? "Exempt" : "Pays"}</span>
                  <input
                    type="checkbox"
        ...
        {/* Preview */}
        <section className="border border-[var(--border)] bg-white p-5 lg:col-span-2">
          ...
          <div className="mt-3 flex flex-wrap items-end gap-3">
            <div>
              ...
              <select
                ...
                className="mt-1 block w-56 rounded-lg border border-[var(--border)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
              >
            ...
            <button
              ...
              className="rounded-lg bg-[var(--ink)] px-4 py-1.5 text-sm font-medium text-white disabled:opacity-60"
            >
              Preview
            </button>
          </div>
          ...
          {preview.data && (
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-lg bg-[var(--forest-soft)] p-3 text-sm">
              ...
              <div className="rounded-lg bg-[var(--gold-soft)] p-3 text-sm">
