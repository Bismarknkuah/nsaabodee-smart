        ...
        {data && (
          <>
            <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
              <div>
                <p className="text-xs text-[var(--text-soft)]">Current rate per funeral</p>
              ...
              {isChief && (
                ...
                >
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-soft)]">Set a new rate (only you can do this)</label>
                    <input
            ...
            {data.members.length === 0 && (
              <p className="text-sm text-[var(--text-soft)]">No one has been transferred to the Town Elders ledger yet.</p>
            )}
                ...
                <li key={m.member_id} className="flex items-center justify-between py-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <p className="font-mono text-sm">{formatCedis(m.collected_total)} <span className="text-[var(--text-soft)]">/ {formatCedis(m.expected_total)}</span></p>
                    ...
                    {canManageMembership && (
                      <button
                        ...
                        className="shrink-0 rounded-lg border border-[var(--clay-red)] px-2 py-1 text-xs text-[var(--clay-red)] disabled:opacity-50"
                      >
                        Remove
                      </button>
                    )}
