  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        {pending.length > 0 && (
          <>
            <p className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">
              Awaiting approval · {pending.length}
            </p>
            <ol className="mb-8 divide-y divide-[var(--border-soft)] border-y-2 border-[var(--gold)]">
              {pending.map((n) => (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 gap-2">
                      <button
                        ...
                        className="rounded-lg border border-[var(--forest)] px-3 py-1.5 text-xs text-[var(--forest)] disabled:opacity-50"
                      >
                        Approve
                      </button>
                      <button
                        ...
                        disabled={decide.isPending}
                        className="rounded-lg border border-[var(--clay-red)] px-3 py-1.5 text-xs text-[var(--clay-red)] disabled:opacity-50"
                      >
                        Reject
                      </button>
                    </div>
        ...
        {decided.length > 0 && (
          <>
            <p className="mb-3 font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-[var(--text-soft)]">Decided</p>
            <ol className="divide-y divide-[var(--border-soft)] border-y border-[var(--border)]">
              {decided.map((n) => (
                <li key={n.id} className="flex items-center justify-between py-3">
                  <div>
                    <p className="text-sm font-medium">{n.member_name}</p>
                    <p className="text-xs text-[var(--text-soft)]">{TYPE_LABEL[n.collector_type]}</p>
                  </div>
                  <span
...
function NominateForm({ nominatableTypes, onDone }: { nominatableTypes: CollectorNomination["collector_type"][]; onDone: () => void }) {
  ...
  return (
    <form
      ...
      className="mb-8 grid gap-3 rounded-lg bg-[var(--bg)] p-4 sm:grid-cols-3"
    >
      <div>
        <label className="block text-xs font-medium text-[var(--text-soft)]">Category</label>
        <select
          value={collectorType}
      ...
      <div className="sm:col-span-2">
        <label className="block text-xs font-medium text-[var(--text-soft)]">Search for a member</label>
        <input
          value={search}
        ...
        {members && members.length > 0 && search.trim().length >= 2 && (
          <ul className="mt-1 max-h-40 overflow-y-auto rounded-lg border border-[var(--border)] bg-white">
            {members.slice(0, 8).map((m) => (
              <li key={m.id}>
                <button
                  type="button"
                  onClick={() => { setMemberId(m.id); setSearch(m.full_name); }}
                  className={`w-full px-3 py-2 text-left text-sm hover:bg-[var(--bg)] ${memberId === m.id ? "bg-[var(--bg)] font-medium" : ""}`}
                >
                  {m.full_name}
                </button>
              </li>
      ...
      <button
        ...
        className="rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60 sm:col-span-3"
      >
