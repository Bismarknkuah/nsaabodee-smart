  ...
  return (
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        ...
        {error && (
          <div className="rounded-sm border border-dashed border-[var(--border)] p-6 text-center text-sm text-[var(--text-soft)]">
            {error.message}
          </div>
        )}
                ...
                {data.by_funeral.map((f) => (
                  <li key={f.funeral_id} className="flex items-center justify-between py-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-[var(--text-soft)]">{f.donation_count} donation{f.donation_count === 1 ? "" : "s"}</p>
                    </div>
                  </li>
                ))}
              </ul>
