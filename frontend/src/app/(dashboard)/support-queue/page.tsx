    ...
    <div className="font-body min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="px-8 py-8">
        <div className="flex flex-wrap items-center gap-4">
          <div className="grid grid-cols-2 gap-px border border-[var(--border)] bg-[var(--rule)] sm:max-w-xs">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-[var(--border)] px-3 py-2 text-sm">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[22rem_1fr]">
          <div className="border border-[var(--border)] bg-white">
            {isLoading && <p className="p-4 text-sm text-[var(--text-soft)]">Loading…</p>}
            {tickets?.length === 0 && <p className="p-4 text-sm text-[var(--text-soft)]">Nothing here.</p>}
            {tickets?.map((t) => (
              <button
                key={t.id}
                onClick={() => setActiveTicketId(t.id)}
                className={`block w-full border-b border-[var(--border)] px-4 py-3 text-left ${t.id === activeTicketId ? "bg-[var(--surface)]" : "hover:bg-[var(--surface)]"}`}
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] font-medium uppercase tracking-wide" style={{ color: STATUS_ACCENT[t.status] }}>
                    {t.status.replace(/_/g, " ")}
                  </span>
                  <span className="text-[10px] text-[var(--text-soft)]">{t.priority}</span>
                </div>
                <p className="text-sm font-medium">{t.subject}</p>
                <p className="text-xs text-[var(--text-soft)]">{t.submitted_by_username}{t.community_name ? ` · ${t.community_name}` : ""}</p>
              </button>
            ))}
          </div>

          <div>
            {activeTicket ? <QueueTicketPanel key={activeTicket.id} ticket={activeTicket} /> : <p className="text-sm text-[var(--text-soft)]">Select a ticket.</p>}
          </div>
        </div>
      </main>
    </div>
...
function QueueTicketPanel({ ticket }: { ticket: { id: string; subject: string; description: string; status: string; submitted_by_username: string } }) {
  ...
  return (
    <div className="flex h-[32rem] flex-col border border-[var(--border)] bg-white">
      <div className="flex items-start justify-between gap-4 border-b border-[var(--border)] p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-soft)]">{ticket.description}</p>
          <p className="mt-1 text-xs text-[var(--text-soft)]">From {ticket.submitted_by_username}</p>
        </div>
        <select
          value={ticket.status}
          onChange={(e) => updateStatus.mutate(e.target.value)}
          className="shrink-0 border border-[var(--border)] px-2 py-1.5 text-xs"
        >
          <option value="open">Open</option>
          <option value="in_progress">In Progress</option>
          <option value="resolved">Resolved</option>
      ...
      <div ref={scrollRef} className="flex-1 space-y-2 overflow-y-auto p-4">
        {messages?.map((m) => (
          <div key={m.id} className="border border-[var(--border)] p-2 text-sm">
            <p className="font-mono text-[10px] uppercase tracking-wide text-[var(--text-soft)]">{m.sender_username}</p>
            <p>{m.content}</p>
          </div>
        ))}
      </div>
      ...
      >
        <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Reply…" className="flex-1 border border-[var(--border)] px-3 py-2 text-sm outline-none focus:border-[var(--forest)]" />
