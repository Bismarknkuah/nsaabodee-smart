...
const DEMO_ROLES: { value: string; label: string }[] = [
  ...
  { value: "bereaved_rep", label: "Deceased Rep" },
  { value: "notification_officer", label: "Notification Officer" },
  { value: "arrears_collector", label: "Arrears Collector" },
  { value: "community_registration_desk", label: "Community Registration Desk" },
  { value: "family_registration_officer", label: "Family Registration Officer" },
  { value: "family_arrears_officer", label: "Family Arrears Officer" },
  { value: "town_registration_officer", label: "Town Registration Officer" },
  { value: "town_elders_arrears_officer", label: "Town Elders Arrears Officer" },
];
    ...
    <div className="font-body flex min-h-screen">
      ...
      <div className="grid w-full max-w-4xl gap-8 lg:grid-cols-2">
        <div className="border border-[var(--border)] bg-white p-8">
          <Link href="/" className="inline-flex items-center gap-1 font-mono text-xs uppercase tracking-widest text-[var(--text-soft)] hover:text-[var(--primary)]">
            ← Back to homepage
          </Link>
          <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.2em] text-[var(--text-soft)]">Folio · Sign In</p>
          <h1 className="font-display mt-1 text-3xl">Welcome back</h1>

          <div className="mt-5 flex gap-1 border border-[var(--border)] bg-[var(--bg)] p-1 text-xs font-medium">
            <button
              type="button"
              onClick={() => setMode("password")}
              className={`flex-1 py-1.5 ${mode === "password" ? "bg-white shadow-sm" : "text-[var(--text-soft)]"}`}
            >
              Password
            </button>
            <button
              ...
              className={`flex-1 py-1.5 ${mode === "phone" ? "bg-white shadow-sm" : "text-[var(--text-soft)]"}`}
            >
              Phone Code
            </button>
          </div>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <div>
              <label className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">Username</label>
              <input
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                autoFocus
                autoComplete="username"
                className="mt-1 w-full border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 text-sm outline-none focus:border-[var(--primary)]"
              />
            </div>
            <div>
              <div className="flex items-baseline justify-between">
                <label className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">Password</label>
                <button type="button" onClick={() => setMode("forgot")} className="text-xs text-[var(--primary)] hover:underline">
                  Forgot password?
                </button>
              </div>
              <div className="relative mt-1">
                <input
                  ...
                  className="w-full border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 pr-14 text-sm outline-none focus:border-[var(--primary)]"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((v) => !v)}
                  className="absolute inset-y-0 right-0 text-xs font-medium text-[var(--text-soft)] hover:text-[var(--primary)]"
                  tabIndex={-1}
                >
            ...
            <button
              ...
              className="w-full bg-[var(--primary)] px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60"
            >
          ...
          <form onSubmit={otpSent ? verifyCode : sendCode} className="mt-6 space-y-4">
            <div>
              <label className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">Phone number</label>
              <input
                value={phoneNumber}
                ...
                disabled={otpSent}
                autoFocus
                className="mt-1 w-full border-0 border-b-2 border-[var(--border)] bg-transparent px-0 py-2 text-sm outline-none focus:border-[var(--primary)] disabled:text-[var(--text-soft)]"
              />
            </div>

            {otpSent && (
              <div>
                <label className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">One-time code</label>
                <input
                  value={otpCode}
                ...
                />
                {otpDemoCode && (
                  <p className="mt-2 rounded-lg border border-dashed border-[var(--gold)] bg-[var(--gold)]/10 px-3 py-2 text-xs text-[var(--text)]">
                ...
                <button
                  ...
                  className="mt-1 text-xs text-[var(--text-soft)] hover:text-[var(--primary)]"
                >
          ...
          {mode === "forgot" && (
            <div className="mt-6">
              ...
              <form onSubmit={resetSent ? submitReset : sendResetCode} className="mt-4 space-y-4">
                ...
                {resetSent && (
                  <>
                    ...
                    <div>
                      <label className="font-mono text-[11px] font-medium uppercase tracking-wide text-[var(--text-soft)]">New password</label>
                      <input
                        type="password"
                    ...
                    <button
                      ...
                      className="text-xs text-[var(--text-soft)] hover:text-[var(--primary)]"
                    >
                      Use a different number
                    </button>
                  </>
                ...
                {resetDone && <p className="text-sm" style={{ color: "var(--primary)" }}>Password reset. Signing you in…</p>}
              ...
              <button
                ...
                className="mt-4 text-xs text-[var(--text-soft)] hover:text-[var(--primary)]"
              >
                ← Back to sign in
              </button>
            </div>
        ...
        <div className="border border-dashed border-[var(--violet)] p-8" style={{ backgroundColor: "var(--violet-soft)" }}>
          ...
          <p className="mt-2 text-sm text-[var(--text-soft)]">
            No account needed. Pick a role to see exactly what that person sees, with
            real demo data, a real family, a real funeral in progress.
          </p>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {DEMO_ROLES.map((r) => (
              <button
                ...
                className="border border-[var(--border)] bg-white px-3 py-2 text-left text-xs font-medium hover:border-[var(--violet)] hover:text-[var(--violet)] disabled:opacity-60"
              >
