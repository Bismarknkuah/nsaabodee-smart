  ...
  return (
    <section className="mt-6 rounded-lg border border-[var(--rule)] bg-white p-5">
      ...
      {positions && positions.length > 0 && (
        <ul className="mt-3 divide-y divide-[var(--border-soft)] rounded-lg border border-[var(--rule)]">
          {positions.map((p) => (
      ...
      <div className="mt-4 border-t border-[var(--rule)] pt-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <select
            ...
            className="rounded-lg border border-[var(--rule)] px-2 py-1.5 text-sm"
          >
          ...
          {usingCustomTitle && (
            <input
              ...
              className="rounded-lg border border-[var(--rule)] px-2 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
            />
          )}
          <input
            value={query}
            ...
            className="flex-1 rounded-lg border border-[var(--rule)] px-3 py-1.5 text-sm outline-none focus:border-[var(--forest)]"
          />
        </div>
        {memberResults && memberResults.length > 0 && (
          <ul className="mt-2 max-h-28 divide-y divide-[var(--border-soft)] overflow-y-auto rounded-lg border border-[var(--rule)]">
            {memberResults.map((m) => (
              <li key={m.id} className="flex items-center justify-between px-3 py-1.5 text-sm">
                ...
                <button
                  ...
                  className="rounded-lg border border-[var(--rule)] px-2 py-1 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)] disabled:opacity-50"
                >
                  Appoint
                </button>
              </li>
