...
import { useExpenses, useExpenseSummary, useFuneralLogisticsActions, useAddFundDeposit } from "@/lib/hooks/useFuneralLogistics";
...
export function ExpensePanel({ funeralId }: { funeralId: string }) {
  ...
  const addFundDeposit = useAddFundDeposit(funeralId);
  ...
  return (
    <section className="mt-6 rounded-lg border border-[var(--rule)] bg-white p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        {canRecord && (
          <div className="flex shrink-0 gap-2">
            <button onClick={() => setShowFundForm((s) => !s)} className="rounded-lg border border-[var(--rule)] px-4 py-2 text-sm font-medium hover:border-[var(--ink)]">
              {showFundForm ? "Cancel" : "Add fund"}
            </button>
            <button onClick={() => setShowForm((s) => !s)} className="rounded-lg border border-[var(--rule)] px-4 py-2 text-sm font-medium hover:border-[var(--ink)]">
      ...
      {summary && (Number(summary.total_deposited) > 0 || summary.expense_count > 0) && (
        <div className="mt-3 rounded-lg bg-[var(--bg)] p-3">
          <p className="text-xs font-medium uppercase tracking-wide text-[var(--ink-soft)]">Expense fund</p>
          <p className="mt-1 text-xs text-[var(--ink-soft)]">
            Recording a purchase never deducts from contributions or gifts — this is a separate,
            explicit fund the committee tops up on purpose, which purchases draw down against.
          </p>
          <div className="mt-2 flex flex-wrap gap-4 font-mono text-sm">
            <span className="text-[var(--forest)]">Deposited: {formatCedis(summary.total_deposited)}</span>
            <span className="text-[var(--clay-red)]">Spent: {formatCedis(summary.total_spent)}</span>
            <span style={{ color: Number(summary.balance) >= 0 ? "var(--forest)" : "var(--clay-red)" }}>
              Balance: {formatCedis(summary.balance)}
            </span>
          </div>
        </div>
      )}

      {showFundForm && canRecord && (
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!fundAmount) return;
            addFundDeposit.mutate(
              { amount: fundAmount, note: fundNote },
              { onSuccess: () => { setFundAmount(""); setFundNote(""); setShowFundForm(false); } }
            );
          }}
          className="mt-3 space-y-2 rounded-lg bg-[var(--bg)] p-3"
        >
          <div className="grid grid-cols-2 gap-2">
            <input
              type="number" min="0.01" step="0.01" value={fundAmount} onChange={(e) => setFundAmount(e.target.value)}
              placeholder="Amount to deposit"
              className="rounded-lg border border-[var(--rule)] bg-white px-2 py-1.5 text-xs outline-none focus:border-[var(--forest)]"
            />
            <input
              value={fundNote} onChange={(e) => setFundNote(e.target.value)}
              placeholder="Note (e.g. initial allocation)"
              className="rounded-lg border border-[var(--rule)] bg-white px-2 py-1.5 text-xs outline-none focus:border-[var(--forest)]"
            />
          </div>
          {addFundDeposit.isError && <p className="text-xs text-[var(--clay-red)]">{(addFundDeposit.error as Error).message}</p>}
          <button
            type="submit" disabled={addFundDeposit.isPending}
            className="rounded-lg bg-[var(--forest)] px-3 py-1.5 text-xs font-medium text-white disabled:opacity-60"
          >
            {addFundDeposit.isPending ? "Adding…" : "Add deposit"}
          </button>
        </form>
      )}
      ...
      {showForm && canRecord && (
        <form onSubmit={submit} className="mt-4 grid grid-cols-2 gap-3 rounded-lg bg-[var(--bg)] p-3">
          ...
          <div className="relative">
            ...
            {!buyerId && buyerResults && buyerResults.length > 0 && (
              <ul className="absolute z-10 mt-1 max-h-28 w-full overflow-y-auto rounded-lg border border-[var(--rule)] bg-white shadow-sm">
                {buyerResults.map((m) => (
                  <li key={m.id}>
                    <button type="button" onClick={() => { setBuyerId(m.id); setBuyerName(m.full_name); }}
                      className="block w-full px-3 py-1.5 text-left text-sm hover:bg-[var(--bg)]">
                      {m.full_name}
                    </button>
                  </li>
                ))}
          ...
          <button type="submit" disabled={recordExpense.isPending}
            className="col-span-2 rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              {canRecord && e.status !== "paid" && e.status !== "cancelled" && (
                <select
                  ...
                  className="ml-auto rounded-lg border border-[var(--rule)] px-2 py-0.5 text-xs"
                >
