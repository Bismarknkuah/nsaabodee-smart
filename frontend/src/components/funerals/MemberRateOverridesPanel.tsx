...
export function MemberRateOverridesPanel({ funeralId, deceasedFamilyId }: { funeralId: string; deceasedFamilyId: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-3 rounded-lg bg-[var(--bg)] p-3">
          ...
          <li key={m.id} className="flex items-center justify-between gap-2 text-sm">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <input
                ...
                className="w-24 rounded-lg border border-[var(--rule)] bg-white px-2 py-1 text-xs outline-none focus:border-[var(--forest)]"
              />
            </div>
          </li>
        ))}
