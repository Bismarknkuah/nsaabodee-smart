...
export function AttendancePanel({ funeralId }: { funeralId: string }) {
  ...
  return (
    <section className="mt-6 rounded-lg border border-[var(--forest)] bg-white p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg bg-[var(--forest-soft)] p-3">
        ...
        <div className="rounded-lg bg-[var(--bg)] p-3">
          ...
          <div className="mt-1 flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-[var(--rule)] px-3 py-2 text-sm"
            />
            <button
              onClick={() => { if (guestName.trim()) { recordAttendance.mutate({ guest_name: guestName.trim() }); setGuestName(""); } }}
              className="rounded-lg bg-[var(--forest)] px-3 py-2 text-sm font-medium text-white"
            >
              Add
            </button>
          </div>
      ...
      <ul className="mt-4 max-h-48 divide-y divide-[var(--border-soft)] overflow-y-auto">
