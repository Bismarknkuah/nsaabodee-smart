              ...
              <td className="px-4 py-3 text-right">
                {o.payment_status !== "paid" && (canRecordForOthers || o.member.id === own_member_id) && (
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() => router.push(`/pay-momo/${o.id}?balance=${o.balance}&label=${encodeURIComponent(o.member.full_name)}`)}
                    ...
                    <button
                      onClick={() => setPayFor(o)}
                      className="rounded-lg border border-[var(--rule)] px-3 py-1 text-xs font-medium hover:border-[var(--forest)] hover:text-[var(--forest)]"
                    >
                      Record payment
                    </button>
                  </div>
