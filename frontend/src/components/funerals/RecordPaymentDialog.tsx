...
export function RecordPaymentDialog({
  ...
  if (recordedPaymentId) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div className="font-body w-full max-w-sm rounded-lg bg-[var(--bg)] p-6 text-center text-[var(--ink)] shadow-xl">
          ...
          <div className="mt-4 flex justify-center gap-2">
            <button onClick={onClose} className="rounded-lg border border-[var(--rule)] px-4 py-2 text-sm">
              Done
            </button>
            <button
              onClick={printReceipt}
              className="rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white"
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body flex max-h-[85vh] w-full max-w-sm flex-col overflow-hidden rounded-lg bg-[var(--bg)] text-[var(--ink)] shadow-xl">
        ...
        {walletBalance > 0 && Number(obligation.balance) > 0 && (
          <div className="mt-3 flex items-center justify-between gap-3 rounded-lg bg-[var(--bg)] p-3 text-sm">
            ...
            <button
              ...
              className="shrink-0 rounded-lg border border-[var(--forest)] px-3 py-1 text-xs font-medium text-[var(--forest)] disabled:opacity-60"
            >
        ...
        <form onSubmit={submit} className="mt-4 space-y-4">
          <Field label="Amount" hint={`Paying more than ${formatCedis(obligation.balance)} is allowed — enter whatever was actually handed over.`}>
            <input
              type="number"
              min="0.01"
              step="0.01"
          ...
          {overpaymentAmount > 0 && (
            <label className="flex items-start gap-2 rounded-lg bg-[var(--bg)] p-3 text-sm">
