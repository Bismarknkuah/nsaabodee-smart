...
export function QrCodePanel({ funeralId }: { funeralId: string }) {
  ...
  return (
    <section className="rounded-lg border border-[var(--rule)] bg-white p-6">
      ...
      {!data ? (
        <button
          onClick={generate}
          disabled={loading}
          className="mt-4 rounded-lg bg-[var(--forest)] px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
        >
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 flex gap-3">
            <a
              ...
              className="rounded-lg border border-[var(--rule)] px-3 py-1.5 text-xs font-medium"
            >
              Download
            </a>
            <button onClick={() => window.print()} className="rounded-lg border border-[var(--rule)] px-3 py-1.5 text-xs font-medium">
              Print
            </button>
          </div>
        </div>
