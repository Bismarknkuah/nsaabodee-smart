...
export function FuneralDailyBreakdownCard({ funeralId }: { funeralId: string }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-[var(--rule)] bg-white p-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--rule)] text-left text-xs uppercase tracking-wide text-[var(--ink-soft)]">
              ...
              <th className="py-1.5 text-right" title="Community ledger + Town Elders ledger, combined">Elders + Community</th>
            </tr>
          </thead>
          <tbody>
            {data.days.map((d) => (
              <tr key={d.date} className="border-b border-[var(--rule)] last:border-b-0">
                ...
                <td className="py-1.5 pr-4 text-right font-mono font-medium">{formatCedis(d.combined_total)}</td>
                <td className="py-1.5 text-right font-mono" style={{ color: "var(--forest)" }}>{formatCedis(d.town_elders_and_community_combined_total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-2 flex justify-between text-sm">
        <span>Town Elders + Community so far: <span className="font-medium" style={{ color: "var(--forest)" }}>{formatCedis(data.town_elders_and_community_grand_total)}</span></span>
        <span className="font-medium">Total so far: {formatCedis(data.grand_total)}</span>
      </div>
    </div>
  );
}
