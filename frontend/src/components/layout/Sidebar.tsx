...
// 'Make it clear for each of the community roles... to have access to
// features they need.' arrears_collector is explicitly "the same job
// as Collector, just scoped to arrears rather than active-funeral
// obligations" (see the Role definition's own comment) — it belongs
// in this set for exactly the same reason Collector does, a gap that
// had gone unnoticed until this sweep. notification_officer already
// has real backend authority to record funeral attendance
// (funeral_logistics.permissions.ATTENDANCE_ROLES) and the actual UI
// for it already exists (AttendancePanel, embedded in a funeral's own
// detail page) — but with no way to navigate to /funerals at all,
// that page, and the panel on it, was completely unreachable.
const FUNERAL_BROWSE_ROLES = [...FINANCE_OVERSIGHT, "collector", "arrears_collector", "notification_officer", ...FAMILY_OFFICERS];
...
// access to what they need to execute their tasks.' Collecting IS the
// job for all three of these roles — none of them could reach the one
// page that actually records a payment before this, only "collector"
// (the community-wide contribution collector) could.
const DESK_ROLES = ["collector", "arrears_collector", "family_arrears_officer", "town_elders_arrears_officer"];
...
const ALL_COMMUNITY_ROLES = [
  ...
  // 'Yes, please continue verifying rather than assuming.' A real,
  // previously-uncaught gap this re-check turned up: every role added
  // across the several turns building out arrears/registration
  // officers was missing from this array entirely, meaning none of
  // them could see their own Tasks, Welfare & Contributions, My
  // Receipts, or Support — pages every community-level executive
  // needs simply because they are also, themselves, a member of the
  // community with their own welfare obligations and their own
  // assigned tasks. None of these six roles is exempt from that the
  // way Guest or Platform Admin structurally are (see the comment
  // above this array) — this was a straightforward omission, not a
  // deliberate exclusion.
  "arrears_collector", "family_arrears_officer", "family_registration_officer",
  "town_registration_officer", "town_elders_arrears_officer", "community_registration_desk",
];
...
const NAV_LINKS: { href: string; label: string; icon: typeof IconDashboard; roles: string[] | null; section: string }[] = [
  ...
  { href: "/plans", label: "Plans", icon: IconCommunities, roles: PLATFORM_TIER, section: "Platform" },
  ...
  { href: "/funerals", label: "Funerals", icon: IconFunerals, roles: [...FUNERAL_BROWSE_ROLES, "traditional_leader", "bereaved_rep"], section: "Funerals & Collections" },
  // "The arrears collector should have more options" — the Arrears Desk is theirs; a plain Collector never sees it.
  { href: "/arrears-desk", label: "Arrears Desk", icon: IconAlert, roles: ["arrears_collector", "family_arrears_officer", "town_elders_arrears_officer", "community_admin", "treasurer", "financial_secretary"], section: "Funerals & Collections" },
  ...
  // 'Make it clear for each of the family executive roles... to have
  // access to what they need to execute their tasks.' Family and Town
  // Registration Officers need this to actually register/edit a
  // member (their whole job); the two Arrears Officer roles need it
  // to look someone up before collecting from them. Extended
  // specifically here rather than adding these roles to
  // FUNERAL_BROWSE_ROLES itself — that set also grants /funerals and
  // /pending-sync, general funeral browsing neither of these roles'
  // jobs actually call for.
  // 'Make it clear for each... to have access to what they need to
  // execute their tasks.' community_registration_desk was already
  // granted backend registration authority (MEMBER_REGISTRATION_ROLES)
  // but had no way to reach the one page that authority is actually
  // exercised on — the same shape of gap family_registration_officer
  // had before it was fixed a few turns ago.
  // "His major role is to have access of the members data: totals by gender, age, family by family." Scoped server-side to each role's jurisdiction.
  { href: "/members/analytics", label: "Member Registry", icon: IconMembers, roles: ["community_admin", "chairman", "secretary", "community_registration_desk", "town_registration_officer", "traditional_leader", "family_head", "family_secretary", "family_registration_officer"], section: "Funerals & Collections" },
  { href: "/members", label: "Members", icon: IconMembers, roles: [...FUNERAL_BROWSE_ROLES, "family_arrears_officer", "family_registration_officer", "town_registration_officer", "town_elders_arrears_officer", "community_registration_desk", "traditional_leader"], section: "Funerals & Collections" },
  ...
  { href: "/town-elders-ledger", label: "Town Elders Ledger", icon: IconCommunities, roles: ["traditional_leader", "community_admin", "chairman", "secretary", "town_elders_arrears_officer"], section: "Finance & Oversight" },
  ...
  { href: "/payment-reversals", label: "Payment Reversals", icon: IconAlert, roles: [...FINANCE_OVERSIGHT, "collector", "arrears_collector", "family_arrears_officer", "town_elders_arrears_officer"], section: "Finance & Oversight" },
  { href: "/arrears-corrections", label: "Arrears Corrections", icon: IconAlert, roles: [...FINANCE_OVERSIGHT, "collector", "arrears_collector", "family_arrears_officer", "town_elders_arrears_officer"], section: "Finance & Oversight" },
  { href: "/expenses", label: "Expenses", icon: IconAlert, roles: ["community_admin", "chairman", "treasurer", "financial_secretary", "auditor"], section: "Finance & Oversight" },
  { href: "/liabilities", label: "Liabilities", icon: IconAlert, roles: ["community_admin", "chairman", "treasurer", "financial_secretary", "auditor"], section: "Finance & Oversight" },
  ...
  { href: "/user-management", label: "User Management", icon: IconAlert, roles: ["community_admin", "chairman", "family_head", "family_secretary", "traditional_leader"], section: "Administration" },
  { href: "/system-settings", label: "System Settings", icon: IconCommunities, roles: ["community_admin", "chairman", "family_head", "family_secretary", "traditional_leader"], section: "Administration" },
...
export function Sidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen w-64 shrink-0 flex-col bg-[var(--sidebar-bg)] text-slate-300 transition-transform duration-200 ease-in-out md:sticky md:top-0 md:translate-x-0 ${
      ...
      >
        ...
        <div className="flex items-center gap-2.5 border-b border-white/10 px-5 py-5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--primary)] text-white">
            <svg viewBox="0 0 40 40" className="h-5 w-5" aria-hidden="true">
              ...
              <circle cx="20" cy="20" r="4" fill="none" stroke="currentColor" strokeWidth="2.2" />
              {Array.from({ length: 8 }, (_, i) => {
                const a = (i / 8) * Math.PI * 2;
                return <circle key={i} cx={20 + Math.cos(a) * 13.5} cy={20 + Math.sin(a) * 13.5} r="1.8" fill="currentColor" />;
              })}
            </svg>
          </div>
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold leading-tight text-white">Nsaabodeɛ Smart</p>
            <p className="truncate text-[10px] uppercase tracking-wide text-slate-400">{user?.community_name ?? "Platform"}</p>
          </div>
        </div>
        ...
        <Link href="/profile" className="flex items-center gap-2.5 border-b border-white/10 px-5 py-4 hover:bg-white/5">
          {user?.profile_photo_url ? (
            ...
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--primary-soft)] text-sm font-semibold text-[var(--primary)]">
              {user?.username?.slice(0, 1).toUpperCase()}
            </span>
          )}
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-white">{user?.username}</p>
            <p className="truncate text-[10px] uppercase tracking-wide text-slate-400">{formatRoleOf(user)}</p>
          </div>
        </Link>
        ...
        {user?.can_switch_dashboard_context && (
          <button
            ...
            className="mx-5 mt-2 flex items-center justify-between rounded-lg border border-white/10 px-3 py-2 text-[11px] font-medium text-slate-300 hover:bg-white/5"
          >
        ...
        <Link
          href="/pending-sync"
          className="mx-5 mt-3 flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1 text-[11px] font-medium text-slate-300 hover:bg-white/10"
        >
        ...
        {user && (user.community_access_expired || (user.community_access_days_remaining !== null && user.community_access_days_remaining <= 7)) && (
          <div className={`mx-5 mt-2 rounded-lg px-3 py-2 text-[11px] ${user.community_access_expired ? "bg-red-500/20 text-red-200" : "bg-amber-500/20 text-amber-200"}`}>
        ...
        {visibleLinks.length > 6 && (
          <div className="mx-5 mt-3">
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[13px] text-white placeholder:text-slate-400 outline-none focus:border-[var(--primary)]"
            />
          </div>
        )}
        ...
        <nav className="mt-3 flex-1 space-y-3 overflow-y-auto px-3 pb-3">
          {navFilter.trim() && groupedLinks.length === 0 && (
            <p className="px-2.5 py-2 text-sm text-slate-400">No pages match &quot;{navFilter}&quot;.</p>
          )}
          {groupedLinks.map((group) => (
            <div key={group.section || "top"} className="space-y-0.5">
              {group.section && (
                <p className="px-2.5 pb-1 pt-2 text-[10px] uppercase tracking-widest text-slate-400">{group.section}</p>
              )}
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 rounded-lg px-2.5 py-2 text-sm transition ${
                      active ? "bg-[var(--primary)] font-medium text-white" : "text-slate-300 hover:bg-white/5 hover:text-white"
                    }`}
                  >
        ...
        <div className="border-t border-white/10 px-3 py-3">
          <p className="px-2.5 pb-1 text-[10px] uppercase tracking-widest text-slate-400">Account</p>
          <Link href="/profile" className="flex items-center gap-3 rounded-lg px-2.5 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white">
            <IconUser className="shrink-0" /> My Profile
          </Link>
          <button onClick={logout} className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left text-sm text-slate-300 hover:bg-white/5 hover:text-red-300">
            <IconSignOut className="shrink-0" /> Sign out
          </button>
        </div>
      </aside>

      <main className="min-w-0 flex-1 bg-[var(--bg)] md:h-screen md:overflow-y-auto">
        {/* Mobile-only top bar with the hamburger toggle — "so when the user wants to see it, he has to click on the three dot or slash" */}
        <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-[var(--border)] bg-[var(--card)] px-4 py-3 md:hidden">
          <button
          ...
          <span className="text-xs font-medium uppercase tracking-widest text-[var(--text-soft)]">Nsaabodeɛ Smart</span>
        </div>
