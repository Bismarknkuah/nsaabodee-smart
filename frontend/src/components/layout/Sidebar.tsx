...
const NAV_LINKS: { href: string; label: string; icon: typeof IconDashboard; roles: string[] | null; section: string }[] = [
  ...
  { href: "/user-management", label: "User Management", icon: IconAlert, roles: ["community_admin", "family_head", "traditional_leader"], section: "Administration" },
  { href: "/system-settings", label: "System Settings", icon: IconCommunities, roles: ["community_admin", "family_head", "traditional_leader"], section: "Administration" },
      ...
      >
        ...
        <Link href="/profile" className="flex items-center gap-2.5 border-b border-white/10 px-5 py-4 hover:bg-white/5">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[10px] uppercase tracking-wide text-slate-400">{formatRole(user?.role)}</p>
          </div>
        </Link>
        ...
        {visibleLinks.length > 6 && (
          <div className="mx-5 mt-3">
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[13px] text-white placeholder:text-slate-500 outline-none focus:border-[var(--primary)]"
            />
          </div>
        )}
        ...
        <nav className="mt-3 flex-1 space-y-3 overflow-y-auto px-3 pb-3">
          {navFilter.trim() && groupedLinks.length === 0 && (
            <p className="px-2.5 py-2 text-sm text-slate-500">No pages match &quot;{navFilter}&quot;.</p>
          )}
          {groupedLinks.map((group) => (
            <div key={group.section || "top"} className="space-y-0.5">
              {group.section && (
                <p className="px-2.5 pb-1 pt-2 text-[10px] uppercase tracking-widest text-slate-500">{group.section}</p>
              )}
        ...
        <div className="border-t border-white/10 px-3 py-3">
          <p className="px-2.5 pb-1 text-[10px] uppercase tracking-widest text-slate-500">Account</p>
