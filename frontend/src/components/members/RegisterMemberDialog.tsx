...
export function RegisterMemberDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="font-body flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-sm bg-[var(--surface)] text-[var(--ink)] shadow-xl">
              ...
              <Field label="Family" hint={isFamilyScoped ? "You can only register members into your own family." : undefined}>
                {isFamilyScoped ? (
                  <select
                    value={familyId}
                    disabled
                    className="w-full cursor-not-allowed rounded-sm border border-[var(--rule)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--ink-soft)]"
                  >
