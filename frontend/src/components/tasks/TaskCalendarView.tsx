  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <button onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))} className="rounded-lg border border-[var(--rule)] px-3 py-1 text-sm hover:border-[var(--ink)]">←</button>
      ...
      <div className="mt-3 grid grid-cols-7 gap-px border border-[var(--rule)] bg-[var(--rule)] text-center font-mono text-[10px] uppercase text-[var(--ink-soft)]">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => <div key={d} className="bg-[var(--bg)] py-1">{d}</div>)}
      </div>
            ...
            >
              ...
              <div className="mt-1 space-y-0.5">
                {dayTasks.slice(0, 3).map((t) => (
                  <div key={t.id} className="truncate rounded-lg px-1 text-[10px] text-white" style={{ backgroundColor: STATUS_ACCENT[t.status] }}>
                    {t.title}
                  </div>
                ))}
        ...
        <div className="mt-4 rounded-lg border border-[var(--rule)] bg-[var(--bg)] p-3">
          ...
          <ul className="mt-2 space-y-2">
            {(byDay.get(selectedDay) ?? []).map((t) => (
              <li key={t.id} className="rounded-lg border border-[var(--rule)] bg-white p-2 text-sm">
