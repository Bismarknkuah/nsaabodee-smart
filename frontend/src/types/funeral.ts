import type { FuneralLedgerBreakdown } from "./reports";
...
/** "Each funeral should have its own fund [so] the family knows the expenditure of that particular funeral... after every funeral each ledger should know the amount they received, and those who didn't pay." */
export interface FuneralClosingReport {
  funeral_id: string;
  deceased_name: string;
  deceased_family: string;
  status: string;
  ...
  total_spent_on_logistics: string;
  total_gifts_received: string;
  /** The same seven-ledger split already used for the live, still-open view of a funeral (see FourLedgerBreakdownCard) — unchanged here, just also included in the closing figures. */
  ledger_breakdown: FuneralLedgerBreakdown;
  who_did_not_pay: {
    member_id: string;
    member_name: string;
