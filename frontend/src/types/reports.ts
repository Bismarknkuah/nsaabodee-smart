...
export interface FuneralLedgerBreakdown {
  ...
  guest_ledger?: DonationLedgerBucket;
  town_leaders_ledger?: DonationLedgerBucket;
  town_elders_contribution_ledger: { member_count: number; expected_total: string; collected_total: string };
  asupede_ledger: {
    active: boolean; amount: string | null; collection_start: string | null; collection_end: string | null;
    obligation_count: number; expected_total: string; collected_total: string; outstanding_total: string; additional_support_total: string;
  };
  in_law_ledger: {
    request_count: number; pending_count: number; approved_count: number; rejected_count: number;
    expected_total: string; collected_total: string; outstanding_total: string;
  };
}
...
export interface OutstandingMembersReport {
  ...
  // 'Must NOT access sensitive personal financial information unless
  // explicitly authorized' — Traditional Leader gets this same
  // endpoint but a genuinely different, aggregate-only shape with no
  // individual names at all (see reports.views.OutstandingMembersReportView).
...
export interface FuneralDailyBreakdownDay {
  ...
  // community ledger and sum them together.'
  town_elders_ledger_total: string;
  community_ledger_total: string;
  town_elders_and_community_combined_total: string;
}
...
export interface FuneralDailyBreakdown {
  ...
  grand_total: string;
  town_elders_and_community_grand_total: string;
}
